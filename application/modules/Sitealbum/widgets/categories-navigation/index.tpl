    ...
    <?php if (!empty($this->categoriesArray)): ?>
      <?php foreach ($this->categoriesArray as $categorylist): ?>
        ...
        <li class="level<?php echo $level . " " . (!empty($subcategories) ? 'parent' : '') ?> ">
          ...
          ?>" href="<?php echo $this->url(array('category_id' => $category->getIdentity(), 'categoryname' => Engine_Api::_()->getItem('album_category', $category->getIdentity())->getCategorySlug()), "sitealbum_general_category"); ?>">
            ...
            <ul class="level<?php echo $level ?>">
              <?php foreach ($subcategories as $subcategorieslist): ?>
                ...
                <li class="level<?php echo $level . "  " ?> ">
                  ...
                  ?>" href="<?php echo $this->url(array('category_id' => $category->getIdentity(), 'categoryname' => Engine_Api::_()->getItem('album_category', $category->getIdentity())->getCategorySlug(), 'subcategory_id' => $subcategory->getIdentity(), 'subcategoryname' => Engine_Api::_()->getItem('album_category', $subcategory->getIdentity())->getCategorySlug()), "sitealbum_general_subcategory"); ?>">
...
<script type="text/javascript">
  en4.core.runonce.add(function() {
    if (!(typeof NavigationSitealbum == 'function')) {
      new Asset.javascript(en4.core.staticBaseUrl + 'application/modules/Sitealbum/externals/scripts/core.js', {
    ...
    function addDropdownMenu() {
      NavigationSitealbum("nav_cat_<?php echo $this->identity ?>", {"show_delay": "100", "hide_delay": "100"});
    }
  });
</script>