...
<br />
<div class="tip"> <span> <?php echo $this->translate('You should only make those photos as "Photo of the Day" which have their view privacy set as \'Everyone\' or \'All Registered Members\'; If you have made "Photo of the Day" a photo which has a different privacy, then its entry below will be specially highlighted.'); ?> </span> </div>
<br />
<div>
...
<div>
	<?php if ($this->photoOfDaysList->getTotalItemCount() > 0): ?>
	  ...
	  <form id='multidelete_form' method="post" action="<?php echo $this->url(array('module' => 'sitealbum', 'controller' => 'photo', 'action' => 'multi-delete-photo'), 'admin_default'); ?>" onSubmit="return multiDelete()">
		  <table class='admin_table' width="100%">
		    <thead>
		      <tr>
						<th style='width: 1%;' align="left"><input onclick="selectAll()" type='checkbox' class='checkbox'></th>
						<?php $class = ( $this->order == 'engine4_album_photos.title' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
						<th width="18%" align="left" class="<?php echo $class ?>"><a href="javascript:void(0);" onclick="javascript:changeOrder('engine4_album_photos.title', 'DESC');"><?php echo $this->translate("Photo") ?></a></th>
						...
						<th width="18%" align="left" class="<?php echo $class ?>"><a href="javascript:void(0);" onclick="javascript:changeOrder('engine4_album_albums.title', 'DESC');"><?php echo $this->translate("Album") ?></a></th>
		    ...
		    <tbody>
		      ...
		      <?php endif; ?>
		        ...
		        <td width="18%" class="sitealbum_table_img"><?php echo $this->htmlLink($photo->getHref(),$this->itemPhoto($photo, 'thumb.normal'),array('title'=>$photo->getTitle())); ?></td>
		        <td width="18%" class="admin_table_bold"><?php echo $this->htmlLink($parent->getHref(), $parent->getTitle()); ?></td>
