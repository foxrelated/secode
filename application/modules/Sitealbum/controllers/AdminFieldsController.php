 ...
 * @version    $Id: AdminFieldsController.php 2011-08-026 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitealbum_AdminFieldsController extends Fields_Controller_AdminAbstract {
  ...
  public function indexAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sitealbum_admin_main', array(), 'sitealbum_admin_main_fields');
  ...
  public function fieldCreateAction() {
    ...
    $form->addElement('select', 'quick_info', array(
        'label' => 'Show in quick information / browse albums widget?',
        'multiOptions' => array(
            1 => 'Show in quick information / browse albums widget',
            0 => 'Hide from quick information / browse albums widget'),
        'value' => 0
    ));
  ...
  public function fieldEditAction() {
    ...
    //START CUSTOMIZATION BY SOCIALENGINEADDONS
    $form->setTitle('Edit Album Question');
    $form->removeElement('show');
    $form->addElement('hidden', 'show', array('value' => 0));
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sitealbum_Form_Admin_Type();
    ...
    if ($form->getValue('duplicate') != 'null') {
      // Create New Option in engine4_album_fields_options
      ...
      $field_meta_array = $db->select()
              ->from('engine4_album_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_album_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_album_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      ...
      Engine_Api::_()->getDbtable('albums', 'sitealbum')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));

      //DELETE MAPPING
      Engine_Api::_()->getDbtable('categories', 'sitealbum')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
