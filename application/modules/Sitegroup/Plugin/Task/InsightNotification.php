 ...
 */
class Sitegroup_Plugin_Task_InsightNotification extends Core_Plugin_Task_Abstract {

  public function execute() {

    // create an object for view
    $view = Zend_Registry::isRegistered('Zend_View') ? Zend_Registry::get('Zend_View') : null;
    $sitegroupInsightmail = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.insightemail', 1);
		if(empty($sitegroupInsightmail))
		{
      return;
		}
    $date = time();
    $days_string = "";
    $insight_email_time = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.insightmail.time', 1);
