 ...
 * @version    $Id: 2Checkout.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitegroup_Plugin_Gateway_2Checkout extends Engine_Payment_Plugin_Abstract {
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($giveBenefit) {
      ...
      $group->gateway_profile_id = $params['order_number']; // This is the same as sale_id
  ...
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
    ...
    // Update subscription
    $groupUpdated = false;
    if (!empty($rawData['sale_id']) && empty($group->gateway_profile_id)) {
      $groupUpdated = true;
      $group->gateway_profile_id = $rawData['sale_id'];
    }
    if (!empty($rawData['invoice_id']) && empty($group->gateway_transaction_id)) {
      $groupUpdated = true;
      $group->gateway_profile_id = $rawData['invoice_id'];
    }
    if ($groupUpdated) {
      $group->save();
    }
  ...
  public function cancelGroup($transactionId) {
    return $this;
  }
  ...
  public function getTransactionDetailLink($transactionId) {
    return 'https://www.2checkout.com/va/sales/get_list_sale_groupd?invoice_id=' . $transactionId;
  }
