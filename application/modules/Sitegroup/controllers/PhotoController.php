 ...
 */
class Sitegroup_PhotoController extends Seaocore_Controller_Action_Standard {
  ...
  public function init() {
    ...
    //CHECK SUBJECT IS EXIST OR NOT IF NOT EXIST THEN SET ACCORDING TO THE GROUP ID AND PHOTO ID
  ...
  public function uploadAlbumAction() {
    ...
    if (Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode')) {
      ...
      if(isset($_FILES['Filedata']))
        $_POST['file'] = $this->uploadPhotoAction();
			...
			$sitegroupalbum_image_format = Zend_Registry::isRegistered('sitegroupalbum_image_format') ? Zend_Registry::get('sitegroupalbum_image_format') : null;
			...
			//GET FORM
			$this->view->form = $form = new Sitegroup_Form_Photo_Album();
			...
			//SET GROUP ID INTO THE FORM
			$form->group_id->setValue($group_id);
			...
			if (empty($sitegroupalbum_image_format)) {
				return;
			}
			...
			//GET DB
			$db = Engine_Api::_()->getItemTable('sitegroup_album')->getAdapter();
			...
			try {
				...
				//UPDATE VALUES
				Engine_Api::_()->getDbtable('photos', 'sitegroup')->update(array('album_id' => $album->getIdentity(), 'group_id' => $group_id, 'collection_id' => $album->getIdentity(),), array('group_id =?' => $group_id, 'album_id =?' => 0, 'collection_id =?' => 0, 'user_id =?' => $viewer_id));
			...
			if ($viewer_id == $sitegroup->owner_id) {
				$this->_helper->redirector->gotoRoute(array('action' => 'edit-photos', 'group_id' => $group_id, 'album_id' => $album->getIdentity(), 'slug' => $album->getSlug(), 'tab' => $this->view->tab_selected_id), 'sitegroup_albumphoto_general', true);
			} else {
				$this->_helper->redirector->gotoRoute(array('action' => 'view', 'group_id' => $group_id, 'album_id' => $album->getIdentity(), 'slug' => $album->getSlug(), 'tab' => $this->view->tab_selected_id), 'sitegroup_albumphoto_general', true);
			}
			...
			//
			//GET FORM
			$this->view->form = $form = new Sitegroup_Form_SitemobilePhoto_Album();
	...
	//upload photo action
			...
			//GET SITEGROUP ITEM
			$sitegroup = Engine_Api::_()->getItem('sitegroup_group', (int) $this->_getParam('group_id'));
			...
			//START MANAGE-ADMIN CHECK
			if ($viewer_id != $sitegroup->owner_id && empty($can_edit)) {
				$isManageAdmin = Engine_Api::_()->sitegroup()->isManageAdmin($sitegroup, 'spcreate');
				if (empty($isManageAdmin)) {
					return $this->_forwardCustom('requireauth', 'error', 'core');
				}
			...
			if (empty($sitegroupalbum_iscollection)) {
				return;
			}
			...
			$sitegroupalbum_upload_type = Zend_Registry::isRegistered('sitegroupalbum_upload_type') ? Zend_Registry::get('sitegroupalbum_upload_type') : null;
			if (empty($sitegroupalbum_upload_type)) {
				return;
			}
  ...
  public function photoEditAction() {
    ...
    //EDIT PHOTO FORM
    $this->view->form = $form = new Sitegroup_Form_Photo_Photoedit();
    ...
    return $this->_forwardCustom('success', 'utility', 'core', array(
                ...
                'parentRedirect' => $this->_helper->url->url(array('action' => 'view',
                    'photo_id' => $photo_id, 'group_id' => $group_id, 'album_id' => $album_id, 'tab' => $this->view->tab_selected_id
                        ), 'sitegroup_imagephoto_specific', true),
  ...
  public function removeAction() {
    ...
    //GET TAB ID
    $tab_selected_id = Engine_Api::_()->sitegroup()->GetTabIdinfo('sitegroup.photos-sitegroup', $group_id, Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.layoutcreate', 0));
    ...
    //END MANAGE-ADMIN CHECK
    //PHOTO OWNER, GROUP OWNER AND SUPER-ADMIN CAN DELETE PHOTO
    if ($viewer_id != $photo->user_id && $can_edit != 1) {
      return $this->_forwardCustom('requireauth', 'error', 'core');
    }
    ...
    $this->view->form = $form = new Sitegroup_Form_Photo_Delete();
    ...
    try {
      ...
      if (empty($count)) {
        Engine_Api::_()->getItemTable('sitegroup_album')->update(array('photo_id' => 0), array('album_id = ?' => $album_id, 'default_value=?' => 1));
      }
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
    ...
    return $this->_forwardCustom('success', 'utility', 'core', array(
                ...
                'smoothboxClose' => 2,
                'parentRedirect' => $this->_helper->url->url(array('action' => 'view', 'group_id' => $group_id, 'album_id' => $album_id, 'slug' => $album->getSlug(), 'tab' => $tab_selected_id), 'sitegroup_albumphoto_general', true),
                'parentRedirectTime' => '2',
            ));
  }
  ...
  public function viewAction() {
    ...
    $this->view->sitegroupmemberEnabled = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupmember');
    ...
    //GET PHOTO SUBJECT
    $this->view->image = $photo = Engine_Api::_()->getItem('sitegroup_photo', $photo_id);
    ...
    //PHOTO OWNER, GROUP OWNER AND SUPER-ADMIN CAN EDIT AND DELETE PHOTO
    if ($viewer_id == $photo->user_id || $can_edit == 1) {
    ...
    if (!empty($viewer_id) && $viewer_id != $photo->user_id) {
      $this->view->report = $report = (int) Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.report', 1);
    }

    if (!empty($viewer_id)) {
      $this->view->share = $share = (int) Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.share', 1);
    }
  ...
  public function rotateAction() {
    ...
    if (!$this->_helper->requireSubject('sitegroup_photo')->isValid())
      return;
  ...
  public function makeGroupProfilePhotoAction() {
    //CHECK USER VALIDATION
    if (!$this->_helper->requireUser()->isValid())
      return;
    ...
    if ($this->getRequest()->isPost() && $this->getRequest()->getPost('confirm') == true) {
      ...
      try {
        ...
        if ($sitegroup->photo_id != $photo->file_id) {
          ...
          //Set it
          $sitegroup->photo_id = $photo->file_id;
          $sitegroup->save();
          $db->commit();
          ...
          if (Engine_Api::_()->sitegroup()->isGroupOwner($sitegroup) && Engine_Api::_()->sitegroup()->isFeedTypeGroupEnable())
            $activityFeedType = 'sitegroup_admin_profile_photo';
          elseif ($sitegroup->all_post || Engine_Api::_()->sitegroup()->isGroupOwner($sitegroup))
            $activityFeedType = 'sitegroup_profile_photo_update';


          if ($activityFeedType) {
            $action = $activityApi->addActivity(Engine_Api::_()->user()->getViewer(), $sitegroup, $activityFeedType);
      ...
      return $this->_forwardCustom('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Your group profile photo has been successfully changed.')),
                  'parentRedirect' => $this->_helper->url->url(array('group_url' => Engine_Api::_()->sitegroup()->getGroupUrl($group_id)), 'sitegroup_entry_view', true),
                  'smoothboxClose' => true,
              ));
    }
  }
  ...
  public function featuredAction() {
    ...
    $this->view->photo = $photo = Engine_Api::_()->getItem('sitegroup_photo', $this->_getParam('photo_id', $this->_getParam('photo_id', null)));
    $photo->featured = !$photo->featured;
    $photo->save();
    exit(0);
  }

  //ACTION FOR ADDING GROUP OF THE DAY
  public function addPhotoOfDayAction() {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        $select = $dayItemTime->select()->where('resource_id = ?', $photo_id)->where('resource_type = ?', 'sitegroup_photo');
  ...
  public function featuredPhotosCarouselAction() {
    ...
    $this->view->featuredPhotos = $featuredPhotos = Engine_Api::_()->sitegroupalbum()->getFeaturedPhotos($values);
  ...
  public function suggestAction()
  {
    ...
    if( !$viewer->getIdentity() ) {
      ...
      $select = Engine_Api::_()->getDbtable('membership', 'sitegroup')->getsitegroupmembersSelect($values);
