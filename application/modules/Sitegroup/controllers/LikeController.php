 ...
 */
class Sitegroup_LikeController extends Seaocore_Controller_Action_Standard {
    ...
    if (empty($like_id)) {
      ...
      if (empty($like_id_temp[0]['like_id'])) {
        ...
        if (!empty($resource)) {
					...
					//START GROUP MEMBER PLUGIN WORK.
					if ($resource_type == 'sitegroup_group' && $sitegroupVersion >= '4.2.9p3') {
						if (Engine_Api::_()->getDbtable( 'modules' , 'core' )->isModuleEnabled('sitegroupmember')) {
							Engine_Api::_()->sitegroupmember()->joinLeave($resource, 'Join');
						}
					Engine_Api::_()->sitegroup()->itemCommentLike($resource, 'sitegroup_contentlike');
					}
					//END GROUP MEMBER PLUGIN WORK.
        ...
        $like_msg = Zend_Registry::get('Zend_Translate')->_('Successfully Liked.');
			...
			if (!empty($resource)) {
			  ...
			  //START GROUP MEMBER PLUGIN WORK
				if ($resource_type == 'sitegroup_group' && $sitegroupVersion >= '4.2.9p3') {
										if (Engine_Api::_()->getDbtable( 'modules' , 'core' )->isModuleEnabled('sitegroupmember')) {
				  Engine_Api::_()->sitegroupmember()->joinLeave($resource, 'Leave');
				  }
				}
				//END GROUP MEMBER PLUGIN WORK				
  ...
  //ACTION FOR NUMBER OF GROUPS LIKES.
  public function likeGroupsAction() {
    ...
    $this->view->resource_id = $resource_id = $this->_getParam('resource_id');
		...
		//CURRENT GROUP NUMBER FOR THE 'AJAX PAGINATION '.
    ...
    $fetch_sub = Engine_Api::_()->sitegroup()->friendPublicLike($call_status, $resource_type, $resource_id, $user_id, $search);
    ...
    $fetch_sub->setCurrentPageNumber($group);
    $fetch_sub->setItemCountPerPage(50);

    //NUMBER OF PEOPLE WHICH LIKED THIS CONTENT.
    $this->view->public_count = Engine_Api::_()->sitegroup()->numberOfLike('sitegroup_group', $resource_id);

    //NUMBER OF MY FRIEND WHICH LIKED THIS CONTENT.
    $this->view->friend_count = Engine_Api::_()->sitegroup()->friendNumberOfLike($resource_type, $resource_id);
...
//     }
  ...
  public function mylikesAction() {
    ...
    $this->view->ratngShow = (int) Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupreview');
		...
		//GET NAVIGATION
		$this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sitegroup_main', array(), 'sitegroup_main');
    $defaultOrder = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.ajax.layouts.oder', 1);
    $ShowViewArray = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.ajax.widgets.layout', array("0" => "1", "1" => "2", "2" => "3"));
    ...
    $like_select = $moduleTable->select()
            ->setIntegrityCheck( false )
						...
						->join($moduleTableName, "$coreLikeTableName.resource_id = $moduleTableName.group_id")
  ...
  public function myJoinedAction() {
	...
	//ACTION FOR SENDING UPDATES TO THE USERS WHO LIKED THIS GROUP
  public function sendUpdateAction() {

    $multi = 'member';
    $multi_ids = '';
    $group_id = $resource_id = $this->_getParam("group_id");
    ...
    if (empty($sitegroup->like_count)) {
      return;
    } elseif ($sitegroup->like_count == 1) {
      $likeTable = Engine_Api::_()->getDbtable('likes', 'core');
    ...
    $form->setDescription('Create the update to be sent to people who like your Group.');

    if ($sitegroup->like_count == 1) {
      $tip_msg = $this->view->translate("<div class='tip'><span>1 person likes this Group.</span></div>");
    } else {
      $tip_msg = "<div class='tip'><span>". $this->view->translate('%s people like this Group.', $sitegroup->like_count). "</span></div>";
    }
    ...
    try {
      ...
      $group_title_with_link = '<a href = http://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array('group_url' => Engine_Api::_()->sitegroup()->getGroupUrl($sitegroup->group_id)), 'sitegroup_entry_view') . ">$sitegroup->title</a>";

      $conversation = Engine_Api::_()->getItemTable('messages_conversation')->send(
              $viewer, $recipients, $values['title'], $this->view->translate("This is an update for the group:" . $group_title_with_link) . "<br />" . $values['body'], $attachment
      );
      ...
      $this->_forwardCustom('success', 'utility', 'core', array(
              'smoothboxClose' => 500,
              'parentRefresh' => false,
              'messages' => array(Zend_Registry::get('Zend_Translate')->_('Update was sent successfully to people who like your Group.'))
      ));
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
