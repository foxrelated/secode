 ...
 */
class Sitegroup_InsightsController extends Core_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    $this->view->sitegroups_view_menu = 8;
    $this->view->group_owner_id = $sitegroup->owner_id;

    if (!Engine_Api::_()->core()->hasSubject('sitegroup')) {
    ...
    $this->view->filterForm = $filterForm = new Sitegroup_Form_Insights_Filter();
    $values = array();
    ...
    $total_likes = Engine_Api::_()->sitegroup()->getGroupLikes($values);

    // check if comments should be displayed or not
    $this->view->show_comments = $show_comments = Engine_Api::_()->sitegroup()->displayCommentInsights();
    if (!empty($show_comments)) {
      $this->view->total_comments = $total_comments = Engine_Api::_()->sitegroup()->getGroupComments($values);
    }
    ...
    $this->view->total_views = $sitegroup->view_count;

    $creation_date = Engine_Api::_()->getItem('sitegroup_group', $group_id)->creation_date;
  ...
  public function groupStatisticsAction() {
    ...
    if (!empty($group_id)) {
      ...
      } else if ($sitegroup->view_count == 1 && empty($raw_views_count)) {
        $sub_status_table->groupViewCount($group_id);
      }
    }
    exit(0);
  ...
  public function chartDataAction() {
    ...
    $statsTable = Engine_Api::_()->getDbtable('groupstatistics', 'sitegroup');
    ...
    do {
      ...
      $currentPeriodCount_likes = Engine_Api::_()->sitegroup()->getGroupLikes($vals);
      if (!empty($show_comments)) {
        $currentPeriodCount_comments = Engine_Api::_()->sitegroup()->getGroupComments($vals);
      }
    ...
    $titleStr = $translate->_('_SITEGROUP_INSIGHTS_' . strtoupper(trim(preg_replace('/[^a-zA-Z0-9]+/', '_', $type), '_')));
    ...
    // Make full chart
    $chart = new OFC_Chart();
    $chart->set_bg_colour(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graph.bgcolor', '#ffffff'));

    $chart->set_x_axis($labels);
    $chart->add_y_axis($yaxis);

    $view_width = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphview.width', '3');
    $like_width = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphlike.width', '3');
    $activeuser_width = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphuser.width', '3');

    $view_color = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphview.color', '#3299CC');
    $like_color = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphlike.color', '#CD6839');
    $activeuser_color = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphuser.color', '#9F5F9F');
    if (!empty($show_comments)) {
      $comment_width = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphcomment.width', '3');
      $comment_color = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.graphcomment.color', '#458B00');
    }
  ...
  public function exportReportAction() {
    ...
    $this->view->sitegroups_view_menu = 23;
    $this->view->group_id = $group_id = $this->_getParam('group_id');
    ...
    // to calculate the oldest group's creation year
    $select = $groupsTable->select();
    $select
            ->from($groupsName, array('group_id', 'MIN(creation_date) as min_year'))
            ->where('owner_id = ?', $this->_viewer_id)
            ->group('group_id')
            ->limit(1);

    $this->view->no_groups = 0;
    $min_year = $groupsTable->fetchRow($select);
    ...
    $this->view->reportform = $reportform = new Sitegroup_Form_Insights_Report();
    $reportform->year_start->setMultiOptions($year_array);
    $reportform->year_end->setMultiOptions($year_array);
    $reportform->group_id->setValue($group_id);
    ...
    if (!empty($_GET['time_summary'])) {
      ...
      if (empty($values['format_report'])) {
        $url = 'http://' . $_SERVER['HTTP_HOST'] . $this->view->url(array('group_id' => $group_id, 'start_daily_time' => $start_tm, 'end_daily_time' => $end_tm), 'sitegroup_webgroupreport', true) . '?' . $url_values[1];
      } else {
        $url = 'http://' . $_SERVER['HTTP_HOST'] . $this->view->url(array('module' => 'sitegroup', 'controller' => 'insights', 'action' => 'export-excel', 'group_id' => $group_id, 'start_daily_time' => $start_tm, 'end_daily_time' => $end_tm), 'default', true) . '?' . $url_values[1];
      }
      // Session Object
  ...
  public function exportExcelAction() {
    ...
    if (!empty($_GET)) {
      ...
      $values = $_GET;
      $this->view->group_id = $group_id = $values['group_id'];
      $this->view->sitegroup = $sitegroup = Engine_Api::_()->getItem('sitegroup_group', $group_id);
      ...
      $statTable = Engine_Api::_()->getDbtable('groupstatistics', 'sitegroup');
      ...
      // FETCH VIEWS
      $statObject2 = $statTable->groupReportInsights($values);
      ...
      $statObject = $statTable->groupReportInsights($values);
      ...
      // FETCH LIKES
      $this->view->total_likes = $total_likes = Engine_Api::_()->sitegroup()->getReportLikes($values);
      foreach ($total_likes as $likes) {
        if (!empty($likes['creation_date'])) {
          ...
          switch ($values['time_summary']) {
            case 'Monthly':
              $date_value = date('F, Y', mktime(0, 0, 0, $date_array[1], $date_array[2], $date_array[0]));
              $rawdata[$date_value[0]]['likes'] = $likes['group_likes'];
              $rawdata[$date_value[0]]['date_value'] = $date_value;
              break;
            ...
            case 'Daily':
              ...
              $labelDate->set($response_time);
              $date_value = $this->view->locale()->toDate($labelDate, array('size' => 'long'));
              $rawdata[$response_date[0]]['likes'] = $likes['group_likes'];
              $rawdata[$response_date[0]]['date_value'] = $date_value;
              break;
          }
        }
      ...
      // FETCH COMMENTS
      if (!empty($show_comments)) {
        $this->view->total_comments = $total_comments = Engine_Api::_()->sitegroup()->getReportComments($values);
        foreach ($total_comments as $comments) {
          if (!empty($comments['creation_date'])) {
            ...
            switch ($values['time_summary']) {
              case 'Monthly':
                $date_value = date('F, Y', mktime(0, 0, 0, $date_array[1], $date_array[2], $date_array[0]));
                $rawdata[$date_value[0]]['comments'] = $comments['group_comments'];
                $rawdata[$date_value[0]]['date_value'] = $date_value;
                break;
              ...
              case 'Daily':
                ...
                $labelDate->set($response_time);
                $date_value = $this->view->locale()->toDate($labelDate, array('size' => 'long'));
                $rawdata[$response_date[0]]['comments'] = $comments['group_comments'];
                $rawdata[$response_date[0]]['date_value'] = $date_value;
                break;
            }
          }
      ...
      $url_string = Zend_Controller_Front::getInstance()->getRequest()->getRequestUri();
      $url_values = explode('?', $url_string);
      $url = 'http://' . $_SERVER['HTTP_HOST'] . $this->view->url(array('group_id' => $values['group_id'], 'empty' => 1), 'sitegroup_reports', true) . '?' . $url_values[1];

      // in case no data, redirect to the report group with an empty parameter
      if (empty($rawdata)) {
  ...
  public function exportWebgroupAction() {
    if (!$this->_helper->requireUser()->isValid())
      return;
    ...
    if (!empty($_GET)) {
      ...
      $totalObject = $statTable->groupReportInsights($values);
      ...
      $totallikes = Engine_Api::_()->sitegroup()->getReportLikes($values);
      if (!empty($totallikes)) {
        $this->view->totallikes = $totallikes = $totallikes[0]['group_likes'];
      } else {
      ...
      $this->view->show_comments = $show_comments = Engine_Api::_()->sitegroup()->displayCommentInsights();
      if (!empty($show_comments)) {
        $totalcomments = Engine_Api::_()->sitegroup()->getReportComments($values);
        if (!empty($totalcomments)) {
          $this->view->totalcomments = $totalcomments = $totalcomments[0]['group_comments'];
        } else {
          $this->view->totalcomments = $totalcomments = 0;
        }
      }

      $totalObject2 = $statTable->groupReportInsights($values);
      ...
      $select_non_loggedins->from($statsName, array('groupstatistic_id', 'viewer_id'))->where('group_id = ?', $group_id)->where('viewer_id = ?', 0)->limit(1);
