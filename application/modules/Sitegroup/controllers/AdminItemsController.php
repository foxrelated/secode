 ...
 */
class Sitegroup_AdminItemsController extends Core_Controller_Action_Admin {

    //ACTION FOR GROUP OF THE DAY
    public function dayAction() {

        //TAB CREATION
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sitegroup_admin_main', array(), 'sitegroup_admin_main_items');
        ...
        //FETCH DATA
        $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('itemofthedays', 'sitegroup')->getItemOfDayList($values, 'group_id', 'sitegroup_group');
    ...
    public function addItemAction() {
        ...
        //FORM GENERATION
        $form = $this->view->form = new Sitegroup_Form_Admin_Item();
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            try {
                ...
                $table = Engine_Api::_()->getDbtable('itemofthedays', 'sitegroup');
                $select = $table->select()->where('resource_id = ?', $values["resource_id"])->where('resource_type = ?', 'sitegroup_group');
    ...
    //ACTION FOR GROUP SUGGESTION DROP-DOWN
    public function getitemAction() {
        ...
        $moduleContents = Engine_Api::_()->getItemTable('sitegroup_group')->getDayItems($search_text, $limit = 10);
    ...
    public function deleteItemAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            $db->beginTransaction();
            try {
                $itemofthedaysTable = Engine_Api::_()->getDbtable('itemofthedays', 'sitegroup')->delete(array('itemoftheday_id =?' => $this->_getParam('id')));
                $db->commit();
            } catch (Exception $e) {
                $db->rollBack();
                throw $e;
    ...
    //ACTION FOR MULTI DELETE GROUP ENTRIES
