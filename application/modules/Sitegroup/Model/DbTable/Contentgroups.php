 ...
 */
class Sitegroup_Model_DbTable_Contentgroups extends Engine_Db_Table implements Engine_Content_Storage_Interface {

  protected $_rowClass = "Sitegroup_Model_Contentgroup";

  public function loadMetaData(Engine_Content $contentAdapter, $name) {
  	
    $select = $this->select()->where('name = ?', $name)->orWhere('contentgroup_id = ?', $name);
  ...
  public function loadContent(Engine_Content $contentAdapter, $name) {
    ...
    $select = $this->select()->where('group_id = ?', $sitegroup_id)->where('name = ?', $name)->orWhere('contentgroup_id = ?', $name);
    $group = $this->fetchRow($select);

    if (is_object($group)) {
			$contentTable = Engine_Api::_()->getDbtable('content', 'sitegroup');
			$select = $contentTable->select()
							->where('contentgroup_id = ?', $group->contentgroup_id)
							->order('order ASC');
      ...
      $group_id = Engine_Api::_()->sitegroup()->getWidgetizedGroup()->page_id;
			$contentTable = Engine_Api::_()->getDbtable('admincontent', 'sitegroup');
			$select = $contentTable->select()
							->where('group_id = ?', $group_id)
							->order('order ASC');
			$content = $contentTable->fetchAll($select);
			$structure = Engine_Api::_()->getDbtable('admincontent', 'sitegroup')->prepareContentArea($content);
			$element = new Engine_Content_Element_Container(array(
  ...
  public function deleteGroup(Sitegroup_Model_Group $group) {
    ...
    Engine_Api::_()->getDbtable('content', 'sitegroup')->delete(array('contentgroup_id = ?' => $group->contentgroup_id));
    $group->delete();
    return $this;
  }
  ...
  public function getContentGroupId($group_id) {
  	...
  	$selectGroupAdmin = $this->select()
            ->from($this->info('name'), array('contentgroup_id', 'description', 'keywords'))
