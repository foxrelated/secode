...
<?php if ($this->direction == 1) { ?>
  ...
  <?php foreach ($this->sitegroups as $sitegroup): ?>
    ...
    ?>	 
  <?php endforeach; ?>
  <?php if ($j < ($this->sponserdSitegroupsCount)): ?>
    <?php for ($j; $j < ($this->sponserdSitegroupsCount); $j++): ?>
...
<?php } else { ?>
  ...
  <?php for ($i = $this->sponserdSitegroupsCount; $i < Count($this->sitegroups); $i++): ?>
    <?php $sitegroup = $this->sitegroups[$i]; ?>
    <?php
    echo $this->partial(
            'list_carousel.tpl', 'sitegroup', array(
        'sitegroup' => $sitegroup,
  ...
  <?php for ($i = 0; $i < $this->sponserdSitegroupsCount; $i++): ?>
    <?php $sitegroup = $this->sitegroups[$i]; ?>
    <?php
    echo $this->partial(
            'list_carousel.tpl', 'sitegroup', array(
