...
<script type="text/javascript">
	...
	function startImporting(file_id) {
	  var confirmation =  confirm('If you want import profile photos also with the group import then use "Upload Photos" link for photos." \n\n If you do not want upload photos or already uploaded then click on "Ok" button otherwise "click" on Cancel button.');
	  if(confirmation == true) {
			Smoothbox.open($('startImporting').innerHTML);
			importfile_id = file_id;
			en4.core.request.send(new Request({
				url : en4.core.baseUrl+'admin/sitegroup/importlisting/data-import?importfile_id='+importfile_id,
				method: 'get',
				data : {
					//'format' : 'json',
				},
	...
	var request = new Request.JSON({
  	'url' : en4.core.baseUrl+'admin/sitegroup/importlisting/stop?importfile_id='+importfile_id,
  	'data' : {
    					'format' : 'json'
	...
	{
		return confirm('<?php echo $this->string()->escapeJavascript($this->translate("Are you sure that you want to delete the selected file entries ? These will not be recoverable after being deleted. Note that deleting them will also delete the corresponding entries which were going to be used to import the Groups from those files.")) ?>');
	}
...
<div id="startImporting" style="display:none;">
	<center class="bold">
		<?php echo $this->translate("Importing groups..."); ?>
	</center>
	<center class="mtop10">
		<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/admin/loader.gif" alt="Importing groups" />
	</center>
	<br />
...
<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'importlisting', 'action' => 'index'), $this->translate('Import a new file'), array('class'=> 'buttonlink icon_sitegroup_admin_import')) ?>

<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'log', 'action' => 'index'), $this->translate('Import History'), array('class'=> 'buttonlink icon_sitegroups_log')) ?><br/><br/>

<?php if($this->paginator->getTotalItemCount()): ?>	

	<h3><?php echo $this->translate("Manage CSV Import Files"); ?></h3>
	<p class="form-description"><?php echo $this->translate('This group contains all the CSV files uploaded by you for importing Groups from them. You can start, stop, rollback and delete the import corresponding to each file now. Rollback will delete all the groups imported from that file. Delete will only delete those entries which were going to be used to import the corresponding Groups from that file and also delete the entry of that file from here. Upload the folder containing profile pictures for the groups uploaded via the csv file by using the ‘Upload Photos” link. Below are the meanings of status for the Files uploaded:'); ?></p> 

	<ul class="importlisting_form_list">

		<li>
			<?php echo $this->translate("Pending: You have not started the group importing from this file. Click on 'start' link and start importing.");?>
		</li>

		<li>
			<?php echo $this->translate("Running: You have started the group importing from this file.");?>
		</li>

		<li>
			<?php echo $this->translate("Stopped: You have stopped the group importing from this file. You can continue it anytime from the same point.");?>
		</li>

		<li>
			<?php echo $this->translate("Completed: Group importing has been done successfully from this file.");?>
		</li>
	...
	<div class="admin_files_groups">
		<?php $groupInfo = $this->paginator->getPages(); ?>
		<?php echo $this->translate("Showing ");?><?php echo $groupInfo->firstItemNumber ?>-<?php echo $groupInfo->lastItemNumber ?><?php echo $this->translate(" of "); ?><?php echo $groupInfo->totalItemCount ?><?php echo $this->translate(" files.")?>
	</div>
				...
				<tr>
					...
					<td width="20%" style="text-align:left;">
						<?php if($item->status != 'Running'): ?>
							...
							<?php endif; ?>
							<?php if($item->status != 'Pending'): ?>
								<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'importlisting', 'action' => 'rollback', 'importfile_id' => $item->importfile_id), $this->translate('Rollback'), array('class' => 'smoothbox')) ?>
								|
							<?php endif; ?>
							<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'importlisting', 'action' => 'delete', 'importfile_id' => $item->importfile_id), $this->translate('Delete'), array('class' => 'smoothbox')) ?>
						<?php else: ?>
							<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'importlisting', 'action' => 'stop', 'importfile_id' => $item->importfile_id, 'forceStop' => 1), $this->translate('Stop')) ?>
						<?php endif;?>
        |
...
<script type="text/javascript" >
  ...
  function uploadphoto(importfile_id) {
    ...
    if(zip_enabled != '') {
      window.location.href = en4.core.baseUrl+'admin/sitegroup/importlisting/upload-photo/importfile_id/'+importfile_id;
    }
    else {
