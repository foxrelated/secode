...
$this->max = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupshow.navigation.tabs', 8);
$headding = "GROUP_NAVIGATION_NAME";
?>
        ...
        <?php foreach ($this->navigation as $nav): ?>			
          <?php if ($key < $this->max): ?>
            ...
            <?php if ($nav->active): echo "class='active'";
              ...
              <?php if ($nav->route == 'sitegroup_general' || $nav->action): ?>
