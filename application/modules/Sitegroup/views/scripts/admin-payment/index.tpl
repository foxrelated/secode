...
<h3><?php echo $this->translate("Group Transactions") ?></h3>
<p>
  <?php echo $this->translate("Browse through the transactions made by users for groups. The search box below will search through the user names, group titles, emails, transaction IDs and order IDs. You can also use the filters below to filter the transactions.") ?>
</p>
...
<?php if( !empty($this->error) ): ?>
  ...
  <br />
<?php return; endif; ?>
  <?php if(Engine_Api::_()->sitegroup()->enablePaymentPlugin()):?>
    ...
    <thead>
      <tr>
        ...
        <th style='width: 1%;' class="<?php echo $class ?>">
         ...
         <?php $class = ( $this->order == 'group_id' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->direction) : '' ) ?>
        <th class="<?php echo $class ?>">
          <a href="javascript:void(0);" onclick="javascript:changeOrder('group_id', 'ASC');">
      ...
      <?php foreach( $this->paginator as $item ):
        ...
        <tr>
          <td><?php echo $item->transaction_id ?></td>
           ...
           <td>
         	<a href="<?php echo $this->url(array('group_url' => Engine_Api::_()->sitegroup()->getGroupUrl($item->group_id)), 'sitegroup_entry_view') ?>"  target='_blank' title="<?php echo  ucfirst($item->title) ?>">
						<?php echo $this->translate(Engine_Api::_()->sitegroup()->truncation($item->getTitle(),25)) ?></a>
           </td>
