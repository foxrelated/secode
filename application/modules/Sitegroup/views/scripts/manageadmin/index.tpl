						...
						<?php foreach ($this->manageHistories as $item):?>
			             ...
			             <?php $url = $this->url(array('action' => 'delete'), 'sitegroup_manageadmins', true);?>
										<?php if ( $this->owner_id != $item->user_id ) :?>
											<a href="javascript:void(0);" onclick="manageinfo('<?php echo $item->manageadmin_id?>','<?php echo $item->getOwner()->getIdentity()?>', '<?php echo $url;?>', '<?php echo $this->group_id ?>')";><?php echo $this->translate('Remove');?></a>
                    <?php endif;?>
									</div>
...
{
	var contentAutocomplete = new Autocompleter.Request.JSON('searchtext', '<?php echo $this->url(array( 'action' => 'manage-auto-suggest', 'group_id' => $this->group_id), 'sitegroup_manageadmins', true) ?>', {
		'postVar' : 'text',
		'minLength': 1,
		'maxChoices': 40,
		'selectMode': 'pick',
