...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
    showDescription('<?php echo $coreSettings->getSetting('sitegroup.description.allow', 1) ?>');
    display_msg=1;
  });

  function showDescription(option) {
    if($('sitegroup_requried_description-wrapper')) {
      if(option == 1) {
        $('sitegroup_requried_description-wrapper').style.display='block';
      } else{
        $('sitegroup_requried_description-wrapper').style.display='none';
      }
    }
  }
