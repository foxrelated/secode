...
<p>
  <?php echo $this->translate('This group lists all the fields which will be displayed to the users in the search form widget at "Groups Home", "Browse Groups" and "My Groups". Below, you can set the sequence of items in the order in which they should appear to users in the search form. To do so, drag-and-drop the items vertically and click on "Save Order" to save the sequence. You can also hide/display the individual fields in this search form.') ?>
</p>
...
<form id='saveorder_form' method='post' action='<?php echo $this->url(array('action' =>'form-search')) ?>' class="clr" style="overflow:hidden;">
								...
								<td>
									...
									<div style="width:20%;" class='admin_table_centered fleft'>
										<?php if($item->display == 1):?>
											<?php   echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroup', 'controller' => 'admin', 'action' => 'diplay-form', 'id' => $item->searchformsetting_id,'display'=>0), $this->htmlImage('application/modules/Sitegroup/externals/images/sitegroup_approved1.gif', '', array('title'=> $this->translate('Hide')))); ?>
										<?php else: ?>
											<?php   echo $this->htmlLink(array('route' => 'default', 'module' => 'sitegroup', 'controller' => 'admin', 'action' => 'diplay-form', 'id' => $item->searchformsetting_id,'display'=>1), $this->htmlImage('application/modules/Sitegroup/externals/images/sitegroup_approved0.gif', '', array('title'=> $this->translate('Display')))); ?>
