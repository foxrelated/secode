...
$string='$ids = Engine_Api::_()->getApi(\'subCore\', \'sitegroup\')->getEveryoneGroupProfileFeeds($about, $this->_getInfo($params));'; ?>
<div id="sitegroupfeed_everyone_dummy-wrapper" class="form-wrapper">
  <div id="sitegroupfeed_everyone_dummy-label" class="tip">
  	<span>
  		<?php echo $this->translate('To show all Activity Feeds on Group Profile irrespective of your setting in Activity Feed Settings, follow ANY ONE of the below 3 options.');?>
  	</span>	
  </div>
	<div class="form-label">&nbsp;</div>
	<div id="sitegroupfeed_everyone_dummy-element" class="form-element">
    <?php $dummyValue=Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupfeed.everyone.dummy', 'c');?>
    <p class="description"><input type="radio" onclick="showTips('tip_everyone_widget','everyone')" value="a" id="sitegroupfeed_everyone_dummy-a" name="sitegroupfeed_everyone_dummy" <?php if($dummyValue=='a'):?> checked ='checked'<?php endif; ?> ><?php echo $this->translate('Use "SocialEngineAddOns Activity Feed" widget instead of core "Activity Feed" widget on Group Profile. (For this, go to "Layout" > "Layout Editor" > "Group Profile" and replace the already placed "Activity Feed" widget with "SocialEngineAddOns Activity Feed" widget. If you have already done this replacement for the above field, then ignore this now.)') ?></p>
    <p class="description"><input type="radio" onclick="showTips('tip_everyone_faq','everyone')" value="b" id="sitegroupfeed_everyone_dummy-b" name="sitegroupfeed_everyone_dummy" <?php if($dummyValue=='b'):?> checked="'checked" <?php endif; ?> >
    <a href="javascript:void(0);" onclick='javascript:$("sitegroupfeed_everyone_dummy-b").checked="checked";showTips("tip_everyone_faq","everyone"); openSmoothboxFeed("<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'settings', 'action' => 'overwrite', 'type' => 'activity_feed_everyone'),'admin_default') ?>");' > <?php echo $this->translate('Click here') ?> </a>
    ...
    <p class="description"><input type="radio" onclick="showTips('tip_everyone_faq','everyone'); faq_show('faq_59');" value="c" id="sitegroupfeed_everyone_dummy-b" name="sitegroupfeed_everyone_dummy"  <?php if($dummyValue=='c'):?> checked="'checked" <?php endif; ?> > <?php echo $this->translate('If you do not want to follow any of the above two options, then you can manually do the minor code modification. (Please see below the steps that you need to follow.)') ?></p>
    ...
    <div id="tip_everyone_faq" style="display: none;" >
      <div class="tip">
        ...
        <?php echo $this->translate("No changes have been done in the activity feed file for the above customization."); ?>
          <?php else:?>
          <?php echo $this->translate("The required changes have been successfully done for displaying all activity feeds on group profiles."); ?>
           <?php endif; ?>
        </span>
      </div>
    </div>    
    <div class='faq' style='display: none;' id='faq_59'>
      <b><?php echo $this->translate('If you do not want to set the value of "Feed Content" setting to "All Members" for User side Activity Feeds. In that also, you can make the display of activity feed on Groups independent of that setting so that group feeds can be seen by all the users. For this, you would have to follow the below steps:');?></b><br /><br />
      ...
      <div class="code">
        <?php echo '// Process ids <br />
                    $ids = array();<br />
                    if ($about->getType() == \'sitegroup_group\'  || $about->getType() == \'sitegroupevent_event\') {<br />
                      $ids = Engine_Api::_()->getApi(\'subCore\', \'sitegroup\')->getEveryoneGroupProfileFeeds($about, $this->_getInfo($params));<br />
                    }<br />
