 ...
 * @version    $Id: _feedLikegroup.tpl 2011-05-05 9:40:21Z SocialEngineAddOns $
...
<?php $dummyValue=Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroupfeed.likegroup.dummy', 'c');?>
<div id="sitegroupfeed_likegroup_dummy-wrapper" class="form-wrapper">    
  <div id="sitegroupfeed_likegroup_dummy-label" class="tip">
  	<span>
  		<?php echo $this->translate("To enable users to receive on their homegroup the activity feeds of Groups that they have liked, follow ANY ONE of the below 3 options.");?>
  	</span>	
  </div>
  <div class="form-label">&nbsp;</div>
  <div id="sitegroupfeed_likegroup_dummy-element" class="form-element">
    <p class="description"><input type="radio" onclick="showTips('tip_like_widget','like')" value="a" id="sitegroupfeed_likegroup_dummy-a" name="sitegroupfeed_likegroup_dummy" <?php if($dummyValue=='a'):?> checked ='checked'<?php endif; ?> > <?php echo $this->translate('Use "SocialEngineAddOns Activity Feed" widget instead of core "Activity Feed" widget on Member Home Group. (For this, go to "Layout" > "Layout Editor" > "Member Home Group" and replace the already placed "Activity Feed" widget with "SocialEngineAddOns Activity Feed" widget.)') ?></p>
    <p class="description"><input type="radio" onclick="showTips('tip_like_faq','like')" value="b" id="sitegroupfeed_likegroup_dummy-b" name="sitegroupfeed_likegroup_dummy" <?php if($dummyValue=='b'):?> checked ='checked'<?php endif; ?> >
    <a href="javascript:void(0);" onclick='javascript:$("sitegroupfeed_likegroup_dummy-b").checked="checked";showTips("tip_like_faq","like"); openSmoothboxFeed("<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'settings', 'action' => 'overwrite', 'type' => 'activity_feed_likegroup'),'admin_default') ?>");' > <?php echo $this->translate('Click here') ?> </a>
    ...
    <p class="description"><input type="radio" onclick="showTips('tip_like_faq','like'); faq_show('faq_37');" value="c" id="sitegroupfeed_likegroup_dummy-c" name="sitegroupfeed_likegroup_dummy" <?php if($dummyValue=='c'):?> checked ='checked'<?php endif; ?> ><?php echo $this->translate('If you do not want to follow any of the above two options, then you can manually do the minor code modification. (Please see below the steps that you need to follow.)') ?></p>
    <div id="tip_like_widget" style="display: none;" >
      <div class="tip">
    <span class="sitegroup_activity_tip">
      <?php if (Engine_Api::_()->getApi('subCore', 'sitegroup')->isCoreActivtyFeedWidget('user_index_home', 'activity.feed')): ?>
      <?php echo $this->translate('The core "Activity Feed" widget is placed on Member Home Group. So, either replace it with "SocialEngineAddOns Activity Feed" widget, or follow one of the other 2 options above.') ?>
      <?php else: ?>
      <?php echo $this->translate('You have not placed any of the activity feed (neither core nor SocialEngineAddOns) widget on the Member Home Group.') ?>
      <?php endif; ?>
    </span>
    </div>
    </div>
      ...
      <div class="tip">
       <span class="sitegroup_activity_tip">
        <?php if (!Engine_Api::_()->getApi('subCore', 'sitegroup')->isContentInFile($path, $string)): ?>
          ...
          <?php else:?>
            <?php echo $this->translate("The required changes have been successfully done for showing Group Activity Feeds to users on Home Group for Groups liked by them."); ?>
           <?php endif; ?>
        </span>
      </div> 
    </div>
    <div class='faq' style='display: none;' id='faq_37'>
      ...
      <?php echo $this->translate('1) First of all, make sure that you have selected "Group\'s Photo and Title" option for the field "Groups Activity Feed Type" above.') ?><br /><br />
