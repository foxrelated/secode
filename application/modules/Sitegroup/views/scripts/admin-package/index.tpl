...
<h3>
  <?php echo $this->translate("Manage Packages for Groups / Communities") ?>
</h3>

<p>
  <?php echo $this->translate('This group allows you to create highly configurable and effective group packages, enabling you to get the best results. You can create both Free and Paid group packages. Below, you can manage existing group packages on your site, or create new ones. You can also set the sequence of group packages in the order in which they should appear to members on the package listing group in the first step of group creation. To do so, drag-and-drop the packages vertically and click on "Save Order" to save the sequence. (Note: You will be able to create a new package over here only if you have enabled Packages from Global Settings.)') ?>
</p>
...
<?php if( !empty($this->error) ): ?>
  ...
  <br />
<?php  endif; ?>
<?php  if (Engine_Api::_()->sitegroup()->hasPackageEnable() && !empty( $this->canCreate)) : ?>
<div>
  <?php echo $this->htmlLink(array('action' => 'create', 'reset' => false), $this->translate('Create New Package'), array(
    'class' => 'buttonlink icon_sitegroup_admin_add',
  )) ?>
</div>
  <?php elseif(!Engine_Api::_()->sitegroup()->hasPackageEnable()):?>
  <div class="tip">
    <span>
    ...
    <thead>
      <tr>
        ...
        <div style='width: 11%;' class='admin_table_centered'>
          <?php echo $this->translate("Total Groups") ?>
        </div>
    ...
    <tbody>
        <tr>
          ...
          <div style="width:7%;" class='admin_table_centered'>
            <?php echo ( $item->enabled ? $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'package', 'action' => 'enabled', 'id' => $item->package_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitegroup/externals/images/enabled1.gif', '', array('title' => $this->translate('Disable Package'))), array('class' => 'smoothbox'))  : $this->htmlLink(array('route' => 'admin_default', 'module' => 'sitegroup', 'controller' => 'package', 'action' => 'enabled', 'id' => $item->package_id), $this->htmlImage($this->layout()->staticBaseUrl.'application/modules/Sitegroup/externals/images/enabled0.gif', '', array('title' => $this->translate('Enable Package')))) ) ?>
          </div>
          ...
          <div style="width:12%;" class='admin_table_options admin_table_centered'>
            <?php  if (Engine_Api::_()->sitegroup()->hasPackageEnable() && !empty( $this->canCreate)) : ?>
            	...
            	|
        		<?php endif;?>
            <a href="javascript:void(0);" onclick="viewGroup(<?php echo $item->package_id ?>)" ><?php echo  $this->translate('View Groups') ?></a>
          </div>
          </td>
        </tr>
    </tbody>
...
<form id='view_selected' method='post' action='<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'viewsitegroup', 'action' => 'index'),'admin_default') ?>'>
