...
?>
<?php $truncation_limit = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.title.truncation', 18);?>
<?php if ($this->direction == 1) { ?>
 <?php  $j=0; $offset=$this->offset; ?>

    <?php foreach ($this->featuredPhotos as $photo): ?>
      <?php if($j% $this->itemsVisible ==0):?>
        <div class="Sitegroupcontent_SlideItMoo_element Sitegroupalbum_SlideItMoo_element" style="width:<?php echo 146 * $this->inOneRow; ?>px;">
          ...
          <?php if ($i < $this->count): ?>
            <div class="featured_thumb_content">
              <a href="<?php echo $this->featuredPhotos[$i]->getHref() ?>"  <?php if ($this->showLightBox): ?> onclick='openSeaocoreLightBox("<?php echo $this->featuredPhotos[$i]->getHref()?>");return false;' <?php endif; ?> title="<?php echo $this->featuredPhotos[$i]->title; ?>" class="thumb_img">
                ...
                $parent = Engine_Api::_()->getItem('sitegroup_album', $this->featuredPhotos[$i]->album_id);
                echo $this->translate('in ').
                     $this->htmlLink($parent->getHref(), $this->string()->truncate($parent->getTitle(),25),array('title' => $parent->getTitle()));
                ?>
                <?php $sitegroup_object = Engine_Api::_()->getItem('sitegroup_group', $this->featuredPhotos[$i]->group_id);?>
								<?php
								$tmpBody = strip_tags($sitegroup_object->title);
								$group_title = ( Engine_String::strlen($tmpBody) > $truncation_limit ? Engine_String::substr($tmpBody, 0, $truncation_limit) . '..' : $tmpBody );
								?>
								<?php echo $this->translate("of ") . $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($this->featuredPhotos[$i]->group_id, $this->featuredPhotos[$i]->user_id, $this->featuredPhotos[$i]->getSlug()),  $group_title,array('title' => $sitegroup_object->title)) ?> 
								<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.postedby', 1)):?>
                <?php echo $this->translate('By ').
                     $this->htmlLink($owner->getHref(), $this->string()->truncate($owner->getTitle(),25),array('title' => $owner->getTitle()));?>
                <?php endif;?>
