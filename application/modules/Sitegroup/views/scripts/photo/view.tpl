...
<div class="layout_middle">
	<div class="sitegroup_viewgroups_head">
		...
		<h2>
		  <?php $link =  $this->htmlLink(array( 'route' => 'sitegroup_entry_view', 'group_url' => Engine_Api::_()->sitegroup()->getGroupUrl($this->sitegroup->group_id), 'tab' => $this->tab_selected_id), $this->translate('Albums')) ?>
...
<?php 
       ...
       <div id="fbsitegroup_photo2">
          <script type="text/javascript">
                    
            var fblike_moduletype = 'sitegroup_photo';
	...
	<div class='sitegroup_photo_view'>
	  <div class="sitegroup_photo_nav">
        ...
        <div id='image_next_div1' style="display:block;">
		      <a href="javascript:void(0);" onclick="photopagination('<?php echo Engine_Api::_()->sitegroup()->getHreflink($this->image->getPrevCollectible()) ?><?php echo "/tab/$this->tab_selected_id"?>')">
		        <?php echo $this->translate('Prev');?>
		      </a>
					<a href="javascript:void(0);" onclick="photopagination('<?php echo Engine_Api::_()->sitegroup()->getHreflink($this->image->getNextCollectible()) ?><?php echo "/tab/$this->tab_selected_id"?>')">
...
<!--FACEBOOK LIKE BUTTON START HERE-->
   ...
   <?php if (!empty($enable_facebookse) ) { ?>
               <div id="fbsitegroup_photo1" style="display:none;">
                 <script type="text/javascript">
...
<div class='sitegroup_photo_container' id='media_image_div'>
  <a id='media_image_next' <?php if($this->album->count()>1):?> onclick="photopagination('<?php echo $this->escape(Engine_Api::_()->sitegroup()->getHreflink($this->image->getNextCollectible())) ?><?php echo "/tab/$this->tab_selected_id"?>')" <?php endif; ?> title="<?php echo $this->image->getTitle();?>">
    <?php
...
<br />
<?php if( $this->canEdit ): ?>
  <div class="group_photo_right_options fright">
      <a class="icon_sitegroup_photos_rotate_ccw" href="javascript:void(0)" onclick="$(this).set('class', 'icon_loading');en4.sitegroup.rotate(<?php echo $this->image->getIdentity() ?>, 90).addEvent('complete', function(){ this.set('class', 'icon_sitegroup_photos_rotate_ccw') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Rotate Left"); ?>" ></a>

      <a class="icon_sitegroup_photos_rotate_cw" href="javascript:void(0)" onclick="$(this).set('class', 'icon_loading');en4.sitegroup.rotate(<?php echo $this->image->getIdentity() ?>, 270).addEvent('complete', function(){ this.set('class', 'icon_sitegroup_photos_rotate_cw') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Rotate Right"); ?>" ></a>

      <a class="icon_sitegroup_photos_flip_horizontal" href="javascript:void(0)" onclick="$(this).set('class', 'icon_loading');en4.sitegroup.flip(<?php echo $this->image->getIdentity() ?>, 'horizontal').addEvent('complete', function(){ this.set('class', 'icon_sitegroup_photos_flip_horizontal') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Flip Vertical"); ?>" ></a>

      <a class="icon_sitegroup_photos_flip_vertical" href="javascript:void(0)" onclick="$(this).set('class', 'icon_loading');en4.sitegroup.flip(<?php echo $this->image->getIdentity() ?>, 'vertical').addEvent('complete', function(){ this.set('class', 'icon_sitegroup_photos_flip_vertical') }.bind(this)); loadingImage();" title="<?php echo $this->translate("Flip Horizontal"); ?>"></a>
...
<div style="overflow:hidden;">
	<?php if ($this->image->getTitle()): ?>
	  <div class="sitegroup_photo_title">
	  <?php echo $this->image->getTitle(); ?>
	  </div>
	<?php endif; ?>
	<?php if ($this->image->getDescription()): ?>
	  <div class="sitegroup_photo_description">
	...
	<div class="sitegroup_photo_date">
	 <?php echo $this->translate('Added'); ?> <?php echo $this->timestamp($this->image->modified_date) ?>
	  <?php if($this->canTag): ?>
		...
		<?php endif; ?>
		<?php $editurl = $this->url(array('action' => 'photo-edit','photo_id' => $this->image->getIdentity(), 'album_id' => $this->album_id, 'group_id' => $this->sitegroup->group_id, 'tab' => $this->tab_selected_id), 'sitegroup_imagephoto_specific', true);?>
		<?php $removeurl = $this->url(array('action' => 'remove','photo_id' => $this->image->getIdentity(), 'album_id' => $this->album_id, 'group_id' => $this->sitegroup->group_id, 'tab' => $this->tab_selected_id), 'sitegroup_imagephoto_specific', true);?>
		<?php if($this->canEdit): ?>
	   ...
	   <?php if( $this->canEdit && SEA_PHOTOLIGHTBOX_MAKEPROFILEPHOTO): ?>
								-
				<a href="javascript:void(0);" onclick="showSmoothbox('<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'photo', 'action' => 'make-group-profile-photo', 'photo' => $this->image->getGuid(), 'group_id' => $this->sitegroup->group_id,'format' => 'smoothbox'), 'sitegroup_imagephoto_specific', true); ?>', 'profilephoto');">
	    		<?php echo $this->translate('Make Group Profile Photo');?>
	      </a>
		<?php endif; ?> 
    <?php if($this->allowFeatured):?>
       	-
		  <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Make Photo of the Day'), array('onclick' => "showSmoothBox('" . $this->escape($this->url(array('module' =>'sitegroup','controller' => 'photo', 'action' => 'add-photo-of-day', 'photo_id' => $this->image->photo_id), 'default', true)) . "'); return false;")) ?>
       -
			<a href="javascript:void(0);"  onclick='featuredgroupalbumPhoto("<?php echo $this->image->photo_id;?>");'><span id="featured_sitegroupalbum_photo" <?php if ($this->image->featured): ?> style="display:none;" <?php endif; ?> title="<?php echo $this->translate("Make Featured"); ?>" ><?php echo $this->translate("Make Featured"); ?> </span> <span id="un_featured_sitegroupalbum_photo" <?php if (!$this->image->featured): ?> style="display:none;" <?php endif; ?> title="<?php echo $this->translate("Make Un-Featured"); ?>" > <?php echo $this->translate("Make Un-featured"); ?> </span></a>  
    <?php endif; ?> 
 ...
 <?php //RENDER FACEBOOK COMMENT WIDGET IF HE HAS ENABLED THIS.
	...
	if(!empty($enable_facebookse)) {
		$success_showFBCommentBox =  Engine_Api::_()->facebookse()->showFBCommentBox ('sitegroupalbum');  
	}
   ...
   include_once APPLICATION_PATH . '/application/modules/Seaocore/views/scripts/_listComment.tpl';
	...
	if ($success_showFBCommentBox) {
		...
		echo $this->content()->renderWidget("facebookse.facebookse-comments", array('module_type' => 'sitegroup_photo', 'curr_url' => $curr_url, 'subject' => $this->subject()->getGuid(), 'task' => 1, 'type' => 'sitegroup_photo', 'id' => $this->image->getIdentity()));
	}
?>
...
var baseY = '0';
 ...
 function getPrevPhoto(){
   return '<?php echo Engine_Api::_()->sitegroup()->getHreflink($this->image->getPrevCollectible()) ?>';
 }
  function getNextPhoto(){
   return '<?php echo Engine_Api::_()->sitegroup()->getHreflink($this->image->getNextCollectible()) ?>';
 }
	...
	<?php if($this->viewer()->getIdentity()):?>
        ...
        taggerInstance = new Tagger('media_image_next', {
			  ...
			  'suggestParam' : <?php if (!empty($this->sitegroupmemberEnabled)): ?><?php echo $this->action('suggest', 'photo', 'sitegroup', array('sendNow' => false, 'includeSelf' => true, 'group_id' => $this->sitegroup->group_id)) ?><?php else: ?><?php echo $this->action('suggest', 'friends', 'user', array('sendNow' => false, 'includeSelf' => true)) ?><?php endif; ?>,
 ...
 window.addEvent('keyup', function(e) { 
  ...
  var loadingImage= function(){
     if(document.getElementById('media_image_div'))
		$('media_image').src = "<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/loader.gif";
