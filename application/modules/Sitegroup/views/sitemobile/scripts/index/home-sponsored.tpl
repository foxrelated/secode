...
<?php } else { ?>
  ...
  <?php for ($i = $count; $i < $this->count; $i++): ?>
    <div class="SlideItMoo_element seaocore_sponsored_carousel_items">
      <div class="seaocore_sponsored_carousel_items_thumb">
        <?php echo $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($this->sitegroupsitegroup[$i]->group_id, $this->sitegroupsitegroup[$i]->owner_id, $this->sitegroupsitegroup[$i]->getSlug()), $this->itemPhoto($this->sitegroupsitegroup[$i], 'thumb.icon', $this->sitegroupsitegroup[$i]->getTitle()), array('rel' => 'lightbox[galerie]', 'class' => "thumb_icon")) ?>
      </div>
      <div class="seaocore_sponsored_carousel_items_info">
        <div class="seaocore_sponsored_carousel_items_title">
          <?php echo $this->htmlLink(Engine_Api::_()->sitegroup()->getHref($this->sitegroupsitegroup[$i]->group_id, $this->sitegroupsitegroup[$i]->owner_id, $this->sitegroupsitegroup[$i]->getSlug()), Engine_Api::_()->sitegroup()->truncation($this->sitegroupsitegroup[$i]->getTitle(), $this->titletruncation), array('title' => $this->sitegroupsitegroup[$i]->getTitle())) ?>
        </div>      
        <?php if($postedBy):?>
          <div class="seaocore_sponsored_carousel_items_stat seaocore_txt_light">
            <?php echo $this->translate('created by'); ?>
