 ...
 * @version    $Id: GroupProfileFieldValueLoop.php 2011-05-05 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitegroup_View_Helper_GroupProfileFieldValueLoop extends Fields_View_Helper_FieldAbstract {

  public function groupProfileFieldValueLoop($subject, $partialStructure, $params = array()) {
    ...
    if (Engine_Api::_()->sitegroup()->hasPackageEnable()) {
      $profileField_level = Engine_Api::_()->sitegroup()->getPackageProfileLevel($subject->group_id);
      if ($profileField_level == 2) {
        $fieldsProfile = Engine_Api::_()->sitegroup()->getProfileFields($subject->group_id);
      }
    } else {
      $group_owner = Engine_Api::_()->getItem('user', $subject->owner_id);
      $profileField_level = Engine_Api::_()->authorization()->getPermission($group_owner->level_id, "sitegroup_group", "profile");
      if ($profileField_level == 2) {
        $fieldsProfile = Engine_Api::_()->sitegroup()->getLevelProfileFields($group_owner->level_id);
      }
    }
