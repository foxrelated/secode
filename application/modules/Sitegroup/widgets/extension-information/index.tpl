 ...
 * @version    $Id: index.tpl (var) 2010-11-18 9:40:21Z SocialEngineAddOns $
...
{
  background-image: url(./application/modules/Seaocore/externals/images/photo.png);
}
ul.seaocores_browse .seaocores_options a.seaocores_type_seaddons
{
  background-image: url(./application/modules/Seaocore/externals/images/seaocore-icon.png);
}
ul.seaocores_browse .seaocores_options a.seaocores_type_se
{
  background-image: url(./application/modules/Seaocore/externals/images/se-icon.png);
}
</style>
<?php 
  $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/styles/slimbox.css'); 
  $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/scripts/slimbox.js');
?>
<h3>
	<?php echo $this->translate("Groups Extension Plugins Information") ?>
</h3>
<br />
      ...
      <form method="post" action="" class="global_form_box" enctype="" id="filter_form" name="form1">
        ...
        <div>  
          ...
          <select id="level_id" name="level_id" onchange="document.form1.submit();">
            <option label="" value="1" <?php if( !empty($all_plugin) ) { echo $all_plugin; } ?>><?php echo $this->translate('All Groups Extensions'); ?> </option>
	...
	<?php 
		...
		?>
			...
			<div class="seaocores_options">
				...
				<?php echo '<a href="'.$item['socialengine_url'].'" class="seaocores_type_se" target="_blank">' . $this->translate('SocialEngine Plugin Group') . '</a>'; ?>
			</div>
