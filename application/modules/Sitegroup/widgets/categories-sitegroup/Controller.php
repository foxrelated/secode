 ...
 */
class Sitegroup_Widget_CategoriesSitegroupController extends Seaocore_Content_Widget_Abstract {

  public function indexAction() {

    $this->_mobileAppFile = true;
     
    $this->view->sitegroup_categories = $sitegroup_categories = Zend_Registry::isRegistered('sitegroup_categories') ? Zend_Registry::get('sitegroup_categories') : null;

    //SET NO RENDER
    if (empty($sitegroup_categories)) {
    ...
    if (Engine_API::_()->seaocore()->checkSitemobileMode('fullsite-mode')) {
      ...
      foreach ($category_info as $value) {
        $sub_cat_array = array();
        $category_info2 = Engine_Api::_()->getDbtable('categories', 'sitegroup')->getAllCategories($value['category_id'], 'subcategory_id', 0, 'subcategory_id', null, 0, 0);
        foreach ($category_info2 as $subresults) {
          $treesubarray = array();
          $subcategory_info2 = Engine_Api::_()->getDbtable('categories', 'sitegroup')->getAllCategories($subresults['category_id'], 'subcategory_id', 0, 'subcategory_id', null, 0, 0);
      ...
      $categoriesAll = Engine_Api::_()->getDbTable('categories', 'sitegroup')->getCategoriesByLevel();
