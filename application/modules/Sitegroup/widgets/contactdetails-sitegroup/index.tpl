...
<?php if ($this->show_phone && isset($this->options_create['phone']) && $this->options_create['phone'] == 'Phone') : ?>    
	<?php if (empty($this->sitegroup->phone) && $this->viewer()->getIdentity() && $this->can_edit) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('phone');" onmouseout="hideImage('phone');">
			<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/icons/mobile.png" alt="Phone" />
			<div class="sitegroup_contect_det">        
				<input type="text" name="phone" value="<?php if(empty($this->sitegroup->phone)): ?><?php echo $this->translate("Contact Number");?><?php else:?><?php echo $this->sitegroup->phone ?><?php endif;?>" id="phone" onblur="saveContactDetails('phone')" onclick="onFocus('phone')" />   
	...
	<?php elseif (!empty($this->sitegroup->phone)) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('phone');" onmouseout="hideImage('phone');">
			<img src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sitegroup/externals/images/icons/mobile.png" alt="Phone" />
			<div class="sitegroup_contect_det">
				<input type="text" name="phone" value="<?php echo $this->sitegroup->phone ?>" id="phone" onblur="saveContactDetails('phone')" style="display:none;" onclick="onFocus('phone')" />  
				<div id="showPhoneNumber" style="display:block;"><?php echo $this->sitegroup->phone ?></div>     
			</div>
			<?php if ($this->can_edit) : ?>	
				<div class="edit_icon" id="phoneimage" style="display:none;" title="<?php echo $this->translate('edit');?>">
					<a href="javascript:void(0);" onclick="onFocus('phone')"></a>
...
<?php if ($this->show_email && isset($this->options_create['email']) && $this->options_create['email'] == 'Email') : ?>   
	<?php if (empty($this->sitegroup->email) && $this->viewer()->getIdentity() && $this->can_edit) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('email');" onmouseout="hideImage('email');">
			<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/invite.png" alt="E-mail" />
			<div class="sitegroup_contect_det">
				<input type="text" name="email" value="<?php if(empty($this->sitegroup->email)): ?><?php echo $this->translate("Email ID");?><?php else:?><?php echo $this->sitegroup->email ?><?php endif;?>" id="email" onblur="saveContactDetails('email')" onclick="onFocus('email')" />
				<div id="showEmailAddress" style="display:none;"></div>
			</div>
			<?php if ($this->can_edit) : ?>
				<div id="showerrormessage" style="display:none;" class="sitegroup_contect_error">  </div> 
	...
	<?php elseif (!empty($this->sitegroup->email)) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('email');" onmouseout="hideImage('email');">
			<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/invite.png" alt="E-mail" />
			<div class="sitegroup_contect_det">
				<input type="text" name="email" value="<?php echo $this->sitegroup->email ?>" id="email" onblur="saveContactDetails('email')"  style="display:none;" onclick="onFocus('email')" />
				<div id="showEmailAddress">
				  <?php if (empty($this->emailme)) :?>
					<a href='mailto:<?php echo $this->sitegroup->email ?>'><?php echo $this->translate('Email Me') ?></a>
			    <?php else: ?>
					<a href="javascript:void(0);" onclick="showSmoothBox('<?php echo $this->escape($this->url(array('route' => 'sitegroup_profilegroup', 'module' => 'sitegroup', 'controller' => 'profile', 'action' => 'email-me', "id" => $this->sitegroup->group_id), 'default' , true)); ?>'); return false;"><?php echo $this->translate('Email Me') ?></a>
					<?php endif; ?>
...
<?php if ($this->show_website && isset($this->options_create['website']) && $this->options_create['website'] == 'Website') : ?>    
	<?php if (empty($this->sitegroup->website) && $this->viewer()->getIdentity() && $this->can_edit) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('website');" onmouseout="hideImage('website');">
			<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/icons/web.png" alt="Website" />
			<div class="sitegroup_contect_det">          
				<input type="text" name="website" value="<?php if(empty($this->sitegroup->website)): ?><?php echo $this->translate("Website");?><?php else:?><?php echo $this->sitegroup->website ?><?php endif;?>" id="website" onblur="saveContactDetails('website')" onclick="onFocus('website')" />        
	...
	<?php elseif (!empty($this->sitegroup->website) ) : ?>
		<div class="sitegroup_contect_field" onmouseover="displayImage('website');" onmouseout="hideImage('website');">
			<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/icons/web.png" alt="Website" />
			<div class="sitegroup_contect_det">
				<input type="text" name="website" value="<?php echo $this->sitegroup->website ?>" id="website" onblur="saveContactDetails('website')"  style="display:none;" onclick="onFocus('website')" />          
				<div id="showWebsite">   
					<?php if (strstr($this->sitegroup->website, 'http://') || strstr($this->sitegroup->website, 'https://')): ?>
						<a href='<?php echo $this->sitegroup->website ?>' target="_blank" title='<?php echo $this->sitegroup->website ?>' ><?php echo $this->translate(''); ?> <?php echo $this->translate('Visit Website') ?></a>
					<?php else: ?>
						<a href='http://<?php echo $this->sitegroup->website ?>' target="_blank" title='<?php echo $this->sitegroup->website ?>' ><?php echo $this->translate(''); ?> <?php echo $this->translate('Visit Website') ?></a>
					<?php endif; ?>
				</div>
			</div>
			<?php if ($this->can_edit) : ?>
...
<script type="text/javascript">
  ...
  function saveContactDetails(blurid) {
    if(blurid == 'phone') {     
      if($('phoneimage'))
        $('phoneimage').src = "<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/edit.png";
    } 
    else if(blurid == 'email') {     
      if(validateEmail($('email').value)){
        if($('emailimage'))
          $('emailimage').src = "<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/edit.png";
      } else {
        if($('showerrormessage')) {
      ...
      if($('websiteimage')) {
        $('websiteimage').src = "<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitegroup/externals/images/edit.png";
      }      
    }
    ...
    en4.core.request.send(new Request.HTML({
      url :'<?php echo $this->url(array('module' => 'sitegroup', 'controller' => 'profile', 'action' => 'contact-detail'), 'default', true) ?>',
      data : {
        format : 'html',
        phone : phone,
        email : email,
