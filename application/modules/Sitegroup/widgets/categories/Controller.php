 ...
 */
class Sitegroup_Widget_CategoriesController extends Engine_Content_Widget_Abstract {

  public function indexAction() {

    //START NETWORK WORK
    $sitegroup_is_category = Zend_Registry::isRegistered('sitegroup_is_category') ? Zend_Registry::get('sitegroup_is_category') : null;
    //SET NO RENDER
    if (empty($sitegroup_is_category)) {
    ...
    if ($this->view->displayNotAll) {
      $category_info = Engine_Api::_()->getDbtable('categories', 'sitegroup')->getAllCategories(0, 'category_id', $displayOnlyUsefulGroups, 'group_id', 1, 0);
      foreach ($category_info as $value) {
        $sub_cat_array = array();
				if (!empty($show2ndlevelCategory)) {
					$category_info2 = Engine_Api::_()->getDbtable('categories', 'sitegroup')->getAllCategories($value['category_id'], 'subcategory_id', $displayOnlyUsefulGroups, 'subcategory_id', null, 0);
					foreach ($category_info2 as $subresults) {
						if (!empty($show3rdlevelCategory)) {
							$subcategory_info2 = Engine_Api::_()->getDbtable('categories', 'sitegroup')->getAllCategories($subresults['category_id'], 'subsubcategory_id', $displayOnlyUsefulGroups, 'subsubcategory_id', null, 0);
