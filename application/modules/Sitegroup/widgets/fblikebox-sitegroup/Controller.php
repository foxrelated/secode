...
class Sitegroup_Widget_FblikeboxSitegroupController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  { 
        
				//DON'T RENDER IF SUNJECT IS NOT THERE
				if (!Engine_Api::_()->core()->hasSubject() || !Engine_Api::_()->getApi('settings', 'core')->getSetting('sitegroup.postfbgroup', 1)) {
					return $this->setNoRender();
				}
				
				//GET SITEGROUP SUBJECT
				$this->view->sitegroup = $sitegroup = Engine_Api::_()->core()->getSubject('sitegroup_group'); 
				//print_r($sitegroup);die;
				if (!$sitegroup || empty($sitegroup->fbpage_url)) {
					return $this->setNoRender();
				}
				
				$this->view->fbpage_url = $sitegroup->fbpage_url;
