<?php
class Ynmultilisting_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Ynmultilisting_Model_Order $order, array $params = array())
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Listing'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Listing from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buy Listing'),
  ...
  public function onPackageTransactionReturn(Ynmultilisting_Model_Order $order, array $params = array())
  {
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'ynmultilisting') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'ynmultilisting') -> findRow($info['PAYMENTREQUESTID']);
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Ynmultilisting_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       }
