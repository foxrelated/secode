...
class Ynmultilisting_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynmultilisting_Model_Node';
	...
	public function updateTree($node = null) {
		...
		if (!is_null($node)){
			$sql = "update $table set level = (select count(parent.title) from (select * from $table where listingtype_id = {$node->listingtype_id}) as parent where $table.pleft BETWEEN parent.pleft and parent.pright)-1 where listingtype_id = {$node->listingtype_id}";
		}
	...
	/**
	 * @return Ynmultilisting_Model_Node
	 */
	public function getNode($node_id, $check_root =true, $listingtype_id = 0) {
		$select = $this -> select() -> where("category_id = ?", $node_id );
		if ($listingtype_id != 0)
		{
			$select = $select -> where("listingtype_id = ?", $listingtype_id );
		}
	...
	public function getRoot() {
		$row = $this -> fetchRow($this -> select() -> where('pleft=1'));
		if(!$row) {
	 ...
	 * @param  $parent_id
	 * @return Ynmultilisting_Model_Node|NULL
	 */
	 ...
	 * @param   Ynmultilisting_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynmultilisting_Model_Node
	 * @throw Exception
	 */
	public function insertAfter(Ynmultilisting_Model_Node $node, $data) {
		if($node -> pleft == 1) {
		...
		try {
			$db -> beginTransaction();
			if (isset($data['listingtype_id']))
			{
				$db -> update($tableName, array('pright' => new Zend_Db_Expr('pright+2'), ), array('pright > ?' => $anchor, 'listingtype_id = ?' => $data['listingtype_id']));
			}
			else 
			{
			...
			if (isset($data['listingtype_id']))
			{
				$db -> update($tableName, array('pleft' => new Zend_Db_Expr('pleft+2'), ), array('pleft > ?' => $anchor, 'listingtype_id = ?' => $data['listingtype_id']));
			}
			else
			{
			...
			if(!empty($data['image']))
			{
				$newNode -> setImage($data['image']);
			}
			$this -> updateTree($newNode);
			$optionId = Engine_Api::_()->getApi('core', 'ynmultilisting')->typeCreate($newNode->title);
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynmultilisting_Model_Node $node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
		...
		try {
			...
			if (isset($data['listingtype_id']))
			{
				$db -> update($tableName, array('pleft' => new Zend_Db_Expr('pleft+2'), ), array('pleft > ?' => $anchor - 1, 'listingtype_id = ?' => $data['listingtype_id']));
			}
			else 
			{
	 ...
	 * @throw Exception
	 */
	public function appendChild(Ynmultilisting_Model_Node $node, $data) {
		...
		try {
			...
			if (isset($data['listingtype_id']))
			{
				$db -> update($tableName, array('pright' => new Zend_Db_Expr('pright+2'), ), array('pright > ?' => $anchor - 1, 'listingtype_id = ?' => $data['listingtype_id']));
			}
			else 
			{
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynmultilisting_Model_Node $node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynmultilisting_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function deleteNode(Ynmultilisting_Model_Node $node, $node_id = NULL) {
		...
		try {
			$db -> beginTransaction();
			$db -> delete($tableName, array('pright <= ?' => $node -> pright, 'pleft >= ?' => $node -> pleft, 'listingtype_id = ?' => $node->listingtype_id));
			$db -> update($tableName, array('pright' => new Zend_Db_Expr('pright-' . $range), ), array('pright > ?' => $anchor, 'listingtype_id = ?' => $node->listingtype_id));
			$db -> update($tableName, array('pleft' => new Zend_Db_Expr('pleft-' . $range), ), array('pleft > ?' => $anchor, 'listingtype_id = ?' => $node->listingtype_id));
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynmultilisting_Model_Node) {
