<?php
class Ynmultilisting_Model_Topic extends Core_Model_Item_Abstract
{
  protected $_searchTriggers = false;  
  protected $_parent_type = 'ynmultilisting_listing';
  protected $_owner_type = 'user';
  protected $_children_types = array('ynmultilisting_post');
  ...
  {
    $table = Engine_Api::_()->getItemTable('ynmultilisting_post');
    $select = $table->select()
      ->where('topic_id = ?', $this->getIdentity())
      ->order('post_id DESC')
      ->limit(1);
  ...
  protected function _delete()
  {
	...
	// Delete topicwatch
    $topicWatchTable = Engine_Api::_()->getDbTable('topicWatches','ynmultilisting');
  ...
  {
    return $this->getParent()->authorization()->isAllowed($user, 'edit') || $this->getParent()->isAllowed('dicussion') || $this->isOwner($user);
  }
}