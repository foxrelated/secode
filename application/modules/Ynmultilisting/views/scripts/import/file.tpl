...
var auth = {};
var url = '<?php echo $this->url(array('action' => 'file-one-by-one'), 'ynmultilisting_import', true)?>';
...
function import_process() {
    if (import_status && !cancel && !rollback && (max_import > 0 || unlimit == 1) && listings.length > 0) {
        ...
        if (import_count > 0 && !rollback) {
            ...
            new Request.JSON({
                url: '<?php echo $this->url(array('action' => 'file-history'), 'ynmultilisting_import', true)?>',
                method: 'post',
                data: {
                    'listings': JSON.encode(listings_imported),
                    'filename': filename
...
<p style="line-height: 1.5em;"><?php echo $this->translate("YNMULTILISTING_IMPORT_DESCRIPTION") ?></p>
