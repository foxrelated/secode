...
<?php if ($this->paginator->getTotalItemCount() > 0): ?>
    <ul class="ynmultilisting_videos_manage videos_manage">
        <?php foreach ($this->paginator as $item): ?>
            <li>
                <div class="ynmultilisting_thumb_wrapper video_thumb_wrapper">
                    <?php if ($item->duration): ?>
                        <?php echo $this->partial('_video_duration.tpl','ynmultilisting', array('video' => $item)) ?>
                    <?php endif; ?>
                    <?php
	                    if ($item->photo_id) {
	                        echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumb.normal'));
	                    } else {
	                        echo '<img alt="" src="' . $this->layout()->staticBaseUrl . 'application/modules/Ynmultilisting/externals/images/video.png">';
	                    }
                    ?>
                </div>
                ...
                <div class="video_info video_info_in_list">
                    <div class="ynmultilisting_title">
                        <?php 
	                         $title = Engine_Api::_()->ynmultilisting()->subPhrase($item->getTitle(),70);
				             if($title == '') $title = "Untitle Video";
	                         echo $this->htmlLink($item->getHref(), htmlspecialchars($title));
                         ?>
                    </div>
                    <div class="video_desc ynmultilisting_block">
                            <?php echo $this->string()->truncate($this->string()->stripTags($item->description), 300) ?>
                    </div>
                     <p class="thumbs_info">
			          <?php echo $this->translate('By');?>
			          <?php if($item->owner_id != 0 ){
			              $name = Engine_Api::_()->ynmultilisting()->subPhrase($item->getOwner()->getTitle(),18);
		...
		<br/>
		<div class="ynmultilisting_pages">
