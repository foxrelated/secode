...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
	...
	<?php if(!empty($this->switchCategory)) :?>
	<?php if(empty($this->theme)) :?>
	if($$('.item-form-theme-choose').length > 0)
		$$('.item-form-theme-choose input')[0].setProperty('checked', 'true');
	<?php endif; ?> 
  ...
  en4.core.runonce.add(function()
  {
    new Autocompleter.Request.JSON('tags', '<?php echo $this->url(array('controller' => 'tag', 'action' => 'suggest'), 'default', true) ?>', {
      ...
      new Autocompleter.Request.JSON('to', '<?php echo $this->url(array('controller' => 'video', 'action' => 'suggest'), 'ynmultilisting_extended', true) ?>', {
       	'postVar' : 'text',
        'minLength': 1,
        'delay' : 250,
        'selectMode': 'pick',
...
<script type="text/javascript">
 var pageAction =function(page){
  ...
  function initialize() {
        ...
        document.getElementById('location').value = document.getElementById('location_address').value;
	}
