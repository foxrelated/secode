...
<script type="text/javascript">
  ...
  var removeWidget = function(name, element) {
    var url = '<?php echo $this->url(array('action' => 'removewidget', 'controller' => 'layout', 'module' => 'ynmultilisting', 'listingtype_id' => $this -> subjectId), 'admin_default', true)?>';
  ...
  var loadPage = function(page_name) {
    if( confirmPageChangeLoss() ) {
      window.location.search = '?page=' + page_name;
 ...
 /* Save current page changes */
  var saveChanges = function()
  {
    ...
    var url = '<?php echo $this->url(array('action' => 'update', 'controller' => 'layout', 'module' => 'ynmultilisting'), 'admin_default', true)?>';
    var request = new Request.JSON({
      'url' : url,
      'data' : {
        'format' : 'json',
        'page_id': pageId,
        'listingtype_id': subjectId,
  ...
  var openWidgetParamEdit = function(name, element) 
  {
    ...
    var url = '<?php echo $this->url(array('action' => 'widget', 'controller' => 'layout', 'module' => 'ynmultilisting', 'listingtype_id'=> $this -> subjectId), 'admin_default', true)?>';
  ...
  var saveCurrentPageInfo = function(formElement) {
    var url = '<?php echo $this->url(array('action' => 'save', 'controller' => 'layout', 'module' => 'ynmultilisting', 'listingtype_id' => $this -> listingtype -> getIdentity()), 'admin_default', true)?>';
  ...
  var resetCurrentPage = function() {
    ...
    var url = '<?php echo $this->url(array('action' => 'reset', 'controller' => 'layout', 'module' => 'ynmultilisting'), 'admin_default', true)?>';
    var request = new Request.JSON({
      'url' : url,
      'data' : {
        'format' : 'json',
...
<?php endif; ?>
<h3><?php echo $this -> translate("Edit Listing Type Pages Layout")." - ";?><a href="<?php echo $this -> listingtype -> getHref()?>"><?php echo $this -> listingtype -> getTitle()?></a></h3>
<p>
      ...
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_openpage">
        <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_pages_wrapper" id="admin_layoutbox_menu_pages_wrapper">
          <ul class="admin_layoutbox_menu_pages">
            <?php foreach( $this->pageList as $page ): ?>
                <li onclick="loadPage('<?php echo $page->name ?>');">
                  <?php echo str_replace($this->translate('Multiple Listings'), $this->translate($this->listingtype->getTitle()), $page->displayname) ?>
                </li>
        <?php endforeach; ?>
          </ul>
        </div>
        <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), $(this));">
          <span><?php echo $this->translate("Editing:"); ?></span>
            <?php echo str_replace($this->translate('Multiple Listings'), $this->translate($this->listingtype->getTitle()), $this->pageObject->displayname) ?><span class="more">&nbsp;</span>
        </a>
      </li>
                        ...
                        ?>
                          ...
                          <?php if( empty($structThreeInfo) ): // Missing widget ?>
                            ...
                            <li id="admin_content_<?php echo $structThree['identity'] ?>" class="disabled admin_content_cell admin_content_buildable <?php echo $draggable?> admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                              ...
                              <span class="remove"><a href='javascript:void(0)' onclick="removeWidget('<?php echo $structThree['name'] ?>',$(this));">x</a></span>
                            </li>
