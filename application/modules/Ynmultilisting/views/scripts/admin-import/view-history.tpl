  ...
  <tbody>
    <?php foreach ($this->paginator as $item): ?>
      ...
      <tr>
		...
		<td><?php echo (!empty($item->file_name)) ? $item->file_name : '-'?></td>
		<td>
		<?php if (!empty($item->module_id)):?>
		<?php $module = Engine_Api::_()->getItem('ynmultilisting_module', $item->module_id);?>
		<?php echo ($module) ? $this->translate($module->getTitle()) : $this->translate('Unknown')?>
		<?php else:?>
		-
		<?php endif;?>
		</td>
        <td>
          <?php echo $this->htmlLink(
                array('route' => 'admin_default', 'module' => 'ynmultilisting', 'controller' => 'import', 'action' => 'view-listing', 'id' => $item->getIdentity()),
