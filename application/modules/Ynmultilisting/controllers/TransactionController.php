<?php
class Ynmultilisting_TransactionController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'ynmultilisting') -> findRow($order_id);
			}
			else
			{
				$this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'ynmultilisting_transaction', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> ynmultilisting();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'ynmultilisting') -> findRow($orderId)) 
	...
	public function finishAction()
	{
		...
		if ($status == 'completed')
		{
			$url = $this -> view -> escape($this -> view -> url(array('id' => $listing -> getIdentity(), 'slug' => $listing -> getSlug()), 'ynmultilisting_profile', true));
		}
		else
		{
			//redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'ynmultilisting_general', true));
