<?php
class Ynmultilisting_AdminTransactionsController extends Core_Controller_Action_Admin {
    public function indexAction() {
        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
        ->getNavigation('ynmultilisting_admin_main', array(), 'ynmultilisting_admin_main_transactions');
        ...
        $select -> setIntegrityCheck(false) 
       			-> joinLeft("$listingTblName as listing", "listing.listing_id = transaction.item_id", "");
				
        $methods = array();
        
        $this->view->form = $form = new Ynmultilisting_Form_Admin_Transactions_Search();
		...
		if ($values['name'] != '') {
            $select->where('listing.title LIKE ?', '%'.$values['name'].'%');
        }
	    ...
	    if (isset($values['order'])) {
	        if (empty($values['direction'])) {
	            $values['direction'] = ($values['order'] == 'listing.name') ? 'ASC' : 'DESC';
	        }
