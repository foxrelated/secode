...
class Ynmultilisting_Api_Mail {
	 ...
	 */
  	public function getTemplate($type, $locale = 'en') {
		...
		$table = Engine_Api::_()->getDbtable('MailTemplates', 'ynmultilisting');
    ...
    $mailTemplateTable = Engine_Api::_()->getDbtable('MailTemplates', 'ynmultilisting');
	...
	$vars = array();
    $params = Engine_Api::_()->getApi('ConvertMailVars', 'ynmultilisting')->process($params, $vars, $type);
    ...
    foreach( $recipients as $recipient ) {
	  ...
	  //send html
	  if(in_array($type, array('ynmultilisting_subscribe_listing')))
	  {
	  	$view = Zend_Registry::get('Zend_View');
	  	$bodyHtmlTemplate = $bodyHtmlContent;
	  	$subjectTemplate = $view -> translate ('Subscribe Listings from ') . $view -> layout() -> siteinfo['title'];
		  // Send
	      $mail = $this->create()
  ...
  public function sendRaw($mail) {
	...
	public function sendMailQueue($send_limit =  NULL){

		if($send_limit==0){
			$send_limit =  Engine_Api::_()->getApi('settings', 'core')->getSetting('core.mail.count', 25);
		}
		
		$table =Engine_Api::_()->getDbTable('emails','ynmultilisting');
		$select =  $table->select()->where('sended=0')->order('priority desc')->limit($send_limit);
		foreach($table->fetchAll($select) as $item){			
			 $mail = $this->create()
	        ->addTo($item->send_to, $item->to_name)
	        ->setFrom($item->send_from, $item->from_name)
