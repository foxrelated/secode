  ...
  $saveFeedTable =  Engine_Api::_() -> getDbTable('saveFeeds', 'ynfeed');
                ...
                <div class='feed_item_date feed_item_icon <?php echo $icon_type ?> yncomment_feed_item_date'>
                   ...
                   $title = $this->translate('Comment');
                        ...
                        <li class="feed_item_option_remove_tag">
                            <a title="<?php echo $this->translate('Remove Tag');?>" href="javascript:void(0);" title="" onclick="removeTag('<?php echo $action->action_id ?>')">
                                <?php echo $title;?></a>
                        </li>
                        <?php endif ?>
                        <!-- // End remove tag -->
                        ...
                        <li class="feed_item_option_remove_preview">
                            <a title="<?php echo $this->translate('Remove preview') ?>" href="javascript:void(0);" title="" onclick="removePreview('<?php echo $action->action_id ?>')">
                                <?php echo $title ?></a>
                        </li>
                        <?php endif ?>
             ...
             if(count($action->getAttachments()) > 0)
                ...
                <?php if ($action->likes()->getLikeCount() > 0 && (count($action->likes()->getAllLikesUsers()) > 0)): ?>
                        <span class="comments_likes">
                            ...
                            <?php echo $this->translate(array('%s like this.', '%s likes this.', $action->likes()->getLikeCount()), $this->feedNCFluentList($action->likes()->getAllLikesUsers(), false, $action)) ?>
                            <?php else:?>
                            <?php echo $this->translate(array('%s likes this.', '%s like this.', $action->likes()->getLikeCount()), $this->feedNCFluentList($action->likes()->getAllLikesUsers(), false, $action)) ?>
                            <?php endif;?>
                        </span>
                <?php endif; ?>
                ...
                <?php endif;?>
                          <span class="comments_likes">
                            <i class="fa fa-thumbs-down ynfontawesome"></i>
                              ...
                              <?php echo $this->translate(array('%s dislike this.', '%s dislikes this.', $this->dislikes->getTotalItemCount()), $this->feedFluentDisLikeList($action->dislikes()->getAllDislikesUsers(), false, $action)) ?>
                              <?php else:?>
                              <?php echo $this->translate(array('%s dislikes this.', '%s dislike this.', $this->dislikes->getTotalItemCount()), $this->feedFluentDisLikeList($action->dislikes()->getAllDislikesUsers(), false, $action)) ?>
                              <?php endif;?>
                          </span>
                <?php else:?>
                          <span class="comments_likes">
    ...
    <li id="comment-<?php echo $comment->comment_id ?>" class="yncomment_nestcomment <?php if($openHide) echo 'yncomment_hidden_open'?>">
        ...
        <div class="comments_info">
            ...
            <div id="comment_edit_<?php echo $comment->comment_id ?>" class="mtop5 comment_edit <?php if (!$this->commentShowBottomPost): ?>yncomment_press_enter<?php endif; ?>" style="display: none;">
                <?php include APPLICATION_PATH . '/application/modules/Yncomment/views/scripts/_editComment.tpl' ?>
            </div>
            <?php if($this->viewer()->getIdentity()):?>
            <div id="close_edit_box-<?php echo $comment->comment_id;?>" class="yncomment_txt_light f_small comment_close" style="display:none;">
                <a class="yncomment_cancel_edit" href='javascript:void(0);' onclick="closeEdit('<?php echo $comment->getIdentity() ?>');">
                    <i class="fa fa-times ynfontawesome"></i>
                    <?php echo $this->translate('Cancel edit');?></a>
            </div>
            <?php endif; ?>
            ...
            <ul class="comments_date">
                ...
                <li>
                    <span>&nbsp;-&nbsp;</span>
                    <a title="<?php echo $this->translate('Remove preview') ?>" href="javascript:void(0)" onclick="en4.ynfeed.removePreview(this, <?php echo sprintf("'%d', %d", $action->getIdentity(), $comment->getIdentity()) ?>);" action-title="<?php echo $this->translate('Remove Preview') ?>">
                    <?php if($showLikeWithoutIconInReplies != 1):?>
                    <i class="fa fa-times ynfontawesome"></i>
                    <?php endif ?>
                    <?php if($showLikeWithoutIconInReplies != 2):?>
        ...
        <?php
                  $replies = $action->getReplies($comment->comment_id);
        if(count($replies)):?>
            ...
            <ul class="yncomment_reply">
                ...
                <div class="comments_info">
                    ...
                    <div id="close_edit_box-<?php echo $reply->comment_id;?>" class="yncomment_txt_light f_small comment_close" style="display:none;">
                        <a class="yncomment_cancel_edit" href='javascript:void(0);' onclick="closeReplyEdit('<?php echo $reply->getIdentity() ?>');">
                            <i class="fa fa-times ynfontawesome"></i>
                            <?php echo $this->translate(' Cancel edit');?></a>
                    </div>
                    <?php endif; ?>
...
<li id='feed-reply-form-open-li_<?php echo $comment->comment_id ?>' onclick='showReplyBox("<?php echo $replyFormId?>", "<?php echo $replyFormBodyId?>");' <?php echo '
                                document.getElementById("' . $this->replyForm->submit->getAttrib('id') . '").style.display = "' . ((!$this->commentShowBottomPost || Engine_Api::_()->getApi('settings', 'core')->core_spam_comment) ? "block" : "none") . '";
document.getElementById("feed-reply-form-open-li_' . $comment->comment_id . '").style.display = "none"; $$(".swiff-uploader-box").each(function(e){e.title = en4.core.language.translate("Attach a Photo");});
document.getElementById("' . $this->replyForm->body->getAttrib('id') . '").focus(); $("'.$this->replyForm->getAttrib('id').'").addClass("yncommnet_click_textarea");' ?>' <?php if (!$this->commentShowBottomPost || Engine_Api::_()->getApi('settings', 'core')->core_spam_comment): ?> style="display:none;"<?php endif; ?> style="display:none;">                               <div></div>
...
<li id='feed-comment-form-open-li_<?php echo $actionBaseId ?>' onclick='<?php echo '
    document.getElementById("' . $this->commentForm->submit->getAttrib('id') . '").style.display = "' . ((!$this->commentShowBottomPost || Engine_Api::_()->getApi('settings', 'core')->core_spam_comment) ? "block" : "none") . '";
document.getElementById("feed-comment-form-open-li_' . $actionBaseId . '").style.display = "none"; $$(".swiff-uploader-box").each(function(e){e.title = en4.core.language.translate("Attach a Photo");});
document.getElementById("' . $this->commentForm->body->getAttrib('id') . '").focus(); $("'.$this->commentForm->getAttrib('id').'").addClass("yncommnet_click_textarea");'?> showCommentBox("<?php echo $commentFormId?>", "<?php echo $commentFormId?>");' <?php if (!$this->commentShowBottomPost || Engine_Api::_()->getApi('settings', 'core')->core_spam_comment): ?> style="display:none;"<?php endif; ?> >                  <div></div>
