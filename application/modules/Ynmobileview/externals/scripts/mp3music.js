...
(function($) {
  var SCROLL_ITEM = 5;
	...
	function mejsPlayNext(currentPlayer, audio) {
		var current_item = '';
		var audio_src = '';
		var mp3Container = $(audio).closest('.ynmobileviewmusic_container');
  ...
  // Preview button
	MediaElementPlayer.prototype.buildprev = function(player, controls, layers, media) {
		var mp3Container = $(controls).closest('.ynmobileviewmusic_container');
		var
  ...
  // Get an random number
	...
	function getNextSong(current_item) {//get next li to get song
		...
		var li = ul.find('li').size();
		var mp3Container = $(current_item).closest('.ynmobileviewmusic_container');
	...
	$(document).ready(function() {
		mp3ContainerList = $('.ynmobileviewmusic_container');
		...
		if($('.ynmobileview-music').next().find("li").size() == 1) {
		...
		$('.ynmobileview-music audio').mediaelementplayer({
        ...
        mediaElement.addEventListener('loadeddata', function(e) {
        	 var currentItem = $(domObject).closest('.ynmobileviewmusic_container').find('li.current:last');
				...
				if($(this).parent().find("li").size() == 1) 
				{
					$(this).find('audio').mediaelementplayer({
						success : function(mediaElement, domObject) 
						{
							...
							mediaElement.addEventListener('loadeddata', function(e) 
							{
								var currentItem = $(domObject).closest('.ynmobileviewmusic_container').find('li.current');
								_changePlayCount(currentItem.find('.song_id').text());
							}, false);
		...
		// Select a song
		$('.mp3music-song-title').click(function(){
			var mp3Container = $(this).closest('.ynmobileviewmusic_container');
	...
	$('.mp3music_expand_thumb').click(function()
	{
