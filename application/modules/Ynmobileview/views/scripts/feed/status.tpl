...
<ul class='feed ynmb_feedDetail_activityFeed' id="activity-feed">
<?php
  ...
  <li id="activity-item-<?php echo $action->action_id ?>" data-activity-feed-item="<?php echo $action->action_id ?>">
    ...
    <div class='feed_item_photo ynmb_feedDetail_item_photo'><?php echo $this->htmlLink($action->getSubject()->getHref(),
      $this->itemPhoto($action->getSubject(), 'thumb.icon', $action->getSubject()->getTitle())
    ) ?></div>

    <div class='feed_item_body ynmb_feedDetail_item_body'>
      ...
      <?php if( $action->getTypeInfo()->attachable && $action->attachment_count > 0 ): // Attachments ?>
        <div class='feed_item_attachments feed_attachment_<?php echo $attachment->meta->type ?> <?php if(count($action->getAttachments()) > 1 && (in_array(current($action->getAttachments())->item->getType(), array('album_photo', 'advalbum_photo', 'advgroup_photo', 'event_photo', 'ynevent_photo', 'group_photo')))) echo "ynmb_feed_manyphotos";?>'>
          <?php if( $action->attachment_count > 0 && count($action->getAttachments()) > 0 ): ?>
            <?php if( count($action->getAttachments()) == 1 &&
			  ...
			  {
			  	 $richContent = Engine_Api::_()->ynmobileview()->getMusicRichContect(current($action->getAttachments())->item, $this->play);
			  } 
              echo $richContent; ?>
            <?php else: ?>
              <?php foreach( $action->getAttachments() as $attachment ): 
              	 $thumb_main = 'thumb.main';
              	 if(count($action->getAttachments()) > 1 && in_array($attachment->meta->type, array('album_photo', 'event_photo', 'group_photo', 'advalbum_photo', 'ynevent_photo', 'advgroup_photo')))
					{
                ...
                <?php elseif( $attachment->meta->mode == 2 ): // Thumb only type actions ?>
                  <div class="feed_attachment_photo">
                    <?php echo $this->htmlLink($attachment->item->getHref(), $this->itemPhoto($attachment->item, $thumb_main, $attachment->item->getTitle()), array('class' => 'feed_item_thumb ynmb_item_onephoto_full')) ?>
                  </div>
