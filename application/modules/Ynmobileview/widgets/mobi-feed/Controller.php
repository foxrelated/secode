<?php
class Ynmobileview_Widget_MobiFeedController extends Engine_Content_Widget_Abstract
{
	public function indexAction(){
		if(Engine_Api::_() -> hasModuleBootstrap('ynfeed'))
		{
			$this->process_ynfeed();
			$this->_action =  'advfeed-index';
		}
		else
		{
			$this -> process_feed();
	...
	public function process_feed()
	{
		...
		do
		{
			...
			if (!empty($subject))
			{
				$actions = Engine_Api::_()->ynmobileview() -> getActivityAbout($subject, $viewer, $tmpConfig, $sort);
			}
			else
			{
				$actions = Engine_Api::_()->ynmobileview() -> getActivity($viewer, $tmpConfig, $sort);
			}
			$selectCount++;
		...
		foreach (Zend_Registry::get('Engine_Manifest') as $data)
		{
			...
			{
				if(!in_array($type, array( 'photo', 'link', 'video')))
				{
					continue;
				}
	...
	public function process_ynfeed()
	{
		// Don't render this if not authorized
		$viewer = Engine_Api::_() -> user() -> getViewer();
		$viewer_id = $viewer->getIdentity();
		...
		$headScript -> appendFile('application/modules/Ynmobileview/externals/scripts/advfeed_core.js');
