    ...
    public function advancedActivity(Activity_Model_Action $action = null, array $data = array(), $method = null, $show_all_comments = false) {
        ...
        if ($viewer->getIdentity()) {
            ...
            if (!Engine_Api::_()->core()->hasSubject()) {
                ...
                if ($subject->getType() == 'siteevent_event' && ($subject->parent_type == 'sitepage_page' || $subject->parent_type == 'sitbusiness_business' || $subject->parent_type == 'sitegroup_group' || $subject->parent_type == 'sitestore_store')) {
                    $subject = Engine_Api::_()->getItem($subject->parent_type, $subject->parent_id);
                }
            ...
            if ($method == 'update') {
                if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('activity.commentreverseorder', false)) {
                    return $this->view->partial(
                                    '_nestedcomment_activityComments.tpl', 'nestedcomment', $data
                    );
                } else {
                     return $this->view->partial(
                                    '_nestedcomment_activityComments_reverse_chronological.tpl', 'nestedcomment', $data
                    );
                }
            } else {
                if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('activity.commentreverseorder', false)) {
