...
class Advancedactivity_Widget_HomeFeedsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
     ...
     $composerList = Engine_Api::_()->getApi('settings', 'core')->getSetting('advancedactivity.composer.menuoptions', Engine_Api::_()->advancedactivity()->getComposerMenuList());
      ...
      foreach (Zend_Registry::get('Engine_Manifest') as $data) {
        ...
        foreach ($data['composer'] as $type => $config) {
          ...
          if(!in_array($type , array('advanced_facebook','advanced_twitter','advanced_linkedin','advanced_instagram','tag')) && !in_array($key, $composerList)){
          continue;
          } 
    ...
    if ($this->view->enableComposer) {
      ...
      if (!empty($instagram_apikey) && !empty($instagram_secret) && ($instagram_key != FALSE || $instagram_enable)) {
      ...
      if(method_exists($Api_facebook,'checkAppReadPermission') && !$Api_facebook->checkAppReadPermission() && $fb_key !== FALSE) {      
            ...
            if ($session_userfeed && $fb_checkconnection) {
              ...
              if (('publish' == $core_fbenable || 'login' == $core_fbenable || $enable_socialdnamodule) && (!$fb_checkconnection)) {
                ...
                try {
                  ...
                  if ($subject && ($subject->getType() == 'sitepage_page' || $subject->getType() == 'sitebusiness_business' || $subject->getType() == 'sitestore_store') && !$session->fb_can_managepages) {
                    $checksiteIntegrate = false;
                  }
                } catch (Exception $e) {
                  $checksiteIntegrate = false;
