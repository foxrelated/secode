 ...
 */
class Advancedactivity_Widget_AdvancedactivityinstagramUserfeedController extends Seaocore_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    try {
      ...
      $response = $instagramObj->getUserFeedNext($feedCount, $nextMaxId, null);
//      $response = $instagramObj->getPopularMedia();

      if (isset($response->meta->code) && $response->meta->code != 200) {
        $this->view->paginator = $this->view->response = null;
      } else {
        $this->view->paginator = $this->view->response = $response->data;
      }
    } catch (Exception $e) {
      $this->view->instagramLoginURL = $instagramloginURL;
