  ...
  <ul> 
    <?php foreach ($this->filterTabs as $key => $tab): ?>
      ...
      <?php if ($key < $this->contentTabMax): ?>
        <li id="tab_advFeed_<?php echo $tab['filter_type'] ?>" class="<?php echo $class ?>">  
          <?php if(!$hasAddEditFeed && $this->viewer()->getIdentity()):?>
            <a href="<?php echo $this->url(array('controller'=>'feed','action' => 'edit-hide-options'),'advancedactivity_extended') ?>" class="smoothbox aaf_tabs_feed_update_edit" title="<?php echo $this->translate("Edit Activity Feed Settings") ?>"></a> <?php $hasAddEditFeed = true;?>  
          <?php endif; ?>
    ...
    <?php if (count($this->filterTabs) > $this->contentTabMax || $this->canCreateCustomList || $this->canCreateCategroyList): ?>
      <li class="aaf_tabs_feed_tab aaf_tabs_feed_tab_more" onclick="moreAdvancedFeedTabSwitch($(this));" >
        ...
        <a href="javascript:void(0);"><span><?php echo count($this->filterTabs)>0 && $this->contentTabMax !=0 ?$this->translate('More'):count($this->filterTabs)<1?$this->translate('More'):$this->translate('Filter') ?></span><i></i></a>
      </li>
