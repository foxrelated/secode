...
class Seshtmlbackground_Plugin_Job_Encode extends Core_Plugin_Job_Abstract
{
  protected function _execute()
  {
    ...
    // No video id?
    if( !($slide_id = $this->getParam('slide_id')) ) {
      $this->_setState('failed', 'No slide identity provided.');
      $this->_setWasIdle();
      return;
    }
    ...
    if( !$slide || !($slide instanceof Seshtmlbackground_Model_Slide) ) {
      $this->_setState('failed', 'Slide is missing.');
      $this->_setWasIdle();
      return;
    }
    ...
    // Check video status
    if( 2 != $slide->status ) {
      $this->_setState('failed', 'Slide has already been encoded, or has already failed encoding.');
      $this->_setWasIdle();
      return;
    }

    $type = $this->getParam('type');
    $type = empty($type) ? 'mp4' : $this->getParam('type');

    // Process
    try {
      $this->_process($slide, $type);
      $this->_setIsComplete(true);
    } catch( Exception $e ) {
      $this->_setState('failed', 'Exception: ' . $e->getMessage());

      // Attempt to set slide state to failed
      try {
        if( 1 != $slide->status ) {
          $slide->status = 3;
          $slide->save();
  ...
  private function getFFMPEGPath() {
    ...
    if( !function_exists('shell_exec') ) {
      throw new Seshtmlbackground_Model_Exception('Unable to execute shell commands using shell_exec(); the function is disabled.');
    }

    if( !function_exists('exec') ) {
      throw new Seshtmlbackground_Model_Exception('Unable to execute shell commands using exec(); the function is disabled.');
    }

    // Make sure FFMPEG path is set
    $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->seshtmlbackground_ffmpeg_path;
    if( !$ffmpeg_path ) {
      throw new Seshtmlbackground_Model_Exception('Ffmpeg not configured');
    }
    ...
    if( !@file_exists($ffmpeg_path) || !@is_executable($ffmpeg_path) ) {
      ...
      if( $return > 0 ) {
        throw new Seshtmlbackgroundideo_Model_Exception('Ffmpeg found, but is not executable');
      }
    }
  ...
  private function getTmpDir() {
    ...
    $tmpDir = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' .
      DIRECTORY_SEPARATOR . 'seshtmlbackground_slide';

    if( !is_dir($tmpDir) && !mkdir($tmpDir, 0777, true) ) {
      throw new Seshtmlbackground_Model_Exception('Slide temporary directory did not exist and could not be created.');
    }

    if( !is_writable($tmpDir) ) {
      throw new Seshtmlbackground_Model_Exception('Slide temporary directory is not writable.');
    }

    return $tmpDir;
  }

  private function getVideo($slide) {
    // Get the video object
    if( is_numeric($slide) ) {
      $slide = Engine_Api::_()->getItem('seshtmlbackground_slide', $slide);
    }

    if( !($slide instanceof Seshtmlbackground_Model_Slide) ) {
      throw new Seshtmlbackground_Model_Exception('Argument was not a valid slide');
    }

    return $slide;
  }

  private function getStorageObject($slide) {
    // Pull video from storage system for encoding
    $storageObject = Engine_Api::_()->getItem('storage_file', $slide->file_id);

    if( !$storageObject ) {
      throw new Seshtmlbackground_Model_Exception('Slide storage file was missing');
    }
  ...
  private function getOriginalPath($storageObject) {
    ...
    if( !file_exists($originalPath) ) {
      throw new Seshtmlbackground_Model_Exception('Could not pull to temporary file');
    }
  ...
  private function notifyOwner($video, $owner) {
    ...
    Engine_Api::_()->getDbtable('notifications', 'activity')
      ->addNotification($owner, $owner, $video, 'seshtmlbackground_video_processed_failed', array(
  ...
  protected function _process($video, $type, $compatibilityMode = false)
  {
    ...
    if( !$success ) {
      ...
      if( APPLICATION_ENV == 'development' ) {
        file_put_contents($tmpDir . '/' . $video->slide_id . '.txt', $videoOutput);
      }
      throw new Seshtmlbackground_Model_Exception($exceptionMessage);
    }
    // Success
    else
    {
      ...
      if ( $thumbSuccess && !$video->thumb_icon ) {
        $video->thumb_icon = $thumbFileRow->file_id;
      }
