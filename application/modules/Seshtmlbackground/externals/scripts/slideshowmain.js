var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSesVideoSlideshow1,clearIntvalSesVideoSlideshow2,autoPlayDelay ;
jQuery(document).ready(function($){
	...
	if ( slidesWrapper.length > 0 ) {
		...
		sliderNav.on('click', 'li', function(event){
			...
			}else if(slidesWrapper.children().length == 1 && slidesWrapper.children().eq(0).hasClass('cd-bg-video')){
				 slidesWrapper.eq(0).find('.cd-bg-video-wrapper').find('video').attr('loop','loop');
				  slidesWrapper.eq(0).find('.cd-bg-video-wrapper').find('video').get(0).play();
			}
		});
	}
