...
class Sitegroupmember_Widget_ListMembersTabsViewController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($is_ajax)) {
      $this->view->tabs = $tabs = Engine_Api::_()->getItemTable('seaocore_tab')->getTabs(array('module' => 'sitegroupmember', 'type' => 'member', 'enabled' => 1));
    ...
    $activeTab = Engine_Api::_()->getItemTable('seaocore_tab')->getTabs(array('module' => 'sitegroupmember', 'type' => 'member', 'enabled' => 1, 'name' => $activeTabName));
    $this->view->activTab = $activTab = $activeTab['0'];

    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('membership', 'sitegroup')->listMemeberTabWidget($activTab);
