 ...
 */
class Sitegroupmember_Widget_SitegroupMemberController extends Seaocore_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $sitegroupmemberBaseDir = Zend_Registry::isRegistered('sitegroupmemberBaseDir') ? Zend_Registry::get('sitegroupmemberBaseDir') : null;
    ...
    $form = new Sitegroupmember_Form_Searchwidget(array('type' => 'sitegroup_group'));
    ...
    $this->view->friendpaginator = $friendpaginator = Engine_Api::_()->getDbtable('membership', 'sitegroup')->getMembers($values, 'friend');

		$friendpaginator->setItemCountPerPage($totalMembers);
		//$this->view->paginator = $paginator->setCurrentPageNumber($values['page']);
	  $this->view->friendpaginator = $friendpaginator->setCurrentPageNumber($front->getParam('group'));
		$this->view->addHelperPath(APPLICATION_PATH . '/application/modules/Sitegroupmember/View/Helper', 'Sitegroupmember_View_Helper');
    //GET MEMBERS DATA
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('membership', 'sitegroup')->getMembers($values, 'otherMember');

		$paginator->setItemCountPerPage($totalMembers);
		//$this->view->paginator = $paginator->setCurrentPageNumber($values['page']);
		$this->view->paginator = $paginator->setCurrentPageNumber($front->getParam('group'));
    
    if( empty($sitegroupmemberBaseDir) ) {
