...
class Sesvideo_Widget_ShowSamePosterController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if ($tableName == 'chanel') {
      $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array('not_chanel_id'=>$subject->getIdentity(),'user_id'=>$subject->owner_id));
    } else if ($tableName == 'video') {
      $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('videos', 'sesvideo')->getVideo(array('not_video_id'=>$subject->getIdentity(),'user_id'=>$subject->owner_id));
    }
