/* $Id: sesVideoimagevieweradvance.js  2015-6-16 00:00:000 SocialEngineSolutions $ */
  ...
  //check function call from image viewer or direct
	...
	var urlChangeState = requestedURL.replace(videoURLsesvideo+'/imageviewerdetail',videoURLsesvideo);
...
function getlastElementDataVideo(type,item_id){
		var URL = en4.core.baseUrl+'sesvideo/index/last-element-data/type/'+type+'/'+item_id;
...
function getAllVideo(requestURL){
  requestVideosesJqueryObjectURL = requestURL.replace(videoURLsesvideo+'/imageviewerdetail',videoURLsesvideo+'/all-videos');
	imageViewerGetRequest = new Request.HTML({
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
				...
				if( !sesJqueryObject.trim( sesJqueryObject('#ses_media_lightbox_all_video_id').html() ).length ) 
					...
					var video_id = sesJqueryObject('#sesvideo_video_id_data_src').attr('data-src');
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
				{
						...
						if(sesJqueryObject('#media_video_next_ses').hasClass('ses-private-image')){
							...
							sesJqueryObject('#media_video_next_ses').remove();
								...
								var password = prompt("Enter the password for video '"+sesJqueryObject('#sesvideo_video_title').html()+"'");
								if(typeof password != 'object' && password.toLowerCase() == trim(sesJqueryObject('#sesvideo_video_password').html())){
									...
									openPhotoSwipeVideo('','','',sesJqueryObject('#video_data_lightbox').find('.sesvideo_view_embed_lightbox').html());
									sesJqueryObject('#media_video_next_ses').remove();
								}else{
									sesJqueryObject('.pswp__top-bar-share').hide();
									sesJqueryObject('.pswp__button--info-show').hide();
									...
									var img = document.createElement('img');
									img.onload = function(){
										openPhotoSwipeVideo(sesJqueryObject('#media_video_next_ses').find('#gallery-img').attr('src'),this.width,this.height);
										sesJqueryObject('.image_show_pswp').attr('src',sesJqueryObject('#media_video_next_ses').find('#gallery-img').attr('src'));
										sesJqueryObject('#media_video_next_ses').remove();
									}
									img.src = sesJqueryObject('#media_video_next_ses').find('#gallery-img').attr('src');
						...
						sesJqueryObject('#media_video_next_ses').remove();
						sesJqueryObject('#sesvideo_video_password').remove();
						sesJqueryObject('#sesvideo_video_title').remove();
						...
						if(changeIframeSize){				
							...
							if(sesJqueryObject('#sesvideo_lightbox_content ').find('iframe').contents().length){
								sesJqueryObject('#sesvideo_lightbox_content ').find('iframe').css('height',parseInt(height-marginTop)+'px');
								sesJqueryObject('#sesvideo_lightbox_content').find('iframe').css('width',width+'px');
								sesJqueryObject('#sesvideo_lightbox_content').find('iframe').css('margin-top',marginTop+'px');
								sesJqueryObject('#sesvideo_lightbox_content').find('iframe').css('margin-bottom',marginTop+'px');
								var srcAttr = sesJqueryObject('#sesvideo_lightbox_content').find('iframe').attr('src');		
							}else{
								sesJqueryObject('#sesvideo_lightbox_content ').find('video').css('margin-top',(height/4)+'px');
							}
						}
						...
						if(typeof srcAttr != 'undefined')
							sesJqueryObject('#sesvideo_lightbox_content').find('iframe').attr('src',srcAttr);
						return true;
				}
			}); 
			en4.core.request.send(imageViewerGetRequest, {
...
sesJqueryObject(document).on("click", '.sesvideo_thumb_img', function (e) {
  ...
  var getImageHref = imageObject.attr('href');
	...
	getImageHref = getImageHref.replace(videoURLsesvideo+'/imageviewerdetail',videoURLsesvideo);
	if(openVideoInLightBoxsesVideo== 0 ){
...
sesJqueryObject(document).on('click','#comments .comments_options a',function(event){
	...
	if(htmlOnclick.search('comments') != -1){
		...
		if(currentURL.search('video_id') != -1)
			...
			var itemType = 'Video';
		if(htmlOnclick.search('unlike') != -1){console.log('yes');
