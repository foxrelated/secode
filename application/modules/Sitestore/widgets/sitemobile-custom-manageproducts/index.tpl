    ...
    <?php endif; ?>
        ...
        function selectAll()
        {
            ...
            for (var i = 0; i < subcatss.length; ++i) {
                ...
                if (document.getElementById(subcatsss)) {
...
//                document.getElementById("selectallchecks").disabled = false;
            ...
            //
            //   var i;
            //    var multidelete_form_table_printing_tag = document.getElementById('multiselect_form');
        ...
        function submitFilter(tempVar) {
            if (document.getElementById('main_printingtag_id'))
                document.getElementById('main_printingtag_id').style.display = 'none';

            if (tempVar != 'exist')
                document.getElementById('product_bulk_edit').style.display = 'none';

            document.getElementById('id_store_manage_products').innerHTML = '<center><img src="application/modules/Sitestore/externals/images/spinner_temp.gif" /></center>';
            var downpayment = 0;
    <?php if (!empty($this->directPayment) && !empty($this->isDownPaymentEnable)) : ?>
                downpayment = document.getElementById('downpayment').value;
    <?php endif; ?>
            en4.core.request.send(new Request.HTML({
                url: '<?php echo $this->layout()->staticBaseUrl; ?>' + 'sitestoreproduct/product/manage',
                data: {
                    format: 'html',
                    ...
                    //                    temp_in_stock: document.getElementById('stock').value,
                    downpayment: downpayment,
                    //          temp_featured : document.getElementById('featured').value,
                    //          temp_sponsored : document.getElementById('sponsored').value,
                    //          temp_newlabel : document.getElementById('new').value,
                    //          temp_status : document.getElementById('temp_status').value,
                ...
                onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                    tempStr = '';
                    if (tempVar == "exist")
                        document.getElementById('product_bulk').innerHTML = '<ul class="form-notices"><li>Products edited successfully.</li></ul>';
//                    document.getElementById('selectallchecks').checked = false;
//                    document.getElementById("selectallchecks").disabled = false;
//                    document.getElementById('selectAllProducts').checked = false;
                    document.getElementById('id_store_manage_products').innerHTML = responseHTML;
                    if (responseHTML.search('<form') >= 0) {
                        if (document.getElementById('main_printingtag_id'))
                            document.getElementById('main_printingtag_id').style.display = 'block';
                        if (tempVar != 'exist')
                            document.getElementById('product_bulk_edit').style.display = 'block';
                    }
                    if (tempVar == "exist")
                        removeChecks();
        ...
        function checkedEntries() {
            ...
            if (document.getElementById('printingtag_id') && (document.getElementById('selectAllProducts').checked || tempStr) && (tempStr != ',<0>') && document.getElementById('printingtag_id').value && document.getElementById('printingtag_id').value != 0) {
                var url = '<?php echo $this->layout()->staticBaseUrl; ?>' + 'sitestoreproduct/printing-tag/print?checked_product=' + tempStr + '&checkAll=' + checkAll + '&printingtag_id=' + document.getElementById('printingtag_id').value + '&store_id=<?php echo $this->store_id ?>&searchParams=' + searchParams;
                Smoothbox.open(url);
            }
    ...
    <?php $url = $this->url(array('action' => 'store', 'store_id' => $this->store_id, 'type' => 'product', 'menuId' => 62, 'method' => 'manage'), 'sitestore_store_dashboard', false) ?>
        function multiFunctionality(tempValue) {
            ...
            if (document.getElementById('printingtag_id'))
                printingtag_id = document.getElementById('printingtag_id').value;
            ...
            en4.core.request.send(new Request.JSON({
                url: '<?php echo $this->layout()->staticBaseUrl; ?>' + 'sitestoreproduct/product/multi-functionality',
                data: {
                    format: 'json',
...
//                    document.getElementById('selectallchecks').checked = false;
                    ...
                    document.getElementById('selectallchecks').checked = true;
                }
            }
            selectAll();
        }
...
//                    document.getElementById('selectAllProducts').checked = false;
        ...
        en4.core.runonce.add(function () {
            document.getElementById('export_products').addEvent('click', function (event)
            {
                var searchParams = {
                    'search': document.getElementById('title').value,
                    //                    'minPrice': document.getElementById('min_price').value,
                ...
                //    if (document.getElementById('selectAllProducts').checked == false){
    ...
    <?php if (empty($this->responseFlag) && !empty($this->is_subject) && !empty($this->viewer_id) && !empty($this->can_edit)): ?>
        <div class="seaocore_add">
            <?php if (empty($this->quota) || (!empty($this->quota) && $this->current_count < $this->quota)): ?>
                <a href="javascript:void(0);" onclick="window.location.href = '<?php echo $this->url(array('action' => 'create', 'store_id' => $this->store_id), 'sitestoreproduct_general', true) ?>';
                                    return false;" class='buttonlink seaocore_icon_add'><?php echo $this->translate('Create Product'); ?>
                </a>
            <?php endif; ?>
            ...
            <form class="field_search_criteria" id="filter_form" onSubmit="return submitFilter();">
                <div>
                    <ul>
                        ...
                        <?php if (!empty($this->directPayment) && !empty($this->isDownPaymentEnable) && false) : ?>
                            <li>
                                            ...
                                            <div class='sr_sitestoreproduct_browse_list_info'>
                                                ...
                                                <div class='sr_sitestoreproduct_browse_list_options clr'>
                                                    ...
                                                    if (!empty($this->can_edit)):
                                                        ...
                                                        echo $this->htmlLink($this->url(array('action' => 'delete-mobile', 'product_id' => $sitestoreproduct->product_id), "sitestoreproduct_general", true), $this->translate('Delete Product'), array('class' => 'buttonlink seaocore_icon_delete'));
//                                                        if (empty($sitestoreproduct->highlighted)):
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'highlighted-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Highlighted'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink icon_sitestore_highlighted'));
//                                                        else:
//
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'highlighted-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-highlighted'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink icon_sitestore_unhighlighted'));
//
//                                                        endif;
                                                    endif;
...
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'featured-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Featured'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_featured'));
//                                                        else:
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'featured-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-featured'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_unfeatured'));
//                                                        endif;


//                                                        if (empty($sitestoreproduct->sponsored)):
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'sponsored-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Sponsored'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_sponsored'));
//                                                        else:
//                                                            echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'sponsored-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-sponsored'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_unsponsored'));
//                                                        endif;
                                                    endif;
...
//                                                        echo $this->htmlLink(array('route' => 'sitestoreproduct_general', 'action' => 'copy-product-mobile', 'product_id' => $sitestoreproduct->product_id), $this->translate('Copy Products'), array('class' => 'buttonlink seaocore_icon_copyproduct'));
//                                                    endif;
                                                    ?>
                                                </div>
                                            </div>
...
//                                                                echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'highlighted-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Highlighted'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink icon_sitestore_highlighted'));
//                                                            else:
//                                                                echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'highlighted-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-highlighted'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink icon_sitestore_highlighted'));
//                                                            endif;
                                                        ...
                                                        if (!empty($this->viewer_level_id) && ($this->viewer_level_id == 1)):
                                                            if (empty($sitestoreproduct->featured)):
                                                                ...
                                                                echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'featured-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-featured'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_featured'));
                                                            endif;
                                                            ...
                                                            if (empty($sitestoreproduct->sponsored)):
                                                                ...
                                                                echo $this->htmlLink(array('route' => 'sitestoreproduct_manage', 'action' => 'sponsored-mobile', 'product_id' => $sitestoreproduct->product_id, 'is_store' => $this->is_subject), $this->translate('Make Un-sponsored'), array('onclick' => 'owner(this);return false', ' class' => 'buttonlink seaocore_icon_sponsored'));
                                                            endif;
                                                        endif;
                                                        ?>
                                                    </div>
        ...
        function switchview(flage) {
            if (flage == 1) {
                if (document.getElementById('image_view')) {
                    if (document.getElementById('sr_sitestoreproduct_map_canvas_view_browse'))
                        document.getElementById('sr_sitestoreproduct_map_canvas_view_browse').style.display = 'none';
                    if (document.getElementById('grid_view'))
    ...
    <script type="text/javascript">
        $(".sr_sitestoreproduct_browse_list_options a").on("click", function () {
