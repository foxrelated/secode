...
<div class="cmad_ad_clm">
	...
	<?php $div_id = 0;
		...
		foreach ($this->communityads_array as $community_ad):
			...
			<div id="store_ads_<?php echo $this->tab; ?>_widget_ad_cancel_<?php echo $div_id; ?>" style="display:none;" class="cmadrem">
				<div class="cmadrem_rl">
					<?php echo '<a class="" title="' . $this->translate('Cancel reporting this ad') . '" href="javascript:void(0);" onclick="adUndo('.$div_id.', \'store_ads_'.$this->tab.'_widget\');">'. $this->translate("Undo"). '</a>'; ?>
				</div>
				<div class="cmadrem_con">
					<?php echo $this->translate("Do you want to report this? Why didn't you like it?"); ?>
					<form>
						<?php $ads_id = $encode_adId; ?>
						<div><input type="radio" name="adAction" value="0" onclick="adSave('Misleading', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'store_ads_<?php echo $this->tab; ?>_widget')"/><?php echo $this->translate('Misleading'); ?></div>
						<div><input type="radio" name="adAction" value="1" onclick="adSave('Offensive', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'store_ads_<?php echo $this->tab; ?>_widget')"/><?php echo $this->translate('Offensive'); ?></div>
						<div><input type="radio" name="adAction" value="2" onclick="adSave('Inappropriate', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'store_ads_<?php echo $this->tab; ?>_widget')"/><?php echo $this->translate('Inappropriate'); ?></div>
						<div><input type="radio" name="adAction" value="3" onclick="adSave('Licensed Material', '<?php echo $ads_id; ?>', <?php echo $div_id; ?>, 'store_ads_<?php echo $this->tab; ?>_widget')"/><?php echo $this->translate('Licensed Material'); ?></div>
						<div><input type="radio" name="adAction" value="4" onclick="otherAdCannel(4, <?php echo $div_id; ?>, 'store_ads_<?php echo $this->tab; ?>_widget')" id="store_ads_<?php echo $this->tab; ?>_widget_other_<?php echo $div_id; ?>"/><?php echo $this->translate('Other'); ?></div>
						<div><textarea name="store_ads_<?php echo $this->tab; ?>_widget_other_text_<?php echo $div_id; ?>" onclick="this.value=''" onblur="if(this.value=='')this.value='<?php echo $this->translate('Specify your reason here..') ?>';"  id= "store_ads_<?php echo $this->tab; ?>_widget_other_text_<?php echo $div_id; ?>" style="display:none;" ><?php echo $this->translate('Specify your reason here..') ?></textarea>
						</div>
						<div><?php echo '<a href="javascript:void(0);" onclick="adSave(\'Other\', \''.$ads_id.'\', '.$div_id.', \'store_ads_'.$this->tab.'_widget\')" id="store_ads_'.$this->tab.'_widget_other_button_'.$div_id.'"  style="display:none" class="cmadrem_button">' . $this->translate('Report') . '</a>'; ?></div>
					</form>
				</div>	
			</div>
		
			<!-- DIV:  Which default show, This div contain the all information about advertisment. start -->
			<div class="cmaddis" id="store_ads_<?php echo $this->tab; ?>_widget_ad_<?php echo $div_id; ?>">
				<div class="cmaddis_close">
					<?php $is_identity = $this->viewer()->getIdentity();
								...
								if(!empty($is_identity)  && !empty($adcancel_enable)) { 
									echo	'<a class="" title="' . $this->translate('Report this ad') . '" href="javascript:void(0);" onclick="adCancel('.$div_id.', \'store_ads_'.$this->tab.'_widget\');"></a>';
								} ?>
				</div>
					...
					// Condition: If Like:1 & Resource Type & Resource Id is available in database then this means that advertisment has existence in community site then we show the 'Like' option of the user.
						...
						if( !empty($this->user_id) && !empty($community_ad['like']) && !empty($community_ad['resource_type']) && !empty($community_ad['resource_id']) && !empty($is_module_enabled) && !empty($resource_url) ) {
							...
							<?php if(empty($like_id)) { // Condition: Show 'Like Link' only when loggden user not like this advertisment. else show message that you have liked this advertismant.   ?>
								<div class="cmad_like_button" id= "<?php echo $resourceType ?>_store_ads_<?php echo $this->tab; ?>_unlikes_<?php echo $encode_adId;?>" style ='display:none;' >
									<?php echo '<div class="cmaddis_cont">' .$this->translate('You like this.'). '</div>'; ?>
									<a href = "javascript:void(0);" onclick = "communityad_likeinfo('<?php echo $encode_adId; ?>', '<?php echo $resourceType ?>', '<?php echo $resource_id ?>', '<?php echo $owner_id ?>', 'store_ads_<?php echo $this->tab; ?>', '<?php echo $resource_url['like']; ?>' )">
									<i class="like_thumbdown_icon"></i>
									<span><?php echo $this->translate('Unlike') ?></span>
									</a>									
								</div>
								<div class="cmad_like_button" id= "<?php echo $resourceType ?>_store_ads_<?php echo $this->tab; ?>_most_likes_<?php echo $encode_adId;?>" style ='<?php echo $like_show;?>'>
									<a href = "javascript:void(0);" onclick = "communityad_likeinfo('<?php echo $encode_adId; ?>', '<?php echo $resourceType; ?>', '<?php echo $resource_id ?>', '<?php echo $owner_id ?>', 'store_ads_<?php echo $this->tab; ?>', '<?php echo $resource_url['like']; ?>' )">
									<i class="like_thumbup_icon"></i>
									<span><?php echo $this->translate('Like') ?></span>
									</a>
								</div>
								<input type ="hidden" id = "store_ads_<?php echo $this->tab; ?>_likeid_info_<?php echo $encode_adId;?>" value = "<?php echo $like_id; ?>"  />
							<?php }
					} 
				?>
...
<style type="text/css">
#store_lightbox_text .cmadrem,
#store_lightbox_text .cmaddis {
	border-top:none;
	clear: both;
	font-size: 11px;
	margin: 10px;
...
#store_lightbox_text .cmaddis_image{
	float:left;
	margin:0 10px 0 0;
}
#store_lightbox_text .cmaddis_body{
	overflow:hidden;
	min-width:90px;
}
</style>