...
foreach ($this->show_slideshow_object as $type => $item) {
  ...
  $content_link = $this->htmlLink(Engine_Api::_()->sitestore()->getHref($item->store_id, $item->owner_id, $item->getSlug()), $this->translate('View Store &raquo;'), array('class' => 'seaocore_slideshow_view_link'));
...
if (!empty($this->sitestore_sponsored)) {
  ?>
    ...
    window.addEvent('domready',function(){
     ...
     for(var i=0;i < divElements.length;i++)
      ...
      var nS8 = new noobSlide({
        box: $('sitestore_seaocore_te_advanced_box'),
        items: $$('#sitestore_seaocore_te_advanced_box h3'),
        size: (width-10),
        handles: $$('#handles8 span'),
        addButtons: {previous: $('sitestore_seaocore_prev8'), stop: $('sitestore_seaocore_stop8'), play: $('sitestore_seaocore_play8'), next: $('sitestore_seaocore_next8') },
        interval: 5000,
        fxOptions: {
          duration: 500,
          transition: '',
        ...
        onWalk: function(currentItem,currentHandle){
          ...
          {
            $('sitestore_seaocore_caption').innerHTML =   current_caption_link;
          }
          else {
            $('sitestore_seaocore_caption').innerHTML =  '';
          }


          $('sitestore_seaocore_current_numbering').innerHTML =  current_index + '/' + num_of_slidehsow ;
        }
      });
...
<div class="seaocore_slideshow_wrapper">
  <div class="seaocore_slideshow_mask">
    <div id="sitestore_seaocore_te_advanced_box" class="seaocore_slideshow_advanced_box">
<?php echo $image_text_var ?>
  ...
  <div class="seaocore_slideshow_option_bar">
    <div>
      <p class="buttons">
        <span id="sitestore_seaocore_prev8" class="seaocore_slideshow_controllers-prev seaocore_slideshow_controllers prev" title="<?php echo $this->translate('Previous'); ?> "></span>
        <span id="sitestore_seaocore_stop8" class="seaocore_slideshow_controllers-stop seaocore_slideshow_controllers" title="<?php echo $this->translate('Stop'); ?> "></span>
        <span id="sitestore_seaocore_play8" class="seaocore_slideshow_controllers-play seaocore_slideshow_controllers" title="<?php echo $this->translate('Play'); ?>"></span>
        <span id="sitestore_seaocore_next8" class="seaocore_slideshow_controllers-next seaocore_slideshow_controllers" title="<?php echo $this->translate('Next'); ?> "></span>
      </p>
    </div>
    <span id="sitestore_seaocore_caption"></span>
    <span id="sitestore_seaocore_current_numbering" class="seaocore_slideshow_pagination"></span>
  </div>
</div>