 ...
 */
class Sitestore_AdminLayoutController extends Core_Controller_Action_Admin {
  ...
  public function layoutAction() {
    ...
    $userlayoutcontentstoresTable = Engine_Api::_()->getDbtable('contentstores', 'sitestore');
    ...
    $session = new Zend_Session_Namespace('sitestorelayout');
  ...
  public function updateAction() {
    ...
    $storeTable = Engine_Api::_()->getDbtable('pages', 'core');
    $contentTable = Engine_Api::_()->getDbtable('admincontent', 'sitestore');
    $userlayoutcontentTable = Engine_Api::_()->getDbtable('content', 'sitestore');
    ...
    $select = $userlayoutcontentTable->select()->from($userlayoutcontentTablename, array('contentstore_id'))->limit($limit, $offset)->group('contentstore_id');
    ...
    try {
      ...
      foreach ($structure as $element) {
        ...
        if (!empty($contentRow) && is_object($contentRow)) {
          ...
          foreach ($content_stores_id as $value) {
            ...
            $content_store_id = $value->contentstore_id;

            if ($contentRow->type == 'container' && $contentRow->name == 'middle') {
              $select_parent = $userlayoutcontentTable->select()->where('type =?', 'container')->where('name =?', $parentContentRow->name)->where('contentstore_id =?', $content_store_id);
              $content_parent = $userlayoutcontentTable->fetchRow($select_parent);
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentstore_id =?', $content_store_id)->where('parent_content_id = ?', $content_parent->content_id);
            } else {
              $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentstore_id =?', $content_store_id);
            }
            ...
            if ($usercontentRow->type == 'container') {
              ...
              $userlayoutcontentTable->update(array('order' => $newOrder), array('type = ?' => $contentRow->type, 'contentstore_id = ?' => $content_store_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }

            if (isset($session->edittitles) && in_array($element['name'], $session->edittitles)) {
							$userlayoutcontentTable->update(array('params' => $element['params']), array('type = ?' => $contentRow->type, 'contentstore_id = ?' => $content_store_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1, 'parent_content_id = ?' => $usercontentRow->parent_content_id, 'params =?' => $oldContentRowParams));
            }

            $parent_id = 0;
            if (!empty($parentContentRow)) {
              $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentstore_id =?', $content_store_id);
              ...
              if (!empty($parentContentRow->parent_content_id)) {
                ...
                if (!empty($parentparentContentRow)) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentparentContentRow->type)->where('name =?', $parentparentContentRow->name)->where('contentstore_id =?', $content_store_id);
                  $contentParent_stores = $userlayoutcontentTable->fetchRow($select);
                  if ($contentParent_stores->name != $parentparentContentRow->name) {
                    $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentstore_id =?', $content_store_id)->where('parent_content_id <> ?', $content_stores->parent_content_id);
              ...
              $parent_id = $content_stores->content_id;
            } else {
              $userlayoutcontentTable->update(array('parent_content_id' => new Zend_Db_Expr('NULL'), 'order' => $newOrder), array('type = ?' => $contentRow->type, 'contentstore_id = ?' => $content_store_id, 'name = ?' => $contentRow->name, 'widget_admin = ?' => 1));
            }
          ...
          foreach ($content_stores_id as $value) {
            ...
            if ($element['type'] == 'container') {
              ...
              if ($element['name'] != 'core.html-block' && $element['name'] != 'core.ad-campaign' && $element['name'] != 'sitestoreintegration.profile-items' && $element['name'] != 'sitestoremember.profile-sitestoremembers') {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('contentstore_id =?', $content_store_id);
                $userlayoutstores = $userlayoutcontentTable->fetchRow($select);
              } else {
                $select = $userlayoutcontentTable->select()->where('name =?', $element['name'])->where('contentstore_id =?', $content_store_id)->where('params =?', json_encode($element['params']));
                $userlayoutstores = $userlayoutcontentTable->fetchRow($select);
              }
            }
            ...
            if (empty($userlayoutstores) && isset($element['type']) && isset($element['name'])) {
              ...
              $usercontentRow->contentstore_id = $content_store_id;
              $usercontentRow->order = $newOrder;
              $usercontentRow->type = $element['type'];
              $usercontentRow->name = $element['name'];
              if (!empty($parentContentRow)) {
                $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentstore_id =?', $content_store_id)->order('content_id DESC')->order('content_id DESC');
                $content_stores = $userlayoutcontentTable->fetchRow($select);
                if (!empty($content_stores)) {
                  $usercontentRow->parent_content_id = $content_stores->content_id;
                }
                if (empty($usercontentRow->parent_content_id)) {
          ...
          foreach ($content_stores_id as $value) {
            ...
            if (!empty($parentContentRow)) {
              ...
              if (!empty($parentparentContentRow)) {
                $select = $userlayoutcontentTable->select()->where('type =?', $parentparentContentRow->type)->where('name =?', $parentparentContentRow->name)->where('contentstore_id =?', $content_store_id);
                $contentParent_stores = $userlayoutcontentTable->fetchRow($select);
                if ($contentParent_stores && $contentParent_stores->name != $parentparentContentRow->name) {
                  $select = $userlayoutcontentTable->select()->where('type =?', $parentContentRow->type)->where('name =?', $parentContentRow->name)->where('contentstore_id =?', $content_store_id)->where('parent_content_id <> ?', $contentParent_stores->content_id);
                  $content_stores = $userlayoutcontentTable->fetchRow($select);
                }
              }
              $parent_id = $content_stores->content_id;
            ...
            if (!empty($parent_id)) {
              if ( $contentRow->name != 'core.ad-campaign' && $contentRow->name != 'sitestoreintegration.profile-items') {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentstore_id =?', $content_store_id)->where('parent_content_id =?', $parent_id);
              } else {
                $select = $userlayoutcontentTable->select()->where('type =?', $contentRow->type)->where('name =?', $contentRow->name)->where('contentstore_id =?', $content_store_id)->where('parent_content_id =?', $parent_id)->where('params =?', $oldContentRow);
              }
              $content_stores = $userlayoutcontentTable->fetchRow($select);
              if (!empty($content_stores)) {
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'contentstore_id =?' => $content_store_id, 'parent_content_id =?' => $parent_id, 'content_id =?' => $content_stores->content_id, 'widget_admin = ?' => 1
                ));
              }
            } else {
              if (($contentRow->name == 'top' || $contentRow->name == 'bottom') && $contentRow->type == 'container') {
                ...
                $userlayoutcontentTable->delete(array('type =?' => 'container',
                    'name =?' => 'middle', 'contentstore_id =?' => $content_store_id, 'parent_content_id = ?' => $content_stores->content_id, 'widget_admin = ?' => 1
                ));
                $userlayoutcontentTable->delete(array(
                    'name =?' => $contentRow->name, 'contentstore_id =?' => $content_store_id, 'content_id =?' => $content_stores->content_id, 'widget_admin = ?' => 1
                ));
              }
            }
          }
      ...
      if (!empty($top)) {
        $select = $contentTable->select()->where('store_id =?', $store)->where('name =?', 'middle')->where('parent_content_id =?', $top->admincontent_id);
        $middle = $contentTable->fetchRow($select);
					if (!empty($middle)) {
					$select = $contentTable->select()->where('store_id =?', $store)->where('parent_content_id =?', $middle->admincontent_id);
					...
					foreach ($content_stores_id as $value) {
						$content_store_id = $value->contentstore_id;
						$select = $userlayoutcontentTable->select()->where('name =?', 'top')->where('contentstore_id =?', $content_store_id);
						$user_top = $userlayoutcontentTable->fetchRow($select);
						if (!empty($user_top)) {
							$select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('contentstore_id =?', $content_store_id)->where('parent_content_id =?', $user_top->content_id);
							$user_middile = $userlayoutcontentTable->fetchRow($select);
							if ($user_middile) {
								foreach ($name as $name_value) {
									$userlayoutcontentTable->update(array('parent_content_id' => $user_middile->content_id), array('contentstore_id = ?' => $content_store_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
								}
							}
						}
					}
        ...
        if (!empty($bottom)) {
          $select = $contentTable->select()->where('store_id =?', $store)->where('name =?', 'middle')->where('parent_content_id =?', $bottom->admincontent_id);
          $middle = $contentTable->fetchRow($select);
          $select = $contentTable->select()->where('store_id =?', $store)->where('parent_content_id =?', $middle->admincontent_id);
          $middle_block = $contentTable->fetchAll($select);
          $name = array();
          ...
          foreach ($content_stores_id as $value) {
            $content_store_id = $value->contentstore_id;
            $select = $userlayoutcontentTable->select()->where('name =?', 'bottom')->where('contentstore_id =?', $content_store_id);
            $user_bottom = $userlayoutcontentTable->fetchRow($select);
            if (!empty($user_bottom)) {
              $select = $userlayoutcontentTable->select()->where('name =?', 'middle')->where('contentstore_id =?', $content_store_id)->where('parent_content_id =?', $user_bottom->content_id);
              $user_middile = $userlayoutcontentTable->fetchRow($select);
              if ($user_middile) {
                foreach ($name as $name_value) {
                  $userlayoutcontentTable->update(array('parent_content_id' => $user_middile->content_id), array('contentstore_id = ?' => $content_store_id, 'name = ?' => $name_value, 'widget_admin = ?' => 1));
      ...
      if (empty($edit_layout_setting)) {
        ...
        if (empty($user_widgetlink)) {
          foreach ($content_stores_id as $value) {
            $content_store_id = $value->contentstore_id;
            $userlayoutcontentTable->delete(array(
                'name =?' => 'sitestore.widgetlinks-sitestore', 'contentstore_id =?' => $content_store_id));
          }
        }
      }
    ...
    if ($store_reload <= $reload_count) {
      ...
      $this->_forward('success', 'utility', 'core', array(
          ...
          'redirect' => $this->view->url(array('module' => 'sitestore', 'controller' => 'layout', 'action' => 'update', 'store' => $store, 'store_reload' => $store_reload, 'admin_sitestore_layout' => $newLayout), 'admin_default', true),
          ...
          'messages' => array(Zend_Registry::get('Zend_Translate')->_('<div class="tip" style="margin:10px auto;width:750px;"><span>Please do not close this store or navigate to another store till you see a layout changes completion or error message.</span></div><div>
					<center><img src="application/modules/Sitestore/externals/images/layout/uploading.gif" alt="" /></center>
      	</div>'))
      ));
    } else {
      ...
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => false,
          'redirect' => $this->view->url(array('module' => 'sitestore', 'controller' => 'layout', 'action' => 'layout', 'store' => $store), 'admin_default', true),
          'parentRedirectTime' => '10',
          //'format' => 'smoothbox',
  ...
  public function saveAction() {
    ...
    $storecoreTable = Engine_Api::_()->getDbtable('pages', 'core');
    ...
    if ($option == 1) {
      ...
      if (empty($admincontentrow)) {
        foreach ($contentstoresrow as $value) {
          Engine_Api::_()->getDbTable('content', 'sitestore')->delete(array('name =?' => $widgetname));
        }
      }
      if (empty($row)) {
