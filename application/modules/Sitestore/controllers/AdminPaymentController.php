 ...
 */
class Sitestore_AdminPaymentController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sitestore_admin_main', array(), 'sitestore_admin_main_transactions');
    ...
    // Make form
    $this->view->formFilter = $formFilter = new Sitestore_Form_Admin_Transaction_Filter();
    ...
    $sitestoreName = Engine_Api::_()->getDbtable('stores', 'sitestore')->info('name');
    $transactionSelect = $transactionsTable->select()
            ...
            ->join($sitestoreName, $sitestoreName . '.store_id=' . $orderssName . '.source_id', array($sitestoreName . '.title', $sitestoreName . '.store_id'));
    ...
    if (!empty($filterValues['query'])) {
      $transactionSelect
          ->joinRight('engine4_users', 'engine4_users.user_id=engine4_sitestore_transactions.user_id', null)
    ...
    if (($user_id = $this->_getParam('user_id', @$filterValues['user_id']))) {
      ...
      $transactionSelect->where('engine4_sitestore_transactions.user_id = ?', $user_id);
    }
  ...
  public function detailAction() {
    ...
    if (!($transaction_id = $this->_getParam('transaction_id')) ||
        !($transaction = Engine_Api::_()->getItem('sitestore_transaction', $transaction_id))) {
      return;
    }
    ...
    $this->view->title = Engine_Api::_()->getItem('sitestore_store', $this->view->order->source_id)->title;
  ...
  public function rawOrderDetailAction() {
    ...
    if (null != ($transaction_id = $this->_getParam('transaction_id')) &&
        null != ($transaction = Engine_Api::_()->getItem('sitestore_transaction', $transaction_id))) {
      $gateway = Engine_Api::_()->getItem('payment_gateway', $transaction->gateway_id);
