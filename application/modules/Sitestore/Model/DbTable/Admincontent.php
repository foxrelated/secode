 ...
 */
class Sitestore_Model_DbTable_Admincontent extends Engine_Db_Table {
  ...
  public function getContentId($contentstore_id, $sitestore) {
    ...
    $select_content = $select
            ->from($this->info('name'))
            ->where('store_id = ?', $contentstore_id)
    ...
    if (!empty($content_id)) {
      ...
      $select_photo = $select
              ...
              ->where('parent_content_id = ?', $content_id)
              ->where('type = ?', 'widget')
              ->where('name = ?', 'sitestore.photos-sitestore')->where('store_id = ?', $contentstore_id)
  ...
  public function getContentInformation($store_id) {
    $select = $this->select()->from($this->info('name'), array('admincontent_id', 'name'))
                    ...
                    ->where('store_id = ?', $store_id)->order('admincontent_id ASC');

    return $this->fetchAll($select);
  }

  /**
   * Set profile store default widget in admin content table without tab
   *
   * @param string $name
   * @param int $store_id
   * @param string $title
   ...
   * @param int $order
   */
  public function setAdminContentDefaultInfoWithoutTab($name = null, $store_id, $title = null, $titleCount = null, $order = null) {
        ...
        if (!empty($container)) {
          ...
          if (!empty($middle)) {
            ...
            if($name != 'sitestoreintegration.profile-items') {
              ...
              $this->settabstoreintwidget('document', '{"title":"Documents","resource_type":"document_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);

              $this->settabstoreintwidget('sitegroup', '{"title":"Groups","resource_type":"sitegroup_group_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);

              $this->settabstoreintwidget('sitepage', '{"title":"Pages","resource_type":"sitepage_page_0","nomobile":"0","name":"sitepageintegration.profile-items"}', $middle_id, $page_id);

              $this->settabstoreintwidget('list', '{"title":"Listings","resource_type":"list_listing_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
              							
							$this->settabstoreintwidget('folder', '{"title":"Folders","resource_type":"folder_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
							
						  $this->settabstoreintwidget('quiz', '{"title":"Quiz","resource_type":"quiz_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
						  
              $this->settabstoreintwidget('sitefaq', '{"title":"FAQs","resource_type":"sitefaq_faq_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
               
              $this->settabstoreintwidget('sitetutorial', '{"title":"Tutorials","resource_type":"sitetutorial_tutorial_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
              
	            $this->settabstoreintwidget('sitebusiness', '{"title":"Businesses","resource_type":"sitebusiness_business_0","nomobile":"0","name":"sitestoreintegration.profile-items"}', $middle_id, $store_id);
