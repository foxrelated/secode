 ...
 */
class Sitestore_Model_DbTable_Contentstores extends Engine_Db_Table implements Engine_Content_Storage_Interface {

  protected $_rowClass = "Sitestore_Model_Contentstore";

  public function loadMetaData(Engine_Content $contentAdapter, $name) {
  	
    $select = $this->select()->where('name = ?', $name)->orWhere('contentstore_id = ?', $name);
  ...
  public function loadContent(Engine_Content $contentAdapter, $name) {
    ...
    $select = $this->select()->where('store_id = ?', $sitestore_id)->where('name = ?', $name)->orWhere('contentstore_id = ?', $name);
    $store = $this->fetchRow($select);

    if (is_object($store)) {
			$contentTable = Engine_Api::_()->getDbtable('content', 'sitestore');
			$select = $contentTable->select()
							->where('contentstore_id = ?', $store->contentstore_id)
							->order('order ASC');
      ...
      $store_id = Engine_Api::_()->sitestore()->getWidgetizedStore()->page_id;
			$contentTable = Engine_Api::_()->getDbtable('admincontent', 'sitestore');
			$select = $contentTable->select()
							->where('store_id = ?', $store_id)
							->order('order ASC');
			$content = $contentTable->fetchAll($select);
			$structure = Engine_Api::_()->getDbtable('admincontent', 'sitestore')->prepareContentArea($content);
			$element = new Engine_Content_Element_Container(array(
  ...
  public function deleteStore(Sitestore_Model_Store $store) {
    ...
    Engine_Api::_()->getDbtable('content', 'sitestore')->delete(array('contentstore_id = ?' => $store->contentstore_id));
    $store->delete();
    return $this;
  }
  ...
  public function getContentStoreId($store_id) {
  	...
  	$selectStoreAdmin = $this->select()
            ->from($this->info('name'), array('contentstore_id', 'description', 'keywords'))
