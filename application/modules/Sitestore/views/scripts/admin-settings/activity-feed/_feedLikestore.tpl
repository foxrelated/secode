 ...
 * @version    $Id: _feedLikestore.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
<?php $dummyValue=Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestorefeed.likestore.dummy', 'c');?>
<div id="sitestorefeed_likestore_dummy-wrapper" class="form-wrapper">    
  <div id="sitestorefeed_likestore_dummy-label" class="tip">
  	<span>
  		<?php echo $this->translate("To enable users to receive on their homestore the activity feeds of Stores that they have liked, follow ANY ONE of the below 3 options.");?>
  	</span>	
  </div>
  <div class="form-label">&nbsp;</div>
  <div id="sitestorefeed_likestore_dummy-element" class="form-element">
    <p class="description"><input type="radio" onclick="showTips('tip_like_widget','like')" value="a" id="sitestorefeed_likestore_dummy-a" name="sitestorefeed_likestore_dummy" <?php if($dummyValue=='a'):?> checked ='checked'<?php endif; ?> > <?php echo $this->translate('Use "SocialEngineAddOns Activity Feed" widget instead of core "Activity Feed" widget on Member Home Page. (For this, go to "Layout" > "Layout Editor" > "Member Home Page" and replace the already placed "Activity Feed" widget with "SocialEngineAddOns Activity Feed" widget.)') ?></p>
    <p class="description"><input type="radio" onclick="showTips('tip_like_faq','like')" value="b" id="sitestorefeed_likestore_dummy-b" name="sitestorefeed_likestore_dummy" <?php if($dummyValue=='b'):?> checked ='checked'<?php endif; ?> >
    <a href="javascript:void(0);" onclick='javascript:$("sitestorefeed_likestore_dummy-b").checked="checked";showTips("tip_like_faq","like"); openSmoothboxFeed("<?php echo $this->url(array('module' => 'sitestore', 'controller' => 'settings', 'action' => 'overwrite', 'type' => 'activity_feed_likestore'),'admin_default') ?>");' > <?php echo $this->translate('Click here') ?> </a>
    ...
    <p class="description"><input type="radio" onclick="showTips('tip_like_faq','like'); faq_show('faq_37');" value="c" id="sitestorefeed_likestore_dummy-c" name="sitestorefeed_likestore_dummy" <?php if($dummyValue=='c'):?> checked ='checked'<?php endif; ?> ><?php echo $this->translate('If you do not want to follow any of the above two options, then you can manually do the minor code modification. (Please see below the steps that you need to follow.)') ?></p>
    <div id="tip_like_widget" style="display: none;" >
      <div class="tip">
    <span class="sitestore_activity_tip">
      <?php if (Engine_Api::_()->getApi('subCore', 'sitestore')->isCoreActivtyFeedWidget('user_index_home', 'activity.feed')): ?>
      ...
      <div class="tip">
       <span class="sitestore_activity_tip">
        <?php if (!Engine_Api::_()->getApi('subCore', 'sitestore')->isContentInFile($path, $string)): ?>
          ...
          <?php else:?>
            <?php echo $this->translate("The required changes have been successfully done for showing Store Activity Feeds to users on Home Page for Stores liked by them."); ?>
           <?php endif; ?>
        </span>
      </div> 
    </div>
    <div class='faq' style='display: none;' id='faq_37'>
      ...
      <?php echo $this->translate('1) First of all, make sure that you have selected "Store\'s Photo and Title" option for the field "Stores / Marketplace Activity Feed Type" above.') ?><br /><br />
