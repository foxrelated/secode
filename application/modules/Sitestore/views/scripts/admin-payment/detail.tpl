...
<dl class="payment_transaction_details">
  ...
  <dt>
    <?php if( $this->user && $this->user->getIdentity() ): ?>
      ...
      <?php endif; ?>
    <?php else: ?>
      <i><?php echo $this->translate('Deleted Store Owner') ?></i>
    <?php endif; ?>
  </dt>
  <dd>
  ...
  <dt>
    	<a href="<?php echo $this->url(array('store_url' => Engine_Api::_()->sitestore()->getStoreUrl($this->order->source_id)), 'sitestore_entry_view') ?>"  target='_blank' title="<?php echo  ucfirst($this->title) ?>">
						<?php echo $this->translate(Engine_Api::_()->sitestore()->truncation($this->title,25)) ?></a>
  </dt>
  <dd>
