 ...
 * @version    $Id: likesitestore.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
//	var likeMemberStore = <?php if(empty($this->no_result_msg)){ echo sprintf('%d', $paginater_vari); } else { echo 1; } ?>;
...
//	
//	//var url = sm4.core.baseUrl + 'sitestore/index/likesitestore';// URL where send ajax request.
//	var url = '<?php echo $this->url(array('action' => 'like-stores'), 'sitestore_like', true);?>'; 
//	function show_myfriend () {
//		$('sitestores_popup_content').innerHTML = '<center><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/sitestore/externals/images/spinner.gif" /></center>';
...
//				$('sitestores_popup_content').innerHTML = responseHTML;
//				}
//      }));
//  }
//
...
//		$('sitestores_popup_content').innerHTML = '<center><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitestore/externals/images/spinner.gif" alt="" class="mtop10" /></center>';
//      sm4.core.request.send(new Request.HTML({
//        'url' : url,
//        'data' : {
//          'format' : 'html',
...
//	var paginateLikeMembers = function(store, call_status) {
...
//  }
	...
	<div class="seaocore_members_popup">
		<div class="top">
			<?php 
				...
				if($this->call_status == 'public') {
					$title = $this->translate('People Who Like This Store') ;
				}else {
					$title = $this->translate('Friends Who Like This Store') ;
				} 
			?>
		...
		<div class="seaocore_members_popup_content" id="sitestores_popup_content">
		<?php } ?>
			...
			<?php if( $this->user_obj->getCurrentPageNumber() > 1 ): ?>
				<div class="seaocore_members_popup_paging">
					<div id="user_like_members_previous" class="paginator_previous">
						<?php echo $this->htmlLink('javascript:void(0);', $this->translate('Previous'), array('onclick' => 'paginateLikeMembers(likeMemberStore - 1, call_status)'
						)); ?>
					</div>
				</div>
			<?php endif; ?>
				...
				<?php if( $this->user_obj->getCurrentPageNumber() < $this->user_obj->count() ): ?>
					<div class="seaocore_members_popup_paging">
						<div id="user_like_members_next" class="paginator_next" style="border-top-width:1px;">
							<?php echo $this->htmlLink('javascript:void(0);', $this->translate('Next') , array('onclick' => 'paginateLikeMembers(likeMemberStore + 1, call_status)')); ?>
						</div>
					</div>
				<?php endif; ?>
			<?php endif; ?>
