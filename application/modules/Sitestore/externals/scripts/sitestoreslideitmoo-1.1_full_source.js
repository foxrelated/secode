...
/**
	...
	- navigators ( forward/back ) no longer added by script. Instead, add them into overallContainer making their display from CSS and after add the CSS selector class to navs parameter
		IE: navs:{ 
				fwd:'.Sitestore_SlideItMoo_forward',
				bk:'.Sitestore_SlideItMoo_back'
			}
...
var SocialengineSlideItMoo = new Class({
  ...
  options: {
    ...
    itemsSelector: null, /* css class for inner elements ( ie: .Sitestore_SlideItMoo_element ) */
      ...
      if (this.options.call_count == 1) {
        ...
        var request = new Request.HTML({
          ...
          {	
            ...
            if(module_name == 'Sitestorevideo' && en4.sitevideoview)
              en4.sitevideoview.attachClickEvent(Array('item_photo_sitestorevideo_video','sitestorevideo_title'));        
          }
        });
        request.send();
      }
