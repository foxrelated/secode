 ...
 */
class Sitestore_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($package->isOneTime()) {
      ...
      if ($paymentStatus == 'okay' ||
        ...
        $store->gateway_profile_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
    ...
    // Recurring
      ...
      if ($rdata['PROFILESTATUS'] == 'ActiveProfile' ||
        ...
        // Enable now
        $store->gateway_id = $this->_gatewayInfo->gateway_id;
        $store->gateway_profile_id = $rdata['PROFILEID'];
        $store->onPaymentSuccess();
  ...
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
    ...
    if (!empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback') {
      $store->onPaymentFailure(); // or should we use pending?
    }
  ...
  public function cancelStore($transactionId, $note = null) {
    ...
    if ($transactionId instanceof Sitestore_Model_Store) {
