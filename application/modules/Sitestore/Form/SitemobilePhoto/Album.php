 ...
 */
class Sitestore_Form_SitemobilePhoto_Album extends Engine_Form {
...
//     $linked_album_title = '<a target = "_parent" href = http://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view', 'store_id' => $album->store_id, 'album_id' => $album->album_id, 'slug' => $album->getSlug(), 'tab' => $content_id), 'sitestore_albumphoto_general') . "><b>$album->title</b></a>";
    ...
    if (count($values['file'] > 0)) {
      ...
      if ($activityFeedType) {
        $action = $api->addActivity(Engine_Api::_()->user()->getViewer(), $sitestore, $activityFeedType, null, array('count' => count($values['file']), 'linked_album_title' => $linked_album_title));
