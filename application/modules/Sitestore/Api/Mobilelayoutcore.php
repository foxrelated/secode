 ...
 */
class Sitestore_Api_MobilelayoutCore extends Core_Api_Abstract {
...
//       Engine_Api::_()->sitestore()->setDefaultDataWidget($contentTable, $contentTableName, $store_id, 'widget', 'sitestore.photorecent-sitestore', $middle_id, 6,'','true');
//     }
...
//     Engine_Api::_()->sitestore()->setDefaultDataWidget($contentTable, $contentTableName, $store_id, 'widget', 'sitestore.insights-sitestore', $left_id, 22, 'Insights','true');
    ...
    if ($sitestoreAlbumEnabled) {
      Engine_Api::_()->sitestore()->setDefaultDataWidget($contentTable, $contentTableName, $store_id, 'widget', 'sitestore.albums-sitestore', $left_id, 24, 'Albums','true');
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitestoreoffer')) {
      $this->setDefaultInfoWithoutTab('sitestoreoffer.profile-sitestoreoffers', $store_id, 'Offers', 'true', '116');
    }
   ...
   * @param int $store_id
   */
  public function setTabbedLayoutContent($store_id, $sitestore_layout_cover_photo) {
    ...
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitestoreoffer')) {
      $this->setContentDefaultInfo('sitestoreoffer.profile-sitestoreoffers', $store_id, 'Offers', 'true', '116');
    }
    //INSERTING EVENT WIDGET 
    if (Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('siteevent')) {
  ...
  public function setContentDefaultLayout($store_id) {
    ...
    $admincontentselected = $admincontentTable->select()->where('store_id =?', $corestoreinfo->page_id)->where('type =?', 'container');
    ...
    foreach ($admintableinfo as $value) {
      $mainContainer = $contentTable->createRow();
      $mainContainer->mobilecontentstore_id = $store_id;
      $mainContainer->type = 'container';
    ...
    //SELECT ADMIN CONTENT
    $admincontentselected = $admincontentTable->select()->where('store_id =?', $corestoreinfo->page_id)->where('type =?', 'widget')->where('name =?', 'sitemobile.container-tabs-columns');
    ...
    foreach ($admintableinfo as $values) {
      ...
      $mainWidgets->mobilecontentstore_id = $store_id;
              ...
              if (!empty($check_sitedocument)) {
                ...
                foreach ($results as $value) {
                  ...
                  $select
                          ...
                          ->where('type = ?', 'widget')
                          ->where('name = ?', 'sitestoreintegration.profile-items')
													->where('params = ?', '{"title":"Documents","resource_type":"document_0","nomobile":"0","name":"sitestoreintegration.profile-items"}');
                  $info = $select->query()->fetch();
                  if (empty($info)) {
                    ...
                    $db->insert('engine4_core_content', array(
                        ...
                        'parent_content_id' => $middle_id,
                        'order' => 999,
                        'params' => '{"title":"c","resource_type":"document_0","nomobile":"0","name":"sitestoreintegration.profile-items"}',
                    ));
                  }
                }
              }
              ...
              if (!empty($check_sitegroup)) {
                ...
                foreach ($results as $value) {
                  ...
                  $select
                          ...
                          ->where('type = ?', 'widget')
                          ->where('name = ?', 'sitestoreintegration.profile-items')
													->where('params = ?', '{"title":"Groups","resource_type":"sitegroup_group_0","nomobile":"0","name":"sitestoreintegration.profile-items"}');
                  $info = $select->query()->fetch();
                  if (empty($info)) {
                    ...
                    $db->insert('engine4_core_content', array(
                        ...
                        'parent_content_id' => $middle_id,
                        'order' => 999,
                        'params' => '{"title":"Groups","resource_type":"sitegroup_group_0","nomobile":"0","name":"sitestoreintegration.profile-items"}',
                    ));
                  }
                }
              }
              ...
              if (!empty($check_sitestore)) {
                ...
                foreach ($results as $value) {
                  ...
                  $select
                          ...
                          ->where('type = ?', 'widget')
                          ->where('name = ?', 'sitestoreintegration.profile-items')
													->where('params = ?', '{"title":"Stores","resource_type":"sitestore_store_0","nomobile":"0","name":"sitestoreintegration.profile-items"}');
                  $info = $select->query()->fetch();
                  if (empty($info)) {
                    ...
                    $db->insert('engine4_core_content', array(
                        ...
                        'parent_content_id' => $middle_id,
                        'order' => 999,
                        'params' => '{"title":"Stores","resource_type":"sitestore_store_0","nomobile":"0","name":"sitestoreintegration.profile-items"}',
                    ));
                  }
                }
              }
              ...
              if (!empty($check_list)) {
                ...
                foreach ($results as $value) {
                  ...
                  $select
                          ...
                          ->where('type = ?', 'widget')
                          ->where('name = ?', 'sitestoreintegration.profile-items')
													->where('params = ?', '{"title":"Listings","resource_type":"list_listing_0","nomobile":"0","name":"sitestoreintegration.profile-items"}');
                  $info = $select->query()->fetch();
                  if (empty($info)) {
                    ...
                    $db->insert('engine4_core_content', array(
                        ...
                        'parent_content_id' => $middle_id,
                        'order' => 999,
                        'params' => '{"title":"Listings","resource_type":"list_listing_0","nomobile":"0","name":"sitestoreintegration.profile-items"}',
                    ));
                  }
                  //}
                }
