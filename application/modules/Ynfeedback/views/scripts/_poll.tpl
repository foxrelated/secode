...
<?php
  $this->headScript()
    ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Ynfeedback/externals/scripts/poll/core.js');
...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    var initializePoll = function() {
      en4.poll.urls.vote = '<?php echo $this->url(array('action' => 'vote'), 'ynfeedback_general') ?>';
    ...
    if( $type(en4) == 'object' && 'poll' in en4 ) {
      ...
      new Asset.javascript(en4.core.staticBaseUrl + 'application/modules/Ynfeedback/externals/scripts/poll/core.js', {
        onload: function() {
          initializePoll();
        }
      });
...
<span class="poll_view_single">
  <form id="poll_form_<?php echo $this->poll->getIdentity() ?>" action="<?php echo $this->url() ?>" method="POST" onsubmit="return false;">
    ...
    <div class="poll_stats">
      <button href='javascript:void(0);' onClick='en4.poll.toggleResults(<?php echo $this->poll->getIdentity() ?>); this.blur();' id="poll_toggleResultsLink" class="poll_toggleResultsLink">
        <?php echo $this->translate($this->hasVoted ? 'Show Questions' : 'Show Results' ) ?>
      </button>
      <?php if (empty($this->hideVote)) :?>
