...
class Ynfeedback_IndexController extends Core_Controller_Action_Standard 
{
	...
	{
		// Check authorization to view feedback.
        if (!$this -> _helper -> requireAuth() -> setAuthParams('ynfeedback_idea', null, 'view') -> isValid())
        {
	...
	public function createPopupAction(){
		...
		//check max feedbacks user can create
		if ($viewer->getIdentity()) {
			$permissionsTable = Engine_Api::_()->getDbtable('permissions', 'authorization');
	        $max = $permissionsTable->getAllowed('ynfeedback_idea', $viewer->level_id, 'max_feedback');
	        if ($max == null) {
	        ...
	        if (($max != 0) && (sizeof($raw_data) >= $max)) {
	        	echo Zend_Json::encode(array('error' => true, 'message' => 'You reach the limitation of creating new feedbacks. Please delete at least one of them if you want to create a new one.'));
	            return;
	        }
		}
   ...
   public function manageFollowAction() {
	...
	public function createAction() {
		...
		if ($viewer->getIdentity()) {
	        ...
	        if (($max != 0) && (sizeof($raw_data) >= $max)) {
	            ...
	            $this->view->message = $this->view->translate('You reach the limitation of creating new feedbacks. Please delete at least one of them if you want to create a new one.');
	            return;
	        }
		}
		...
		if(!empty($values['guest_email'])) {
	        $max = Engine_Api::_()->getApi('settings', 'core')->getSetting('ynfeedback_email_maxfeedback', 0);
	        $table = Engine_Api::_()->getItemTable('ynfeedback_idea');
	        $select = $table->select()
	            -> where('guest_email = ?', $values['guest_email'])
	            -> where('deleted = ?', 0);
	            
	        $raw_data = $table->fetchAll($select);
	        if (($max != 0) && (sizeof($raw_data) >= $max)) {
	        	$form->addError($this->view->translate('Sorry but you reach the limitation of creating new feedbacks for this email address.'));
	            return;
	        }
		}
	...
	public function renderCreatePopupAction() {
        $this->_helper->layout->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
		$isFinal = $this->_getParam('isFinal', false);
        if ($isFinal) {
        	echo $this->view->partial('_popup_feedback.tpl', 'ynfeedback', array('isFinal' => true));
        }
		else {
			echo $this->view->partial('_popup_feedback.tpl', 'ynfeedback', array());
		}
    }
	
	public function checkLimitEmailAction(){
		$this -> _helper -> layout -> disableLayout();
		$this -> _helper -> viewRenderer -> setNoRender(true);
		
		$email = $this->_getParam('email');
		if (empty($email)) {
			echo Zend_Json::encode(array('error' => true, 'message' => $this->view->translate('The email must not be empty.')));
            return;
		}
        ...
        $raw_data = $table->fetchAll($select);
        if (($max != 0) && (sizeof($raw_data) >= $max)) {
        	echo Zend_Json::encode(array('error' => true, 'message' => $this->view->translate('Sorry but you reach the limitation of creating new feedbacks for this email address.')));
            return;
        }
		
		echo Zend_Json::encode(array('error' => false));
        return;
