...
<?php if($this->isAllowCreate) :?>
	<?php if ($this->isReachLimit) :?>
		<?php if($this -> isFinal) :?>
			<h3><?php echo $this -> translate('Awesome!!!'); ?></h3>
			<p class='form-description'><?php echo $this -> translate('Your feedback has just been created successfully. This is sent to Admin Panel and we will respond it soonest. However, you reach the limitation of creating new feedbacks. Please delete at least one of them if you want to create a new one.');?></p>
		<?php else:?>
			<div class='tip'>
        		<span><?php echo $this -> translate('You reach the limitation of creating new feedbacks. Please delete at least one of them if you want to create a new one.');?></span>
        	</div>
		<?php endif;?>
