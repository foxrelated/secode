...
function imageupload(){
    tempImageFlag++;
    if( tempImageFlag > 5 ) return;
     $('imageName').value='';
      $('imageenable').value=0;
      $('photo_id_filepath').value='';
      ...
      var listContainer = $('ul_images');
       var childCount = listContainer.getElementsByTagName('li').length;
       
      $('uploaded_images_' + tempImageFlag).innerHTML='';     
      ajaxUpload($('copyForm'), url_action, 'uploaded_images_' + tempImageFlag, '<center><img src=\"<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sitestoreproduct/externals/images/loader.gif\" border=\'0\' />', '');
