...
<div class='seaocore_db_tabs'>
  ...
  <?php if( !empty($dashboard_tabs) ) : ?>
    <ul>
	  <?php $mobileShow = array(1,3,4,5,7);?>
      <?php foreach( $dashboard_tabs as $storeTab ) : ?>
		<?php if(!in_array($storeTab->tab, $mobileShow)) continue;?>
        <?php $tempStyle = ""; ?>
        ...
        <li id="id_<?php echo $storeTab->tab ?>" <?php if( $this->sitestores_view_menu == $storeTab->tab ) : echo "class='selected'"; endif; ?> <?php echo $tempStyle; ?>>
          <?php echo $this->htmlLink($storeTab->getHref(), $this->translate($storeTab->getLabel()), array_merge($attribs,array("onclick" => "window.location.href='".$storeTab->getHref()."';return false;"))); ?>
        </li>
      <?php endforeach; ?>
    </ul>
  <?php endif; ?>
