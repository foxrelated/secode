  ...
  <span class="fright">
    ...
    <?php
      if( $this->sitestoreproduct->store_id ) {
        ...
        echo "<a href='" . $this->url(array('action' => 'store', 'store_id' => $page->store_id), 'sitestore_dashboard', true) . "' class='sitestoreproduct_buttonlink'>" . $this->translate('Manage Products') . "</a>";
      }
    ?>
  </span>
...
{
  $("productStatusMessageLoadingImage").innerHTML = '<img src='+en4.core.staticBaseUrl+'application/modules/Sitestoreproduct/externals/images/loading.gif height=15 width=15>';
  en4.core.request.send(new Request.JSON({
    url : en4.core.baseUrl + 'sitestoreproduct/product/change-product-status/product_id/' + product_id,
    method : 'POST',
    data : {format : 'json'},
    onSuccess : function(responseJSON)  
    {
      $("productStatusMessageLoadingImage").innerHTML = '';
      if( responseJSON.success )
        $("productStatusMessage").innerHTML = '<ul class="form-notices"><li><?php echo $this->translate("Your product has been enabled successfully.") ?></li></ul>';
      $("tipMessageElement").removeClass('tip')
    }
    })
    )
}
