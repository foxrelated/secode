 ...
 * @version    $Id: list.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
"'.$this->subject->getIdentity().'", "'.($this->comments->getCurrentPageNumber()).'","'.$this->widget_id .'")'
    ...
    <li class="comment-<?php echo $comment->comment_id ?>" id="comment-<?php echo $comment->comment_id ?>">
      <?php if( $canDelete ): ?>
         <a href="javascript:void(0);" onclick="en4.srpinboard.comments.deleteComment('<?php
  echo $this->subject->getType()?>', '<?php echo $this->subject->getIdentity() ?>', '<?php echo
  $comment->comment_id ?>', '<?php echo
  $this->widget_id ?>')" class="sr_sitestoreproduct_comment_remove" title="<?php echo $this->translate('delete') ?>">
         </a>
      <?php endif; ?>
...
?></span>
        ...
        <?php if( $this->canComment ||$comment->likes()->getLikeCount() > 0 ): ?>
          <?php if( $this->canComment ):
              ...
              ?>
            <?php if( !$isLiked ): ?>
              <a href="javascript:void(0)" onclick="en4.srpinboard.comments.like(<?php echo
sprintf("'%s', %d, %d, %d", $this->subject->getType(), $this->subject->getIdentity(), $this->widget_id, $comment->getIdentity())
?>)">
                <?php echo $this->translate('like') ?>
              </a>
            <?php else: ?>
              <a href="javascript:void(0)" onclick="en4.srpinboard.comments.unlike(<?php echo
sprintf("'%s', %d, %d, %d ", $this->subject->getType(), $this->subject->getIdentity(), $this->widget_id, $comment->getIdentity())
?>)">
...
<script type="text/javascript">
  en4.core.runonce.add(function(){
    ...
    en4.srpinboard.comments.attachCreateComment($('comment-form_<?php echo $this->element_id ?>'),'<?php echo
