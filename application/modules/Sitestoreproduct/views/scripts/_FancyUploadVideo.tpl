 ...
 * @version    $Id: _FancyUploadVideo.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
window.addEvent('domready', function() { // wait for the content
	...
	up = new FancyUpload2($('demo-status'), $('demo-sitestore'), { // options object
    ...
    // url is read from the form, so you just have to change one place
		...
		onFileRemove: function(file) {
		  ...
		  file_id = file.photo_id;
                  ...
                  var demositestore = document.getElementById("demo-sitestore");
                  var demosubmit = document.getElementById("upload-wrapper");

                  demositestore.style.display = "none";
		...
		onSelectSuccess: function(file) {
                  $('demo-sitestore').style.display = 'block';
		...
		onFileSuccess: function(file, response) {
			...
			if (json.get('status') == '1') {
				...
				file.info.set('html', '<br/><b><?php echo $this->translate("Upload has failed: ") ?></b> <?php echo $this->translate("The video you tried to upload exceeds the maximum file size.") ?> <br/>' + (json.get('error') ? (json.get('error')) : response));
