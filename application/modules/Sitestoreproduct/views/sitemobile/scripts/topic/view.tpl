...
<?php  
   ...
   array("href"=>$this->sitestoreproduct->getHref(array('tab' => $this->tab_selected_id)),"title"=>"Discussions","icon"=>"arrow-r"),
   array("title"=>$this->topic->getTitle(),"icon"=>"arrow-d","class" => "ui-btn-active ui-state-persist"));
  
	echo $this->breadcrumb($breadcrumb);
?>

<div class="sitestoreproduct_discussion_view">
  ...
  <ul data-inset="true" data-role="listview" class="ui-listview ui-listview-inset ui-corner-all ui-shadow sm-ui-topic-view">
      ...
      <li class="<?php echo $liClass ?> ui-li-has-count">
       ...
       <div class="sitestoreproduct_sitestoreproducts_thread_info">
				<div class="sitestoreproduct_sitestoreproducts_thread_details_options">
					<div class="thread_options" data-role="controlgroup" data-type="horizontal" data-mini="true">
						<?php if ($post->user_id == $this->viewer()->getIdentity() || $this->canEdit == 1 || $this->topic->user_id == $this->viewer()->getIdentity()): ?>
							<?php
								echo $this->htmlLink(array('route' => 'sitestoreproduct_extended', 'controller' => 'post', 'action' => 'edit', 'post_id' => $post->getIdentity(), 'format' => 'smoothbox'), $this->translate('Edit'), array(
						'data-role'=>"button", 'data-icon'=>"edit",'data-iconpos'=>'notext'
							))?>
							<?php
							echo $this->htmlLink(array('route' => 'sitestoreproduct_extended', 'controller' => 'post', 'action' => 'delete', 'post_id' => $post->getIdentity(), 'format' => 'smoothbox'), $this->translate('Delete'), array(
				...
				<div class="sitestoreproduct_sitestoreproducts_thread_details">
					<h3 class="ui-li-heading"><?php echo $this->htmlLink($user->getHref(), $user->getTitle()) ?></h3>
          <div class="sitestoreproduct_sitestoreproducts_thread_details_date">
						<p class="ui-li-desc"><?php echo $this->timestamp(strtotime($post->creation_date)) ?></p>
          </div>
				</div>
				<div class="sitestoreproduct_sitestoreproducts_thread_body">
					<?php echo $this->viewMore(nl2br($this->BBCode($post->body))) ?>
				</div>
				<span class="sitestoreproduct_sitestoreproducts_thread_body_raw" style="display: none;">
