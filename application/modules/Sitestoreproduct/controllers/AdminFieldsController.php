 ...
 */
class Sitestoreproduct_AdminFieldsController extends Fields_Controller_AdminAbstract {
  ...
  public function fieldCreateAction() {
    ...
    $form->setTitle('Add Product Question');
    //$form->setDescription('If you have created a profile field of “Select Box” type, then all the combinations created by you for this product (configurable / virtual products) category will get deleted and you need to create new combinations with the newly created select box type for your product.');
    
    $form->removeElement('show');
    $form->addElement('hidden', 'show', array('value' => 0));

    $display = $form->getElement('display');
    $display->setLabel('Show on product page?');
    $display->setOptions(array('multiOptions' => array(
            1 => 'Show on product page',
            0 => 'Hide on product page'
            )));
    ...
    $form->addElement('select', 'quick_info', array(
        'label' => 'Show in quick specifications widget?',
        'multiOptions' => array(
            1 => 'Show in quick specifications widget',
            0 => 'Hide from quick specifications widget'),
        'value' => 0
    ));
...
//    if (!empty($cfType) && $cfType == 'select') {
//      $option_id = $this->_getParam('option_id');
//      $child_ids = Engine_Api::_()->getDbTable('maps', 'sitestoreproduct')->getChildIds($option_id);
//      if (!empty($child_ids)) {
...
//    }
  ...
  public function fieldEditAction() {
    ...
    //START CUSTOMIZATION BY SOCIALENGINEADDONS
    $form->setTitle('Edit Product Question');
    $form->removeElement('show');
    $form->addElement('hidden', 'show', array('value' => 0));
...
//    }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sitestoreproduct_Form_Admin_Type();
    ...
    if ($form->getValue('duplicate') != 'null') {
      // Create New Option in engine4_sitestoreproduct_product_fields_options
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sitestoreproduct_product_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sitestoreproduct_product_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sitestoreproduct_product_fields_maps', array(
...
//      }
      ...
      Engine_Api::_()->getDbtable('products', 'sitestoreproduct')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));

      //DELETE MAPPING
      Engine_Api::_()->getDbtable('categories', 'sitestoreproduct')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
  ...
  public function mapDeleteAction() {
    ...
    //DELETE CONFIGURABLE / VIRTUAL PRODUCT COMBINATIONS OF THE DELETED FIELDS
//    if(!empty($child_id)){
//      Engine_Api::_()->sitestoreproduct()->deleteCombinations($child_id);
//    }
