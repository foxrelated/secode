 ...
 * @version    $Id: AdminFieldsReviewController.php 2013-09-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitestoreproduct_AdminFieldsReviewController extends Fields_Controller_AdminAbstract {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sitestoreproduct_admin_reviewmain', array(), 'sitestoreproduct_admin_reviewmain_fields');
  ...
  public function fieldEditAction() {
    ...
    //START CUSTOMIZATION BY SOCIALENGINEADDONS
    $form->setTitle('Edit Review Question');
    $form->removeElement('show');
    $form->addElement('hidden', 'show', array('value' => 0));
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sitestoreproduct_Form_Admin_Typereview();
    ...
    if ($form->getValue('duplicate') != 'null') {
      // Create New Option in engine4_sitestoreproduct_review_fields_options
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sitestoreproduct_review_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sitestoreproduct_review_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sitestoreproduct_review_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      ...
      Engine_Api::_()->getDbtable('reviews', 'sitestoreproduct')->update(array('profile_type_review' => 0), array('profile_type_review = ?' => $option_id));

      //DELETE MAPPING
      Engine_Api::_()->getDbtable('categories', 'sitestoreproduct')->update(array('profile_type_review' => 0), array('profile_type_review = ?' => $option_id));
    }
