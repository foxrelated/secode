 ...
 * @author     John
 */
class Sitestoreproduct_View_Helper_FieldValueLoopStore extends Fields_View_Helper_FieldValueLoop
{
  public function fieldValueLoopStore($subject, $partialStructure)
  {
      ...
      if( $field->type == 'heading' ) {
        ...
        if( !empty($firstValue->value) && !empty($tmp) ) {
          ...
          if( !$isHidden || $show_hidden ) {
            ...
            $lastContents .= <<<EOF
  <span data-field-id={$field->field_id}>
    {$notice}
    {$label}:
    <b>
      {$tmp}
