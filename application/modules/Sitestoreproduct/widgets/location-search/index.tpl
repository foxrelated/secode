...
?>
<?php if( $this->form ): ?>
  <div class="global_form_box sitestore_advanced_search_form">
...
<script type="text/javascript">
  ...
  var myLatlng;
	window.addEvent('domready', function() {
	  ...
	  if(document.getElementById('sitestoreproduct_location').value == '') {
			submiForm();
		}
	  ...
	  $('store_location_pops_loding_image').injectAfter($('done-element'));
		new google.maps.places.Autocomplete(document.getElementById('sitestoreproduct_location'));
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function(position){
			...
			mapGetDirection = new google.maps.Map(document.getElementById("sitestore_location_map_none"), myOptions);
        ...
        if(!position.address) {
          ...
          service.search(request, function(results, status) { 
            if (status  ==  'OK') {
              ...
              document.getElementById('sitestoreproduct_location').value = (results[index].vicinity) ? results[index].vicinity :'';
          ...
          document.getElementById('sitestoreproduct_location').value = location;
					document.getElementById('Latitude').value = lat;
					document.getElementById('Longitude').value = lng;
					document.getElementById('locationmiles').value = 1000;
          //form submit by ajax
	...
	function submiForm() { 
  	...
  	if ($('category_id').options[$('category_id').selectedIndex].value == 0) { 
		...
		document.getElementById('store_location_pops_loding_image').style.display ='';
		en4.core.request.send(new Request.HTML({
			method : 'post',
			'url' : url,
			'data' : param,
			onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
				document.getElementById('store_location_pops_loding_image').style.display ='none';
				$('sitestore_map_container_topbar').style.display ='block';
				document.getElementById('sitestore_location_map_anchor').getParent().innerHTML = responseHTML;
				setMarker();
			  en4.core.runonce.trigger();
				$('sitestore_map_container').style.visibility = 'visible'; 
	...
	function locationStore() {
		var  sitestore_location = document.getElementById('sitestoreproduct_location');
	...
	function advancedSearchSitestores() {
		...
		if (flag == 0) {
		  if ($('fieldset-grp2'))
			...
			if ($('sitestoreproduct_street'))
			$('sitestoreproduct_street').value = '';
// 			if ($('sitestore_postalcode'))
// 			$('sitestore_postalcode').value = '';
			if ($('sitestoreproduct_country'))
			$('sitestoreproduct_country').value = '';
			if ($('sitestoreproduct_state'))
			$('sitestoreproduct_state').value = '';
			if ($('sitestoreproduct_city'))
			$('sitestoreproduct_city').value = '';
			if ($('profile_type'))
			$('profile_type').value = '';
...
<script type="text/javascript">
  ...
  var location_subcategoryies = function(category_id, sub, subcatname, subsubcat) {
    ...
    if(category_id != 0) {
  	...
  	var url = '<?php echo $this->url(array('action' => 'subcategory'), 'sitestoreproduct_general', true);?>';
...
//     var contentAutocomplete = new Autocompleter.Request.JSON('sitestore_location', '<?php //echo $this->url(array('module' => 'sitestoremember', 'controller' => 'widgets', 'action' => 'getitem'), 'admin_default', true) ?>', {
//       'postVar' : 'text',
//       'minLength': 1,
//       'selectMode': 'pick',
