...
<?php $latitude = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestoreproduct.map.latitude', 0); ?>
<?php $longitude = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestoreproduct.map.longitude', 0); ?>
<?php $defaultZoom = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestoreproduct.map.zoom', 1); ?>
...
<script type="text/javascript">
  ...
  var paginateStoreLocations = function(store) {
    ...
    var url = en4.core.baseUrl + 'widget/index/mod/sitestoreproduct/name/browselocation-sitestoreproduct';
    //clearOverlays();
    gmarkers = [];
				...
				<?php //if(!empty($item->location) || !empty($this->locationVariable)) : ?>
					<li>
						<div class="seaocore_browse_list_photo">
							<?php echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumb.normal'), array('title' => $item->getTitle(), 'target' => '_parent', 'class' => !empty($item->location)? "marker_photo_".$item->product_id :'un_location_sitestore')); ?>
						</div>
							...
							<div class='seaocore_browse_list_info'>
								<div class='seaocore_browse_list_info_title'>
									...
									<?php echo $this->htmlLink($item->getHref(), $item->getTitle(), array('title'
										=> $item->getTitle(), 'target' => '_parent', 'class' =>!empty($item->location)? "marker_".$item->product_id :'un_location_sitestore')); ?>
							  </div>
...
//										$statistics .= $this->translate(array('%s follower', '%s followers', $item->follow_count), $this->locale()->toNumber($item->follow_count)).', ';
										...
										<?php  echo $this->translate("Location: "); echo $this->translate($item->location); ?>
											- <b>
													<?php if (!empty($this->mobile)) : ?>
														<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->product_id, 'resouce_type' => 'sitestoreproduct_product', 'is_mobile' => $this->mobile), $this->translate("Get Directions"), array('target' => '_blank')) ; ?>
													<?php else: ?>
														<?php if (!empty($this->is_ajax)) : ?>
															<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->product_id, 'resouce_type' => 'sitestoreproduct_product'), $this->translate("Get Directions"), array('onclick' => 'owner(this);return false')) ; ?>
															<?php else : ?>
																<?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $item->product_id, 'resouce_type' => 'sitestoreproduct_product'), $this->translate("Get Directions"), array('class' => 'smoothbox')) ; ?>
															<?php endif; ?>
													<?php endif; ?>
												</b>
								</div>
			...
			<div class="clr sitestore_browse_location_paging" style="margin-top:10px;">
				<?php echo $this->paginationControl($this->result, null, array("pagination/pagination.tpl", "sitestoreproduct"), array("orderby" => $this->orderby)); ?>
		...
		<?php else: ?>
			<div class="tip"> 
				<span><?php echo $this->translate('Nobody has created a product with that criteria.'); ?></span>
			</div>
		<?php endif; ?>
		<?php if (empty($this->is_ajax)) : ?>	
	</div>
...
<script type="text/javascript" >
  ...
  window.addEvent('domready',function() {
    //smallLargeMap(1);
    var Clientwidth = $('global_content').getElement(".layout_sitestoreproduct_browselocation_sitestoreproduct").clientWidth;
...
<script type="text/javascript" >
  ...
  function createMarker(latlng, name, html,title_store, product_id) {
    ...
    //Show tooltip on the mouse over.
	  $$('.marker_' + product_id).each(function(locationMarker) {
    ...
    //Show tooltip on the mouse over.
	  $$('.marker_photo_' + product_id).each(function(locationMarker) {
			locationMarker.addEvent('mouseover',function(event) {
				google.maps.event.trigger(map, 'resize');
				map.setCenter(marker.position);
				infoBubbles.open(map,marker);
  ...
  function setMapCenterZoomPoint(bounds, maplocation) {
    ...
    if (bounds &&  bounds.center_lat &&  bounds.center_lng) {
      ...
      maplocation.setCenter(new google.maps.LatLng('<?php echo $latitude; ?>', '<?php echo $longitude;?>'), 4);
    }
    if (bds) {
...
<?php   foreach ($this->locations as $location) : ?>
	...
	var product_id = <?php echo $this->sitestoreproduct[$location->product_id]->product_id  ?>;
	<?php if(!empty ($enableBouce)):?>
	var sponsored = <?php echo $this->sitestoreproduct[$location->product_id]->sponsored ?>
	<?php else:?>
	var sponsored =0;
	<?php endif; ?>
	// create the marker

	<?php $product_id = $this->sitestoreproduct[$location->product_id]->product_id; ?>
	var contentString = '<div id="content">'+
		'<div id="siteNotice">'+
		'</div>'+'  <ul class="sitestores_locationdetails"><li>'+

		'<div class="sitestores_locationdetails_info_title">'+
		'<?php echo $this->htmlLink($this->sitestoreproduct[$location->product_id]->getHref(), $this->string()->escapeJavascript($this->sitestoreproduct[$location->product_id]->getTitle())); ?>'+

		'<div class="fright">'+
		'<span >'+
					<?php if ($this->sitestoreproduct[$location->product_id]->featured == 1): ?>
							...
							'</span>'+
								'<span>'+
					<?php if ($this->sitestoreproduct[$location->product_id]->sponsored == 1): ?>
		...
		'<?php echo $this->htmlLink($this->sitestoreproduct[$location->product_id]->getHref(), $this->itemPhoto($this->sitestoreproduct[$location->product_id], 'thumb.normal', '', array('align' => 'center'))); ?>'+
		'</div>'+
		'<div class="sitestores_locationdetails_info">'+

		<?php if ($this->ratngShow): ?>
			<?php if (($this->sitestoreproduct[$location->product_id]->rating > 0)): ?>
					'<span class="clr">'+
					<?php for ($x = 1; $x <= $this->sitestoreproduct[$location->product_id]->rating; $x++): ?>
							'<span class="rating_star_generic rating_star"></span>'+
					<?php endfor; ?>
					<?php if ((round($this->sitestoreproduct[$location->product_id]->rating) - $this->sitestoreproduct[$location->product_id]->rating) > 0): ?>
		...
		<?php endif; ?>
            ...
            '<?php echo $this->htmlLink("https://maps.google.com/?daddr=".urlencode($location->location), $this->translate($location->location), array('target' => 'blank')) ?>'+     
					'</div>'+
					'</div>'+
					'<div class="clr"></div>'+
					' </li></ul>'+
					'</div>';
				var marker = createMarker(point,sponsored,contentString,"<?php echo $this->string()->escapeJavascript($this->sitestoreproduct[$location->product_id]->getTitle()); ?>", product_id);
