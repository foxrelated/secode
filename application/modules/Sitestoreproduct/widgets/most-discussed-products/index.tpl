    ...
    <?php foreach ($this->products as $sitestoreproduct): ?>
      <li> 
        ...
        <div class='seaocore_sidebar_list_info'>
          ...
          <div class='seaocore_sidebar_list_details'>
            <?php echo $this->translate(array('%s Discussion', '%s Discussions', $sitestoreproduct->counttopics), $this->locale()->toNumber($sitestoreproduct->counttopics)) ?>,  
            <?php echo $this->translate(array('%s Reply', '%s Replies', $sitestoreproduct->total_count), $this->locale()->toNumber($sitestoreproduct->total_count)) ?>  
          </div>
...
<?php else: ?>
   ...
   <ul class="sitestoreproduct_grid_view sitestoreproduct_sidebar_grid_view mtop10">
    <?php foreach ($this->products as $sitestoreproduct): ?>
     <li class="g_b <?php if($isLarge): ?>largephoto<?php endif;?>" style="width: <?php echo $this->columnWidth; ?>px;height:<?php echo $this->columnHeight; ?>px;">
      <div>
        ...
        <div class="sitestoreproduct_grid_view_thumb_wrapper"> 
          <a class="sitestoreproduct_quick_view_btn" href="javascript:void(0);" onclick="Smoothbox.open('<?php echo $this->url(array('action' => 'quick-view', 'product_id' => $sitestoreproduct->product_id), "sitestoreproduct_product_general", true) ?>')">
