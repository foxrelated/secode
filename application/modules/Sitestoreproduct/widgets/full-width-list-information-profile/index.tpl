...
<script type="text/javascript">
  tempImageZoomWidth = 550;
  tempImageZoomHeight = 500;
...
<?php if((!empty($this->showProductRating) && !empty($this->sitestoreproduct->review_count)) || !empty($this->storeInfo)): ?>
<div class="clr sitestoreproduct_psi mtop10 mbot5">
  <?php endif; ?>
...
<!--            <a href="javascript:void(0);" class = 'sea_add_tooltip_link' rel='sitestoreproduct_product 1'>
              <?php // echo $this->showRatingStarSitestoreproduct($this->sitestoreproduct->rating_users, 'user', 'small-star'); ?>
            </a>-->
          <!--</div>-->
        </div>
      ...
      <?php endif; ?>
        ...
        <?php if(!empty($this->showProductRating) && !empty($this->sitestoreproduct->review_count)): ?>
          <?php $ratingDataTopbox = Engine_Api::_()->getDbTable('ratings', 'sitestoreproduct')->ratingbyCategory($this->sitestoreproduct->product_id, 'user', $this->sitestoreproduct->getType()); ?>
          <!--<div class="pro_rating">-->
          <div class="sr_sitestoreproduct_profile_review_stars">
            <div class="rate_head f_small fright bold"><?php echo $this->translate("Product Rating"); ?></div>
            <div class="sr_sitestoreproduct_profile_review_rating seaocore_txt_light clr">
              <span class="fleft">
                <?php echo $this->showRatingStarSitestoreproduct($this->sitestoreproduct->rating_users, 'user', 'small-star'); ?>
              </span>
              <?php // if (count($ratingDataTopbox) > 1): ?>
              ...
              <div class="sr_sitestoreproduct_ur_bdown_box_wrapper br_body_bg b_medium">
                <div class="sr_sitestoreproduct_ur_bdown_box">
                  ...
                  <div class="sr_sitestoreproduct_profile_rating_parameters">
                    <?php $iteration = 1; ?>
                    <?php foreach ($ratingDataTopbox as $reviewcatTopbox): ?>
...
<?php endif; ?>
    ...
    <?php if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestoreproduct.locationfield', 0) && !empty($this->sitestoreproduct->location) && !empty($this->isQuickView)) : ?>
      <div class="sr_sitestoreproduct_profile_info_sep b_medium">
        <?php  echo $this->translate("Location: "); echo $this->translate($this->sitestoreproduct->location); ?>
        - <b>
          <?php echo  $this->htmlLink(array('route' => 'seaocore_viewmap', "id" => $this->sitestoreproduct->product_id, 'resouce_type' => 'sitestore_store'), $this->translate("Get Directions"), array('class' => 'smoothbox')) ; ?>
          </b>
        </div>
      <?php endif; ?>

<?php $temp_allowed_selling = Engine_Api::_()->sitestoreproduct()->getIsAllowedSellingProducts($this->sitestoreproduct->store_id);
if (!empty($temp_allowed_selling) && !empty($this->sitestoreproduct->allow_purchase)):
  ?>
...
<?php endif; ?>
      ...
      // Check Enetred combination quantity is available or not.
        if (form_error != 1){
...
<?php endif; ?>
    ...
    if ($('sitestoreproduct_product_image_zoom')) {
      ...
      setTimeout(function() {
        ...
        if (tempImgWidth >= 550 && tempImgHeight >= 500)
