...
<ul class="seaocore_sidebar_list">
    ...
    <?php  $this->partial()->setObjectKey('sitestoreproduct_video');
		...
		?>		    
					<?php
          $sitestoreproduct->product_title  = Engine_Api::_()->getItem('sitestoreproduct_product', $sitestoreproduct_video->product_id);
					$truncation_limit = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitestoreproduct.title.truncation', 18);
					$tmpBody = strip_tags($sitestoreproduct->product_title);
					$product_title = ( Engine_String::strlen($tmpBody) > $truncation_limit ? Engine_String::substr($tmpBody, 0, $truncation_limit) . '..' : $tmpBody );
          ?>
          <?php echo $this->translate("in ") . $this->htmlLink($sitestoreproduct_video->getHref(), $product_title, array('title' => $sitestoreproduct->product_title)) ?>    
          </div>
