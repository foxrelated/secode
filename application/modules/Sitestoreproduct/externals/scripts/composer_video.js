/* $Id: composer_video.js 2013-09-02 00:00:00Z SocialEngineAddOns Copyright 2012-2013 BigStep Technologies Pvt. Ltd. $ */

Composer.Plugin.Sitestoreproduct = new Class({
  ...
  activate: function() {
    ...
    this.elements.formInput = new Element('select', {
      'id': 'compose-sitestoreproduct_video-form-type',
      'class': 'compose-form-input',
      'option': 'test',
      'events': {
        'change': this.updateSitestoreproductvideoFields.bind(this)
      }
    }).inject(this.elements.body);
    var options = 0;
    $('compose-sitestoreproduct_video-form-type').options[options++] = new Option(this._lang('Choose Source'), '0');
    if (this.options.youtubeEnabled == 1) {
      $('compose-sitestoreproduct_video-form-type').options[options++] = new Option(this._lang('YouTube'), '1');
    }
    $('compose-sitestoreproduct_video-form-type').options[options++] = new Option(this._lang('Vimeo'), '2');

//    if (this.options.allowed == 1) {
//      $('compose-sitestoreproduct_video-form-type').options[options++] = new Option(this._lang('My Computer'), '3');
//    }
    //link to full composer for now
    //$('compose-sitestoreproduct_video-form-type').options[options++] = new Option('My Computer','3');

    this.elements.formInput = new Element('input', {
      'id': 'compose-sitestoreproduct_video-form-input',
      'class': 'compose-form-input',
      'type': 'text',
      'style': 'display:none;'
    }).inject(this.elements.body);
   ...
   if(DetectMobileQuick() || DetectIpad()){
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-sitestoreproduct_video-form-type').options[options++] = new Option(this._lang('My Device'), '3');
    }
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitestoreproduct_video-upload',
        'class' : 'compose-sitestoreproduct_video-upload',
        'html' : this._lang('To upload a video from your device, please use our <a href="'+en4.core.baseUrl+'sitestoreproduct/video/create/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    else{
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-video-form-type').options[options++] = new Option(this._lang('My Computer'), '3');
    }
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitestoreproduct_video-upload',
        'class' : 'compose-sitestoreproduct_video-upload',
        'html' : this._lang('To upload a video from your computer, please use our <a href="'+en4.core.baseUrl+'sitestoreproduct/video/create/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    ...
    this.elements.formSubmit = new Element('button', {
      'id': 'compose-sitestoreproduct_video-form-submit',
  ...
  deactivate: function() {
    ...
    if (this.params.video_id)
      new Request.JSON({
        url: en4.core.basePath + 'sitestoreproduct/video/delete',
        data: {
          format: 'json',
  ...
  doAttach: function(e) {
    ...
    var sitestoreproduct_video_element = document.getElementById("compose-sitestoreproduct_video-form-type");
    var type = sitestoreproduct_video_element.value;
  ...
  doProcessResponse: function(responseJSON, responseText) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id': 'compose-sitestoreproduct_video-preview-image',
  ...
  doImageLoaded: function() {
    ...
    this.elements.previewInfo = new Element('div', {
      'id': 'compose-sitestoreproduct_video-preview-info',
      'class': 'compose-preview-info'
    }).inject(this.elements.body);

    this.elements.previewTitle = new Element('div', {
      'id': 'compose-sitestoreproduct_video-preview-title',
    ...
    this.elements.previewDescription = new Element('div', {
      'id': 'compose-sitestoreproduct_video-preview-description',
  ...
  updateSitestoreproductvideoFields: function(element) {
    var sitestoreproduct_video_element = document.getElementById("compose-sitestoreproduct_video-form-type");
    var url_element = document.getElementById("compose-sitestoreproduct_video-form-input");
    var post_element = document.getElementById("compose-sitestoreproduct_video-form-submit");
    var upload_element = document.getElementById("compose-sitestoreproduct_video-upload");
    // clear url if input field on change
    $('compose-sitestoreproduct_video-form-input').value = "";

    // If sitestoreproduct_video source is empty
    if (sitestoreproduct_video_element.value == 0)
    {
    ...
    // If sitestoreproduct_video source is youtube or vimeo
    if (sitestoreproduct_video_element.value == 1 || sitestoreproduct_video_element.value == 2)
    {
      upload_element.style.display = "none";
    ...
    // if sitestoreproduct_video source is upload
    if (sitestoreproduct_video_element.value == 3)
    {
