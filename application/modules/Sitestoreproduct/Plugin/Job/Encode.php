 ...
 */
class Sitestoreproduct_Plugin_Job_Encode extends Core_Plugin_Job_Abstract {
  ...
  protected function _execute() {
    ...
    // Get video object
    $video = Engine_Api::_()->getItem('sitestoreproduct_video', $video_id);
    if (!$video || !($video instanceof Sitestoreproduct_Model_Video)) {
   ...
   protected function _process($video, $type, $compatibilityMode = false){
    ...
    if (!($video instanceof Sitestoreproduct_Model_Video)) {
    ...
    if (!$success) {
      ...
      $notificationMessage = 'Video conversion failed. You may be over the site upload limit.  Try %1$suploading%2$s a smaller file, or delete some files to free up space.';
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'sitestoreproduct_video_processed_failed', array(
      ...
      throw new Sitestoreproduct_Model_Exception($exceptionMessage);
    }
    ...
    // Success
      ...
      try {
        // new action
        $sitestoreproduct = Engine_Api::_()->getItem('sitestoreproduct_product', $video->product_id);
        ...
        // notify the owner
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'sitestoreproduct_video_processed');
