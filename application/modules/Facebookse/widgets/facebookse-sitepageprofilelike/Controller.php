...
class Facebookse_Widget_FacebookseSitepageprofilelikeController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    if (!Engine_Api::_()->getDbtable('mixsettings', 'facebookse')->checkLikeButton($module, ''))
		...
		$fbpage_url = $front->getRequest()->getparam('page_url');
		if (!empty ($fbpage_url)) {
		  $fblike_moduletype_id = Engine_Api::_()->sitepage()->getPageId($fbpage_url);
		}
		else {
		  $fblike_moduletype_id = '';
		}
    $this->view->fblike_moduletype_id = $fblike_moduletype_id;
		
    if (($module == 'sitepage' && $action == 'view')) {
