...
class Facebookse_Widget_FacebookseGroupprofilelikeController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    if (!Engine_Api::_()->getDbtable('mixsettings', 'facebookse')->checkLikeButton($module, ''))
		...
		$facebookse_page_type = Engine_Api::_()->getApi('settings', 'core')->getSetting('facebookse.page.type');
		$fblike_moduletype_id = $front->getRequest()->getparam('id');
    $this->view->fblike_moduletype_id = $fblike_moduletype_id;
    if (($module == 'sitegroup' && $action == 'view') || ($module == 'group' && $action == 'index')) {
