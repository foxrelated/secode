 ...
 */
class Ynvideo_Plugin_Job_Maintenance_RebuildPrivacy extends Core_Plugin_Job_Abstract
{
	...
	protected function _execute()
	{
		...
		if (null === $total)
		{
			$total = $table -> select() -> from($table -> info('name'), new Zend_Db_Expr('COUNT(*)')) -> query() -> fetchColumn(0);
		...
		while (!$break && $count <= $limit)
		{
			...
			$item = $table -> fetchRow($table -> select() -> where('`' . $primaryCol . '` >= ?', (int)$position + 1) -> order($primaryCol . ' ASC') -> limit(1));
			...
			else
			{
				...
				foreach ($actions as $action)
				{
					...
					if ($itemOwner)
					{
						$options = (array)Engine_Api::_() -> authorization() -> getAdapter('levels') -> getAllowed($item -> getType(), $itemOwner, 'auth_' . $action);
