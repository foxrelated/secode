...
<div class="generic_list_widget">
    <ul class="ymb_menuRight_wapper ynidea-category">
        <?php foreach ($this->categories as $category) : ?>
            <li value ='<?php echo $category->getIdentity() ?>' class="ynidea-category_row <?php if ($category->level > 1) echo 'ynidea-category-sub-category child_'.$category->parent_id.' level_'.$category->level?>">
                <div 
                ...
                if($request-> getParam('category_id') == $category -> category_id) echo 'class = "active"';?>>
                    <?php if(count($category->getChildList()) > 0 && !$session-> mobile) : ?>
                        <div class="ynidea-category-collapse-control ynidea-category-collapsed"></div>
                    <?php else : ?>
                        <div class="ynidea-category-collapse-nocontrol"></div>
                    <?php endif; ?>
