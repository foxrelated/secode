...
class Ynidea_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Ynidea_Model_Node';
	 ...
	 * @param   Ynidea_Model_Node  $node
	 * @param   Array                $data
	 * @return  Ynidea_Model_Node
	 * @throw Exception
	 */
	public function insertAfter(Ynidea_Model_Node $node, $data) {
		if($node -> pleft == 1) {
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Ynidea_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function appendChild(Ynidea_Model_Node $node, $data) {
	 ...
	 * @throw Exception
	 */
	public function prependChild(Ynidea_Model_Node$node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Ynidea_Model_Node $node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	 ...
	 * @throw Exception
	 */
	public function deleteNode(Ynidea_Model_Node $node, $node_id = NULL) {
		$anchor = $node -> pleft;
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Ynidea_Model_Node) {
