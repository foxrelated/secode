 ...
 * @version    $Id: Package.php 2015-05-11 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteeventpaid_Model_Package extends Core_Model_Item_Abstract {
    ...
    public function getProduct() {
        if (null === $this->_product) {
            ...
            $this->_product = $productsTable->fetchRow($productsTable->select()
                            ->where('extension_type = ?', 'siteeventpaid_package')
    ...
    public function getProductParams() {
        return array(
            ...
            'price' => $this->price,
            'extension_type' => 'siteeventpaid_package',
    ...
    public function getPackageDescription() {
        ...
        if ($this->duration > 0 && $this->duration_type != 'forever') {
            if ($this->duration > 1)
                $typeStr = $translate->translate($this->duration_type . 's');
            else
                $typeStr = $translate->translate($this->duration_type);
...
//      $typeStr = $translate->translate($typeStr);
    ...
    public function getPackageQuantity() {
        ...
        // Add duration, if not forever
        if ($this->duration > 0 && $this->duration_type != 'forever') {
            $typeStr = $translate->translate(array($this->duration_type, $this->duration_type . 's', $this->duration), $view->locale()->toNumber($this->duration));
            // $typeStr = $translate->translate($typeStr);
            $str = $this->duration.' '.$typeStr;
