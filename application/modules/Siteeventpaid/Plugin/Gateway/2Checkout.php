 ...
 * @version    $Id: 2Checkout.php 2015-05-11 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteeventpaid_Plugin_Gateway_2Checkout extends Engine_Payment_Plugin_Abstract {
    ...
    Payment_Model_Order $order, array $params = array()) {
        ...
        // Get related info
        $user = $order->getUser();
        $eventOtherinfo = Engine_Api::_()->getDbTable('otherinfo', 'siteevent')->getOtherinfo($order->source_id);
        ...
        // Get benefit setting
        $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'siteeventpaid')
        ...
        if ($giveBenefit) {
            ...
            // Update subscription
            $eventOtherinfo->gateway_id = $this->_gatewayInfo->gateway_id;
            $eventOtherinfo->gateway_profile_id = $params['order_number'];
            $eventOtherinfo->save();
// This is the same as sale_id
            $event->onPaymentSuccess();

            // send notification
            if ($event->didStatusChange()) {
                Engine_Api::_()->siteeventpaid()->sendMail("ACTIVE", $event->event_id);
            }
        ...
        // Enable later
            ...
            $eventOtherinfo->save();
            //This is the same as sale_id  
            $event->onPaymentPending();

            // send notification
            if ($event->didStatusChange()) {
                Engine_Api::_()->siteeventpaid()->sendMail("PENDING", $event->event_id);
            }
    ...
    /**
     * Process ipn of event transaction
     *
     * @param Payment_Model_Order $order
     * @param Engine_Payment_Ipn $ipn
     */
    ...
    Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
        ...
        // Update subscription
        $eventUpdated = false;
        if (!empty($rawData['sale_id']) && empty($event->gateway_profile_id)) {
            $eventUpdated = true;
            $event->gateway_profile_id = $rawData['sale_id'];
        }
        if (!empty($rawData['invoice_id']) && empty($event->gateway_transaction_id)) {
            $eventUpdated = true;
            $event->gateway_profile_id = $rawData['invoice_id'];
        }
        if ($eventUpdated) {
            $event->save();
        }
        ...
        switch ($rawData['message_type']) {
            ...
            case 'INVOICE_STATUS_CHANGED':
                ...
                if (strtolower($rawData['invoice_status']) == 'declined' ||
                    ...
                    // send notification
                    if ($event->didStatusChange()) {
                        Engine_Api::_()->siteeventpaid()->sendMail("OVERDUE", $event->event_id);
                    }
            ...
            case 'REFUND_ISSUED':
                ...
                // send notification
                if ($event->didStatusChange()) {
                    Engine_Api::_()->siteeventpaid()->sendMail("REFUNDED", $event->event_id);
                }
                break;
            ...
            case 'RECURRING_INSTALLMENT_SUCCESS':
                ...
                // send notification
                if ($event->didStatusChange()) {
                    //  @todo Siteeventpaid_event_recurrence
                    Engine_Api::_()->siteeventpaid()->sendMail("RECURRENCE", $event->event_id);
                }
                break;
            ...
            case 'RECURRING_STOPPED':
                ...
                // send notification
                if ($event->didStatusChange()) {
                    Engine_Api::_()->siteeventpaid()->sendMail("CANCELLED", $event->event_id);
                }
                break;
            ...
            case 'RECURRING_COMPLETE':
                ...
                // send notification
                if ($event->didStatusChange()) {
                    Engine_Api::_()->siteeventpaid()->sendMail("EXPIRED", $event->event_id);
                }
                break;
    ...
    /**
     * Generate href to a event detailing the order
     *
    ...
    /**
     * Generate href to a event detailing the transaction
     *
     * @param string $transactionId
     * @return string
     */
    ...
    public function onIpn(Engine_Payment_Ipn $ipn) {
        ...
        if ($order) {
            // Subscription IPN
            if ($order->source_type == 'siteevent_event') {
