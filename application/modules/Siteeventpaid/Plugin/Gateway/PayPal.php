 ...
 */
class Siteeventpaid_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
    ...
    Payment_Model_Order $order, array $params = array()) {
        ...
        $event = Engine_Api::_()->getItem('siteevent_event', $order->source_id);
        ...
        if ($package->isOneTime()) {
            ...
            if ($paymentStatus == 'okay' ||
                ...
                $eventOtherinfo->gateway_profile_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
                $eventOtherinfo->save();
        ...
        // Recurring
            ...
            if ($rdata['PROFILESTATUS'] == 'ActiveProfile' ||
                ...
                // Enable now
                $eventOtherinfo->gateway_id = $this->_gatewayInfo->gateway_id;
                $eventOtherinfo->gateway_profile_id = $rdata['PROFILEID'];
                $eventOtherinfo->save();
                $event->onPaymentSuccess();
    ...
    Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
        ...
        if (!empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback') {
            $event->onPaymentFailure(); // or should we use pending?
        }
                    ...
                    if ($package->isOneTime()) {
                        switch ($rawData['payment_status']) {
                            ...
                            case 'Pending':
                                ...
                                // Get benefit setting
                                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'siteeventpaid')->getBenefitStatus($user);
                                if ($giveBenefit) {
                                    $event->onPaymentSuccess();
                                } else {
                                    $event->onPaymentPending();
