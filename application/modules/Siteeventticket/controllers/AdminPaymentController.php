 ...
 */
class Siteeventticket_AdminPaymentController extends Core_Controller_Action_Admin {
    ...
    public function init() {
        ...
        $this->_session = new Zend_Session_Namespace('Payment_Siteeventtickets');
        ...
        if (!$this->_user_request) {
            if (!empty($this->_session->user_request_id)) {
                $this->_user_request = Engine_Api::_()->getItem('siteeventticket_paymentrequest', $this->_session->user_request_id);
            }
        }
    }
    ...
    public function indexAction() {
        ...
        $this->view->navigationGeneral = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('siteeventticket_admin_main_ticket', array(), 'siteeventticket_admin_main_payment_requests');
    ...
    public function processPaymentAction() {
        //GET NAVIGATION
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('siteevent_admin_main', array(), 'siteeventticket_admin_main_payment');

        $this->view->payment_req_obj = $payment_req_obj = Engine_Api::_()->getItem('siteeventticket_paymentrequest', $this->_getParam('request_id'));

        if (empty($payment_req_obj))
            return $this->_forward('notfound', 'error', 'core');

        if ($payment_req_obj->request_status == 1) {
            $this->view->siteeventticket_payment_req_delete = true;
            return;
        }
        $gateway_id = Engine_Api::_()->getDbtable('gateways', 'siteeventticket')->getEventGateway($payment_req_obj->event_id);

        $this->view->siteevent = $siteevent = Engine_Api::_()->getItem('siteevent_event', $payment_req_obj->event_id);
        ...
        if (empty($gateway_id)) {
            ...
            $this->view->form = $form = new Siteeventticket_Form_Admin_Payment_PaymentTransfer(array('amount' => @round($payment_req_obj->request_amount, 2)));
            ...
            $currency_symbol = Engine_Api::_()->siteeventticket()->getCurrencySymbol();

            $values = $form->getValues();

            if (@round($values['user_req_amount'], 2) != @round($payment_req_obj->request_amount, 2)) {
                $user_previous_req_amount = Engine_Api::_()->siteeventticket()->getPriceWithCurrency($values['user_req_amount']);
                $user_current_req_amount = Engine_Api::_()->siteeventticket()->getPriceWithCurrency($payment_req_obj->request_amount);
            ...
            $gateway = Engine_Api::_()->getItem('siteeventticket_paymentreq', $gateway_id); 
            ...
            //UPDATE 
            Engine_Api::_()->getDbtable('paymentrequests', 'siteeventticket')->update(array(
            ...
            return $this->_helper->redirector->gotoRoute(array('module' => 'siteeventticket', 'controller' => 'payment', 'action' => 'process', 'gateway_id' => $gateway_id), "admin_default", true);
        }
    }
    ...
    public function processAction() {
        ...
        // Get order
        if (!$request_id ||
                !($user_request = Engine_Api::_()->getItem('siteeventticket_paymentrequest', $request_id))) {
            return $this->_helper->redirector->gotoRoute(array(), 'admin_default', true);
        }
        ...
        $gateway = Engine_Api::_()->getItem('siteeventticket_paymentreq', $user_gateway_id); 
        ...
        $ordersTable->insert(array(
            ...
            'source_type' => 'siteeventticket_paymentrequest',
        ...
        $params['return_url'] = $schema . $host
                . $this->view->url(array('action' => 'return', 'controller' => 'payment', 'module' => 'siteeventticket'), 'admin_default', true)
                . '?order_id=' . $order_id
                . '&state=' . 'return'
        ...
        $params['source_type'] = 'siteeventticket_paymentrequest';
    ...
    public function returnAction() {
        ...
        if (!$this->_user ||
                ...
                $order->source_type != 'siteeventticket_paymentrequest' ||
    ...
    public function finishAction() {
        //GET NAVIGATION
        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('siteevent_admin_main', array(), 'siteeventticket_admin_main_payment');
        ...
        if (isset($this->_success->succes_id)) {
            ...
            if ($state == 'active') {
                $payment_request_obj = Engine_Api::_()->getItem('siteeventticket_paymentrequest', $request_id);
                $event_id = $payment_request_obj->event_id;
                ...
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'siteeventticket_payment_request_approve', array(
                    'object_title' => $siteevent->getTitle(),
                    'object_name' => $event_name,
                    'response_amount' => Engine_Api::_()->siteeventticket()->getPriceWithCurrency($payment_request_obj->response_amount)
                ));
            }
    ...
    public function deletePaymentRequestAction() {
        ...
        $siteevent = Engine_Api::_()->getItem('siteevent_event', $event_id);
        if (empty($request_id) || empty($payment_req_obj) || empty($siteevent))
            return $this->_forward('notfound', 'error', 'core');
    ...
    public function viewPaymentRequestAction() {
        ...
        $this->view->payment_req_obj = $payment_req_obj = Engine_Api::_()->getItem('siteeventticket_paymentrequest', $request_id);
        $this->view->siteevent = $siteevent = Engine_Api::_()->getItem('siteevent_event', $payment_req_obj->event_id);
        $this->view->user_obj = Engine_Api::_()->getItem('user', $this->view->siteevent->owner_id);

        if (empty($siteevent) || empty($request_id) || empty($payment_req_obj))
            return $this->_forward('notfound', 'error', 'core');
