 ...
 */
class Siteeventticket_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
    ...
    public function createUserOrderTransaction($order_id, array $params = array(), $user = NULL) {
        ...
        $order_tickets = Engine_Api::_()->getDbtable('orders', 'siteeventticket')->getAllOrders($order_id);

        foreach ($order_tickets as $ticket) {

            $siteevent = Engine_Api::_()->getItem('siteevent_occurrence', $ticket['occurrence_id']);
                ...
                if (!empty($couponDetail)) {
                    ...
                    if (!empty($couponAmt)) {
                        ...
                        $orderCouponDetail[] = array(
                            'NAME' => Zend_Registry::get('Zend_Translate')->_('Coupon Discount'),
                            'AMT' => -@round($discountedCouponPrice, 2),
                            'QTY' => 1,
                        );
                    }
                }
            ...
            if ($temp_order_id != $ticket['order_id']) {
                ...
                $tax += @round($ticket['tax_amount'], 2);
                $temp_order_id = $ticket['order_id'];

                $grand_total += @round($ticket['grand_total'], 2);
                $tickets_sub_total += @round($ticket['sub_total'], 2);
            }

            $buy_tickets[] = array(
                'NAME' => $siteevent->getTitle() . ': ' . $ticket['title'],
                'AMT' => @round($ticket['price'], 2),
                'QTY' => $ticket['quantity'],
            );
        }
        if (!empty($orderCouponDetail)) {
            $buy_tickets = array_merge($buy_tickets, $orderCouponDetail);
        }

        if ($grand_total != ($tax + $tickets_sub_total))
            $tickets_sub_total = $grand_total - ($tax);

        $params['driverSpecificParams']['PayPal'] = array(
            'AMT' => @round($grand_total, 2),
//        'CUSTOM' => '5',
//        'INVNUM' => '1',
            'ITEMAMT' => @round($tickets_sub_total, 2),
            'SELLERID' => '1',
            'TAXAMT' => @round($tax, 2),
            'ITEMS' => $buy_tickets,
            'SOLUTIONTYPE' => 'sole',
                ...
                case 'express_checkout':
                    ...
                    switch ($rawData['payment_status']) {
                        ...
                        case 'Pending':
                            ...
                            // Get benefit setting
                            $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'siteeventticket')->getBenefitStatus($user);
                            if ($giveBenefit) {
                                $user_order->onPaymentSuccess();
                            } else {
                                $user_order->onPaymentPending();
