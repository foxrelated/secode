 ...
 */
class Siteeventticket_Plugin_Gateway_Event_PayPal extends Engine_Payment_Plugin_Abstract {
    ...
    public function createEventBillTransaction($event_id, $bill_id, $params = array()) {
        ...
        $params['driverSpecificParams']['PayPal'] = array(
            'AMT' => round($eventBillDetail->amount, 2),
            'ITEMAMT' => round($eventBillDetail->amount, 2),
            'ITEMS' => array(
                array(
                    'NAME' => $event_title,
                    'DESC' => $eventBillDetail->message,
                    'AMT' => round($eventBillDetail->amount, 2),
                ),
            ),
            'SOLUTIONTYPE' => 'sole',
    ...
    public function onEventBillTransactionReturn(Payment_Model_Order $order, array $params = array()) {
        ...
        $eventBill = $order->getSource();
        $user = $order->getUser();
        if ($eventBill->status == 'pending') {
            return 'pending';
        }
        ...
        // Get benefit setting
        $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'siteeventticket')
        ...
        if ($paymentStatus == 'okay' ||
            ...
            $eventBill->gateway_profile_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];

            // Payment success
            $eventBill->onPaymentSuccess();
    ...
    Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
        ...
        if (!empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback') {
            $eventBill->onPaymentFailure(); // or should we use pending?
        }
                ...
                case 'express_checkout':
                    ...
                    switch ($rawData['payment_status']) {
                        ...
                        case 'Canceled_Reversal': // Not sure about this one
                            $eventBill->onPaymentSuccess();
                            // send notification
                            if ($eventBill->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("ACTIVE", $eventBill->userad_id);
                            }
                            break;
                        ...
                        case 'Reversed':
                            ...
                            if ($eventBill->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("OVERDUE", $eventBill->userad_id);
                            }
                            break;
                        ...
                        case 'Refunded':
                            ...
                            if ($eventBill->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("REFUNDED", $eventBill->userad_id);
                            }
                            break;

                        case 'Expired': // Not sure about this one
                            $eventBill->onExpiration();
                            // send notification
                            if ($eventBill->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("EXPIRED", $eventBill->userad_id);
                            }
                            break;
     ...
     *
     * @param User_Model_User $user
     * @param Zend_Db_Table_Row_Abstract $eventBill
     * @param Zend_Db_Table_Row_Abstract $package
     * @param array $params
     * @return Engine_Payment_Gateway_Transaction
     */
    public function createSubscriptionTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $eventBill, Payment_Model_Package $package, array $params = array()) {
