 ...
 */
class Siteeventticket_Plugin_Gateway_Admin_PayPal extends Engine_Payment_Plugin_Abstract {
    ...
    public function createUserRequestTransaction(User_Model_User $user, $request_id, array $params = array()) {
        ...
        $event_title = Engine_Api::_()->getItem('siteevent_event', $response[0]['event_id'])->getTitle();
...
//      $user_request->gateway_id = $this->_gatewayInfo->gateway_id;
    ...
    Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
        ...
        $user = $order->getUser();
        $user_request = $order->getSource();
        $package = $user_request->getPackage();
        ...
        if (!empty($rawData['case_type']) && $rawData['case_type'] == 'chargeback') {
            $user_request->onPaymentFailure(); // or should we use pending?
        }
            ...
            switch ($rawData['txn_type']) {
                ...
                case 'express_checkout':
                    ...
                    switch ($rawData['payment_status']) {
                        ...
                        case 'Refunded':
                            $user_request->onRefund();
                            // send notification
                            if ($user_request->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("REFUNDED", $user_request->userad_id);
                            }
                            break;

                        case 'Expired': // Not sure about this one
                            $user_request->onExpiration();
                            // send notification
                            if ($user_request->didStatusChange()) {
                                // SEND OVERDUE MAIL HERE
                                Engine_Api::_()->communityad()->sendMail("EXPIRED", $user_request->userad_id);
                            }
                            break;
     ...
     *
     * @param User_Model_User $user
     * @param Zend_Db_Table_Row_Abstract $user_request
     * @param Zend_Db_Table_Row_Abstract $package
     * @param array $params
     * @return Engine_Payment_Gateway_Transaction
     */
    public function createSubscriptionTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $user_request, Payment_Model_Package $package, array $params = array()) {
