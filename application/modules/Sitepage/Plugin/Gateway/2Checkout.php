 ...
 */
class Sitepage_Plugin_Gateway_2Checkout extends Engine_Payment_Plugin_Abstract {
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($giveBenefit) {
      ...
      $page->gateway_profile_id = $params['order_number']; // This is the same as sale_id
  ...
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
    ...
    // Update subscription
    $pageUpdated = false;
    if (!empty($rawData['sale_id']) && empty($page->gateway_profile_id)) {
      $pageUpdated = true;
      $page->gateway_profile_id = $rawData['sale_id'];
    }
    if (!empty($rawData['invoice_id']) && empty($page->gateway_transaction_id)) {
      $pageUpdated = true;
      $page->gateway_profile_id = $rawData['invoice_id'];
    }
    if ($pageUpdated) {
      $page->save();
    }
  ...
  public function cancelPage($transactionId) {
    return $this;
  }
