...
<?php if ($this->loaded_by_ajax): ?>
    <script type="text/javascript">
        var params = {
            ...
            responseContainer: $$('.layout_sitepage_horizontal_searchbox_sitepage')
        }
...
<div class="sitepage_form_quick_search">
...
<script type="text/javascript">
    ...
    var doSearching = function(searchboxcategory_id) {
        ...
        if (searchboxcategory_id != 0) {
            ...
            var categoriesArray = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'sitepage')->getCategoriesDetails($this->categoriesLevel)); ?>;
    ...
    en4.core.runonce.add(function()
    {
        var item_count = 0;
        if($('titleAjax')) {
        var contentAutocomplete = new Autocompleter.Request.JSON('titleAjax', '<?php echo $this->url(array('action' => 'get-search-events'), "sitepage_general", true) ?>', {
            'postVar': 'text',
            'minLength': 1,
            'selectMode': 'pick',
            ...
            'injectChoice': function(token) {
                if (typeof token.label != 'undefined') {
                    if (token.sitepage_url != 'seeMoreLink') {
                        var choice = new Element('li', {'class': 'autocompleter-choices', 'html': token.photo, 'id': token.label, 'sitepage_url': token.sitepage_url, onclick: 'javascript:getPageResults("' + token.sitepage_url + '")'});
                    ...
                    if (token.sitepage_url == 'seeMoreLink') {
                        ...
                        var choice = new Element('li', {'class': 'autocompleter-choices1', 'html': '', 'id': 'stopevent', 'sitepage_url': ''});
            ...
            window.addEvent('keyup', function(e) {
                if (e.key == 'enter') {
                    if (selected.retrieve('autocompleteChoice') != 'null') {
                        var url = selected.retrieve('autocompleteChoice').sitepage_url;
                        if (url == 'seeMoreLink') {
                            Seemore();
                        }
                        else {
