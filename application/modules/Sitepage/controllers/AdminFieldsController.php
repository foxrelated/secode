...
class Sitepage_AdminFieldsController extends Fields_Controller_AdminAbstract {
  ...
  public function showCustomfieldsAction() {
    ...
    if ($this->getRequest()->getPost()) {
      ...
      if ($tabel_isvalid)
        $db->query('DROP TABLE IF EXISTS `engine4_sitepage_imports`');

      $table_exist = $db->query('SHOW TABLES LIKE \'engine4_sitepage_imports\'')->fetch();
      if (empty($table_exist)) {
        $db->query("CREATE TABLE IF NOT EXISTS `engine4_sitepage_imports` (
  ...
  PRIMARY KEY (`import_id`)
      ...
      if (isset($_POST['addtional'])) {
        ...
        foreach ($fields as $field) {
          ...
          $column_exist = $db->query("SHOW COLUMNS FROM engine4_sitepage_imports LIKE '$short_value'")->fetch();
          if (empty($column_exist)) {
            $db->query("ALTER TABLE `engine4_sitepage_imports` ADD `$short_value` TEXT CHARACTER SET utf8 COLLATE utf8_unicode_ci NULL");
          }
          $CommonCustomfields[] = $field;
          $field = explode('_', $field);
          $field = $field[2];
          $fieldType = $db->select()->from('engine4_sitepage_page_fields_meta', 'type')->where('field_id = ?', $field)->query()->fetchColumn();
      ...
      $path = $this->_getPath();
      $file_path = "$path/new_page_import.csv";
      ...
      if (isset($_POST['addtional'])) {
        ...
        $file_string .= "Title|Page_url|Category|Sub-Category|3rd Level Category|Description|Overview|Tag_String|Location|Price|Email_Id|Web_Address|Phone_Number|Claim_a_Page|Photo Name.ext" . "$string_custom_fields";
        ...
        $file_string = "Title|Page_Url|Category|Sub_Category|3rd Level Category|Description|Overview|Tag_String|Location|Price|Email_Id|Web_Address|Phone_Number|Claim_a_Page|Photo Name.ext" . "$string_custom_fields";
      }

      if (is_dir(APPLICATION_PATH . "/public/sitepage_page") != 1) {
        @mkdir(APPLICATION_PATH . "/public/sitepage_page", 0777, true);
      }

      $path_importfile = $this->_getPathImport();
      $file_path_import = "$path_importfile/previous_page_import.csv";
      @chmod($path_importfile, 0777);
      @chmod($file_path_import, 0777);
      $fp = fopen(APPLICATION_PATH . '/public/sitepage_page/previous_page_import.csv', 'w+');
      @chmod(APPLICATION_PATH . "/public/sitepage_page", 0777);
      fwrite($fp, $file_string);
      fclose($fp);

      @chmod($path, 0777);
      @chmod($file_path, 0777);
      $fp = fopen(APPLICATION_PATH . '/temporary/new_page_import.csv', 'w+');
      fwrite($fp, $file_string);
      fclose($fp);
      ...
      $path = APPLICATION_PATH . "/temporary/new_page_import.csv";
