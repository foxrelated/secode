 ...
 */
class Poke_Widget_ListMostpokersController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    ...
    $select = $userTable->select()
			...
			->from($userTable->info('name'), array('*', 'count(*) as poke_count'))
			->join($pokeTableName, "`{$pokeTableName}`.userid=`{$userTableName}`.`user_id`")
     // ->where("$pokeTableName.`isdeleted` = 1")
     ->group("$pokeTableName.userid")
