 ...
 * @license    http://www.hire-experts.com
 */
abstract class Hecore_Controller_Action_Bridge extends Core_Controller_Action_Standard
{
  ...
  protected function getOptions(Core_Model_Item_Abstract $item, $name = false, $module = null)
  {
    ...
    if (is_array($setting['options']))
      foreach ($setting['options'] as $option) {
        ...
        if(isset($option['data-icon'])) {
          $tmp['attrs']['data-icon'] = $option['data-icon'];
        }
        $options[] = $tmp;
      }
