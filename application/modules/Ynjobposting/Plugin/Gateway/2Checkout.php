...
class Ynjobposting_Plugin_Gateway_2Checkout extends Payment_Plugin_Gateway_2Checkout
{
  ...
  public function onPackageTransactionReturn(Ynjobposting_Model_Order $order, array $params = array())
  {
	...
	$package_id = $order -> package_id;
	 ...
	 $view = Zend_Registry::get('Zend_View');
	 $transactionType = $view -> translate('Publish Job');
     ...
     try {
     	...
     	if($order -> type == 'job')
		{
			...
			{
				$params['module'] = 'ynjobposting';
				$params['user_id'] = $order->user_id;
				$params['rule_name'] = 'publish_job';
		...
		else 
		{
			...
			if(Engine_Api::_() -> hasModuleBootstrap('ynaffiliate'))	
			{
				$params['module'] = 'ynjobposting';
				$params['user_id'] = $order->user_id;
				$params['rule_name'] = 'sponsor_company';
				$params['total_amount'] = $order->price;
				$params['currency'] = $order->currency;
	        	Engine_Hooks_Dispatcher::getInstance()->callEvent('onPaymentAfter', $params);
			}
		...
		//save transaction
		 ...
		 {
		 	$transactionType = $view -> translate('Sponsor Company');
