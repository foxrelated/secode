 ...
 * @license    http://www.socialengine.net/license/
 */
class Heevent_AdminGatewayController extends Core_Controller_Action_Admin
{
  public function init()
  {
    $this->view->menu       = $this->_getParam('action');
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('heevent_admin_main', array(), 'heevent_admin_main_gateways');
  }
  ...
  public function indexAction()
  {
    ...
    // Make paginator
    $select = Engine_Api::_()->getDbtable('gateways', 'heevent')
      ->select()
      ->where('`plugin` != ?', 'Heevent_Plugin_Gateway_Testing')
      ->where('`plugin` != ?', 'heevent_Plugin_Gateway_Credit')
    ;
  ...
  public function buttonAction()
  {
     ...
     * @var $gateway Store_Model_Gateway
     */
    $this->view->gateway = $gateway = Engine_Api::_()->getDbtable('gateways', 'heevent')
      ->find($this->_getParam('gateway_id'))
      ->current();
