 ...
 * @license    http://www.socialengine.net/license/
 */
class Heevent_Model_DbTable_Gateways extends Engine_Db_Table
{
  protected $_rowClass = 'Heevent_Model_Gateway';
  ...
  {
    return $this->fetchRow(array('gateway_id = ?' => $gateway_id));
  }
  ...
  static private function _getCryptKey()
  {
    if( null === self::$_cryptKey ) {
      $key = Engine_Api::_()->getApi('settings', 'core')->core_secret
        . '^'
        . Engine_Api::_()->getApi('settings', 'core')->store_secret;
  ...
  public function getCreditGateway()
  {
    return $this->fetchRow(array('title = ?' => 'Credit'));
  }
}