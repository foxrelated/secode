 ...
 * @license    http://hire-experts.com/
 */
class Heevent_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
      ...
      array $params = array())
  {
    ...
    for($i=1; $i<=$subscription->quantity;$i++){
     $quan[$i]['NAME']  = $event->getTitle();
     $quan[$i]['DESC'] = $desc;
     $quan[$i]['AMT'] = $event->getPrice();
     $quan[$i]['NUMBER'] = $subscription->subscription_id;
     $quan[$i]['QTY'] = 1;
    ...
    $params['driverSpecificParams']['PayPal'] = array(
      'AMT' => $event->getPrice() * $subscription->quantity,
      'DESC' => $desc,
      'CUSTOM' => $subscription->subscription_id,
      'INVNUM' => $params['vendor_order_id'],
      'ITEMAMT' => $event->getPrice() * $subscription->quantity,
      'ITEMS' =>array(
        array(
          'NAME' => $event->getTitle(),
          'DESC' => $desc,
   ...
   *
   * @param Heevent_Model_Order $order
   * @param array $params
   */
   ...
   */
  public function onEventTransactionIpn(
      Heevent_Model_Order $order,
        ...
        case 'express_checkout':
          switch( $rawData['payment_status'] ) {
            ...
            case 'Reversed':
              ...
              // send notification
              if( $subscription->didStatusChange() ) {
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'heevent_subscription_overdue', array(
                  'subscription_title' => $event->title,
                  'subscription_description' => $event->description,
                  'subscription_terms' => $event->getEventDescription(),
                  'object_link' => 'http://' . $_SERVER['HTTP_HOST'] .
            ...
            case 'Refunded':
              ...
              // send notification
              if( $subscription->didStatusChange() ) {
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'heevent_subscription_refunded', array(
                  'subscription_title' => $event->title,
                  'subscription_description' => $event->description,
                  'subscription_terms' => $event->getEventDescription(),
                  'object_link' => 'http://' . $_SERVER['HTTP_HOST'] .
            ...
            case 'Expired': // Not sure about this one
              ...
              // send notification
              if( $subscription->didStatusChange() ) {
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'heevent_subscription_expired', array(
                  'subscription_title' => $event->title,
                  'subscription_description' => $event->description,
                  'subscription_terms' => $event->getEventDescription(),
                  'object_link' => 'http://' . $_SERVER['HTTP_HOST'] .
  ...
  public function onIpn(Engine_Payment_Ipn $ipn)
  {
    ...
    if( $order ) {
      $ipnProcessed = false;
      // Subscription IPN
      if( $order->source_type == 'events_subscription' ) {
        $this->onEventTransactionIpn($order, $ipn);
        $ipnProcessed = true;
      }
      // Custom IPN
      else {
  ...
  {
    return new Heevent_Form_Admin_Gateway_PayPal($options);
  }
