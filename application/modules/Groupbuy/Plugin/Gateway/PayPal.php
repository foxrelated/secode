<?php
class Groupbuy_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal {
  	...
  	public function createPackageTransaction(Groupbuy_Model_Order $order, array $params = array()) {
		...
		if ($order->params['type'] == 'publish') {
		    $payment_item = array_merge($payment_item, array(
		      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Publish Deal'),
		      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Publish Deal from %s', $view->layout()->siteinfo['title']),
		      'L_PAYMENTREQUEST_0_AMT0' => $order->price
		    ));
		}
		else {
			$payment_item = array_merge($payment_item, array(
		      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Deal'),
		      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Deal from %s', $view->layout()->siteinfo['title']),
		      'L_PAYMENTREQUEST_0_AMT0' => $order->price
		    ));
		}
    	...
    	$payment_requests = array_merge($payment_requests, array_merge(array(
      		...
      		'PAYMENTREQUEST_0_DESC' => $view->translate('Buy/Publish Deal'),
	...
	public function createTransaction(array $params) {
  	...
  	public function onPackageTransactionReturn(Groupbuy_Model_Order $order, array $params = array()) {
     	...
     	if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'groupbuy') -> findRow($request['PAYMENTREQUESTID'])) ) {
       		$order->onPaymentFailure();
     	...
     	$order = Engine_Api::_()->getDbTable('orders', 'groupbuy') -> findRow($info['PAYMENTREQUESTID']);
     	...
     	try {
     		if ($order->params['type'] == 'publish') {
     			Engine_Api::_() -> groupbuy() -> processPublishPayment($order -> security_code, $order -> invoice_code, $info['TRANSACTIONID'], 'Paypal');
     		}
			else {
     			Engine_Api::_() -> groupbuy() -> processPayment($order -> security_code, $order -> invoice_code, $info['TRANSACTIONID'], $order->params['gift_id'], 'Paypal');
			}
      		$db->commit();
    	} catch (Exception $e) {
      		$db->rollBack();
     	...
     	if( $paymentStatus == 'okay' ) {
       		...
       		if ($order instanceof Groupbuy_Model_Order) {
         		$order->onPaymentFailure();
         		throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try again later.');
       		}
