...
class Groupbuy_Model_DbTable_Nodes extends Engine_Db_Table {

	protected $_rowClass = 'Groupbuy_Model_Node';
	 ...
	 * @param  $parent_id
	 * @return Groupbuy_Model_Node|NULL
	 */
	 ...
	 * @throw Exception
	 */
	public function insertAfter(Groupbuy_Model_Node$node, $data) {
		if($node -> pleft == 1) {
	 ...
	 * @throw Exception
	 */
	public function insertBefore(Groupbuy_Model_Node$node, $data) {
		if($node -> pleft == 1) {
			throw new Exception("can not insert after root");
		}
	 ...
	 * @throw Exception
	 */
	public function appendChild(Groupbuy_Model_Node$node, $data) {
	 ...
	 * @throw Exception
	 */
	public function prependChild(Groupbuy_Model_Node$node, $data) {
		$newNode = $this -> fetchNew();
		$newNode -> setFromArray($data);
		$anchor = $node -> pleft;
		$newNode -> pleft = $anchor + 1;
	 ...
	 * @throw Exception
	 */
	public function addChild(Groupbuy_Model_Node$node, $data) {
		if(!is_object($node)) {
			$node = $this -> getRoot();
		}
	...
	public function getDescendent($node, $include_own =true) {
		if(!$node instanceof Groupbuy_Model_Node) {
