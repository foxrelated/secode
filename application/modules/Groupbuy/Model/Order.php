<?php
class Groupbuy_Model_Order extends Core_Model_Item_Abstract {
	protected $_searchTriggers = false;
	protected $_type = 'groupbuy_order';
	...
	public function getPackageParams($arr = array()) {
        ...
        // General
        $params['name'] = $view -> translate('Publish/Buy Deal');
        $params['price'] = $arr['price'];
        $params['description'] = $view -> translate('Publish/Buy Deal from %s', $view -> layout() -> siteinfo['title']);
    ...
    public function getGatewayIdentity($user_id = 0, $fee = 100) {
        return 'groupbuy_' . $user_id . '_fee_' . $fee;
    }
	...
	public function isChecked() {
		if ($this -> status != 'completed')
			return false;
		$table = Engine_Api::_() -> getDbTable('transactions', 'groupbuy');
	...
	public function onPackageTransactionReturn(array $params = array()) {
		...
		if (isset($params['state']) && $params['state'] == 'cancel')
		{
		 ...
		 $title = ($gateway) ? $gateway->title : 'Unknown Method';
	     try {
	     	if ($order->params['type'] == 'publish') 
				Engine_Api::_() -> groupbuy() -> processPublishPayment($this -> security_code, $this -> invoice_code, $params['transaction_id'], $title);
			else
		    	Engine_Api::_() -> groupbuy() -> processPayment($this -> security_code, $this -> invoice_code, $params['transaction_id'], $this->params['gift_id'], $title);
		    $db->commit();
	    } catch (Exception $e) {
	      $db->rollBack();
	      throw $e;
