 ...
 * @author     Minh Nguyen
 */
class Groupbuy_AdminLocationController extends Core_Controller_Action_Admin {

	protected $_paginate_params = array();

	public function init() {
		$this -> view -> navigation = $navigation = Engine_Api::_() -> getApi('menus', 'core') -> getNavigation('groupbuy_admin_main', array(), 'groupbuy_admin_main_localtions');
	}

	/**
	 *
	 *
	 * @return Groupbuy_Model_DbTable_Location
	 */
	public function getDbTable() {
		return Engine_Api::_() -> getDbTable('locations', 'groupbuy');
	}
	...
	public function addLocationAction() {
		...
		$form = $this -> view -> form = new Groupbuy_Form_Admin_Location();
	...
	public function deleteLocationAction() {
        ...
        $options = Engine_Api::_()->getDbTable('locations','groupbuy')->getDeleteOptions($id);
		if(!$options)
		...
		$moveNode = $this->view->moveNode = new Zend_Form_Element_Select('node_id', array(
			'label'=>'Location',
			'multiOptions'=> Engine_Api::_()->getDbTable('locations','groupbuy')->getDeleteOptions($id),
	...
	public function editLocationAction() {
		...
		if($this -> getRequest() -> isPost() && $form -> isValid($this -> getRequest() -> getPost())) {
			...
			try {
				...
				// Transaction
				$row = Engine_Api::_() -> getItem('groupbuy_location', $values["id"]);
		...
		// Generate and assign form
		$location = Engine_Api::_() -> getItem('groupbuy_location', $id);
