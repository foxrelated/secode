...
class Ynvideochannel_VideoController extends Core_Controller_Action_Standard
{
    ...
    public function favoriteAction() {
        ...
        if (!$this->_helper->requireAuth()->setAuthParams($video, null, 'view')->isValid()) {
            $data = array(
                'result' => 0,
                'message' => $translate->_('You do not have the authorization to favorite this video.'),
            );
        ...
        if ($added) {
            ...
            $favorite = $favoriteTable->addVideoToFavorite($video->video_id, $viewer_id);
            if ($favorite) {
                // CREATE AUTH STUFF HERE
                $auth = Engine_Api::_()->authorization()->context;
                ...
                $actionTable = Engine_Api::_()->getDbtable('actions', 'activity');
                $action = $actionTable->addActivity($viewer, $favorite, 'ynvideochannel_add_favorite');
    ...
    public function rateAction()
    {
        ...
        return $this -> _helper -> json(array('rating' => $video -> rating));
    }
}
