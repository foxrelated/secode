...
class Ynvideochannel_IndexController extends Core_Controller_Action_Standard
{
    ...
    public function browseVideosAction()
    {
        ...
        $params['search'] = 1;
        if (!$page) $page = 1;
        $items_per_page = Engine_Api::_()->getApi('settings', 'core')->getSetting('video.page', 10);
        $this -> view -> paginator = $paginator = Engine_Api::_() -> getItemTable('ynvideochannel_video') -> getVideosPaginator($params);
    ...
    public function browseChannelsAction()
    {
        ...
        $params['search'] = 1;
        if (!$page) $page = 1;
        $items_per_page = Engine_Api::_()->getApi('settings', 'core')->getSetting('channel.page', 10);
    ...
    public function manageVideosAction()
    {
        ...
        $this->view->paginator = $paginator = $table->getVideosPaginator(array('user_id' => $viewer->getIdentity()));
        $items_per_page = Engine_Api::_()->getApi('settings', 'core')->getSetting('video.page', 10);
        $this->view->paginator->setItemCountPerPage($items_per_page);
        $this->view->paginator->setCurrentPageNumber($this->getParam('page', 1));
    }
    ...
    public function manageChannelsAction()
    {
        ...
        $this->view->paginator = $paginator = $table->getChannelsPaginator(array('user_id' => $viewer->getIdentity()));
        $items_per_page = Engine_Api::_()->getApi('settings', 'core')->getSetting('channel.page', 10);
        $this->view->paginator->setItemCountPerPage($items_per_page);
        $this->view->paginator->setCurrentPageNumber($this->getParam('page', 1));
    }
    ...
    public function favoritesAction()
    {
        $this->_helper->content->setEnabled();
        $viewer = Engine_Api::_()->user()->getViewer();
        $videos = Engine_Api::_() -> ynvideochannel() -> getFavoriteVideos($viewer -> getIdentity());
        $itemperPage = $this->_getParam('numberOfItems', 10);
    ...
    public function getChannelAction()
    {
        ...
        if (count($items)) {
            ...
            if (!empty($items[0]->brandingSettings->image)) {
                $channelImg = $items[0]->brandingSettings->image->bannerTvHighImageUrl;
            }
        }
        ...
        try {
            ...
            if (!empty($values['cover_photo'])) {
                ...
                $channel->setCoverPhoto($channelImg);
            }
    ...
    public function shareVideoAction()
    {
        ...
        if (Engine_Api::_()->ynvideochannel()->isExistVideoCode($values['code'],null, $viewer->getIdentity())) {
            $form->addError('You have already shared this video.');
            return;
        }
