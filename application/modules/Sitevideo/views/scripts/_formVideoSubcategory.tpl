 ...
 * @version    $Id: _formVideoSubcategory.tpl 6590 2016-3-3 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
?>

<?php
$defaultProfileFieldId = Engine_Api::_()->getDbTable('metasVideos', 'sitevideo')->defaultProfileId();
$defaultProfileFieldId = "0_0_$defaultProfileFieldId";
$video_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('video_id', 0);
$profile_type = 0;

if (!empty($video_id)) {
    $video = Engine_Api::_()->getItem('sitevideo_video', $video_id);
    $profile_type = Engine_Api::_()->getDbTable('videoCategories', 'sitevideo')->getProfileType(array('categoryIds' => null, 'category_id' => $video->category_id));
}
...
echo "
	...
	<div id='subcategory_id-wrapper' class='form-wrapper' style='display:none;'>
		<div id='subcategory_id-label' class='form-label'>
		 	<label for='subcategory_id' class='optional'>" . $this->translate('Sub-Category') . "</label>
		</div>
...
<script type="text/javascript">
    ...
    var actionType = '<?php echo $video_id; ?>';
    ...
    var subcatname = '';
    var show_subcat = 1;
<?php if (!empty($this->sitevideo->category_id)) : ?>
        show_subcat = 0;
<?php endif; ?>

    var subcategories = function (category_id, sub, subcatname)
    {
        var aQuick = document.getElementById('sitevideos_create_quick');
        ...
        content.getElementById('subcategory_backgroundimage').innerHTML = '<div class="form-label"></div><div class="form-element"><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Core/externals/images/loading.gif" alt="" /></div>';
    ...
    function changesubcategory(subcatid) {
        ...
        if (content.getElementById('subsubcategory_id-label'))
            content.getElementById('subsubcategory_id-label').style.display = 'none';
        content.getElementById('subsubcategory_backgroundimage').innerHTML = '<div class="form-label"></div><div  class="form-element"><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Core/externals/images/loading.gif" /></center></div>';
        en4.core.request.send(new Request.JSON({
            url: url,
            data: {
                format: 'json',
