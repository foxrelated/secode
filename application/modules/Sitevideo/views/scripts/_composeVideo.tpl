...
<?php
 ...
 $allowedSources = array_flip(Engine_Api::_()->getApi('settings', 'core')->getSetting('sitevideo.allowed.video', array(1, 2, 3,4)));
  ...
  $allowed_upload = isset($allowedSources) && isset($allowedSources[4]) && (bool) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('video', $user, 'upload');
  ...
  $youtubeEnabled = isset($allowedSources) && isset($allowedSources[1]) && (bool) $currentYouTubeApiKey;
  
  $vimeoEnabled = isset($allowedSources) && isset($allowedSources[2]) ;
  $dailymotionEnabled = isset($allowedSources) && isset($allowedSources[3]) ;
...
<script type="text/javascript">
    function showVideoInLightboxInFeed() {
        SmoothboxSEAO.open({
