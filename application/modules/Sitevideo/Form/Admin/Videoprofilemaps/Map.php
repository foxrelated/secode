 ...
 * @version    $Id: Map.php 6590 2016-3-3 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitevideo_Form_Admin_Videoprofilemaps_Map extends Engine_Form {
    ...
    public function init() {
        ...
        $this->setMethod('post')
                ...
                ->setDescription("After selecting a profile type, if you click on 'Save', then the already created videos of this category will also be associated with this profile type.");
        ...
        if (!empty($getCountChieldMapping)) {
            ...
            $description = Zend_Registry::get('Zend_Translate')->_("<div class='tip'><span>Note: It seems that sub-categories of this category have been already mapped. Thus, if you add this mapping, then earlier videos associated with the mapped sub-categories will loose their custom profile data.</span></div>");
        ...
        if (count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type') {
            ...
            if (count($options) > 0) {
                $options = $profileTypeField->getElementParams('sitevideo_video');
