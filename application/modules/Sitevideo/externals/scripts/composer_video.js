...
Composer.Plugin.SiteVideo = new Class({
  ...
  activate : function() {
    ...
    if (this.options.youtubeEnabled == 1) {
      $('compose-video-form-type').options[options++] = new Option(this._lang('YouTube'), '1');
    }
    
    if (this.options.vimeoEnabled == 1) {
    $('compose-video-form-type').options[options++] = new Option(this._lang('Vimeo'), '2');
}
    if (this.options.dailymotionEnabled == 1) {
    $('compose-video-form-type').options[options++] = new Option(this._lang('Dailymotion'), '4');
}
    ...
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-video-form-type').options[options++] = new Option(this._lang('My Device'), '3');
    }
      ...
      this.elements.previewDescription = new Element('div', {
        ...
        'html' : this._lang('To upload a video from your device, please use our <a href="'+this.options.videoHref+'" class="seao_smoothbox" onclick="showVideoInLightboxInFeed();return false;" data-SmoothboxSEAOClass="seao_add_video_lightbox" >full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
  } else {
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-video-upload',
        'class' : 'compose-video-upload',
        'html' : this._lang('To upload a video from your device, please use our <a href="'+this.options.videoHref+' ">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
  }
    }
    ...
    if(this.options.openVideosInLightboxMode) {
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-video-upload',
        'class' : 'compose-video-upload',
        'html' : this._lang('To upload a video from your computer, please use our <a href="'+this.options.videoHref+'" onclick="showVideoInLightboxInFeed();return false;" class="seao_smoothbox" data-SmoothboxSEAOClass="seao_add_video_lightbox">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
  } else {
       this.elements.previewDescription = new Element('div', {
        'id' : 'compose-video-upload',
        'class' : 'compose-video-upload',
        'html' : this._lang('To upload a video from your computer, please use our <a href="'+this.options.videoHref+'">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
  }
    }
  ...
  // If video source is empty
    ...
    if (video_element.value == 1 || video_element.value == 2 || video_element.value == 4)
    {
      upload_element.style.display = "none";
