        ...
        <li>
            <div>
                <?php foreach ($this->tag_array as $key => $frequency): ?>
                    ...
                    <a href='<?php echo $this->url(array('action' => 'browse'), "sitevideo_general"); ?>?tag=<?php echo urlencode($key) ?>&tag_id=<?php echo $this->tag_id_array[$key] ?>' style="font-size:<?php echo $step ?>px;" title=''><?php echo $key ?><sup><?php echo $frequency ?></sup></a> 
                <?php endforeach; ?>
            </div>		
        </li>
        <?php if (empty($this->notShowExploreTags)) : ?>
            <li>
                <?php echo $this->htmlLink(array('route' => "sitevideo_channel_general", 'action' => 'tagscloud'), $this->translate('Explore Tags &raquo;'), array('class' => 'more_link')) ?>
            </li>
        <?php endif; ?>
    </ul>
<?php else: ?>
