    ...
    <?php if ($this->autoload): ?>
        ...
        <script type="text/javascript">
            ...
            PinBoardSeaoObject[layoutColumn].add({
                ...
                responseContainerClass: 'layout_sitevideo_pinboard_browse_videos_sitevideo'
            });
    ...
    <?php else: ?>
        ...
        <?php foreach ($this->paginator as $video): ?>
            ...
            if ($this->show_buttons):
                $alllowComment = (in_array('comment', $this->show_buttons) || in_array('like', $this->show_buttons)) && $video->authorization()->isAllowed($this->viewer(), "comment");
                ...
                <div class="seaocore_board_list b_medium" style="width:<?php echo $this->params['itemWidth'] - 18 ?>px;"> 
                    <div>
                        <div class="seaocore_board_list_thumb">
                            <a href="<?php echo $video->getHref() ?>" class="seaocore_thumb sitevideo_thumb_viewer">
                                <table>
                                    <tr valign="middle">
                                        <td>
                                            ...
                                            ?>  
                                            <?php echo $this->itemPhoto($video, $this->thumbnailType, '', $options); ?>  
                                        </td> 
                                    </tr> 
                                </table>
                            </a>
                        ...
                        <div class="seaocore_board_list_btm">       
                            <?php if (!empty($this->videoOption) && in_array('owner', $this->videoOption)): ?>                
                                <?php echo $this->itemPhoto($video->getOwner(), 'thumb.icon'); ?>                 
                                <div class="o_hidden seaocore_stats seaocore_txt_light">            
                                    <b> <?php echo $this->htmlLink($video->getOwner()->getHref(), $video->getOwner()->getTitle(), array('class' => 'thumbs_author')) ?> </b>     
                                </div>
                            <?php endif; ?>             
                        </div>
                        <div class="seaocore_board_list_cont">
                            <?php if (!empty($this->videoOption) && in_array('title', $this->videoOption)): ?>
                                <div class="seaocore_title">
                                    <?php echo $this->htmlLink($video, Engine_Api::_()->seaocore()->seaocoreTruncateText($video->getTitle(), $this->titleTruncation)) ?>
                                </div>
                            <?php endif; ?>
                            <?php echo $this->videoInfo($video, $this->videoOption, array('truncationLocation' => $this->truncationLocation)); ?>
                            <?php if ($this->descriptionTruncation): ?>
                                <div class="seaocore_description">
                                    <?php echo Engine_Api::_()->seaocore()->seaocoreTruncateText($video->getDescription(), $this->descriptionTruncation) ?>
                                </div>  
                            <?php endif; ?>
                        ...
                        <?php if (!empty($this->userComment)) : ?>
                            <div class="seaocore_board_list_comments o_hidden">
                                <?php echo $this->action("list", "pin-board-comment", "seaocore", array("type" => $video->getType(), "id" => $video->video_id, 'widget_id' => $this->identity)); ?>
                            </div>
                        <?php endif; ?>

                        <?php if (!empty($this->show_buttons)): ?>
                            <div class="seaocore_board_list_action_links">
                                <?php $urlencode = urlencode(((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $video->getHref()); ?>
                                <?php if ((in_array('comment', $this->show_buttons) || in_array('like', $this->show_buttons)) && $alllowComment && !empty($this->userComment)): ?>
                                    <?php if (in_array('comment', $this->show_buttons)): ?>
                                        <a href='javascript:void(0);' onclick="en4.seaocorepinboard.comments.addComment('<?php echo $video->getGuid() . "_" . $this->identity ?>')" class="seaocore_board_icon comment_icon" title="Comment"><!--<?php echo $this->translate('Comment'); ?>--></a> 
                                    <?php endif; ?>
                                <?php endif; ?>

                                <?php $this->pinboardShareLinks($video, $this->show_buttons); ?>
        ...
        <div class="tip">
            <span>
                <?php if (isset($this->formValues['tag_id']) || isset($this->formValues['category_id']) || isset($this->formValues['location']) || isset($this->formValues['search'])): ?> 
                    <?php echo $this->translate('Nobody has created an video with that criteria.'); ?>
                <?php else: ?>  
                    <?php echo $this->translate('No videos have been posted yet.'); ?>
                <?php endif; ?>  
                <?php if ($this->canCreate): ?>
                    <?php echo $this->translate('Be the first to %1$screate%2$s one!', '<a href="' . $this->url(array('action' => 'create'), "sitevideo_video_general") . '">', '</a>'); ?>
                <?php endif; ?>
            </span>
        </div>
    <?php endif; ?>
