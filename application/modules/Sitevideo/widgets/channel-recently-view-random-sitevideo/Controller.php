 ...
 */
class Sitevideo_Widget_ChannelRecentlyViewRandomSitevideoController extends Seaocore_Content_Widget_Abstract {
    ...
    public function indexAction() {
        ...
        $this->view->widgetPath = 'widget/index/mod/sitevideo/name/channel-recently-view-random-sitevideo';
        ...
        $showTabArray = $params['ajaxTabs'] = $this->_getParam('ajaxTabs', array('most_recent', 'most_liked', 'most_subscribed', 'most_commented', 'most_rated', 'most_favourites', 'random'));
        ...
        switch ($paramsContentType) {
            ...
            case 'most_liked' :
                $orderby = 'like_count';
                $filter = 'like_count';
                $this->view->message = 'Nobody has liked a channel yet.';
                break;
            case 'most_subscribed' :
                $orderby = 'subscribe_count';
                $filter = 'subscribe_count';
                $this->view->message = 'Nobody has subscribed a channel yet.';
                break;
            case 'most_commented' :
                $orderby = 'comment_count';
                $filter = 'comment_count';
                $this->view->message = 'Nobody has commented on channel yet.';
            case 'most_rated' :
                $orderby = 'rating';
                $filter = 'rating';
                $this->view->message = 'Nobody has rated a channel yet.';
                break;
            case 'most_favourites' :
                $filter = 'favourite_count';
                $orderby = 'favourite_count';
                $this->view->message = 'Nobody has favourited a channel yet.';
                break;
            case 'random' :
                $orderby = 'random';
                break;
        ...
        $paginatorVideoView = $this->view->paginatorVideoView = Engine_Api::_()->getDbTable('channels', 'sitevideo')->getChannelPaginator($params);
        $paginatorListView = $this->view->paginatorListView = Engine_Api::_()->getDbTable('channels', 'sitevideo')->getChannelPaginator($params);
        $paginatorGridView = $this->view->paginatorGridView = Engine_Api::_()->getDbTable('channels', 'sitevideo')->getChannelPaginator($params);
    ...
    public function setTabsOrder($tabs) {
        ...
        $tabsOrder['most_subscribed'] = $this->_getParam('subscribed_order', 3);
        ...
        $tabsOrder['random'] = $this->_getParam('random', 7);
        $tempTabs = array();
        foreach ($tabs as $tab) {
            $order = $tabsOrder[$tab];
