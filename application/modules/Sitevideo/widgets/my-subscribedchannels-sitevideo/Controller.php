 ...
 */
class Sitevideo_Widget_MySubscribedchannelsSitevideoController extends Seaocore_Content_Widget_Abstract {
    ...
    public function indexAction() {
        ...
        $this->view->widgetPath = 'widget/index/mod/sitevideo/name/my-subscribedchannels-sitevideo';
        $this->view->message = "You do not have any subscribed channels.";

        if (isset($params['search']) && !empty($params['search']))
            $this->view->message = 'No subscribed channels found in search criteria.';

        $this->view->id = $params['idenity'] = $this->_getParam('identity');
        $this->view->isViewMoreButton = true;

        if (is_null($this->view->channelOption)) {
            $this->view->channelOption = $params['channelOption'] = array('subscribe', 'subscriptionSettings');
        }

        $this->view->channelOption[] = 'owner';

        if (!in_array('subscribe', $this->view->channelOption)) {
            $this->view->channelOption[] = 'subscribe';
        }

        if (!in_array('subscriptionSettings', $this->view->channelOption)) {
            $this->view->channelOption[] = 'subscriptionSettings';
        }
        ...
        $paginator = $this->view->paginator = Engine_Api::_()->getDbTable('channels', 'sitevideo')->getSubscribedChannelPaginator($params);
