 ...
 * @license    http://www.socialengine.com/license/
 */
class Store_Plugin_Job_Encode extends Core_Plugin_Job_Abstract
{
  protected function _execute()
  {
    ...
    // Get video object
    $video = Engine_Api::_()->getItem('store_video', $video_id);
    if( !$video || !($video instanceof Video_Model_Video) ) {
      $this->_setState('failed', 'Store video is missing.');
      $this->_setWasIdle();
      return;
    }
    ...
    // Check video status
    if( 0 != $video->status ) {
      $this->_setState('failed', 'Store video has already been encoded, or has already failed encoding.');
      $this->_setWasIdle();
      return;
    }
  ...
  private function getTmpDir() {
    ...
    $tmpDir = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' .
      DIRECTORY_SEPARATOR . 'store_video';

    if( !is_dir($tmpDir) && !mkdir($tmpDir, 0777, true) ) {
      throw new Video_Model_Exception('Store video temporary directory did not exist and could not be created.');
    }

    if( !is_writable($tmpDir) ) {
      throw new Video_Model_Exception('Store video temporary directory is not writable.');
    }
  ...
  private function getVideo($video) {
    ...
    if( is_numeric($video) ) {
      $video = Engine_Api::_()->getItem('store_video', $video);
    }
  ...
  private function getStorageObject($video) {
    ...
    if( !$storageObject ) {
      throw new Video_Model_Exception('Store video storage file was missing');
    }
