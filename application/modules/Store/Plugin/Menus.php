...
class Store_Plugin_Menus
{
  ...
  public function onMenuInitialize_CoreMiniCart($row)
  {
    ...
    $contentTable = new Core_Model_DbTable_Content();
    $select = $contentTable->select()->where('page_id=1');
    $widgets = $contentTable->fetchAll($select);
    foreach ($widgets as $widget) {
      if ($widget->name == 'advancedsearch.mini-menu') {
        return array(
          'label' => $label,
          "uri" => $view->url(array('controller' => 'cart'), 'store_extended', 1),
        );
      }
  ...
  public function onMenuInitialize_StoreMainPurchases(Engine_Db_Table_Row $row)
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    if (!$viewer->getIdentity()) {
      return true;
  ...
  public function onMenuInitialize_StoreMainShipping(Engine_Db_Table_Row $row)
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    if (!$viewer->getIdentity()) {
      return true;
  ...
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    $isAllowOrder = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('store_product', $viewer, 'order');
  ...
  public function onMenuInitialize_StoreProductProfileEdit($row)
  {
    ...
    $subject = Engine_Api::_()->core()->getSubject();
 	...
 	if ($subject->isOwner($viewer) && ($subject->page_id == 0)) {
      $view = Zend_Registry::get('Zend_View');
  ...
  public function onMenuInitialize_StoreAdminMainAddons($row)
  {
    /**
     * @var $addons Store_Model_DbTable_Addons
     */
    ...
    $cls = (isset($row->params) && isset($row->params['class'])) ? $row->params['class'] : '';
