  ...
  public function onCartTransactionReturn(Store_Model_Order $order, array $params = array())
  {
    ...
    if ($paymentStatus == 'okay') {
      ...
      if ($order->didStatusChange()) {
        try {
          Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'store_cart_complete', array(
            ...
            'order_link'    => ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] .
	  ...
	  foreach ($items as $item) {
	  	if ($item->item_type != 'store_product') continue;
		$product = Engine_Api::_()->getItem('store_product', $item->item_id);
		if ($product) {
		      $owner = $product->getOwner();
			  if ($owner) {
				  $notificationTable->addNotification($owner, $user, $product, 'store_product_transaction_okay');
			  }
		  }
	  }
  	  return $order->status;
