...
class Store_Form_Cart_Details extends Engine_Form
{
    ...
    {
        $title = 'STORE_Update Shipping Details';
        $descrption = 'STORE_CART_SHIPPING_CART_UPDATE_ON_CHECK_OUT';
        ...
        if (!$this->_viewer->getIdentity()) {
            ...
            $this->addElement('text', 'middle_name', array(
                'label' => 'Middle Name',
                'required' => true,
                'value' => $this->_getDetail('middle_name'),
            ));
    ...
    {
        $token = ($this->_viewer->getIdentity()) ? null : Engine_Api::_()->store()->getToken();

        if (($value = $this->_details->getDetail($this->_viewer, $name, $token))) {
            return $value;
        }
