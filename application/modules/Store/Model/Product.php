...
class Store_Model_Product extends Core_Model_Item_Collection
{
  ...
  {
    return isset($this->description) ? Engine_String::strip_tags($this->description) : '';
  }
  ...
  public function getShortDescription($length = 300)
  {
    if (isset($this->description)) {
      $view = Zend_Registry::get('Zend_View');
      $description = Engine_String::strip_tags($this->description);

      $moreLink = $view->htmlLink($this->getHref(), " ... ", array('target' => '_blank'));

      $shortDescription = $view->string()->truncate($description, $length, $moreLink);
  ...
  public function hasStore()
  {
    ...
    $page = Engine_Api::_()->getItem('page', $this->page_id);
...
//    $table = Engine_Api::_()->fields()->getTable('products', 'store');
  ...
  {
    return ($this->price <= 0 && $this->type == 'digital');
  }
  ...
  public function isLocationSupported($location_id)
  {
     ...
     * @var $table Store_Model_DbTable_Productships
     */
    $table = Engine_Api::_()->getDbTable('locationships', 'store');
    $select = $table->select()
      ->from($table, new Zend_Db_Expr('IF(location_id > 0, 1, 0)'))
      ->where('location_id = ?', $location_id);

    $settings = Engine_Api::_()->getDbTable('settings', 'core');
    $isAdmin = $settings->__get('store.store.locations.' . $this->page_id, 0);

    if (!$isAdmin) {
      $select->where('page_id = ?', $this->page_id);
    }

    return (bool)$select->query()
      ->fetchColumn();
  ...
  public function isAdminLocations()
  {
    // true - admin locations
    // false - store locations
    if (!$this->page_id) {
      return true;
    }
    $settings = Engine_Api::_()->getDbTable('settings', 'core');
    $setting = $settings->__get('store.store.locations.' . $this->page_id);
    return $setting;
  }
  ...
  public function getShippingPrice($location_id)
  {
    if ($this->isDigital()) {
	 ...
	 $settings = Engine_Api::_()->getDbTable('settings', 'core');
	  ...
	  $setting = $settings->__get('store.ww.enabled');
	 

    if ($this->isLocationSupported($location_id)&&$setting==0) {
      if ($this->isAdminLocations()) {
        $page_id = 0;
      } else {
     ...
     $a = (double)$table->select()
      ...
      return Engine_Api::_()->store()->getWorldWideShippingPrice();
    }
  ...
  public function getShippingDays($location_id)
  {
    ...
    if ($this->isLocationSupported($location_id)) {
      if ($this->isAdminLocations()) {
        $page_id = 0;
      } else {
        $page_id = $this->page_id;
      ...
      return Engine_Api::_()->store()->getWorldWideShippingDays();
    }
  }
  ...
  public function getSimilarProducts()
  {
    ...
    $catId = $this->getCategory()->value;
    //все продукты, где или категория, или самые частые теги совпадают
    ...
    if ($tags) {
      $select
        ->joinLeft($prefix . 'core_tags', $prefix . "core_tags.tag_id in({$tags})", array())
        ->joinLeft($prefix . 'core_tagmaps', $prefix . "core_tagmaps.tag_id in({$tags})", array())
        ...
        ->orWhere('o.option_id = ?', $catId);
    } else {
      $select->where('o.option_id = ?', $catId);
    }


    $select->where($prefix . "store_products.product_id <> ?", $this->getIdentity())
      ->limit(5)
      ->order('RAND()');
  ...
  public function getTags()
  {
    ...
    $select
      ->from($prefix . 'core_tagmaps', array('tag_id', 'freq' => 'COUNT(' . $prefix . 'core_tagmaps.tag_id)'))
    ...
    $where = $prefix . "core_tagmaps.resource_id = {$this->getIdentity()}";
    $select->where($where);
    ...
    foreach ($tags as $tag) {
      ...
      $select = $db->select()
        ...
        ->joinInner($prefix . 'store_products', $prefix . 'core_tagmaps.resource_id = ' . $prefix . 'store_products.product_id AND (' . $prefix . 'store_products.quantity <> 0 OR ' . $prefix . 'store_products.type = "digital")', array());
      $select = $table->setStoreIntegrity($select);
      $select->where($prefix . "core_tagmaps.tag_id = {$tag['tag_id']}");
  ...
  public function getBundle()
  {
    ...
    if (!$modules->isModuleEnabled('storebundle')) {
      return null;
    }

    /**
     * var $sbTable Storebundle_Model_DbTable_Storebundles
     */
    $sbTable = Engine_Api::_()->getItemTable('storebundle');
    $select = $sbTable->select()
      ->setIntegrityCheck(false)
      ->from(array('sb' => $sbTable->info('name')))
      ->join(array('sbp' => $sbTable->getTablePrefix() . 'storebundle_products'), 'sb.storebundle_id = sbp.bundle_id')
      ->where('sbp.product_id=?', $this->getIdentity())
    ;
    $t = $sbTable->fetchRow($select);


    return (isset($t->enabled)) ? $t : null;
  }

  public function getBundleId()
  {
    $bundle = $this->getBundle();
    return ($bundle) ? $bundle->getIdentity() : 0;
  }
  ...
  public function getCondition()
  {
	...
	if (in_array($this->item_condition, array_keys($conditions)))
		return $conditions[$this->item_condition];
	return '';
  }
	
	public function getRequestCount()
	{
		return Engine_Api::_()->getDbTable('brequests', 'store')->getRequestCountByProductId($this->getIdentity());
	}
	
	public function getRequests()
	{
		return Engine_Api::_()->getDbTable('brequests', 'store')->getRequestsByProductId($this->getIdentity());
	}
  ...
  /*public function getRichContent($view = false, $params = array())
  {
      $view = Zend_Registry::get('Zend_View');
      $view->product = $this;
      $content = $view->render('application/modules/Store/views/scripts/_product_feed_item.tpl');
      return $content;
  }*/
}
