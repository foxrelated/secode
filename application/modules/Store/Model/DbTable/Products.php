...
class Store_Model_DbTable_Products extends Engine_Db_Table
{
  ...
  public function modifyProducts($ids = array(), $action = 0)
  {
    if (empty($ids) || !$action) {
      return;
    }
    ...
    $where = $this->getAdapter()->quoteInto('product_id IN (?)', $tmp);

    switch($action) {
      case 1:
        // Set selected as featured
        $this->update(array('featured' => 1), $where);
        break;
      case 2:
        $this->update(array('sponsored' => 1), $where);
        // Set selected as sponsored
        break;
      case 3:
        $this->update(array('featured' => 0), $where);
        // Set selected as not featured
        break;
      case 4:
        $this->update(array('sponsored' => 0), $where);
        // Set selected as not sponsored
        break;
      case 5:
  ...
  public function getSelect($params = array())
  {
    ...
    if (!empty($params['ids'])) {
      $select
        ->where($prefix . 'store_products.product_id in (?)', $params['ids']);
    }
  ...
  public function setStoreIntegrity(Zend_Db_Select $select)
  {
    ...
    if (!Engine_Api::_()->getDbtable('modules', 'hecore')->isModuleEnabled('page')) {
      ...
      $select
        ...
        ->where($prefix . 'page_pages.approved=1')//->where('apis.enabled = 1')
      ;
