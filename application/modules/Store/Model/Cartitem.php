...
class Store_Model_Cartitem extends Core_Model_Item_Abstract
{
  ...
  public function isUserLocationSupported()
  {
    ...
    $hasLocations = $table->select()
      ->setIntegrityCheck(false)
      ->from(array('i' => $table->info('name')), "IF((ls.locationship_id && p.page_id=ls.page_id),TRUE,FALSE)")
      ...
      ->joinLeft(array('ls' => $prefix . 'store_locationships'),
        'ls.page_id = p.page_id AND ( (ls.location_id = d.location_id_2) || (ls.location_id = d.location_id_1 && d.location_id_2 = 0))', array())
      ->joinLeft(array('l' => $prefix . 'store_locations'), 'l.location_id = ls.location_id', array())
  ...
  public function isCheckable($via_credits = false)
  {
    ...
    if (!$this->isUserLocationSupported() && !Engine_Api::_()->store()->isWorldWideEnabled())
      return false;
  ...
  public function getPrice($activeBundle = false)
  {
    ...
    $bundle = $product->getBundle();

    if ($bundle && $activeBundle && ($this->bundle_id == $bundle->getIdentity())) {
      $bundle = $product->getBundle();
      $price = $price - $price * $bundle->percent / 100;
    }
