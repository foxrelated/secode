...
class Store_PanelController extends Core_Controller_Action_Standard
{
  /**
   * @var $_isPublic boolean
   */
  ...
  public function init()
  {
    ...
    if ($action != 'address' && $action != 'purchases' && $action != 'purchase' && $this->_isPublic) {
      $this->_helper->redirector->gotoRoute(array(), 'store_general');
    }
  }
  ...
  public function indexAction()
  {
    if (
      ...
      $this->_helper->redirector->gotoRoute(array('action' => 'products'));
    }
  ...
  {
    $this->view->order_id = $order_id = $this->_getParam('order_id', 0);
    ...
    if (isset($order->shipping_details) &&
      ...
      if (isset($order->shipping_details['location_id_2']) && $order->shipping_details['location_id_2'] &&
  ...
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    $details = Engine_Api::_()->getDbtable('details', 'store');
  ...
  public function wishListAction()
  {
	...
	public function productsAction() {
		...
		if ($this->_getParam('format') == 'json') {
	      	$this->view->html = $this->view->render('_product_list_edit.tpl');
	    }
	}
    ...
    //    $page = $this->view->page;
        ...
        // Make form
        $this->view->formFilter = $formFilter = new Store_Form_Transaction_Filter(array());
		...
		$select
            ->joinLeft(array('p' => $prefix . 'store_products'), 'p.product_id=oi.item_id', array())
            ->where('p.owner_id = ?', $viewer->getIdentity());
