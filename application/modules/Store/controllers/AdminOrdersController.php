...
class Store_AdminOrdersController extends Core_Controller_Action_Admin
{
    ...
    {
        $this->view->activeMenu = 'store_admin_main_orders';
    ...
    public function indexAction()
    {
        ...
        elseif (Engine_Api::_()->getDbtable('gateways', 'store')->getEnabledGatewayCount() <= 0) {
            $this->view->error = $this->view->translate('STORE_No enabled gateways', '<a href="' .
...
//      'payment_state'=> 't.state'))
            ...
            ->where("oi.status IN('processing', 'completed', 'shipping', 'delivered', 'pending')");
    ...
    public function detailAction()
    {
        ...
        $this->view->user = $user = ($order->user_id) ? $item->getOwner() : new User_Model_User(array());
