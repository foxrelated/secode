...
//    !$this->_helper->requireAuth()->setAuthParams($page, null, 'edit')->isValid() ||
  ...
  public function indexAction()
  {
    ...
    $setting = $settings->__get('store.store.locations.' . $page->getIdentity());

    $this->view->isAdmin = $isAdmin = $this->_getParam('admin');

    $params = $this->_getAllParams();
    if (isset($params['admin'])) {
      $settings->__set('store.store.locations.' . $page->getIdentity(), $isAdmin);
    }
  }
  ...
  public function editAction()
  {
    ...
    try {
      ...
      if ((float)$data['shipping_tax'] <= 0)
        $ship->shipping_tax = null;
      else
        $ship->shipping_tax = (float)$data['shipping_tax'];
