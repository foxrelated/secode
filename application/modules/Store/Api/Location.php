...
class Store_Api_Location extends Core_Api_Abstract
{
    ...
    public function getPaginator($page_id, $page = 1, $parent_id = 0, $type = 'supported', $product_id = 0, $location_name = null)
    {
        switch ($type) {
            ...
            case 'supported':
                ...
                $select = $this->_table->select()
                    ->setIntegrityCheck(false)
                    ->from(array('a' => $this->_table->info('name')), array('a.location', 'shipping_tax', 'sub_locations' => new Zend_Db_Expr('(' . $selectB . ')'), 'supported' => 'IF(l.locationship_id > 0, 1, 0)'))
            ...
            case 'product':
                ...
                $select = $this->_table->select()
                    ->setIntegrityCheck(false)
                    ->from(array('a' => $this->_table->info('name')), new Zend_Db_Expr('a.location, a.shipping_amt, a.shipping_days, (' . $selectB . ') as sub_locations '))
                    ->joinInner(array('s2' => $this->_tableShips->info('name')), 's2.location_id = a.location_id', array())
                    ->where('s2.page_id = ?', $page_id)
                    ->where('a.parent_id = ?', $parent_id)
                    ->order('a.location ASC');
            ...
            default:
                ...
                if ($location_name) {
                    $select->where('a.location like ?', $location_name . '%');
                }
