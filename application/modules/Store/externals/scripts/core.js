...
var StoreCart = new Class({
      ...
      'onSuccess': function ($response) {
        if ($response.status) {
          ...
          for(var i=0;i<ids.length;i++) {
            if ($('store-product-profile-details-' + ids[i])) {
              self.refreshWidget(ids[i]);
            }
          }
  ...
  product: {
    add: function (mode, $product_id, $values, $quantity, options) {
      // mode 0 - std, 1 - details popup, 2 - quickview
      var self = store_cart;
      var href = '';
      var $params = {
        'format': 'json',
      ...
      new Request.JSON({
        ...
        'onSuccess': function ($response) {
          if ($response.status) {
            ...
            if (mode == 2) {
              var btn = $(options.el).getElement('span');
              if(btn.hasClass('adding')) {
                btn.removeClass('adding');
              }
              if(btn.hasClass('store-add-button')) {
                btn.removeClass('store-add-button');
              }
              btn.addClass('store-remove-button');
              btn.set('text', options.text);
              btn.getParent().set('onclick', '').removeEvents().addEvent('click', function () {
                quickToCart.remove($product_id, $response.item_id, options.el);
              });
              // quickToCart.remove('+$product_id+', '+options.item_id+', this);
              $$(".quick-product-options").setStyle('display', 'none');
            }
			...
			window.location = en4.core.baseUrl + 'store/cart/checkout/'+$product_id;
			return;
            if (self.show_cart) {
          ...
          if (mode == 1) {
            product_manager.hideProductDetails();
          } else if (mode == 2) {
    ...
    remove: function (mode, $product_id, $item_id, options) {
      var self = store_cart;
        ...
        'onSuccess': function ($response) {
          if ($response.status) {
            ...
            if (mode == 2) {
              ...
              if(btn.hasClass('store-remove-button')) {
                btn.removeClass('store-remove-button');
              }
              btn.addClass('store-add-button');
              btn.set('text', options.text);
              btn.getParent().set('onclick', '').removeEvents().addEvent('click', function () {
                quickToCart.add($product_id, 0, options.el);
              });
              $$(".quick-product-options").setStyle('display', 'block');
            }
	...
	removeone: function (mode, $product_id, $item_id, options) {
      var self = store_cart;
    ...
    addToWishList: function (mode, $product_id) {
      var self = store_cart;
      var $params = {
        'format': 'json',
        'product_id': $product_id,
      ...
      new Request.JSON({
        ...
        'onRequest': function () {
          ...
          if (!mode) {
            ...
            $('add-to-wish-list-' + $product_id).addClass('adding');
          }
        },
        ...
        'onSuccess': function ($response) {
          if ($response.status) {
            ...
            if (mode) {
              product_manager.updateWishListButton(mode, 0, $product_id, $response.count);
            } else {
              if ($('store-product-profile-details-' + $product_id)) {
                self.refreshWidget($product_id);
              }
    ...
    removeFromWishList: function (mode, $product_id) {
      var self = store_cart;
      var $params = {
        'format': 'json',
        'product_id': $product_id,
      ...
      new Request.JSON({
        ...
        'onSuccess': function ($response) {
          if ($response.status) {
            he_show_message($response.message);
            if (mode) {
              product_manager.updateWishListButton(mode, 1, $product_id, $response.count);
            } else {
              if ($('store-product-profile-details-' + $product_id)) {
                self.refreshWidget($product_id);
              }
