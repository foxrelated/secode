 ...
 * @version    $Id: _FancyUploadAudio.tpl 2011-09-12 16:20:23 taalay $
 * @author     Taalay
 */
?>
...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() { // wait for the content
    var up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
      ...
      url: $('form-upload-audio').action + '?ul=1',
      ...
      onFileRemove: function(file) {
        ...
        request = new Request.JSON({
          'format' : 'json',
          'url' : $('form-upload-audio').action + '?rm=1',
          'data': {
            'format': 'json',
            'file_id' : file_id
          },
        ...
        if (json.get('status') == '1') {
          ...
          if (fileids) {
            if (fileids.value.length)
              fileids.value += ' ';
            fileids.value += json.get('file_id');
          }
        } else {
...
<fieldset id="demo-fallback">
  ...
  <label for="demo-musiclabel">
    <?php echo $this->translate('Upload Audios:') ?>
...
<div id="demo-status" class="hide">
  <div>
    <?php echo $this->translate('_AUDIO_UPLOAD_DESCRIPTION') ?>
  </div>
  <div>
    <a class="buttonlink icon_audio_new product_audios_new" href="javascript:void(0);" id="demo-browse"><?php echo $this->translate('Add Audios') ?></a>
