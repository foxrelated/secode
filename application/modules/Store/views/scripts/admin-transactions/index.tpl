    ...
    <tbody>
      <?php foreach( $this->paginator as $item ):
        ...
        <tr>
          ...
          <td class='admin_table_bold'>
            <?php
                if($user) {
                    ...
                    if($item->token) {
                        echo $item->getPublicName();
                    } else {
                        echo ( $user ? $user->__toString() : '<i>' . $this->translate('Deleted or Unknown Member') . '</i>' );
                    }
                }
            ?>
          </td>
          ...
          <td class='center'>
            <?php echo ( $gateway ? $gateway->title : '<i>' . (($item->gateway_id == '99') ? $this->translate('Item') : $this->translate('Unknown Gateway')) . '</i>' ) ?>
          </td>
          <td class='center'>
