...
<?php if ($this->products->getTotalItemCount() > 0): ?>
  <?php if ($this->page->getStorePrivacy() && !$this->isGatewayEnabled) : ?>
    <div class="tip">
      <span>
        <?php echo $this->translate('STORE_No enabled gateways', '<a href="' .
          $this->escape($this->url(array('action' => 'gateway', 'page_id' => $this->page->getIdentity()), 'store_settings', true)) .
          '">', '</a>');
        ?>
      </span>
    </div>
        ...
        <li>
          ...
          <div class="he-item-info">
            ...
            <div class="he-item-desc">
              <?php echo $this->viewMore(Engine_String::strip_tags($item->getDescription()), 33) ?>
            </div>
          </div>
        </li>
      <?php endforeach; ?>
