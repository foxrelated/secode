...
<script type="text/javascript">
  ...
  function switchCheckoutFormCheckout($via_credits)
  {
    product_manager.switchCheckoutFormCheckout($via_credits);
  }
...
<div id="accordion">
  <h3 onclick="switchCheckoutFormCheckout(0)"><?php echo $this->translate('Order Summary')?></h3>
    ...
    <ul id="store-checkout-panel">
      ...
      <div>
		...
		<div class="checkout-item">
          <button class="gateway-button button" onclick="checkout(false);">
            <?php echo $this->translate('Select your Item'); ?>
          </button>
        </div>
      </div>
    </ul>
  ...
  <?php if ($this->api->isCreditEnabled() && $this->viewer()->getIdentity()) : ?>
    <h3 onclick="switchCheckoutFormCheckout(1)"><?php echo $this->translate('Order via Credits')?></h3>
          ...
          <?php if ($this->taxesPrice + $this->shippingPrice + $this->totalPrice > 0): ?>
            <?php foreach ($this->gateways as $gateway): ?>
              <?php if ($gateway->title == 'Credit') : ?>
                <div class="checkout-item">
                  <button class="gateway-button button" onclick="checkout(true);">
                    <?php echo $this->translate('Select the OGV amount'); ?>
                  </button>
                </div>
              <?php endif;?>
            <?php endforeach; ?>
