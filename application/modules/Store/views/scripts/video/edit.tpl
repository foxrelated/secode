...
en4.core.runonce.add(function () {
  ...
  up = new FancyUpload2($('demo-status'), $('demo-list'), { // options object
    ...
    url: "store/index/edit-video<?php// echo $this->url(array('module'=>'store', 'controller'=>'videos', 'action'=>'edit-video'), 'admin_default', 1) . '?ul=1'; ?>",
    ...
    onLoad: function () {
      ...
      <?php if($this->preview){?>
       var li1 =  document.getElementById('type-1').parentElement;
       var li2 =  document.getElementById('type-3').parentElement;

      li1.style.display = "none";
      li2.style.display = "none";

        $('store-video-preview').set('html' , '<?php echo $this->preview;?>');
      <?php } ?>
  ...
  $('upload').addEvent('click', function () {
    ...
    new Request.JSON({
      method: 'post',
      url: 'store/video/edit<?php// echo $this->url(array('module'=>'store', 'controller'=>'videos', 'action'=>'edit-video'), 'admin_default', 1); ?>',
      data: data,
...
var video = {
  ...
  youtube: function (url) {
    ...
    if (youtube_code) {
      (new Request.HTML({
        'format': 'html',
        'url': 'store/video/validation/product_id/<?php echo $this->product->getIdentity();?>   <?php //echo $this->url(array('module' => 'store', 'controller' => 'video', 'action' => 'validation', 'product_id' => $this->product->getIdentity())) ?>',
        'data': {
          'ajax': true,
          'code': youtube_code,
          'type': 'youtube'
...
<?php echo $this->htmlLink($this->url(array('action' => 'edit', 'product_id' => $this->product->getIdentity()), 'store_products'), $this->translate('Edit Item'), array(
                    ...
                    )) ?> <img src="/application/modules/Core/externals/images/next.png"> <?php echo $this->translate("Edit Video") ?> <img src="/application/modules/Core/externals/images/next.png">
  <?php echo $this->translate('%s', $this->htmlLink($this->product->getHref(), $this->product->getTitle())); ?>
</h3>


    <p class="flying-text-imp">
      <?php echo $this->translate("STORE_EDIT_VIDEO_DESCRIPTION_FORM") ?>
    </p>
