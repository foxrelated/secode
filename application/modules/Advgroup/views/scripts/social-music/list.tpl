	...
	<div class="generic_layout_container layout_middle">
        <div class="group_discussions_options">
            ...
            <?php if ($this->canCreate):?>
            	<?php $label = ($this->type == 'album') ? $this->translate('Create Album') : $this->translate('Create Song');?>
                ...
                'class' => 'buttonlink icon_group_music_new'
                ))
                ?>
            <?php endif; ?>
        </div>      

        <div class="group-profile-header-content">
           <?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
            <span class="group-numeric"><?php echo $this->paginator->getTotalItemCount(); ?></span>
            <?php 
            if ($this->type == 'album') 
		...
		<?php 
        if ($this->type == 'album') 
        	echo $this->partial('_album-listing.tpl', 'ynmusic', array('paginator' => $this->paginator, 'formValues' => $this->formValues, 'group_id' => $this->subject()->getIdentity(), 'paging' => true));	
		else
		 	echo $this->partial('_song-listing.tpl', 'ynmusic', array('paginator' => $this->paginator, 'formValues' => $this->formValues, 'group_id' => $this->subject()->getIdentity(), 'paging' => true));
        ;?>
    </div>
</div>
