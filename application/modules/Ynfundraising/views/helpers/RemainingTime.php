 ...
 * @license    http://www.socialengine.com/license/
 */
class Ynfundraising_View_Helper_RemainingTime extends Zend_View_Helper_HtmlElement
{
  const MINUTE = 60;
  const HOUR = 3600;
  const DAY = 86400;
  ...
  public function remainingTime($time, $endtime, $attribs = array())
  {
    if( null === $time )
    {
      $time = time();
    ...
    if( is_string($endtime) && !is_numeric($endtime) )
    {
    ...
    $content = $this->calculateDefaultTimestamp($time, $endtime);
