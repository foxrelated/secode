...
Element.Properties.slideshow = {
	...
	set: function(options){
		this.get('slideshowynfundraising').setup(options);
		return this;
	},

	get: function(){
		var instance = this.retrieve('slideshowynfundraising');
		if (!instance){
			instance = new SlideShow(this, {}, true);
			this.store('slideshowynfundraising', instance);
		}
		return instance;
	}
...
Element.implement({
	...
	playSlideShow: function(options){
		this.get('slideshowynfundraising').setup(options).play();
		return this;
	},

	pauseSlideShow: function(){
		this.get('slideshowynfundraising').pause();
		return this;
	}
...
(function(){
	...
	function go(type, styles, data){
		...
		if (type != 'slide') {
			...
			data.next.tween(styles.property, 2);
		}
		if (type != 'blind'){
			data.previous
			    .set('tween', tweenOptions)
