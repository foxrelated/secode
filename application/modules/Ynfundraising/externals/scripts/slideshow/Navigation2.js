...
document.addEvent('domready', function(){
	...
	var navs = $('ynfundraising_pagination').getElements('a');

	// create a basic slideshow
	navSlideShow = new SlideShow('ynfundraising_navigation-slideshow', {
		autoplay: true,
		delay: 5000,
	...
	if($$('.ynfundraising_newsalbums').length > 0) {
		var itemWidth =  parseInt($$('.ynfundraising_newsalbums')[0].getStyle('width'));
		...
		$('ynfundraising_navigation-slideshow').style.width = slideWidth + "px";
		console.log(itemWidth);
		if($$('.ynfundraising_newsalbums').length >= 4) {
			$$('.ynfundraising_newsalbums').each(function(item, index){
