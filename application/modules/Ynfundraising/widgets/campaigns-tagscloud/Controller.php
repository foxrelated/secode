...
class Ynfundraising_Widget_CampaignsTagscloudController extends Engine_Content_Widget_Abstract
{
    public function indexAction()
    {
        $t_table = Engine_Api::_()->getDbtable('tags', 'core');
        $tm_table = Engine_Api::_()->getDbtable('tagMaps', 'core');
        $p_table = Engine_Api::_()->getDbTable('campaigns', 'ynfundraising');
        ...
        $filter_select = $tm_table->select()->from($tmName,"$tmName.*")
                         ...
                         ->join($pName,"$pName.campaign_id = $tmName.resource_id",'')
                         ->where("$pName.status not in ('draft')");
        ...
        $select  ->where("t.resource_type = ?","ynfundraising_campaign");
