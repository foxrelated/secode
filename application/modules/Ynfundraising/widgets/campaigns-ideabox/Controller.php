<?php
class Ynfundraising_Widget_CampaignsIdeaboxController extends Engine_Content_Widget_Abstract {
	public function indexAction() {
		...
		$params = array('limit' => $limit, 'parent_type' => "'idea','trophy'", 'status' => Ynfundraising_Plugin_Constants::CAMPAIGN_ONGOING_STATUS);
		$campaigns = Engine_Api::_()->getApi('core', 'ynfundraising')->getCampaignPaginator($params);
		$this->view->campaigns = $campaigns;
		if(!$campaigns->getTotalItemCount() || !Engine_Api::_ ()->getApi ( 'core', 'ynfundraising' )->checkIdeaboxPlugin ())
			$this->setNoRender();
	    $view = $this->view;
