  ...
  setContainersSize: function(){
    ...
    if( this.options.slideVertical ){
      ...
      if( this.options.showControls ){
        var s1 = $('SlideItMoo_forward').getSize();
        var s2 = $('SlideItMoo_back').getSize();
			...
			if (call_count == 1) {
				...
				if (direction == 1) {
					...
					if (forward_link == 1) {
						...
						startindex = parseInt(startindex) + parseInt(curnt_limit);
						if (startindex > total) { 
							startindex = parseInt(startindex) - parseInt(curnt_limit);
						}
					}
				...
				var obj = this;
				var url = en4.core.baseUrl+'list/index/homesponsored';	
				var request = new Request.HTML({
					...
					'onSuccess' : function(responseTree, responseElements, responseHTML, responseJavaScript)
					{
						...
						$('SlideItMoo_items').innerHTML = responseHTML;
