...
<h2>
  <?php $album = Engine_Api::_()->getItem('list_album', $this->list[0]['album_id']);?>
  <?php echo $this->translate('%1$s '."&#187; Photos", $album->getOwner()->__toString()) ?>
</h2>

<?php if($enable_facebookse): ?>
	<div id="fblisting_photo">
		<script type="text/javascript">
			var fblike_moduletype = 'list_photo';
...
<div class='list_photo_view'>
  <div class="list_photo_nav">
    <div>
      <?php
				echo $this->translate('Picture %1$s of %2$s',
              $this->locale()->toNumber($this->image->getCollectionIndex() + 1),
              $this->locale()->toNumber($this->list->count()))
      ?>
    </div>
    <?php if ($this->list->count() > 1): ?>
			<div>
	...
	<div class='list_photo_info'>
		<div class='list_photo_container' id='media_image_div'>
	  ...
	  <div class="event_photo_tags" id="media_tags" class="tag_list" style="display: none;">
	    ...
	    <div class="list_photo_date">
	      ...
	      <?php if( $this->canEdit ): ?>
					...
					- <?php echo $this->htmlLink(array('route' => 'list_photo_extended', 'controller' => 'photo', 'action' => 'edit', 'photo_id' => $this->image->getIdentity(), 'format' => 'smoothbox'), $this->translate('Edit'), array('class' => 'smoothbox')) ?>
				<?php endif; ?>

				<?php if( $this->canEdit && $this->canDelete): ?>
					- <?php echo $this->htmlLink(array('route' => 'list_photo_extended', 'controller' => 'photo', 'action' => 'remove', 'photo_id' => $this->image->getIdentity(), 'format' => 'smoothbox'), $this->translate('Delete'), array('class' => 'smoothbox')) ?>
				<?php endif; ?>
