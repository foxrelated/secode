...
<script type="text/javascript">
	...
	function startImporting(file_id) {
		...
		en4.core.request.send(new Request({
			url : en4.core.baseUrl+'admin/list/importlisting/data-import?importfile_id='+importfile_id,
			method: 'get',
			data : {
				//'format' : 'json',
			},
	...
	var request = new Request.JSON({
  	'url' : en4.core.baseUrl+'admin/list/importlisting/stop?importfile_id='+importfile_id,
  	'data' : {
    					'format' : 'json'
...
<div id="startImporting" style="display:none;">
	...
	<center class="mtop10">
		<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/List/externals/images/loader.gif" alt="Importing lists" />
	</center>
	<br />
...
<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'list', 'controller' => 'importlisting', 'action' => 'index'), $this->translate('Import a new file'), array('class' => 'buttonlink icon_list_admin_import')) ?>

<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'list', 'controller' => 'log', 'action' => 'index'), $this->translate('Import History'), array('class' => 'buttonlink icon_lists_log')) ?><br/><br/>

<?php if($this->paginator->getTotalItemCount()): ?>	

	<h3><?php echo $this->translate("Manage CSV Import Files"); ?></h3>
	<p class="form-description"><?php echo $this->translate('This page contains all the CSV files uploaded by you for importing Listings from them. You can start, stop, rollback and delete the import corresponding to each file now. Rollback will delete all the listings imported from that file. Delete will only delete those entries which were going to be used to import the corresponding Listings from that file and also delete the entry of that file from here. Below are the meanings of status for the Files uploaded:'); ?></p> 
	...
	<div class="admin_files_lists">
		<?php $listInfo = $this->paginator->getPages(); ?>
		<?php echo $this->translate("Showing ");?><?php echo $listInfo->firstItemNumber ?>-<?php echo $listInfo->lastItemNumber ?><?php echo $this->translate(" of "); ?><?php echo $listInfo->totalItemCount ?><?php echo $this->translate(" files.")?>
	</div>
				...
				<tr>
					...
					<td width="20%" style="text-align:left;">
						<?php if($item->status != 'Running'): ?>
							...
							<?php endif; ?>
							<?php if($item->status != 'Pending'): ?>
								<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'list', 'controller' => 'importlisting', 'action' => 'rollback', 'importfile_id' => $item->importfile_id), $this->translate('Rollback'), array('class' => 'smoothbox')) ?>
								|
							<?php endif; ?>
							<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'list', 'controller' => 'importlisting', 'action' => 'delete', 'importfile_id' => $item->importfile_id), $this->translate('Delete'), array('class' => 'smoothbox')) ?>
						<?php else: ?>
							<?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'list', 'controller' => 'importlisting', 'action' => 'stop', 'importfile_id' => $item->importfile_id, 'forceStop' => 1), $this->translate('Stop')) ?>
						<?php endif;?>
					</td>
