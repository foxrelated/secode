 ...
 * @version    $Id: aolapi.php 2013-09-02 00:00:00Z SocialEngineAddOns $
...
function getaolcontacts($username,$password, $login=true) {
  ...
  //SETP:1:-GETTING AOL LOGIN STORE.
	$ch	= curl_init();
	...
	$loginhtmlstore 	= curl_exec( $ch );
	
	//GETTING THE REQUIRED HIDDEN FILEDS FROM AOL LOGIN FORM.
	preg_match_all("/name=\"usrd\" value=\"(.*?)\"/", $loginhtmlstore, $arr_user_usrd);
	preg_match_all("/name=\"siteState\" value=\"(.*?)\"/", $loginhtmlstore, $arr_user_sitestate);
  ...
  $login_confirmurl = 'https://my.screenname.aol.com/_cqr/login/login.psp';
	...
	$loginconfirmstore = curl_exec( $ch );
