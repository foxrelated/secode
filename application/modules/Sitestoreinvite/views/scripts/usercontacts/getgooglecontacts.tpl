 ...
 * @version    $Id: getgooglecontacts.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
if (!$this->errormessage) {
	...
	if ($total > 0) { ?>
		<div class="header">	
			<div class="title">	
				<?php echo $this->translate("Found %s Gmail contacts you can promote this Store to.", $total);?>
			</div>
			<div>
	...
	if (!empty($this->addtononfriend)) { ?>
		...
		<?php  
			$getGooglePer = Zend_Registry::isRegistered('sitestoreinvite_getGooglePer') ? Zend_Registry::get('sitestoreinvite_getGooglePer') : null; 
