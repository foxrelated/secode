 ...
 * @version    $Id: getwindowlivecontacts.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
...
if (!$this->errormessage) {
	...
	if ($total > 0) { ?>
		<div class="header">	
			<div class="title">	
				<?php echo $this->translate("Found %s Window Live contacts you can promote this Store to.", $total);?>
			</div>
			<div>
	...
	if (!empty($this->addtononfriend)) { ?>
		...
		<?php  
			$getWindowLivePer = Zend_Registry::isRegistered('sitestoreinvite_getWindowLivePer') ? Zend_Registry::get('sitestoreinvite_getWindowLivePer') : null;
