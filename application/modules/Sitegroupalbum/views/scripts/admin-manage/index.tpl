...
<?php endif; ?>
<h3><?php echo $this->translate('Manage Group Albums'); ?></h3>
<p>
  <?php echo $this->translate('Here, you can see all the albums for the Groups on your site. Here you can monitor these albums and delete offensive ones if necessary.');?>
</p>
<br />
...
<div class='admin_members_results'>
  <?php 
  	...
  	if(!empty($counter)): ?>
      <div class="">
        <?php  echo $this->translate(array('%s group album found.', '%s group albums found.', $counter), $this->locale()->toNumber($counter)) ?>
      </div>
  <?php else:?>
    <div class="tip"><span>
    ...
    <thead>
      <tr>
        ...
        <th style='width: 5%;' align="left"><a href="javascript:void(0);" onclick="javascript:changeOrder('username', 'ASC');"><?php echo $this->translate('Owner');?></a></th>
				<th style='width: 5%;' align="left"><a href="javascript:void(0);" onclick="javascript:changeOrder('sitegroup_title', 'ASC');"><?php echo $this->translate('Group Title');?></a></th>
        ...
        <?php foreach( $this->paginator as $item ): ?>
          <tr>         
            ...
            <td class="admin_table_bold"><?php  echo $this->htmlLink($this->item('user', $item->owner_id)->getHref(), $item->truncateOwner($this->user($item->owner_id)->username), array('title' => $item->username, 'target' => '_blank')) ?></td>
	          <?php             
             	$truncation_limit = 13;
							$tmpBodytitle = strip_tags($item->sitegroup_title);
            ...
            ?>          
						...
						<td class='admin_table_bold'><?php echo $this->htmlLink($this->item('sitegroup_group', $item->group_id)->getHref(), $item_sitegrouptitle, array('title' => $item->sitegroup_title, 'target' => '_blank')) ?></td>
            ...
            <?php if($item->type == NULL):?>
							<?php if($item->featured == 1):?>
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('module' => 'sitegroupalbum','controller'=>'admin-manage','action'=>'featured','route' => 'default', 'id' => $item->album_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitegroupalbum/externals/images/sitegroupalbum_goldmedal1.png', '', array('title'=> $this->translate('Featured')))) ?> 
								</td>       
							<?php else: ?>  
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('module' => 'sitegroupalbum','controller'=>'admin-manage','action'=>'featured','route' => 'default', 'id' => $item->album_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitegroupalbum/externals/images/sitegroupalbum_goldmedal0.png', '', array('title'=> $this->translate('Un-featured')))) ?>
								</td>
							<?php endif; ?> 
            <?php else :?>
            ...
            <td class='admin_table_options' style="white-space: nowrap;">	            						 
              ...
              |
              <?php echo $this->htmlLink(array('module' => 'sitegroupalbum','controller'=>'admin-manage','action'=>'delete','route' => 'default', 'id' => $item->album_id), $this->translate('delete'), array('class' => 'smoothbox')) ?> 
            </td>
