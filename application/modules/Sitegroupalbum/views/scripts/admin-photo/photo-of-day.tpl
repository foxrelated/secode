...
<br />
<div class="tip"> <span> <?php echo $this->translate('You should only make those photos as "Photo of the Day" which have their group view privacy set as \'Everyone\' or \'All Registered Members\'.'); ?> </span> </div>
<br />
<div>
...
<div>
	<?php if ($this->photoOfDaysList->getTotalItemCount() > 0): ?>
	  ...
	  <form id='multidelete_form' method="post" action="<?php echo $this->url(array('module' => 'sitegroupalbum', 'controller' => 'settings', 'action' => 'multi-delete-photo'), 'admin_default'); ?>" onSubmit="return multiDelete()">
		  <table class='admin_table' width="100%">
		    <thead>
		      <tr>
						<th style='width: 1%;' align="left"><input onclick="selectAll()" type='checkbox' class='checkbox'></th>
						<?php $class = ( $this->order == 'engine4_sitegroup_photos.title' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
						<th width="18%" align="left" class="<?php echo $class ?>"><a href="javascript:void(0);" onclick="javascript:changeOrder('engine4_sitegroup_photos.title', 'DESC');"><?php echo $this->translate("Photo") ?></a></th>
						<th width="18%" align="left"><?php echo $this->translate("Album") ?></th>
            <th width="18%" align="left"><?php echo $this->translate("Group Name") ?></th>
          ...
          <?php $sitegroupphoto_object = Engine_Api::_()->getItem('sitegroup_photo', $photo->resource_id);?>
		      <?php  $parent = Engine_Api::_()->getItem('sitegroup_album', $photo->album_id);?>
		        ...
		        <td width="18%" class="sitealbum_table_img"><?php echo $this->htmlLink($sitegroupphoto_object->getHref(),$this->itemPhoto($sitegroupphoto_object, 'thumb.normal'),array('title'=>$sitegroupphoto_object->getTitle())); ?></td>
            ...
            ?>          
						...
						<td width="18%" class='admin_table_bold'><?php echo $this->htmlLink($sitegroup_object->getHref(), $item_sitegrouptitle, array('title' => $sitegroup_object->title, 'target' => '_blank')) ?></td>
