 ...
 */
class Sitegroupalbum_Widget_ListAlbumsTabsViewController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($is_ajax)) {
      $this->view->tabs = $tabs = Engine_Api::_()->getItemTable('seaocore_tab')->getTabs(array('module' => 'sitegroupalbum', 'type' => 'albums', 'enabled' => 1));
    ...
    $activeTab = Engine_Api::_()->getItemTable('seaocore_tab')->getTabs(array('module' => 'sitegroupalbum', 'type' => 'albums', 'enabled' => 1, 'name' => $activeTabName));
    $this->view->activTab = $activTab = $activeTab['0'];

    switch ($activTab->name) {
      case 'recent_groupalbums':
        break;
      case 'liked_groupalbums':
        $select->order($tableName .'.like_count DESC');
        break;
      case 'viewed_groupalbums':
        $select->order($tableName .'.view_count DESC');
        break;
      case 'commented_groupalbums':
        $select->order($tableName .'.comment_count DESC');
        break;
      case 'featured_groupalbums':
        $select->where($tableName .'.featured = ?', 1);
        $select->order('Rand()');
        break;
      case 'random_groupalbums':
        $select->order('Rand()');
        break;
    }
 
    if ($activTab->name != 'featured_groupalbums' && $activTab->name != 'random_groupalbums') {
