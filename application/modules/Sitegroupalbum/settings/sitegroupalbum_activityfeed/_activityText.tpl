 ...
 * @version    $Id: _activityText.tpl 8897 2011-04-13 22:42:20Z alex $
 * @author     Jung
 */
?>
<?php $sitegroupAlbumEnable = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitegroupalbum'); ?>
<?php if(Engine_Api::_()->sitegroup()->canShowPhotoLightBox() && $sitegroupAlbumEnable):
include_once APPLICATION_PATH . '/application/modules/Sitegroupalbum/views/scripts/_lightboxImage.tpl';
endif; ?> 
...
<?php
  foreach( $actions as $action ): // (goes to the end of the file)
    try { // prevents a bad feed item from destroying the entire group
            ...
            <?php else: ?>
              <?php foreach( $action->getAttachments() as $attachment ): ?>
               ...
               <?php elseif( $attachment->meta->mode == 2 ): // Thumb only type actions ?>
							...
							<?php if($sitegroupAlbumEnable && $attachment->item->getType() == "sitegroup_photo" && Engine_Api::_()->sitegroup()->canShowPhotoLightBox()):?>
							<a class="feed_item_thumb" href="<?php echo $attachment->item->getHref() ?>" onclick='openLightBox("<?php echo $attachment->item->getPhotoUrl()?>","<?php echo Engine_Api::_()->sitegroup()->getHreflink($attachment->item) ?>");return false;' > <?php echo $this->itemPhoto($attachment->item, 'thumb.normal', $attachment->item->getTitle()) ?></a>
							<?php else: ?>
