...
Composer.Plugin.SitegroupPhoto = new Class({
  ...
  doRequest : function() {
    this.elements.iframe = new IFrame({
      'name' : 'composeSitegroupPhotoFrame',
      ...
      'events' : {
        'load' : function() {
          this.doProcessResponse(window._composeSitegroupPhotoResponse);
          window._composeSitegroupPhotoResponse = false;
        }.bind(this)
      }
    }).inject(this.elements.body);

    window._composeSItegroupPhotoResponse = false;
    this.elements.form.set('target', 'composeSitegroupPhotoFrame');
  ...
  doProcessResponse : function(responseJSON) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id' : 'compose-sitegroupphoto-preview-image',
