 ...
 */
class Sitegroupalbum_AdminAlbumController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->tabs = Engine_Api::_()->getItemTable('seaocore_tab')->getTabs(array('module' => 'sitegroupalbum', 'type' => 'albums'));
  }
  ...
  public function manageDayItemsAction() {
    ...
    $this->view->navigationGroup = Engine_Api::_()->getApi('menus', 'core')
		...
		//TAB CREATION
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sitegroupalbum_admin_main', array(), 'sitegroupalbum_admin_widget_settings');
    $this->view->subNavigation = $subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sitegroupalbum_admin_submain', array(), 'sitegroupalbum_admin_submain_dayitems');
    ...
    $this->view->albumOfDaysList = $albumOfDay = Engine_Api::_()->getDbtable('itemofthedays', 'sitegroup')->getItemOfDayList($values, 'album_id', 'sitegroup_album');
    $albumOfDay->setItemCountPerPage(50);
    $albumOfDay->setCurrentPageNumber($group);
  }
  ...
  public function addAlbumOfDayAction() {
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        $select = $dayItemTime->select()->where('resource_id = ?', $values["resource_id"])->where('resource_type = ?', 'sitegroup_album');
  ...
  public function enabledAction() {
    ...
    $this->_redirect('admin/sitegroupalbum/album');
  }
}
