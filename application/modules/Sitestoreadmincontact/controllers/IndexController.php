 ...
 */
class Sitestoreadmincontact_IndexController extends Core_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    //IF SUPERADMIN,ADMIN IS NOT THERE THEN RETURN THE PRIVATE STORE
    if (empty($level_id)) {
    ...
    $this->view->results = $results = Engine_Api::_()->getDbtable('stores', 'sitestore')->checkStore();
    if(!empty($results)) {
      //MAKE FORM
      $this->view->form = $form = new Sitestoreadmincontact_Form_Compose();
    } 
    ...
    try {
      ...
      if ((isset($package_id_array[0]) && empty($package_id_array[0])) && (isset($categories_id_array[0]) && empty($categories_id_array[0])) && (isset($status_id_array[0]) && empty($status_id_array[0]))) {
        $select = $manageadminsTable->select()->setIntegrityCheck(false)
                ...
                ->join($storeTableName, $storeTableName . '.store_id = ' . $manageadminsTableName . '.store_id', array())
