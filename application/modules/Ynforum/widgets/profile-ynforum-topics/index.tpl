...
<ul class="forum_topics" id="forum_topics">
    <?php foreach( $this->paginator as $topic ):
          ...
          <?php endif; ?>
            ...
            <div class="forum_topics_lastpost">
              <?php echo $this->htmlLink($this->url(array(
                  ...
                  'post_id' => $last_post->getIdentity()), 'ynforum_topic'), $this->itemPhoto($last_user, 'thumb.icon')) ?>
              <span class="forum_topics_lastpost_info">
              <?php if($last_post):
                list($openTag, $closeTag) = explode('-----', $this->htmlLink($this->url(array(
                                            'slug' => $topic->getSlug(), 
                                            'action' => 'view', 
                                            'topic_id' => $topic->getIdentity(),
                                            'post_id' => $last_post->getIdentity()), 'ynforum_topic'), '-----'));
                ?>
      ...
      <div class="forum_topics_replies">
        <span>
          <?php echo $this->translate(array('%1$s %2$s reply', '%1$s %2$s replies', $topic->approved_post_count-1), $this->locale()->toNumber($topic->approved_post_count-1), '</span><span>') ?>
        </span>
      </div>
