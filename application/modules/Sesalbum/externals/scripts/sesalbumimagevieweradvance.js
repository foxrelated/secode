...
sesJqueryObject(document).on('click','#saveDetailsChanelSes',function(e){
	e.preventDefault();
	var thisObject = this;
    ...
    type: "POST",  
    url: en4.core.baseUrl+'sesvideo/chanel/edit-detail/photo_id/'+photo_id,  
    data: formData,  
...
sesJqueryObject(document).on('click','#saveDetailsSes',function(e){
	...
	sesJqueryObject.ajax({  
    type: "POST",  
    url: en4.core.baseUrl+'albums/photo/edit-detail/album_id/'+album_id+'/photo_id/'+photo_id,  
    data: formData,  
    success: function(response) {  
      var data = JSON.parse(response);
			if(data.status && !data.error){
