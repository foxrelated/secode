 ...
 * @version    $Id: _activityText.tpl 2013-09-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
?>
<?php $sitestoreAlbumEnable = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sitestorealbum'); ?>
<?php if(Engine_Api::_()->sitestore()->canShowPhotoLightBox() && $sitestoreAlbumEnable):
include_once APPLICATION_PATH . '/application/modules/Sitestorealbum/views/scripts/_lightboxImage.tpl';
endif; ?> 
...
<?php
  foreach( $actions as $action ): // (goes to the end of the file)
    try { // prevents a bad feed item from destroying the entire store
            ...
            <?php else: ?>
              <?php foreach( $action->getAttachments() as $attachment ): ?>
               ...
               <?php elseif( $attachment->meta->mode == 2 ): // Thumb only type actions ?>
							...
							<?php if($sitestoreAlbumEnable && $attachment->item->getType() == "sitestore_photo" && Engine_Api::_()->sitestore()->canShowPhotoLightBox()):?>
							<a class="feed_item_thumb" href="<?php echo $attachment->item->getHref() ?>" onclick='openLightBox("<?php echo $attachment->item->getPhotoUrl()?>","<?php echo Engine_Api::_()->sitestore()->getHreflink($attachment->item) ?>");return false;' > <?php echo $this->itemPhoto($attachment->item, 'thumb.normal', $attachment->item->getTitle()) ?></a>
							<?php else: ?>
