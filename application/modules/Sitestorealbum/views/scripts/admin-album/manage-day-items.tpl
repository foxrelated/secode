...
<div>
	<?php if ($this->albumOfDaysList->getTotalItemCount() > 0): ?>
	  ...
	  <form id='multidelete_form' method="post" action="<?php echo $this->url(array('module' => 'sitestorealbum', 'controller' => 'settings', 'action' => 'multi-delete-album'), 'admin_default'); ?>" onSubmit="return multiDelete()">
		  <table class='admin_table' width="100%">
		    <thead>
		      <tr>
						<th style='width: 1%;' align="left"><input onclick="selectAll()" type='checkbox' class='checkbox'></th>
						<?php $class = ( $this->order == 'engine4_sitestore_albums.title' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
						<th width="24%" align="left" class="<?php echo $class ?>"><a href="javascript:void(0);" onclick="javascript:changeOrder('engine4_sitestore_albums.title', 'DESC');"><?php echo $this->translate("Album") ?></a></th>
            <th width="24%" align="left"><?php echo $this->translate("Store Name") ?></th>
            ...
            <?php $sitestorealbum_object = Engine_Api::_()->getItem('sitestore_album', $album->resource_id);?>
						...
						<td width="24%" class=""><?php echo $this->htmlLink($sitestorealbum_object->getHref(array('tab' => $tab_id)),$this->itemPhoto($sitestorealbum_object, 'thumb.normal'),array('title'=>$sitestorealbum_object->getTitle())); ?></td>
