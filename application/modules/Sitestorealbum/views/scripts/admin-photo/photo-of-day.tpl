...
<br />
<div class="tip"> <span> <?php echo $this->translate('You should only make those photos as "Photo of the Day" which have their store view privacy set as \'Everyone\' or \'All Registered Members\'.'); ?> </span> </div>
<br />
<div>
...
<div>
	<?php if ($this->photoOfDaysList->getTotalItemCount() > 0): ?>
	  ...
	  <form id='multidelete_form' method="post" action="<?php echo $this->url(array('module' => 'sitestorealbum', 'controller' => 'settings', 'action' => 'multi-delete-photo'), 'admin_default'); ?>" onSubmit="return multiDelete()">
		  <table class='admin_table' width="100%">
		    <thead>
		      <tr>
						<th style='width: 1%;' align="left"><input onclick="selectAll()" type='checkbox' class='checkbox'></th>
						<?php $class = ( $this->order == 'engine4_sitestore_photos.title' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
						<th width="18%" align="left" class="<?php echo $class ?>"><a href="javascript:void(0);" onclick="javascript:changeOrder('engine4_sitestore_photos.title', 'DESC');"><?php echo $this->translate("Photo") ?></a></th>
						<th width="18%" align="left"><?php echo $this->translate("Album") ?></th>
            <th width="18%" align="left"><?php echo $this->translate("Store Name") ?></th>
          ...
          <?php $sitestorephoto_object = Engine_Api::_()->getItem('sitestore_photo', $photo->resource_id);?>
		      <?php  $parent = Engine_Api::_()->getItem('sitestore_album', $photo->album_id);?>
		        ...
		        <td width="18%" class="sitealbum_table_img"><?php echo $this->htmlLink($sitestorephoto_object->getHref(),$this->itemPhoto($sitestorephoto_object, 'thumb.normal'),array('title'=>$sitestorephoto_object->getTitle())); ?></td>
