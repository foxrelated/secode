...
/* $Id: composer_photo.js 2013-09-02 00:00:00Z SocialEngineAddOns Copyright 2012-2013 BigStep Technologies Pvt. Ltd. $ */

Composer.Plugin.SitestorePhoto = new Class({
  ...
  activate : function() {
    ...
    this.elements.form = new Element('form', {
      'id' : 'compose-sitepagae-photo-form',
      'class' : 'compose-form',
      'method' : 'post',
      'action' : fullUrl,
    ...
    this.elements.formInput = new Element('input', {
      'id' : 'compose-sitepagae-photo-form-input',
    ...
    if( !DetectMobileQuick() && !DetectIpad() && this.options.fancyUploadEnabled && this.options.fancyUploadOptions ) {
      ...
      this.elements.formFancyFile = new Element('a', {
        ...
        'id' : 'compose-sitepagae-photo-form-fancy-file',
        'class' : 'buttonlink',
  ...
  doRequest : function() {
    this.elements.iframe = new IFrame({
      'name' : 'composeSitestorePhotoFrame',
      ...
      'events' : {
        'load' : function() {
          this.doProcessResponse(window._composeSitestorePhotoResponse);
          window._composeSitestorePhotoResponse = false;
        }.bind(this)
      }
    }).inject(this.elements.body);

    window._composeSItestorePhotoResponse = false;
    this.elements.form.set('target', 'composeSitestorePhotoFrame');
  ...
  doProcessResponse : function(responseJSON) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id' : 'compose-sitestorephoto-preview-image',
      'class' : 'compose-photo-preview',
