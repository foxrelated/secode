...
* @version    $Id: _birthdaylist.tpl 6590 2010-17-11 9:40:21Z SocialEngineAddOns $
* @author     SocialEngineAddOns
*/
?>
<?php
  $this->headLink()
  	->prependStylesheet($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/styles/style_infotooltip.css');
?>
<script type="text/javascript">
    ...
    function another_month(date_current) {
      ...
      var data_another_month = { 'date_current'  : date_current, 'format' : 'html'};
      en4.core.request.send(new Request.HTML({
	
        'url' : '<?php echo $this->url(array('module' => 'birthday', 'controller' => 'index', 'action' => 'index'), 'default', true) ?>',
...
<ul class="layout_birthday_widget" id="dyanamic_code">
  ...
  <?php if($this->display_action == 0) : ?>
		<li class="birthday_widget_name_list">
			<ul class="birthday-jq-checkpoints">
			 	<li>
					<?php $url = $this->url(array('module' => 'birthday', 'controller' => 'index', 'action' => 'view'), 'default', true); ?>
					<a href='<?php echo $url ?>' title="<?php echo $this->translate('View Birthdays') ?>">
						<img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Birthday/externals/images/wish.png" alt="" style="float:left;" />
					</a>	
				</li>
	      ...
	      <?php if($this->display_count < $this->display_entries) : ?>
            ...
            .$values->item_id ."'><a href=". $profile_url. ">". $this->user($user_subject)->getTitle().
						"</a></div>"; ?>
		      <?php
            $tooltip_string.= '<div class="intu_stats"><textarea rows="2" cols="10"
            id="activity-write-body-'. $values->item_id .'" onfocus="activitywriteonclick($(this), \'' .
            $this->translate("Write on " . $this->user($user_subject)->getTitle() . "s wall...") .'\', 1,
            '. $values->item_id . ')"  onblur="activitywriteonclick($(this),\''. $this->translate("Write on "
            . $this->user($user_subject)->getTitle() . "s wall...") . '\', 2, ' . $values->item_id .')"
            onkeyup="statusubmit(event, '. $values->item_id . ', \''.
            $this->translate($this->user($user_subject)->getTitle()) . '\')">' . $this->translate("Write on "
            . $this->user($user_subject)->getTitle() . "s wall...") . '</textarea></div>';
          ?>
					<?php //$tooltip_string.= "<a class='buttonlink icon_type_wish_birthday' href='". $profile_url.
					"'>". $this->translate('Wish'). "</a>\t"; ?>
  ...
  <?php if($this->display_action == 1) :?>
  	<li>
	      ...
	      <?php $tooltip_string = ""; ?>		
	      <?php if($this->display_count < $this->display_entries) : ?>
      		<li class="birthday_widget_userlist">
						<?php $user_subject = Engine_Api::_()->user()->getUser($values->item_id);
						$profile_url = $this->url(array('id' => $values->item_id), 'user_profile'); ?>
						<a href="<?php echo $profile_url ?>" class="birthday_userlist_thumb"> <?php echo $this->itemPhoto($this->user($user_subject), 'thumb.icon') ?></a>
            <!--Tooltip code start here-->
						<?php
							$tooltip_string.= "<div class='intu_list'><div class='intu_thumb'><a href=". $profile_url.
							">". $this->itemPhoto($this->user($user_subject), 'thumb.icon'). "</a></div>"  ?>
          ...
          ?>
    		...
    		<?php endforeach; ?>
		      ...
		      <?php 
						echo $this->htmlLink(
							array('route' => 'default', 'module' => 'birthday', 'controller' => 'index', 'action' => 'view'),
							$this->translate('More &raquo;')
						)
		      ?>
	      </li>
    	</ul>
  	...
  	<li>
	      ...
	      <?php if($this->display_count < $this->display_entries) : ?>
      		<li class="birthday_widget_userlist">
						...
						<a href="<?php echo $profile_url ?>" class="birthday_userlist_name"><?php echo $this->user($user_subject)->getTitle() ?></a>
            <!--Tooltip code start here-->
						<?php
							$tooltip_string.= "<div class='intu_list'><div class='intu_thumb'><a href=". $profile_url.
							">". $this->itemPhoto($this->user($user_subject), 'thumb.icon'). "</a></div>"  ?>
  ...
  <?php if($this->display_action == 3) : ?>
    ...
    <div id="calender" class="birthday_calendar">
      ...
      <table cellpadding="0" cellspacing="0">
      	<thead>
					<?php $day_start = Engine_Api::_()->getApi('settings', 'core')->getSetting('birthday.daystart', 1);?>
					<tr>
					  <?php if($day_start == 1) :?>
					    <td class='' title="<?php echo $this->translate('Sunday') ?>"><div class="day"><?php echo $this->translate("Su") ?></div></td>
					  <?php endif; ?>
					  <td class='' title="<?php echo $this->translate('Monday') ?>"><div class="day"><?php echo $this->translate("Mo"); ?></div></td>
					  <td class='' title="<?php echo $this->translate('Tuesday') ?>"><div class="day"><?php echo $this->translate("Tu") ?></div></td>
					  <td class='' title="<?php echo $this->translate('Wednesday') ?>"><div class="day"><?php echo $this->translate("We") ?></div></td>
					  <td class='' title="<?php echo $this->translate('Thursday') ?>"><div class="day"><?php echo $this->translate("Th") ?></div></td>
					  <td class='' title="<?php echo $this->translate('Friday') ?>"><div class="day"><?php echo $this->translate("Fr") ?></div></td>
					  <td class='' title="<?php echo $this->translate('Saturday') ?>"><div class="day"><?php echo $this->translate("Sa") ?></div></td>
					  <?php if($day_start == 0) :?>
					    <td class='' title="<?php echo $this->translate('Sunday') ?>"><div class="day"><?php echo $this->translate("Su") ?></div></td>
					  <?php endif; ?>
					</tr>
	  ...
	  <?php if(!empty($this->result)) : ?>
	    <td> 
	      <?php $date_entry = 0; 
		    $days_count = 1;
        ?>
	      <?php //if($this->current_month == 02 && ($date-1) == 28 && $february_flag == 0 ) { $february_flag = 1; } ?>
		    ...
		    <?php foreach($this->result as $values) : ?>
			  ...
			  <li class="blist_calendar">
			    <?php $entry_counter = $entry_counter+1; ?>
			      ...
			      <?php $user_subject = Engine_Api::_()->user()->getUser($values->item_id);
						$profile_url = $this->url(array('id' => $values->item_id), 'user_profile'); ?>
			      <?php $tooltip_string.= "<div class='intu_list'><div class='intu_thumb'><a href=". $profile_url. ">". $this->itemPhoto($this->user($user_subject), 'thumb.icon'). "</a></div>"  ?>
			    
			      <?php $tooltip_string.= "<div class='intu_body'><div class='intu_title'><a href=". $profile_url. ">". $this->user($user_subject)->getTitle(). "</a></div><div class='intu_stats'>"; ?>
			        <?php if($values->Day == date('d', time()) && $values->Month == date('m', time())) :?>
			      		 <?php $tooltip_string.= "<a class='buttonlink icon_type_wish_birthday' href='". $profile_url. "'>". $this->translate('Wish'). "</a>\t"; ?>
			      	<?php endif; ?>
			     		<?php $tooltip_string.= "<a class='icon_type_message_birthday buttonlink' href='". $this->sugg_baseUrl. "/messages/compose/to/". $values->item_id. "'>". $this->translate('Send Message'). "</a></div></div></div>"; ?>
		...
		<?php 
		  if($entry_counter != 0) {
		    echo $this->htmlLink(
			    array('route' => 'default', 'module' => 'birthday', 'controller' => 'index', 'action' => 'viewallcalender', 'year' => $this->current_year, 'month' => $this->current_month ,'date' => $date), 
