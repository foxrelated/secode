...

{
    ...
    public function _initynmediaimporter1343294423()
    {
        ...
        else
        {
            ...
            {
                $res = @mysql_query("INSERT IGNORE INTO `engine4_younetcore_license` (`name`, `title`, `descriptions`, `type`, `current_version`, `lasted_version`, `is_active`, `date_active`, `params`, `download_link`, `demo_link`) VALUES ('ynmediaimporter', 'Social Media Importer', '', 'module', '4.03', '4.03', '0', NULL, NULL, NULL, NULL);", $connection);
                $this -> e($module_name, 0);
            }
            else
            {
                $res = @mysql_query("Update `engine4_younetcore_license` set `lasted_version` = '4.03' , `current_version` = '4.03' where `name`='ynmediaimporter' ");
