 ...
 */
class Sitestorevideo_Plugin_Job_Encode extends Core_Plugin_Job_Abstract {
  ...
  protected function _execute() {
    ...
    if (!$video || !($video instanceof Sitestorevideo_Model_Video)) {
  ...
  protected function _process($video, $type, $compatibilityMode = false){
    ...
    if (!($video instanceof Sitestorevideo_Model_Video)) {
    ...
    if (!$success) {
      ...
      try {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ...
                ->addNotification($owner, $owner, $video, 'sitestorevideo_processed_failed', array(
                    'message' => $notificationMessage,
                    'message_link' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view'), 'video_general', true),
                ));
      ...
      throw new Sitestorevideo_Model_Exception($exceptionMessage);
    }
    ...
    // Success
      ...
      try {
        // new action
        $subject = Engine_Api::_()->getItem('sitestore_store', $video->store_id);
        $action = $actionsTable->addActivity($owner, $subject, 'sitestorevideo_new');
        if ($action) {
        ...
        // notify the owner
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'sitestorevideo_processed');
