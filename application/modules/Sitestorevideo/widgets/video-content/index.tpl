...
<div class="sitestorevideo_view">
        ...
        if (!empty ($enable_facebookse) && !empty($fbmodule->version)) :
          ...
          if (!empty($fbversion) && ($fbversion >= '4.1.5')) { ?>
             <div class="sitestorevideo_fb_like">
                <script type="text/javascript">
                    var fblike_moduletype = 'sitestorevideo_video';
  ...
  <?php if( $this->video->type == 3): ?>
      ...
      <video id="sitestorevideo" controls preload="auto" width="480" height="386">
    ...
    <?php if( $this->can_embed ): ?>
	  ...
	  <?php endif ?>
		...
		<?php if($this->allowView ): ?>    
			...
			'class' => 'buttonlink smoothbox item_icon_sitestorevideo_video'
		)) ?>
      &nbsp; | &nbsp;
		<?php endif;?>
...
<script type="text/javascript">
  function featured(video_id)
  {
    en4.core.request.send(new Request.HTML({
      method : 'post',
      'url' : en4.core.baseUrl + 'sitestore/index/featured',
      'data' : {
        format : 'html',
        'video_id' : video_id
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {       
       if($('featured_sitestorevideo').style.display=='none'){
        $('featured_sitestorevideo').style.display="";
        $('un_featured_sitestorevideo').style.display="none";
       }else{
          $('un_featured_sitestorevideo').style.display="";
        $('featured_sitestorevideo').style.display="none";
       }
      }
    }));
