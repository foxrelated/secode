...
/* $Id: composer_sitestorevideo.js 7244 2010-09-01 01:49:53Z john $ */


Composer.Plugin.SitestoreVideo = new Class({
  ...
  activate : function() {
    ...
    this.elements.formInput = new Element('select', {
      'id' : 'compose-sitestorevideo-form-type',
      'class' : 'compose-form-input',
      'option' : 'test',
      'events' : {
        'change' : this.updateSitestorevideoFields.bind(this)
      }
    }).inject(this.elements.body);
    var options = 0;
    $('compose-sitestorevideo-form-type').options[options++] = new Option(this._lang('Choose Source'), '0');
    if (this.options.youtubeEnabled == 1) {
      $('compose-sitestorevideo-form-type').options[options++] = new Option(this._lang('YouTube'), '1');
    }
    $('compose-sitestorevideo-form-type').options[options++] = new Option(this._lang('Vimeo'), '2');

//    if (this.options.allowed == 1){
//      $('compose-sitestorevideo-form-type').options[options++] = new Option(this._lang('My Computer'), '3');
//    }
    //link to full composer for now
    //$('compose-sitestorevideo-form-type').options[options++] = new Option('My Computer','3');

    this.elements.formInput = new Element('input', {
      'id' : 'compose-sitestorevideo-form-input',
      'class' : 'compose-form-input',
      'type' : 'text',
      'style': 'display:none;'
    }).inject(this.elements.body);
    ...
    if (this.options.allowed == 1 && this.options.type != 'message'){
      $('compose-sitestorevideo-form-type').options[options++] = new Option(this._lang('My Device'), '3');
    }
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitestorevideo-upload',
        'class' : 'compose-sitestorevideo-upload',
        'html' : this._lang('To upload a video from your device, please use our <a href="'+en4.core.baseUrl+'sitestorevideo/create/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    else{
    if (this.options.allowed == 1 && this.options.type != 'message'){
      ...
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-sitestorevideo-upload',
        'class' : 'compose-sitestorevideo-upload',
        'html' : this._lang('To upload a video from your computer, please use our <a href="'+en4.core.baseUrl+'sitestorevideo/create/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    ...
    this.elements.formSubmit = new Element('button', {
      'id' : 'compose-sitestorevideo-form-submit',
  ...
  deactivate : function() {
    ...
    if (this.params.video_id)
      new Request.JSON({
        url: en4.core.basePath + 'sitestorevideo/index/delete',
        data: {
          format: 'json',
  ...
  doAttach : function(e) {
    ...
    var sitestorevideo_element = document.getElementById("compose-sitestorevideo-form-type");
    var type = sitestorevideo_element.value;
  ...
  doProcessResponse : function(responseJSON, responseText) {
    ...
    this.elements.preview = Asset.image(responseJSON.src, {
      'id' : 'compose-sitestorevideo-preview-image',
  ...
  doImageLoaded : function() {
    ...
    this.elements.previewInfo = new Element('div', {
      'id' : 'compose-sitestorevideo-preview-info',
      'class' : 'compose-preview-info'
    }).inject(this.elements.body);

    this.elements.previewTitle = new Element('div', {
      'id' : 'compose-sitestorevideo-preview-title',
    ...
    this.elements.previewDescription = new Element('div', {
      'id' : 'compose-sitestorevideo-preview-description',
  ...
  updateSitestorevideoFields : function(element) {
    var sitestorevideo_element = document.getElementById("compose-sitestorevideo-form-type");
    var url_element = document.getElementById("compose-sitestorevideo-form-input");
    var post_element = document.getElementById("compose-sitestorevideo-form-submit");
    var upload_element = document.getElementById("compose-sitestorevideo-upload");
    // clear url if input field on change
    $('compose-sitestorevideo-form-input').value = "";

    // If sitestorevideo source is empty
    if (sitestorevideo_element.value == 0)
    {
    ...
    // If sitestorevideo source is youtube or vimeo
    if (sitestorevideo_element.value == 1 || sitestorevideo_element.value == 2)
    {
      upload_element.style.display = "none";
    ...
    // if sitestorevideo source is upload
    if (sitestorevideo_element.value == 3)
    {
