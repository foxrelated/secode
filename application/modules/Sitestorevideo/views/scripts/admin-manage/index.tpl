...
<h3>
  <?php echo $this->translate('Manage Store Videos'); ?>
</h3>

<p>
  <?php echo $this->translate('Here, you can see all the videos your users have created for the Stores on your site. Here you can monitor these videos and delete offensive ones if necessary. You can also make store videos featured / un-featured, highlighted / un-highlighted by clicking on the corresponding icons. 
...
<div class='admin_members_results'>
  ...
  ?>
		<div class="">
			<?php  echo $this->translate(array('%s store video found.', '%s store videos found.', $counter), $this->locale()->toNumber($counter)) ?>
		</div>
  <?php else:?>
		<div class="tip"><span>
...
<script type="text/javascript">
  ...
  function killProcess(video_id) {
    (new Request.JSON({
      'format': 'json',
      'url' : '<?php echo $this->url(array('module' => 'sitestorevideo', 'controller' => 'admin-manage', 'action' => 'kill'), 'default', true) ?>',
      'data' : {
        'format' : 'json',
        'video_id' : video_id
      },
    ...
    <thead>
      <tr>
        ...
        <th style='width: 1%;'><a href="javascript:void(0);" onclick="javascript:changeOrder('username', 'ASC');"><?php echo $this->translate('Owner');?></a></th>
				<th style='width: 1%;'><a href="javascript:void(0);" onclick="javascript:changeOrder('sitestore_title', 'ASC');"><?php echo $this->translate('Store Title');?></a></th>
          ...
          <tr>
            ...
            <td class='admin_table_bold'>
              <?php if(!empty($item->store_id)):?>
								<?php echo $this->htmlLink($this->item('sitestore_store', $item->store_id)->getHref(),$item_sitestoretitle, array('title' => $item->sitestore_title, 'target' => '_blank')) ?>
							<?php endif;?>
            </td>
           <?php if($item->highlighted == 1):?>
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'sitestorevideo_highlightedvideo', 'id' => $item->video_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitestore/externals/images/icons/highlighted.png', '', array('title'=> $this->translate('Highlighted')))) ?> 
								</td>       
							<?php else: ?>  
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'sitestorevideo_highlightedvideo', 'id' => $item->video_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sitestore/externals/images/icons/unhighlighted.png', '', array('title'=> $this->translate('Un-highlighted')))) ?>
								</td>
							<?php endif; ?>
           <?php if($item->featured == 1):?>
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'sitestorevideo_featuredvideo', 'id' => $item->video_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/images/featured.png', '', array('title'=> $this->translate('Featured')))) ?> 
								</td>       
							<?php else: ?>  
								<td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'sitestorevideo_featuredvideo', 'id' => $item->video_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/images/unfeatured.png', '', array('title'=> $this->translate('Un-featured')))) ?>
								</td>
							<?php endif; ?>
            ...
            <td class='admin_table_options'>
						...
						|
						<?php echo $this->htmlLink(array('route' => 'sitestorevideoadmin_delete', 'video_id' => $item->video_id,'store_id' => $item->store_id), $this->translate('delete'), array(
							'class' => 'smoothbox',
						)) ?> 
            </td>
          </tr>
