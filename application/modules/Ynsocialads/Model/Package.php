...
class Ynsocialads_Model_Package extends Core_Model_Item_Abstract
{
	...
	public function getAllModuleNames()
	{
		...
		$select = $modulesTbl -> select() -> from($tblName);
		if (count($this -> modules))
		{
			...
			{
			    if (Engine_Api::_()->hasModuleBootstrap($value->module_name))
				    $moduleNameArr[] = $view->translate($value->module_title);
			}
	...
	public function getAllModules()
	{
		...
		{
		    $result = array();
			$data = $modulesTbl -> fetchAll($select -> where('module_name IN (?)', $this -> modules));
			foreach ($data as $row) {
	...
	public function getPageName($page_id)
	{
		...
		if ($page)
		{
			return $view->translate($page -> displayname);
		}
		elseif (Engine_Api::_() -> hasModuleBootstrap('ynsocialadspage'))
		{
			...
			if ($page)
			{
				return $view->translate($page -> title);
			}
			else
			{
				return "";
