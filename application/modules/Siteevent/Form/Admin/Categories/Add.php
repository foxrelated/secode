 ...
 */
class Siteevent_Form_Admin_Categories_Add extends Engine_Form {
    ...
    public function init() {
        ...
        $this->addElement('Text', 'category_slug', array(
            'label' => 'URL Component',
            'description' => "This will be the end of the URL of your event browse page, for example: $link/events/categoryid/URL-COMPONENT"
        ));
        ...
        if (empty($isThirdLevelCat)) {
            ...
            if (!empty($category_id) && $perform == 'edit') {
                ...
                if ($category->photo_id && !empty($getCategoryPhoto)) {
                    ...
                    $description = "<img src='$photoName' class='sr_siteevent_categories_banner_img' />";
        ...
        if (!empty($category_id) && $perform == 'edit') {
            ...
            if ($category->file_id) {
                ...
                $description = "<img src='$photoName' class='siteevent_categories_icon' />";
        ...
        if (!empty($category_id) && $perform == 'edit') {
            ...
            if ($category->banner_id) {
                ...
                $description = "<img src='$photoName' class='siteevent_categories_banner_img' />";
        ...
        $this->addElement('Textarea', 'top_content', array(
            'label' => 'Top Content',
            'description' => 'Please enter the content to be shown below the category name and above the events on the category page. (Useful for SEO. For ex: You can enter the category description here.)'
        ));

        $this->addElement('Textarea', 'bottom_content', array(
            'label' => 'Bottom Content',
            'description' => 'Please enter the content to be shown below the events on the category page. (Useful for SEO.)'
        ));
        ...
        if(empty($category->cat_dependency)) {
					$this->addElement('Checkbox', 'allow_guestreview', array(
							'label' => 'Do you want to allow members of your site to review and rate guests of events belonging to this category?',
							'Description' => 'Allow Reviews on Guests',
							'value' => 0,
					));
        }
