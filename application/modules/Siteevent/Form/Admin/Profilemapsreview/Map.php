 ...
 */
class Siteevent_Form_Admin_Profilemapsreview_Map extends Engine_Form {
    ...
    public function init() {
        ...
        $this->setMethod('post')
                ...
                ->setDescription("After selecting a profile type, if you click on 'Save', then the already created reviews for events of this category will also be associated with this profile type.");
        ...
        if (!empty($getCountChieldMapping)) {
            ...
            if ($category->cat_dependency == 0 && $category->sucat_dependency == 0) {
                $description = Zend_Registry::get('Zend_Translate')->_("<div class='tip'><span>Note: It seems that sub-categories of this category have been already mapped. Thus, if you add this mapping, then earlier reviews for events associated with the mapped sub-categories will loose their custom profile data.</span></div>");
            } else {
                $description = Zend_Registry::get('Zend_Translate')->_("<div class='tip'><span>Note: It seems that 3rd level categories of this sub-category have been already mapped. Thus, if you add this mapping, then earlier reviews for events associated with the mapped 3rd level categories will loose their custom profile data.</span></div>");
            }
