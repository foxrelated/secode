 ...
 * @version    $Id: _composeSiteeventVideo.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
?>

<?php if (Engine_Api::_()->core()->hasSubject() && in_array($this->subject()->getType(), array('siteevent_event', 'siteeventevent_event'))): ?>
    <style type="text/css">
        /*
    ...
    if (in_array($subject->getType(), array('siteeventevent_event'))):
        $subject = Engine_Api::_()->getItem('siteevent_event', $subject->event_id);
    endif;
    if (!Engine_Api::_()->siteevent()->isLeader($subject, 'svcreate')):
        return;
    endif;
    ?>
    <?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Siteevent/externals/scripts/composer_video.js') ?>
    ...
    $allowed_upload = 1;
    $ffmpeg_path = (bool) Engine_Api::_()->getApi('settings', 'core')->siteevent_video_ffmpeg_path;
        ...
        en4.core.runonce.add(function() {
            ...
            composeInstance.addPlugin(new Composer.Plugin.Siteevent({
                ...
                lang: {
                    ...
                    'To upload a video from your computer, please use our full uploader.': '<?php echo addslashes($this->translate('To upload a video from your computer, please use our <a href="%1$s">full uploader</a>.', $this->url(array('action' => 'create', 'event_id' => $subject->getIdentity(), 'type' => 3), 'siteevent_video_general'))) ?>'
                },
                allowed: <?php echo $allowed; ?>,
                youtubeEnabled: <?php echo (int) $youtubeEnabled?>,
                type: type,
                requestOptions: {
                    'url': en4.core.baseUrl + 'siteevent/video/compose-upload/format/json/c_type/' + type
                }
            }));
        });
    </script>
