 ...
 * @version    $Id: list.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
            ...
            <li>
                ...
                <div class="comments_viewall">
                    ...
                    echo $this->htmlLink('javascript:void(0);', $this->translate('View previous comments'), array(
                        'onclick' => 'en4.srpinboard.comments.loadComments("' . $this->subject->getType() . '",
...
"' . $this->subject->getIdentity() . '", "' . ($this->comments->getCurrentPageNumber()) . '","' . $this->widget_id . '")'
        ...
        for (; $i != $e; $i += $d):
            ...
            <li class="comment-<?php echo $comment->comment_id ?>" id="comment-<?php echo $comment->comment_id ?>">
                <?php if ($canDelete): ?>
                    <a href="javascript:void(0);" onclick="en4.srpinboard.comments.deleteComment('<?php echo $this->subject->getType() ?>', '<?php echo $this->subject->getIdentity() ?>', '<?php
                    echo
                    $comment->comment_id
                    ?>', '<?php
                       echo
                       $this->widget_id
                       ?>')" class="siteevent_comment_remove" title="<?php echo $this->translate('delete') ?>">
                    </a>
                    <?php endif; ?>
                    ...
                    <div class="comments_date">
                        <?php if ($this->canComment || $comment->likes()->getLikeCount() > 0): ?>
                            ...
                            if ($this->canComment):
                                ...
                                ?>
                                <?php if (!$isLiked): ?>
                                    <a href="javascript:void(0)" onclick="en4.srpinboard.comments.like(<?php
                                       echo
                                ...
                                <?php else: ?>
                                    <a href="javascript:void(0)" onclick="en4.srpinboard.comments.unlike(<?php
                                       echo
                                       sprintf("'%s', %d, %d, %d ", $this->subject->getType(), $this->subject->getIdentity(), $this->widget_id, $comment->getIdentity())
                                       ?>)">
                            ...
                            <?php if ($comment->likes()->getLikeCount() > 0): ?>
                                ...
                                <a href="javascript:void(0);" id="comments_comment_likes_<?php echo $comment->comment_id ?>"
                                   class="comments_comment_likes">
                                       <?php
                                       echo $this->translate(array('%s likes this', '%s like this',
...
"' . $this->subject->getIdentity() . '", "' . ($this->page + 1) . '","' . $this->widget_id . '")'
        ...
        <?php if (isset($this->form)): ?>
            <li id='comment-form-open-li_<?php echo $this->element_id ?>' onclick="en4.srpinboard.comments.addComment('<?php echo $this->element_id ?>');"> 
                <div/></div>
    ...
    <?php endif; ?>
        <li id='comment-form-loading-li_<?php echo $this->element_id ?>' style="display: none;"> 
            <div class="comments_author_photo">
                <?php
                echo $this->htmlLink($this->viewer()->getHref(), $this->itemPhoto($this->viewer(), 'thumb.icon', $this->viewer()->getTitle())
                )
                ?>
            </div>
            <div class="comments_info">
    ...
    <script type="text/javascript">
        en4.core.runonce.add(function() {
            ...
            en4.srpinboard.comments.attachCreateComment($('comment-form_<?php echo $this->element_id ?>'), '<?php echo $this->subject->getType()?>',<?php echo $this->subject->getIdentity() ?>, '<?php echo $this->widget_id ?>');

            <?php if ($this->submit_post): ?>
                $('comment-form_<?php echo $this->element_id ?>').style.display = '';
                $('comment-form_<?php echo $this->subject->getGuid()?>').body.focus()
            <?php endif; ?>

        });
    </script>

    <?php if (isset($this->form)) echo $this->form->setAttribs(array('id' => 'comment-form_' . $this->element_id, 'style' => 'display:none;'))->render()
    ?>
