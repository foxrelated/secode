 ...
 * @version    $Id: form-search.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
...
<p>
    <?php echo $this->translate('This page lists all the fields which will be displayed to the users in the search form widget at "Events Home", "Browse Events" and "My Events". Below, you can set the sequence of items in the order in which they should appear to users in the search form. To do so, drag-and-drop the items vertically and click on "Save Order" to save the sequence. You can also hide/display the individual fields in this search form.') ?>
</p>
                    ...
                    <li>
                        ...
                        <div style="width:45%;" class="admin_table_centered">
                            <?php if ($item->display == 1): ?>
                                <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'siteevent', 'controller' => 'admin-settings', 'action' => 'diplay-form', 'id' => $item->searchformsetting_id, 'name' => $item->name, 'display' => 0,), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/images/approved.gif', '', array('title' => $this->translate('Hide')))); ?>
                            <?php else: ?>
                                <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'siteevent', 'controller' => 'admin-settings', 'action' => 'diplay-form', 'id' => $item->searchformsetting_id, 'name' => $item->name, 'display' => 1,), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/images/disapproved.gif', '', array('title' => $this->translate('Display')))); ?>
                            <?php endif; ?>
                        </div>
                    </li>
                <?php endforeach; ?>
