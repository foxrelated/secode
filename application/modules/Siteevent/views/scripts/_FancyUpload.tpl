 ...
 * @version    $Id: _FancyUpload.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
...
$siteevent = Engine_Api::_()->getItem('siteevent_event', Zend_Controller_Front::getInstance()->getRequest()->getParam('event_id', null));
    ...
    if (Engine_Api::_()->siteevent()->hasPackageEnable()) {
      $photoCount = Engine_Api::_()->getDbTable('packages', 'siteeventpaid')->getPackageOption($siteevent->package_id, 'photo_count');
    }else {
      $photoCount = 0;
    }
...
// url is read from the form, so you just have to change one place
             ...
             *
             * This example creates a list of error elements directly in the file siteevent, which
             * hide on click.
             */
            ...
            onFileRemove: function(file) {
                ...
                request = new Request.JSON({
                    'format': 'json',
                    'url': '<?php echo $this->url(Array('module' => 'siteevent', 'controller' => 'photo', 'action' => 'remove'), 'default') ?>',
                    'data': {
                        'photo_id': file_id,
                        'is_ajax': 1,
                    },
                ...
                if (uploadCount == 0)
                {
                    ...
                    var demositeevent = document.getElementById("demo-list");
                    var demosubmit = document.getElementById("submit-wrapper");
                    democlear.style.display = "none";
                    demositeevent.style.display = "none";
...
<div id="demo-status" class="hide">
    <div>
        <?php echo $this->translate("Click 'Add Photos' to select one or more photos from your computer. After you have selected the photos, they will begin to upload right away. When your upload is finished, click the button below your photo to save them to your event."); ?>
    </div>
    <?php if(Engine_Api::_()->siteevent()->hasPackageEnable() && $photoCount):?>
    <div class="tip">
      <span>
        <?php echo $this->translate("You are allowed to upload maximum %s photos.", $photoCount);?>
      </span>
    </div>
  <?php endif;?>
    <div>
