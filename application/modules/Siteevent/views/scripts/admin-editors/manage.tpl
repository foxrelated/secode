...
<script type="text/javascript">
    ...
    {
        return confirm('<?php echo $this->string()->escapeJavascript($this->translate("Are you sure you want to remove selected Editors in which this user has been added as editor?")) ?>');
    }
        ...
        <form id='multidelete_form' method="post" action="<?php echo $this->url(array('action' => 'multi-delete')); ?>" onSubmit="return multiDelete()">
            <div>
                ...
                <p class="form-description">
                    <?php echo $this->translate('Editor reviews are helpful in displaying accurate, trusted and unbiased reviews that will showcase events\' (for example: events of hotels, events, etc.) quality, features, and value. This will bring more user engagement to your site, as editor reviews provide reviews from expert people (editors) on the events of their interest.<br />Below, you can add new editor by using "Add New Editor" link. You can also edit and remove editors added by you by clicking on the links for each. You can also make an editor as Super Editor, who will be assigned all the reviews if other editors delete their user accounts from your site.<br /><br /><b>Badge:</b> You can assign badge to the editors. You can add / manage editor badges by clicking on "Manage Editor Badges" link.<br /><br /><b>Note:</b> You can not remove Super Editor from editor of your site. To do so, you have to first make some other editor as Super Editor. You can disable editor reviews by using "Allow Reviews" field from the "Reviews & Ratings" section of this plugin.') ?>
                </p>            

                <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'editors', 'action' => 'create'), $this->translate('Add New Editor'), array('class' => 'buttonlink seaocore_icon_add')) ?> <t/><t/>

                <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'badge', 'action' => 'manage'), $this->translate('Manage Editor Badges'), array('class' => 'buttonlink', 'style' => 'background-image: url('.$this->layout()->staticBaseUrl.'application/modules/Siteevent/externals/images/badge.png);')) ?> <br /><br />        

                <?php if (Count($this->paginator) > 0): ?>

                    <table class='admin_table siteevent_editor_list' width="100%">
                        <thead>
                            <tr>
              <!--								<th style='width: 1%;' class='admin_table_short'><input onclick="selectAll()" type='checkbox' class='checkbox'></th>-->
                                ...
                                <?php $class = ( $this->order == 'engine4_siteevent_editors.designation' ? 'admin_table_ordering admin_table_direction_' . strtolower($this->order_direction) : '' ) ?>
                        ...
                        <tbody>
                            <?php foreach ($this->paginator as $editor): $i = 0;
                                $i++;
                                $id = 'admin_file_' . $i; ?>
                                <?php $username = $editor->username ? $editor->username : $editor->displayname; ?>
                                <tr>
                                    <td class='admin_table_user'><?php echo $this->htmlLink(array('route' => 'siteevent_review_editor_profile', 'username' => $username, 'user_id' => $editor->user_id), $this->itemPhoto($editor, 'thumb.icon'), array('target' => '_blank', 'title' => $editor->getTitle())) ?></td>					

                                    <td class='admin_table_user'><?php echo $this->htmlLink(array('route' => 'siteevent_review_editor_profile', 'username' => $username, 'user_id' => $editor->user_id), $editor->getTitle(), array('target' => '_blank')) ?></td>	
                                    ...
                                    $params['type'] = 'editor';
                                    ?>  
                                    <td class='admin_table_centered'><?php echo Engine_Api::_()->getDbtable('reviews', 'siteevent')->totalReviews($params); ?></td>  

                                    <td>
                                        <?php if (!empty($editor->badge_id)): ?>
                                            <?php $badge = Engine_Api::_()->getItem('siteevent_badge', $editor->badge_id); ?>
                                                ...
                                                <?php if (!empty($main_path)): ?>
                                                    <div class="admin_file admin_file_type_image" id="<?php echo $id ?>">
                                                        <div class="slideshow-image-preview">
                                                            ...
                                                            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'badge', 'action' => 'assign-badge', 'editor_id' => $editor->editor_id), '<img src="' . $main_path . '" class="photo" width="50" />', array('class' => 'smoothbox', 'title' => 'Click to change or remove badge')) ?>
                                            ...
                                            <?php endif; ?>    
                                        <?php else: ?>
                                            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'badge', 'action' => 'assign-badge', 'editor_id' => $editor->editor_id), $this->translate('Assign'), array('class' => 'smoothbox', 'title' => 'Click to assign badge')) ?>
                                        <?php endif; ?>
                                    </td>         
                                    ...
                                    <?php else: ?>
                                        <td align="center" class="admin_table_centered"> <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'editors', 'action' => 'super-editor', 'editor_id' => $editor->editor_id, 'super_editor' => 0), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Seaocore/externals/images/disapproved.gif', '', array('title' => $this->translate('Make Super Editor'))), array('class' => 'smoothbox')) ?></td>
                                    <?php endif; ?>                  

                                    <td align="left">

                                        <?php echo $this->htmlLink(array('route' => 'siteevent_review_editor_profile', 'username' => $username, 'user_id' => $editor->user_id), $this->translate("Profile"), array('target' => '_blank')) ?> | 

                                        <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'editors', 'action' => 'edit', 'editor_id' => $editor->editor_id), $this->translate('Edit'), array('class' => 'smoothbox')) ?> | 

                                        <?php if ($editor->super_editor == 1): ?>  
                                            <span><?php echo $this->translate('Remove'); ?></span>
                                        <?php else: ?>                
                                            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'siteevent', 'controller' => 'editors', 'action' => 'delete', 'editor_id' => $editor->editor_id), $this->translate('Remove'), array('class' => 'smoothbox',)) ?>
                                        <?php endif; ?>                    
