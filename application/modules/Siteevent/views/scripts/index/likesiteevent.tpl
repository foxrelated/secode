 ...
 * @version    $Id: likesiteevent.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
...
if (!empty($this->user_obj)) {
    $paginater_vari = $this->user_obj->getCurrentPageNumber();
}
?>

<script type="text/javascript">

    var likeMemberPage = <?php
        if (empty($this->no_result_msg)) {
            echo sprintf('%d', $paginater_vari);
        } else {
            echo 1;
        }
    ?>;
    ...
    var url = en4.core.baseUrl + 'siteevent/index/likesiteevent';// URL where send ajax request.
    function show_myfriend() {
        $('siteevents_popup_content').innerHTML = '<center><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/siteevent/externals/images/loading.gif" /></center>';
        en4.core.request.send(new Request.HTML({
            ...
            onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
                $('siteevents_popup_content').innerHTML = responseHTML;
            }
        }));
    }
    ...
    en4.core.runonce.add(function() {
        ...
        $('like_members_search_input').addEvent('keyup', function(e) {
            $('siteevents_popup_content').innerHTML = '<center><img src="<?php echo $this->layout()->staticBaseUrl ?>application/modules/Siteevent/externals/images/loading.gif" alt="" class="mtop10" /></center>';
            en4.core.request.send(new Request.HTML({
                'url': url,
                'data': {
                    'format': 'html',
            ...
            <div class="seaocore_members_search_box">
                <div class="link">
                    <a href="javascript:void(0);" class="<?php
                       ...
                       ?>" id="show_all" onclick="likedStatus('public');"><?php echo $this->translate('All'); ?>(<?php echo number_format($this->public_count); ?>)</a>
                    <a href="javascript:void(0);" class="<?php
                           if ($this->call_status == 'friend') {
                               echo 'selected';
                           }
                           ?>" onclick="likedStatus('friend');"><?php echo $this->translate('Friends'); ?>(<?php echo number_format($this->friend_count); ?>)</a>
                </div>

                <div class="seaocore_members_search fright">
                    <input id="like_members_search_input" type="text" placeholder="<?php echo $this->translate('Search Friends');?>" value="<?php echo $this->search; ?>"/>
                </div>

                <div class="clear"></div>
            </div>
        </div>
        <div class="seaocore_members_popup_content" id="siteevents_popup_content">
                <?php } ?>
