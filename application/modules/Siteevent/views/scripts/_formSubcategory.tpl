 ...
 * @version    $Id: _formSubcategory.tpl 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
...
$siteeventGetCategoryType = Zend_Registry::isRegistered('siteeventGetCategoryType') ? Zend_Registry::get('siteeventGetCategoryType') : null;
$profile_type = 0;
if (empty($siteeventGetCategoryType))
    return;

if (!empty($event_id)) {
    $event = Engine_Api::_()->getItem('siteevent_event', $event_id);
    $profile_type = Engine_Api::_()->getDbTable('categories', 'siteevent')->getProfileType(null, $event->category_id, 'profile_type');
}
...
<script type="text/javascript">
    ...
    var actionType = '<?php echo $event_id; ?>';
    ...
    var subcatname = '';
    var show_subcat = 1;
<?php if (!empty($this->siteevent->category_id)) : ?>
        show_subcat = 0;
<?php endif; ?>

    var subcategories = function(category_id, sub, subcatname)
    {
var aQuick = document.getElementById('siteevents_create_quick');
