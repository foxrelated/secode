  ...
  <ul> 
    <?php if ($this->checkPage == "eventProfile") : ?>
      ...
      <?php if (!empty($this->can_reply) && $review->owner_id) : ?>
        <li> 
          <?php echo $this->htmlLink($review->getHref() . "#comments-form_$reviewGuid", $this->translate('<b>Comment on this review</b>'), array('title' => $this->translate("Comment on this review"), 'class' => 'reply icon_siteevent_comment')) ?>
        </li> 
        <li>|</li>  
      <?php endif; ?>
      <?php
  ...
  function reviewHelpful(option, review_id) {
    ...
    //  $.mobile.showPageLoadingMsg();
      $.ajax({
          type: "GET", 
          dataType: "JSON",
          url: sm4.core.baseUrl + 'siteevent/review/helpful',
          data : {
            format : 'json',
            review_id : review_id,
            helpful: option
