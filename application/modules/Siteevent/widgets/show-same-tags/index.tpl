...
<ul class="seaocore_sidebar_list">
    <?php foreach ($this->paginator as $siteevent_video): ?>
        ...
        $tmpBody = strip_tags($siteevent_video->getParent()->getTitle());
        $event_title = ( Engine_String::strlen($tmpBody) > $truncation_limit ? Engine_String::substr($tmpBody, 0, $truncation_limit) . '..' : $tmpBody );
        ?>
        <?php echo $this->translate("in ") . $this->htmlLink($siteevent_video->getParent()->getHref(), $event_title, array('title' => $siteevent_video->getParent()->getTitle())) ?>    
        </div>
