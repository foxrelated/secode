...
<?php if ($this->loaded_by_ajax): ?>
    <script type="text/javascript">
        var params = {
            ...
            responseContainer: $$('.layout_siteevent_searchbox_siteevent')
        }
...
<div class="siteevent_form_quick_search">
    <?php echo $this->form->setAttrib('class', 'siteevent-search-box')->render($this) ?>
</div>	
<?php endif; ?>
...
<script type="text/javascript">
    ...
    var doSearching = function(searchboxcategory_id) {
        ...
        if (searchboxcategory_id != 0) {
            ...
            var categoriesArray = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'siteevent')->getCategoriesDetails($this->categoriesLevel)); ?>;
    ...
    en4.core.runonce.add(function()
    {
        var item_count = 0;
        if($('titleAjax')) {
        var contentAutocomplete = new Autocompleter.Request.JSON('titleAjax', '<?php echo $this->url(array('action' => 'get-search-events'), "siteevent_general", true) ?>', {
            'postVar': 'text',
            'minLength': 1,
            'selectMode': 'pick',
            ...
            'injectChoice': function(token) {
                if (typeof token.label != 'undefined') {
                    if (token.siteevent_url != 'seeMoreLink') {
                        var choice = new Element('li', {'class': 'autocompleter-choices', 'html': token.photo, 'id': token.label, 'siteevent_url': token.siteevent_url, onclick: 'javascript:getPageResults("' + token.siteevent_url + '")'});
