...
<?php if ($this->showContent): ?>
  <?php if($this->addEditorReview): ?>
    <script type="text/javascript">
      var editorPageAction = function(page){
        ...
        var url = sm4.core.baseUrl + 'core/widget/index/mod/siteevent/name/editor-reviews-siteevent';
      ...
      <?php if($this->addEditorReview): ?>
         ...
         <section class="sm-widget-block" id="review_content">
                      ...
                      <strong><?php echo $this->translate("Editor Rating"); ?></strong>
										...
										<td>
											<?php echo $this->showRatingStarSiteeventSM($reviewCat['rating'], 'editor', 'big-star', $this->siteevent->event_id); ?>
										</td>
									</tr>
									<?php break; ?>
								<?php endif; ?>
							...
							<?php foreach($ratingData as $reviewCat):?>
								<?php if (!empty($reviewCat['ratingparam_name'])): ?>
									<tr valign="middle">
										...
										<td>
											<?php echo $this->showRatingStarSiteeventSM($reviewCat['rating'], 'editor', 'small-box', $this->siteevent->event_id); ?>
										</td>
									</tr>
									<?php continue; ?>
								<?php endif; ?>
                ...
                <tr>
                  <td colspan="2">
                    ...
                    <?php elseif($this->min_price > 0 && $this->max_price > 0): ?>
                      <?php echo $this->translate("%s to %1s", "<span style='font-size:18px;'>" . $this->locale()->toCurrency($this->min_price, Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD')) . "</span>", $this->locale()->toCurrency($this->max_price, Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD'))); ?>
                    <?php endif; ?>
                  </td>
                </tr>
                <tr>
  ...
  <?php if( empty($this->isAjax) && $this->showComments):?>
			<?php echo $this->content()->renderWidget("sitemobile.comments", array('type' => $this->siteevent->getType(), 'id' => $this->siteevent->getIdentity())); ?>
  <?php endif;?>
<?php endif; ?>