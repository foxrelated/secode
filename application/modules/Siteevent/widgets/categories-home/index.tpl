...
<ul class="siteevent_categories_box">
    <li class="b_light">
        ...
        ?>
            <?php for ($i = 0; $i <= $this->totalCategories; $i++) { ?>
                ...
                <div class="b_medium clr o_hidden">
                    <?php endif; ?>
                <div class="siteevent_categories_list_col">
                    <?php $ceil_count++; ?>
                    <?php
                    $category = "";
                    ...
                    ?>
                    <div class="siteevent_categories_list">
                            <?php $total_subcat = count($category['sub_categories']); ?>
                        <h6>
                        <?php echo $this->htmlLink($this->url(array('category_id' => $category['category_id'], 'categoryname' => Engine_Api::_()->getItem('siteevent_category', $category['category_id'])->getCategorySlug()), $categoryRouteName), $this->translate($category['category_name'])) ?><?php if (isset($category['count'])): ?> (<?php echo $category['count'] ?>) <?php endif; ?>
                        </h6>

                        <?php if (!empty($this->show2ndlevelCategory)): ?>
                            <div class="sub_cat clr" id="subcat_<?php echo $category['category_id'] ?>">
                                <?php $subCatCount = 0;
                                foreach ($category['sub_categories'] as $subcategory) : $subCatCount++;
                                    ?>
                                    ...
                                    <?php if ($subCatCount > 4): ?> 
                                        <div style="display:none;" class="clr o_hidden sub_cat_list categories_more_subcats_<?php echo $category['category_id']; ?>">
                                            ...
                                            <?php endif; ?>
                                            <?php echo $this->htmlLink($this->url(array('category_id' => $category['category_id'], 'categoryname' => Engine_Api::_()->getItem('siteevent_category', $category['category_id'])->getCategorySlug(), 'subcategory_id' => $subcategory['sub_cat_id'], 'subcategoryname' => Engine_Api::_()->getItem('siteevent_category', $subcategory['sub_cat_id'])->getCategorySlug()), "siteevent_general_subcategory"), $this->translate($subcategoryname)) ?> 								 <?php
    ...
    <?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Siteevent/externals/images/gray_bullet_bottom.png', '', array('title' => $this->translate('More'))) ?>
                                        </a>
                                    </div>                      
                                    ...
                                    <?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Siteevent/externals/images/gray_bullet_top.png', '', array('title' => $this->translate('Fewer'))) ?>
                                        </a>
                                    </div>  
                                <?php endif; ?>                       
                            <?php endforeach; ?>
...
<?php if (empty($this->showCount)): ?>
    <div class="siteevent_categories_list_link fright">
        [ <?php echo $this->htmlLink(array('route' => 'siteevent_review_categories', 'showCount' => 1), $this->translate('See item counts')); ?> ]
    </div> 
<?php endif; ?>