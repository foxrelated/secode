...
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Siteevent/externals/styles/style_siteevent_video.css')
?>
...
<script type="text/javascript">
    var event_id = <?php echo $this->video->event_id; ?>;
    ...
    function rate(rating) {
        ...
        (new Request.JSON({
            'format': 'json',
            'url': '<?php echo $this->url(array('module' => 'siteevent', 'controller' => 'video', 'action' => 'rate'), 'default', true) ?>',
            'data': {
                'format': 'json',
                'rating': rating,
                'video_id': video_id,
...
<div class="siteevent_video_view">
    <h3>
        <?php echo $this->video->title; ?>
    </h3>

    <div class="siteevent_video_date seaocore_txt_light">
    ...
    <?php if ($this->video->type == 3): ?>
        <div id="video_embed" class="siteevent_video_embed">
    ...
    <?php else: ?>
        <div class="siteevent_video_embed">
...
<?php echo $this->translate(array('%s view', '%s views', $this->video->view_count), $this->locale()->toNumber($this->video->view_count)) ?>
        ...
        <?php if (count($this->videoTags)): ?>
            ...
            <?php foreach ($this->videoTags as $tag): ?>
                <?php if (!empty($tag->getTag()->text)): ?>
                    <a href="<?php echo $this->url(array('tag' => $tag->getTag()->tag_id), 'siteevent_video_general', true) ?>"><?php endif; ?>#<?php echo $tag->getTag()->text ?></a>&nbsp;
            <?php endforeach; ?>
        <?php endif; ?>
    </div>
    ...
    <div class='siteevent_video_view_options'>
        <?php if ($this->can_create): ?>
            <?php echo $this->htmlLink(array('route' => "siteevent_video_create", 'event_id' => $this->siteevent->event_id, 'content_id' => $this->tab_selected_id), $this->translate('Add Video'), array('class' => 'buttonlink icon_siteevents_video_new')) ?>
            &nbsp; | &nbsp;
        <?php endif; ?> 

        <?php if ($this->can_edit): ?>
            <a href='<?php echo $this->url(array('event_id' => $this->siteevent->event_id, 'video_id' => $this->video->video_id, 'content_id' => $this->tab_selected_id), "siteevent_video_edit", true) ?>'  class='buttonlink seaocore_icon_edit'><?php echo $this->translate('Edit Video'); ?></a>
            &nbsp; | &nbsp;
            <?php echo $this->htmlLink(Array('route' => "siteevent_video_delete", 'event_id' => $this->siteevent->getIdentity(), 'video_id' => $this->video->video_id, 'content_id' => $this->tab_selected_id), $this->translate("Delete Video"), array('class' => 'buttonlink seaocore_icon_delete')); ?>
            &nbsp; | &nbsp;
        <?php endif; ?>

        <?php echo $this->htmlLink(Array('module' => 'activity', 'controller' => 'index', 'action' => 'share', 'route' => 'default', 'type' => 'siteevent_video', 'id' => $this->video->getIdentity(), 'format' => 'smoothbox'), $this->translate("Share"), array('class' => 'smoothbox buttonlink seaocore_icon_share')); ?>&nbsp; | &nbsp;

        <?php if ($this->can_embed): ?>
            <?php echo $this->htmlLink(Array('route' => "siteevent_video_embed", 'id' => $this->video->getIdentity(), 'format' => 'smoothbox'), $this->translate("Embed"), array('class' => 'smoothbox buttonlink icon_siteevent_video_embed')); ?>&nbsp; | &nbsp; 
        <?php endif ?>
