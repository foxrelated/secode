 ...
 * @version    $Id: AdminBadgeController.php 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteevent_AdminBadgeController extends Core_Controller_Action_Admin {
    ...
    public function manageAction() {
        ...
        //MAKE QUERY
        $select = Engine_Api::_()->getDbTable('badges', 'siteevent')->select()
    ...
    public function createAction() {
        ...
        //PREPARE FORM
        $this->view->form = $form = new Siteevent_Form_Admin_Badge_Create();
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            $table = Engine_Api::_()->getItemTable('siteevent_badge');
            $db = $table->getAdapter();
            $db->beginTransaction();
    ...
    public function editAction() {
        ...
        //PREPARE FORM
        $this->view->form = $form = new Siteevent_Form_Admin_Badge_Edit();

        //FETCH BADGE ITEM
        $this->view->badge_id = $badge_id = $this->_getParam('id');
        $siteevent = Engine_Api::_()->getItem('siteevent_badge', $badge_id);
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            if (isset($_FILES['badge_main']) && is_uploaded_file($_FILES['badge_main']['tmp_name'])) {
                ...
                if (!empty($siteevent->badge_main_id)) {
                    $storage_main_file = Engine_Api::_()->getItem('storage_file', $siteevent->badge_main_id);
            ...
            try {
                ...
                if (!empty($new_file_id)) {
                    $siteevent->badge_main_id = $new_file_id;
                }
                $siteevent->save();
                $db->commit();
            } catch (Exception $e) {
    ...
    public function assignBadgeAction() {
        ...
        //FATCH BADGE DATA
        $this->view->badgeData = Engine_Api::_()->getDbTable('badges', 'siteevent')->getBadgesData(null);
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                //ASSIGN BADGE
                Engine_Api::_()->getDbTable('editors', 'siteevent')->update(array('badge_id' => $badge_id), array('user_id = ?' => $editor->user_id));
    ...
    public function deleteAction() {
        ...
        if ($this->getRequest()->isPost()) {
            ...
            try {
                ...
                Engine_Api::_()->getDbTable('editors', 'siteevent')->update(array('badge_id' => 0), array('badge_id = ?' => $badge_id));

                $siteevent = Engine_Api::_()->getItem('siteevent_badge', $badge_id);

                //DELETE BADGE IMAGE FILE
                $badgeFile = Engine_Api::_()->getItem('storage_file', $siteevent->badge_main_id);
                if (!empty($badgeFile->file_id)) {
                    $badgeFile->delete();
                }

                //DELETE BADGE
                Engine_Api::_()->getDbtable('badges', 'siteevent')->delete(array('badge_id = ?' => $badge_id));
        ...
        if ($this->getRequest()->isPost()) {
            ...
            foreach ($values as $key => $value) {
                if ($key == 'delete_' . $value) {
                    ...
                    $badge = Engine_Api::_()->getItem('siteevent_badge', $badge_id);
