 ...
 * @version    $Id: AdminFieldsReviewController.php 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteevent_AdminFieldsReviewController extends Fields_Controller_AdminAbstract {
    ...
    public function indexAction() {
        ...
        $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('siteevent_admin_reviewmain', array(), 'siteevent_admin_reviewmain_fields');
    ...
    public function typeCreateAction() {
        ...
        // Create form
        $this->view->form = $form = new Siteevent_Form_Admin_Typereview();
        ...
        if ($form->getValue('duplicate') != 'null') {
            // Create New Option in engine4_siteevent_review_fields_options
            ...
            $field_meta_array = $db->select()
                    ->from('engine4_siteevent_review_fields_meta')
            ...
            // Copy each row
            for ($c = 0; $c < $field_map_array_count; $c++) {
                $db->insert('engine4_siteevent_review_fields_meta', array(
            ...
            for ($i = 0; $i < $map_count; $i++) {
                $db->insert('engine4_siteevent_review_fields_maps', array(
    ...
    public function typeDeleteAction() {
        ...
        if (!empty($option_id)) {
            ...
            Engine_Api::_()->getDbtable('reviews', 'siteevent')->update(array('profile_type_review' => 0), array('profile_type_review = ?' => $option_id));

            //DELETE MAPPING
            Engine_Api::_()->getDbtable('categories', 'siteevent')->update(array('profile_type_review' => 0), array('profile_type_review = ?' => $option_id));
        }
