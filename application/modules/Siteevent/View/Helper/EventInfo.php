 ...
 * @license    http://www.socialengineaddons.com/license/Others
 * @version    $Id: AddToDiary.php 6590 2014-01-02 00:00:00Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Siteevent_View_Helper_EventInfo extends Zend_View_Helper_Abstract {

    public function eventInfo($siteevent, $eventInfo, $params = array()) {
            ...
            if (isset($params['view_type']) && $params['view_type'] == 'grid_view') {
                ...
                if (!empty($hostDisplayName)) {
                    ...
                    if (is_array($hostDisplayName)) {
                        ...
                        echo '<div class="siteevent_listings_stats siteevent_listings_host ' . $className . ' ">';
                        echo '<img class="thumb_icon item_photo_user item_nophoto " alt="" title="'.$view->translate('Host: ') .$hostDisplayName.'" src="' . $view->layout()->staticBaseUrl . 'application/modules/User/externals/images/nophoto_user_thumb_icon.png">';
                        echo '</div>';
                    }
                }
            } else {
        ...
        if ($locationEnabled && !empty($siteevent->location) && in_array('location', $eventInfo)) {
          ...
          <div class="o_hidden">';
            if (!in_array('directionLink', $eventInfo)) {
                echo "<span title='$siteevent->location'>$location</span>";
            } else if (in_array('directionLink', $eventInfo)) {
                echo $view->htmlLink(array('route' => 'seaocore_viewmap', "id" => $siteevent->event_id, 'resouce_type' => 'siteevent_event'), $location, array('onclick' => 'openSmoothbox(this);return false', 'title' => $siteevent->location));
            }
        ...
        if (in_array('categoryLink', $eventInfo) && $siteevent->category_id) {
          ...
          <div class="o_hidden">
            <a href="' . $view->url(array('category_id' => $siteevent->category_id, 'categoryname' => $siteevent->getCategory()->getCategorySlug()), $categoryRouteName) . '"> ' .
            $view->translate($siteevent->getCategory()->getTitle(true)) .
            '</a>
          </div>
        </div>';
        }
          ...
          <div class="o_hidden stats_rating_star">
            ...
            if (in_array('ratingStar', $eventInfo)) {
                if ($params['ratingValue'] == 'rating_both') {
                    echo $view->ShowRatingStarSiteevent($siteevent->rating_editor, 'editor', $params['ratingShow']);
                    echo $view->ShowRatingStarSiteevent($siteevent->rating_users, 'user', $params['ratingShow']);
                } else
                    echo $view->ShowRatingStarSiteevent($siteevent->$params['ratingValue'], $params['ratingType'], $params['ratingShow']);
            }
            echo '</div>';
