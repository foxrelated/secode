 ...
 */
class Siteevent_Model_DbTable_Videos extends Engine_Db_Table {

    protected $_rowClass = "Siteevent_Model_Video";
    ...
    public function getVideosSelect($params = array()) {
        ...
        if (!empty($params['text'])) {
            ...
            $select
                    ...
                    ->where($sName . '.type = ?', 'siteevent_video')
        ...
        $eventsName = $eventsTable->info('name');
        $select
                ->joinLeft($eventsName, "$eventsName. event_id = $rName. event_id", NULL)
        ;

        if (!empty($params['tag'])) {
            $select
                    ->joinLeft($tmName, "$tmName.resource_id = $rName.video_id", NULL)
                    ->where($tmName . '.resource_type = ?', 'siteevent_video')
     ...
     * @return video count
     */
    public function getEventVideoCount($event_id) {
    ...
    public function widgetVideosData($params = array(), $videoType = null, $widgetType = null) {
        ...
        if (isset($params['view_action']) && !empty($params['view_action'])) {
            ...
            if ($widgetType == 'sameposter') {
                $select = $this->select()
                        ->where($tableVideoName . '.event_id = ?', $params['event_id'])
    ...
    public function getTagCloud($limit = 100) {
        ...
        $tableSiteevents = $this->info('name');
        $select = $this->select()
                ->setIntegrityCheck(false)
                ->from($tableSiteevents, 'title')
                ->joinInner($tableTagmaps, "$tableSiteevents.video_id = $tableTagmaps.resource_id", array('COUNT(engine4_core_tagmaps.resource_id) AS Frequency'))
                ->joinInner($tableTags, "$tableTags.tag_id = $tableTagmaps.tag_id", array('text', 'tag_id'));

        $select->where($tableSiteevents . ".search = ?", 1);
        $select = $select
                ->where($tableTagmaps . '.resource_type = ?', 'siteevent_video')
