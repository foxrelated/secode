...
'<div style="display:none;">\n\
  ...
  deactivate : function() {
    if (this.params.albumsong_id)
      new Request.JSON({
        url: en4.core.basePath + '/music/remove-song',
        data: {
          format: 'json',
          albumsong_id: this.params.albumsong_id
        }
      });
    if( !this.active ) return;
    this.parent();
  ...
  doProcessResponse : function(responseJSON) {
    ...
    if (  $type(parseInt(responseJSON.albumsong_id)) != 'number' ) {
      if( this.elements.loading )
          this.elements.loading.destroy();
      //if ($type(console))
    ...
    */
    ...
    this.params.set('albumsong_id',    responseJSON.albumsong_id);
      ...
      'events' : {
        'click' : function(event) {
          ...
          var song = (responseJSON.song_url.match(/\.mp3$/)
            ? soundManager.createSound({id:'s'+responseJSON.albumsong_id, url:responseJSON.song_url})
          : soundManager.createVideo({id:'s'+responseJSON.albumsong_id, url:responseJSON.song_url}));
          song.togglePause();
          this.blur();
        }
      }
