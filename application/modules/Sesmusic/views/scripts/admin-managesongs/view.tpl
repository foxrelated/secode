...
<div class="sesbasic_view_stats_popup">
  ...
  <table>
    ...
    <?php if($this->item->photo_id): ?>
      ...
      $path = $img_path; 
      ?>
    <?php elseif($this->album->photo_id): ?>
      <?php $img_path = Engine_Api::_()->storage()->get($this->album->photo_id, '')->getPhotoUrl();
      $path = $img_path; 
      ?>
     <?php else: ?>
    ...
    <tr>
      <td><?php echo $this->translate('Album') ?>:</td>
      <td><?php echo $this->htmlLink($this->album->getHref(), $this->album->getTitle(), array('title' => $this->translate($this->album->getTitle()), 'target' => '_blank')) ?></td>
    </tr>
    <tr>
    ...
    <tr>
      <td><?php echo $this->translate('Plays') ?>:</td>
      <td><?php echo $this->locale()->toNumber($this->item->play_count) ?></td>
    </tr>
    <tr>
      <?php if($this->artists_array): ?>
          <td><?php echo $this->translate('Artists') ?>:</td>
          <td>
          <?php $artists = '';
          foreach($this->artists_array as $key => $artist):  ?>
            <?php $artists .= $this->htmlLink(array('module'=>'sesmusic', 'controller'=>'artist', 'action'=>'view', 'route'=>'default', 'artist_id' => $key), $artist, array('target' => "_blank")) . ', '; ?>            
          <?php endforeach; ?>
