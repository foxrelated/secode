<?php
class SocialConnect_SignupController extends Core_Controller_Action_Standard
{
  ...
  public function indexAction()
  {
	...
	$actionUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('controller'=>'signup','module'=>'social-connect','action'=>'index'));
    
    $formSequenceHelper = $this->_helper->formSequence2;
    foreach( Engine_Api::_()->getDbtable('signup', 'user')->fetchAll() as $row ) {
      if( $row->enable == 1 ) 
      {
         $class = $row->class;
		 $plugin =  new $class;
		 $plugin->getForm()->setAction($actionUrl);
        $formSequenceHelper->setPlugin($plugin, $row->order);
      }
    }

    // This will handle everything until done, where it will return true
    if( !$this->_helper->formSequence2() ) {
      return;
    }
