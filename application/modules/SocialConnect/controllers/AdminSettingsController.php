...
class SocialConnect_AdminSettingsController extends Core_Controller_Action_Admin
{
	...
	public function editAction()
	{
		...
		$class = 'SocialConnect_Form_Service_' . ucfirst($service);
		$form = NULL;
		...
		{
			$form = $this -> view -> form = new SocialConnect_Form_Service_Openid();
		}
		...
		{
			if ($form -> saveCommit($service))
			{
				$front = Zend_Controller_Front::getInstance();
				$base_url = $front -> getBaseUrl();
				$this -> _redirect('/admin/social-connect/settings');
			}
		}
	}
