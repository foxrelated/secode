...
class Ynrestapi_Api_Core extends Ynrestapi_Api_Base
{
    ...
    public function postTags($params)
    {
        ...
        // STRING tagging
        else if (!empty($text = $params['label'])) {
            $tag = $text;
        }
     ...
     * @param Zend_Http_Response $response
     */
    protected function _previewText($uri, Zend_Http_Response $response, &$result)
    {
        ...
        $result['data']['title'] = substr($body, 0, 63);
        $result['data']['description'] = substr($body, 0, 255);
    }
     ...
     * @param Zend_Http_Response $response
     */
    protected function _previewHtml($uri, Zend_Http_Response $response, &$result)
    {
        $body = $response->getBody();
        $body = trim($body);
        if (preg_match('/charset=([a-zA-Z0-9-_]+)/i', $response->getHeader('content-type'), $matches) ||
            preg_match('/charset=([a-zA-Z0-9-_]+)/i', $response->getBody(), $matches)) {
            $result['data']['charset'] = $charset = trim($matches[1]);
        } else {
            $result['data']['charset'] = $charset = 'UTF-8';
        }
        ...
        $result['data']['image_count'] = count($images);
