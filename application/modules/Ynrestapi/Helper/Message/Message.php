...
class Ynrestapi_Helper_Message_Message extends Ynrestapi_Helper_Base
{
    ...
    public function field_from()
    {
        ...
        $from = array(
            'id' => $message->user_id,
            'title' => $user->getTitle(),
            'img' => Ynrestapi_Helper_Utils::prepareUrl(Ynrestapi_Helper_ItemPhoto::getInstance()->itemPhoto($user, 'thumb.icon')),
        );
    ...
    public function field_attachment()
    {
        ...
        if (!empty($message->attachment_type) && null !== ($attachment = $this->view->item($message->attachment_type, $message->attachment_id))) {
            $rs = array(
                ...
                'img' => Ynrestapi_Helper_Utils::prepareUrl(Ynrestapi_Helper_ItemPhoto::getInstance()->itemPhoto($attachment, 'thumb.normal')),
            );
