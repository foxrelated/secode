...
(function(){
	...
	Slideshow = new Class({
		...
		options: {/*
			...
			captions: true,
			center: true,
			classes: [/*'slideshow', 'first', 'prev', 'play', 'pause', 'next', 'last', 'images', 'captions'+this.options.slideshowName, 'controller', 'thumbnails'+this.options.slideshowName, 'hidden', 'visible', 'inactive', 'active', 'loader'*/],
			controller: true,
			data: null,
			delay: 2000,
			duration: 1000,
	...
	*/
		...
		initialize: function(el, data, options){
			...
			var thumbnail_variable = 'thumbnails' + this.options.slideshowName;
			var caption_variable = 'captions' + this.options.slideshowName;
			var keys_variable = 'slideshow first prev play pause next last images '+ caption_variable +' controller ' + thumbnail_variable + ' hidden visible inactive active loader';

			var keys = keys_variable.split(' '),
			...
			if (!data){
				this.options.hu = '';
				data = {};
				var thumbnails = this.el.getElements(this.classes.get('thumbnails'+this.options.slideshowName) + ' img');
	...
	*/
		...
		load: function(data){
			...
			if (this.options.thumbnails && this.el.retrieve('thumbnails'+this.options.slideshowName))
				this._thumbnails();
			...
			if (loaded && Date.now() > this.timeToNextTransition && Date.now() > this.timeToTransitionComplete){
				...
				if (this.stopped){
					if (this.options.captions)
						this.caption.get('morph').cancel().start(this.classes.get('captions'+this.options.slideshowName, 'hidden'));
					this.pause(1);
					if (this.end)
						this.fireEvent('end');
	...
	var Caption = new Class({
		...
		initialize: function(slideshow){
			...
			if (options === true)
				options = {};
			this.setOptions(options);
			var el = slideshow.el.getElement(slideshow.classes.get('captions'+slideshow.options.slideshowName)),
				caption = el ? el.dispose().empty()
					: new Element('div', {'class': slideshow.classes.get('captions'+slideshow.options.slideshowName).substr(1)});
		...
		update: function(fast){
		    ...
		    if (fast){
		      var p = empty ? 'hidden' 
				: 'visible';
		      this.caption.set({'aria-hidden': empty, 'html': this.data.captions[this._slide]}).get('morph').cancel().set(this.classes.get('captions'+this.options.slideshowName, p));
		    }
		    else {
		      var fn1 = empty ? function(){} 
				...
				this.caption.store('timer', setTimeout(function(caption){
			        this.caption.set('html', caption).morph(this.classes.get('captions'+this.options.slideshowName, hide_caption_strip));
				}.pass(caption, this), this.caption.retrieve('delay')));
		      }.pass(this.data.captions[this._slide], this);    
		      var fn2 = function(){ 
		        this.caption.set('aria-busy', false); 
		      }.bind(this);
		      this.caption.set('aria-busy', true).get('morph').cancel().start(this.classes.get('captions'+this.options.slideshowName, 'hidden')).chain(fn1, fn2);
		    }
		}
	});
	...
	*/
	...
	var Thumbnails = new Class({
		...
		initialize: function(slideshow){
			...
			this.setOptions(options);
			var el = slideshow.el.getElement(slideshow.classes.get('thumbnails'+slideshow.options.slideshowName)),
				thumbnails = el ? el.empty() 
					: new Element('div', {'class': slideshow.classes.get('thumbnails'+slideshow.options.slideshowName).substr(1)});
			...
			slideshow.data.thumbnails.each(function(thumbnail, i){
				var li = new Element('li', {'id': uid + i}).inject(ul),
					a = new Element('a', {
						'class': slideshow.classes.get('thumbnails'+slideshow.options.slideshowName, 'hidden').substr(1),
						'events': {
			...
			if (a){
				(function(a){
					var visible = i == this.slide ? 'active' 
						: 'inactive';					
					a.store('loaded', true).get('morph').set(this.classes.get('thumbnails'+this.options.slideshowName, 'hidden')).start(this.classes.get('thumbnails'+this.options.slideshowName, visible));	
