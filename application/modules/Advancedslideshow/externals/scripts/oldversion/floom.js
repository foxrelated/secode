...
var Floom = new Class({
	...
	createCaptions: function(){
		this.captions = new Element('div', {
			'class': this.options.prefix + 'caption'+this.options.slideshowName,
			'html': 'caption'+this.options.slideshowName,
			'styles': {
				'opacity': 0
			}
		});
...
//			'href' : 'http://www.google.com'     
		...
		this.link.inject(this.wrapper.el);
		...
		this.container.inject(this.link);
    ...
    if (this.slides[this.current.slide].url){
			this.link.set('href', this.slides[this.current.slide].url);
			if (this.slides[this.current.slide].target){
				this.link.set('target', this.slides[this.current.slide].target);
			}
		} else {
			this.link.set('href', 'javascript:void(0);');
		}
