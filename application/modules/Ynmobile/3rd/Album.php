...
class Ynmobile_Api_Album extends Core_Api_Abstract
{
	...
	public function delete($aData)
	{
		...
		if (!$album)
		{
			return array(
				'error_code' => 1,
				'error_message' => Zend_Registry::get('Zend_Translate') -> _("Album does not exists or not authorized to delete")
			);
		}
	...
	public function list_songs($aData)
	{
		...
		if (!$album || !Engine_Api::_() -> authorization() -> isAllowed($album, null, 'view'))
		{
			return array(
				'error_code' => 1,
				'error_message' => Zend_Registry::get('Zend_Translate') -> _("Playlist does not exists or not authorized to view")
			);
		}
