...
class Ynmobile_Helper_Base{
    ...
    function field_disliked(){
        $this->data['bIsDisliked'] = 0;
        $viewer = self::getViewer();
        if(Engine_Api::_()->hasModuleBootstrap('yncomment')) {
            $this->data['bIsDisliked'] = (Engine_Api::_() -> getDbtable('dislikes', 'yncomment') -> isDislike($this->entry, $viewer))?1:0;
        }
    }
    ...
    function field_totalDislike(){
        ...
        if(Engine_Api::_()->hasModuleBootstrap('yncomment')){
            $this->data['iTotalDislike'] = Engine_Api::_() -> getDbtable('dislikes', 'yncomment') -> getDislikeCount($this->entry);
        }
    }

    function field_dislikes(){
        $this->data['aDislikes']=Engine_Api::_()-> getApi('like','ynmobile') -> getUserDislike($this->entry);
    }
    ...
    function field_canDislike(){
        ...
        $commentOptions = $this->getYnmobileApi()->getCommentOptions($this->entry->getType());
        $this->data['bCanDislike'] = ($commentOptions['bShowComments'] && $canComment && $advancedCommentOptions['bCanDislike']) ? 1 : 0;
    }

    function field_showDislikeUsers() {
        $itemType = $this->entry->getType();
        $advancedCommentOptions = $this->getYnmobileApi()->getAdvancedCommentOptions($itemType);

        $this->data['bShowDislikeUsers'] = ($advancedCommentOptions['bEnabled'] && $advancedCommentOptions['bShowDislikeUsers']) ? 1 : 0;
    }
    ...
    function field_totalComment(){
        ...
        if(method_exists($this->entry, 'comments')) {
            ...
            $advancedCommentOptions = $this->getYnmobileApi()->getAdvancedCommentOptions($this->entry->getType());
            if ($advancedCommentOptions['bEnabled']){
                $select->where('parent_comment_id = ?', 0);
            }
            $allComments = $table->fetchAll($select);
            $this->data['iTotalComment'] = count($allComments);
        }
     ...
     *
     */
    function field_advancedComment(){
        $action =  $this->entry;
        ...
        if (in_array($actionType, array('activity_comment', 'core_comment'))) {
            ...
            $itemType = $actionType;
            $this->data['aAdvancedComment'] = $this->getYnmobileApi()->getAdvancedCommentOptions($itemType);
        }
    ...
    function field_resume_photos() {
        $item = $this->entry;

        $photosResult = array();
        $photos = Engine_Api::_()->getItemTable('ynresume_photo')->getPhotosItem($item);
        foreach ($photos as $key=>$photo) {
            $url = $photo->getPhotoUrl();
            $photosResult[$key]['sPhotoUrl'] = $this->finalizeUrl($url);
            $photosResult[$key]['iPhotoId'] = $photo->photo_id;
        }

        $this->data['aPhotos'] = $photosResult;
    }

    function field_resume_period($withRange = false) {
        ...
        if ($item->end_year) {
            ...
            $end_time = $view-> translate('Present');
        }

        $periodString = '';

        $diff = date_diff($start_date, $end_date);
        $period = $diff->format('%y')*12 + $diff->format('%m') + 1;

        if ($period > 12) {
            $years = intval($period / 12);
            $months = $period % 12;
            ($years > 0) ? $periodString = $periodString . $view->translate(array('%s year','%s years',$years),$years) . ' '  : '';
            ($months > 0) ? $periodString = $periodString . $view->translate(array('month_diff','%s months',$months),$months): '';
        } else {
            $periodString = $periodString . $view->translate(array('month_diff','%s months',$period),$period);
        }

        if ($withRange) {
            $periodString = $start_time . " - " . $end_time . " (" . $periodString .")";
        } else {
            $periodString = $start_time . " - " . $end_time;
        }

        $this->data['sPeriod'] = $periodString;
    }
}