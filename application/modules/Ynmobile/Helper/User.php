...
class Ynmobile_Helper_User extends Ynmobile_Helper_Base{
    ...
    function field_cover(){
        ...
        if(isset($user->cover_id) && $user->cover_id){
            $sCoverUrl = $this->get_cover_from_file_id($user->cover_id);
        }

        if(!$sCoverUrl && isset($user->user_cover) && $user->user_cover){
			$sCoverUrl =  $this->get_cover_from_photo_id($user->user_cover);
        }


        if(!$sCoverUrl){
            $sCoverUrl = $this->get_cover_from_ynmobile();
        }
     ...
     * @return string
     */
    function get_cover_from_ynmobile(){
        $sCoverUrl = '';
	...
	function get_cover_from_file_id($file_id){
		$file  = Engine_Api::_()->getDbtable('files','storage')->getFile($file_id);
		
		if($file){
			$url =  $file->getPhotoUrl();
			if($url){
    ...
    function get_cover_from_photo_id($iPhotoId){

        $item =  $this->getYnmobileApi()->getWorkingItem('album_photo', $iPhotoId);

        if($item){
            return $this->finalizeUrl($item->getPhotoUrl());
        }
    }
    ...
    function field_ynbusinesspagesRole(){

        $this->data['sRole'] = $this->entry->role_name;
    }
    ...
    function field_subscription(){
        $subscriptionsTable = Engine_Api::_() -> getDbtable('subscriptions', 'payment');

        $this->data['bSubscription'] = $subscriptionsTable->check($this->entry) ? 1:0;
    }
}
