...
class Ynmobile_Service_Photo extends Ynmobile_Service_Base
{
	...
	private function getPhotos($aData, $type)
	{
		...
		$sParentType =  isset($sParentType)?$sParentType: "album";

		if(!$sParentType){
			$sParentType = "album";
		}

		if($sParentType == "album"){
			// album table
			$albumTable = $this->getWorkingTable('albums','album');
			$photoTable = $this->getWorkingTable('photos','album');
		}else{
			// album table
			$sPhotoType =  str_replace('_album','_photo', $sParentType);
			$albumTable = Engine_Api::_()->getItemTable($sParentType);
			$photoTable = Engine_Api::_()->getItemTable($sPhotoType);
		}
		...
		if (isset($iAlbumId))
		{
			...
			$album  = $albumTable ->fetchRow(array(
				'album_id=?'=>$iAlbumId,
			));
		...
		if($albumIds){
			if($photoTableName != 'engine4_album_photos'){
				$select
	...
	public function albumcreate($aData)
	{
		...
		if($aData['parent_type'] == 'ynlistings_listing' && $aData['parent_id'] > 0){
			$aData['iListingId'] =  $aData['parent_id'];
			return Engine_Api::_()->getApi('ynlisting','ynmobile')->albumcreate($aData);
		}
	...
	public function albumview($aData)
	{
		...
		$sItemType = isset($aData['sItemType']) ? $aData['sItemType'] : $this->getWorkingType('album');

		if(!$sItemType){
			$sItemType =  $this->getWorkingType('album');
		}

		$albumTable = Engine_Api::_()->getItemTable($sItemType);
	...
	public function albumedit($aData)
	{
		...
		if (isset($iCategoryId) && !is_numeric($iCategoryId))
		{
			return array(
				'result' => 0,
				'error_code' => 1,
				'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Invalid Category Id")
			);
		}
	...
	public function albumdelete($aData)
	{
		...
		if(@$aData['sItemType'] == 'ynlistings_album'){
	...
	{
		if (($aData['sParentType'] == "group" || $aData['sParentType'] == 'advgroup')
			&& isset($aData['iParentId']))
		{
			$aData['iGroupId'] = $aData['iParentId'];
			return Engine_Api::_()->getApi('group','ynmobile')->upload_photo($aData);
		}else if($aData['sParentType'] == 'ynlistings_listing'
			|| $aData['sItemType'] == 'ynlistings_album'){
			return Engine_Api::_()->getApi('ynlisting','ynmobile')->upload_photo($aData);
		}
	...
	function fetch_photo($aData){
		...
		if(!isset($sParentType) || empty($sParentType)){
			...
			if(!$parent){
				return array(
					'error_code'=>1,
					'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Parent not found"),
				);
			}
	...
	function slider($aData){
		...
		if(!$photo){
			return array(
				'error_code'=>1,
				'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Photo not found"),
			);
		}

		if($sItemType == 'ynlistings_photo'){
			$sParentType =  'ynlistings_album';
		}

		if($sItemType == 'ynbusinesspages_photo'){
			$sParentType = 'ynbusinesspages_album';
			$table = Engine_Api::_() -> getItemTable('ynbusinesspages_album');
			$select = $table -> select() -> where('business_id = ?', $iParentId) -> order('album_id ASC') -> limit(1);
			$album = $table -> fetchRow($select);
			$iParentId = $album -> getIdentity();
		}
	...
	public function edit($aData)
	{
		...
		try
		{
			$photo->setFromArray(array(
				...
				'image_title'=>(string) $sTitle,
				'image_description' => (string) $sDescription,
			));
	...
	public function delete($aData)
	{
		...
		$viewer = Engine_Api::_() -> user() -> getViewer();
		// check edit permission
		if ($aData['sItemType'] == 'ynbusinesspages_photo') {
			// get business
			$business = $photo->getParent('ynbusinesspages_business');
			if ($business) {
				$canDelete = $business->isAllowed('album_delete', null, $photo);
			} else {
				$canDelete = false;
			}
		} else {
			$canDelete = $photo -> authorization() -> isAllowed($viewer, 'edit');
		}
	...
	public function delete_photos($aData)
	{
		...
		if($sParentType == 'ynlistings_album'){
			$albumTable = Engine_Api::_()->getItemTable('ynlistings_album');
		...
		foreach($iPhotoIds as $iPhotoId){
			...
			if($sItemType == 'ynlistings_photo'){
				$listing = Engine_Api::_()->getItem('ynlistings_listing', $photo->listing_id);
				$album =  Engine_Api::_()->getItem('ynlistings_album', $photo->album_id);
				...
				if($album->photo_id == $photo->file_id){
					$album->photo_id = 0;
					$photo->save();
				}
			}
	...
	public function filter_album($aData)
	{
		...
		if($aData['sParentType'] == 'ynlistings_listing'){
			return Engine_Api::_()->getApi('ynlisting','ynmobile')->fetch_album(array(
				'iListingId'=>$aData['iParentId'],
				'iPage'=>$aData['iPage'],
				'iLimit'=>$aData['iLimit'],
			));
		}
	...
	public function postfeed($aData)
	{
		...
		if(@$aData['sParentType'] == 'event' && $aData['iParentId']){
			$iEventId =  $aData['iParentId'];

			if($this->getWorkingType('event') != 'ynevent_event'){
				$actionType =  'ynevent_photo_upload';
				$subject  =  Engine_Api::_()->user()->getViewer();
				$object =  Engine_Api::_()->getItem('event', $iEventId);
				...
				$actionType =  'ynevent_photo_upload';
				$subject  =  Engine_Api::_()->user()->getViewer();
				$object =  Engine_Api::_()->getItem('ynevent_event', $iEventId);

				$photoTable = Engine_Api::_()->getItemTable('event_photo');

			}

			$action = $api->addActivity($subject, $object, $actionType, null, array('count' => count($ids)));

			$count =  0;

			foreach( $ids as $photo_id )
			{
				$photo = $photoTable->fetchRow(array('photo_id=?'=> $photo_id));
		...
		else if(@$aData['sParentType'] == 'ynlistings_listing' && $aData['iParentId']){
			$actionType =  'ynlistings_photo_upload';
			$iListingId =  $aData['iParentId'];

			$subject  =  Engine_Api::_()->user()->getViewer();
			$object =  Engine_Api::_()->getItem('ynlistings_listing', $iListingId);
		...
		else if(@$aData['sParentType'] == 'ynbusinesspages_business' && $aData['iParentId']){
			$actionType =  'ynbusinesspages_photo_upload';
			$iBusinessId =  $aData['iParentId'];

			$subject  =  Engine_Api::_()->user()->getViewer();
			$object =  Engine_Api::_()->getItem('ynbusinesspages_business', $iBusinessId);
			...
			$count =  0;
			$album = $object->getSingletonAlbum();
		...
		else if (isset($aData['sParentType']) && ($aData['sParentType']  == "group" || $aData['sParentType']  == "advgroup" ))
		{
	...
	public function formadd($aData) {
		...
		if(!empty($aData['parent_type']) && $aData['parent_type'] == 'ynlistings_listing'){
			return array(
				'viewOptions' => array(),
