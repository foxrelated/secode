...
class Ynmobile_Service_Ynchat extends Ynmobile_Service_Base{
    ...
    public function pingStatus(){
        ...
        if(null == $user || $user->getIdentity() == 0){
            return array(
                    'error_code'=>0,
                    'error_message'=>Zend_Registry::get('Zend_Translate') -> _("invalid user")
                );
        }
...
//                    }
    ...
    public function update_agent($aData) {
		...
		if(!$row){
			return array(
				'error_code'=>'1',
				'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Can not find status"),
			);	
		}
...
//                    $aRows = $this->_getFriendsList(Phpfox::getUserId(), null, $sSearch);
    ...
    public function upload($aData) {
        ...
        $origin_file_name  = $params['fileName'];
        $file_name = $name = $params['fileName'] . uniqid();
    ...
    public function download($aData){
        ...
        if (false == Engine_Api::_()->hasItemType('ynchat_file')){
            return array(
                'error_code'=>1,
                'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Social Chat plugin is disabled")
            );
        }
        ...
        if (!$file) {
            return array(
                'error_code'=>1,
                'error_message'=>Zend_Registry::get('Zend_Translate') -> _("file not found")
            );
        }
		
		$fileStorage =  Engine_Api::_()->storage()->get($file->storage_file_id);
        ...
        // $file_link = Ynmobile_Helper_Base::getBaseUrl(). '/ynchat/files' . '/user_' . $file->user_id . '/' .$filename;
        $file_link  = $fileStorage->map();
	...
	function pushNotification($aMessage){
		...
		$fromUser =  Engine_Api::_()->getItemTable('user')->findRow(intval($fromUserId));
// 		
		if(!$fromUser){
			return ;
		}
...
// 		
		...
		$content  = strip_tags(sprintf('%s: %s', $fromUser->getTitle(), $msg));
// 		
		$params =  array(
			'ios' => array(
					'aps' => array(
		...
		Engine_Api::_()->getApi('PushNotification','ynmobile')->send($params, $toUserId);
	}
}
