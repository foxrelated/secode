...
class Ynmobile_Service_Blog extends Ynmobile_Service_Base
{
	...
	public function upload_photo($aData) {
		if (!empty($aData['photo'])) {
			if ($filePath = $this->_save_photo($aData['photo'])) {
			...
			return array(
				'error_code' => 1,
				'error_message' => 'Image saving failed!'
			);
		} else {
			return array(
				'error_code' => 1,
	...
	function _save_photo($img) {
		$user_id = Engine_Api::_ ()->user ()->getViewer ()->getIdentity ();
		$destination = "public/blog/";
		if (! is_dir ( $destination )) {
			mkdir ( $destination );
		}
		$destination = "public/blog/" . $user_id . "/";
		if (! is_dir ( $destination )) {
			mkdir ( $destination );
		}
		$upload_dir = APPLICATION_PATH . '/' . $destination;
