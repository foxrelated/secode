...
class Ynmobile_Service_Poll extends Ynmobile_Service_Base
{
    ...
    public function formadd($aData) {
        ...
        $response = array(
            ...
            'iMaxOptions' => (int) Engine_Api::_()->getApi('settings', 'core')->getSetting('poll.maxoptions', 15),
        );
	...
	public function vote($aData)
	{
		...
		try 
		{
			...
			return array(
			 'error_code'=>0,
			 'message'=>Zend_Registry::get('Zend_Translate') -> _("Voted successfully."),
    ...
    function voters($aData){
        ...
        if(!$poll){
            return array(
                'error_code'=>1,
                'error_message'=>Zend_Registry::get('Zend_Translate') -> _("This poll not found"),
            );
        }
