...
class Ynmobile_Service_Event extends Ynmobile_Service_Base
{
    ...
    public function get_ynevent_select($aData = array())
    {
        ...
        if($aData['sOrder'] == 'starttime'){
			...
			$values['direction'] = 'desc';
		}else if($aData['sOrder'] == 'distance') {
			$values['within'] = $aData['within'];
			$values['lat'] = $aData['lat'];
			$values['long'] = $aData['long'];
        }else{
	...
	{
		$isAdvEvents = $this->getWorkingModule('event') == 'ynevent';
		$categories = $this->categories();
		return array(
			'isAdvEvents' => $isAdvEvents ? 1: 0,
	...
	public function create($aData)
	{
		...
		if ( isset($aData['zip_code']) && !is_numeric($aData['zip_code']) )
		{
			return array(
					'error_code' => 4,
					'error_message' => Zend_Registry::get('Zend_Translate') -> _("Zip code is invalid!")
			);
		}
		...
		try
		{
			...
			// support ynbusiness pages mapping
			if (!empty($aData['parent_type']) && $aData['parent_type']=='ynbusinesspages_business')
			{
//				$business = $this->getWorkingItem('ynbusinesspages_business', $aData['parent_id']);
				$business_id = $aData['parent_id'];
	...
	public function detail($aData){
        ...
        if(!$event){
            ...
            'error_message'=>Zend_Registry::get('Zend_Translate') -> _("Event not found"));
        }        
	...
	public function addrsvp($aData)
	{
		...
		try
		{
			...
			return array(
				'error_code' => 0,
				'result' => 1,
				'message'=>Zend_Registry::get('Zend_Translate') -> _("Your RSVP has been updated!"),
		...
		try
		{
			...
			if( $event->membership()->isResourceApprovalRequired() ) {
				return array(
						'error_code' => 0,
						'result' => 1,
						'message'=>Zend_Registry::get('Zend_Translate') -> _("You have requested invite this event!"),
						'event_data' =>$this->detail(array('iEventId'=> $event->getIdentity())),
				);
			}
			else {
				return array(
						'error_code' => 0,
						'result' => 1,
						'message'=>Zend_Registry::get('Zend_Translate') -> _("You have joined this event!"),
						'event_data' =>$this->detail(array('iEventId'=> $event->getIdentity())),
				);
			}
	...
	public function leave($aData)
	{
		...
		try
		{
			...
			return array(
					'error_code' => 0,
					'result' => 1,
					'message'=>Zend_Registry::get('Zend_Translate') -> _("You have left this event!"),
					'event_data' =>$this->detail(array('iEventId'=> $event->getIdentity())),
			);
		}
		catch( Exception $e )
	...
	public function uploadPhotos($aData){
		...
		$canUpload = $auth->setAuthParams($event, null, 'photo') -> isValid()?1:0;
    	...
    	$db->beginTransaction();
		 ...
		 try {
		      ...
		      $photoTable = $this->getWorkingItemTable('event_photo');
		      ...
		      $this->view->name = $_FILES['image']['name'];
