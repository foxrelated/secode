...
class Yncontest_Model_Contest extends Core_Model_Item_Abstract {
	...
	public function checkAllow($params= array()){
		...
		//if(in_array($params['action'], $perms) ){
	...
	public function deniedContest(){
		...
		// 		$notifyApi = Engine_Api::_() -> getDbtable('notifications', 'activity');
		// 		$notifyApi -> addNotification($owner,$viewer, $this, "contest_denied");
		//send mail
		//$params["contest_link"] = $this->getHref();
		//Engine_Api::_() -> getApi('mail', 'yncontest') -> send($this -> contest_email, 'contest_denied', $params);
	...
	public function checkPublish(){
		//$rules = Engine_Api::_() -> getDbTable('rules', 'yncontest') -> getRuleByContest($this->contest_id);
		//$awards = Engine_Api::_() -> getDbtable('awards', 'yncontest') -> getAwardByContest($this -> contest_id);
		$settings = Engine_Api::_() -> getDbTable('settings', 'yncontest') -> getSettingByContest($this->contest_id);

		// $viewer = Engine_Api::_()->user()->getViewer();
		// if(!$this->isOwner($viewer )&& ($viewer->getIdentity() !=1 || $viewer->getIdentity() !=2 ))
		// return false;
		//
		//if(count($rules)==0 || count($awards) ==0 || count($settings)==0) return false;
	...
	public function createPhoto($album_id, $file){
		...
		// Resize image (main)
		$image = Engine_Image::factory();
		//$image -> open($url) -> resize(440, 255) -> write($path . '/m_' . $name) -> destroy();
