 ...
 * @version    $Id: IndexController.php 2012-06-028 9:40:21Z SocialEngineAddOns $
 * @author     SocialEngineAddOns
 */
class Sitevideoview_IndexController extends Core_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    $this->view->viewPermission = $video->authorization()->isAllowed($viewer, 'view');
    if ($message_view) {
    ...
    switch ($modulename) {
      case 'video' :
        ...
        $this->view->rating_count = Engine_Api::_()->video()->ratingCount($video_id);

        $this->view->rated = Engine_Api::_()->video()->checkRated($video_id, $viewer_id);
        ...
        $this->view->tagFilterUrlArray = array('module' => 'video', 'controller' => 'index', 'action' => 'browse');
    ...
    if ($this->_getParam('count', null)) {
      ...
      $params['count'] = Engine_Api::_()->sitevideoview()->getCountTotal($video, $params);
    }
    ...
    if ($params['count'] > 1) {
      ...
      if ($this->_getParam('offset', null)) {
        ...
        $params['offset'] = Engine_Api::_()->sitevideoview()->getCollectibleIndex($video, $params);
      }
      $this->view->prevVideo = Engine_Api::_()->sitevideoview()->getPrevVideo($video, $params);
      if (empty($params['offset'])) {
        $this->view->PrevOffset = $params['count'] - 1;
      } else {
        $this->view->PrevOffset = $params['offset'] - 1;
      }
      $this->view->nextVideo = Engine_Api::_()->sitevideoview()->getNextVideo($video, $params);
