<ul class="ynmember-most-reviewed-members">
  <?php foreach( $this->list_show_users as $arr_item ): ?>
  	<?php 
  		$user_id = $arr_item['resource_id'];
    ...
    <li class="ynmember-most-item ynmember-clearfix">
    	...
    	<div class="ynmember-most-item-info">
			...
			<!-- mutual friend -->
			<a href='<?php echo $this->url(array('controller' => 'review', 'action' => 'user', 'id' => $user_id),'ynmember_extended') ?>'><?php echo $this -> translate(array("%s review", "%s reviews" ,$arr_item['count']), $arr_item['count']) ?></a>
		</div>
    </li>
  <?php endforeach; ?>
</ul>