 ...
 */
class Socialstore_Payment_Gateway_2Checkout extends Socialstore_Payment_Gateway_Abstract
{
	...
	const METHOD_OCHECK = 'CK';
	const METHOD_PAYPAL = 'PPI';
	const METHOD_PAYPAL_LATER = 'PPL';
	const METHOD_FXSOURCE = 'FXS';

    const TRXTYPE_AUTH_CAPTURE         = 'AUTH_CAPTURE';
    const TRXTYPE_AUTH_ONLY            = 'AUTH_ONLY';
    const TRXTYPE_CAPTURE_ONLY         = 'CAPTURE_ONLY';
    const TRXTYPE_CREDIT               = 'CREDIT';
    const TRXTYPE_VOID                 = 'VOID';
    const TRXTYPE_PRIOR_AUTH_CAPTURE   = 'PRIOR_AUTH_CAPTURE';
    ...
    protected function _getRequirements()
    {
        ...
        /*$requirements->setOnMethod(
            ...
            array('Socialstore_Payment_Method_Card', 'Socialstore_Payment_Method_CC')
        );
        $requirements->setOnMethod(
    ...
    protected function _processSale($request)
    {
        ...
        $redirect_url = $this->getUrl()."?".http_build_query($this->_queryParams) . "\n";
    ...
    protected function _initQueryOrder($request)
    {
        ...
        $this->_queryParams['total']         = $order->getTotalAmount();
        //$this->_queryParams['x_currency_code']  = $order->getCurrency();
        //$this->_queryParams['x_po_num']         = $order->getId();
        $this->_queryParams['cart_order_id']    = $order->getId();
        /*if ($order->getShipping()) {
            $this->_queryParams['x_freight']    = $order->getShipping();
        }
        if ($order->getTax()) {
    ...
    protected function _initQueryOrderAddresses($order)
    {
       ...
       /* if ($billing) {
            ...
            $this->_queryParams['x_address']    = $billing->getFullStreet();
