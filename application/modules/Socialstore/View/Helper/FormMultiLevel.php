...
class Socialstore_View_Helper_FormMultiLevel extends Zend_View_Helper_Abstract{
	
	public function formMultiLevel($name, $value = null, $attributes = array()){
		...
		if(!is_object($item)){
			...
			for($i =  0; $i< $level; ++$i){
				...
				if ($model_class == 'Socialstore_Model_DbTable_Locations') {
					$element = new Zend_Form_Element_Select(
						sprintf("%s_%s",$name, $i),
						array(
							'multiOptions'=> $options,
