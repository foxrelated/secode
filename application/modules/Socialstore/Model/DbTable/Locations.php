...
class Socialstore_Model_DbTable_Locations extends Engine_Db_Table {

	/**
	 * model table name
	 * @var string
	 */
	protected $_name = 'socialstore_locations';

	/**
	 * model class name
	 * @var string
	 */
	protected $_rowClass = 'Socialstore_Model_Location';
	...
	/**
	 * @param  Socialstore_Model_Location  $node
	 * @param  int                   $new_pid
	 */
	public function deleteNode(Socialstore_Model_Location $node, $new_pid = 0){
	...
	public function addNode($data, $pid = 0) {
		...
		try {
			...
			if(is_object($parent)) {
				for($i = 0; $i <= Socialstore_Model_Location::MAX_LEVEL; ++$i) {
					$key = "p{$i}";
					$node -> {$key} = $parent -> {$key};
				}
	...
	public function getMaxLevel(){
		return Socialstore_Model_Location::MAX_LEVEL;
	}
