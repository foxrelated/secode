...
class Socialstore_Model_Customcategory extends Core_Model_Item_Abstract {
	...
	/*public function getAscendant(){
		...
		$table =  $this->_table;
		$select =  $table->select()->where('category_id in (?)', $pids)->order('level');
	...
	public function getAscendant($category_id = null) {
		...
		$select = $table->select();
		if (!$category_id) {
			$select->where('customcategory_id = ?', $this->parent_category_id);
		}
		else {
			$select->where('customcategory_id = ?', $category_id);
		}
	...
	public function getDescendantIds($parent_id = null, $descendantIds = array()) {
		$temp = array();
		if (!$parent_id) {
			$temp = $this->getDirectDescendant($this->customcategory_id);
		}
		else {
	    ...
	    foreach ($temp as $te) {
	        $this->getDescendantIds($te->customcategory_id, $descendantIds);
	    }   
	...
	public function getUsedCount() {
		$table = Engine_Api::_() -> getDbTable('SocialProducts', 'Socialstore');
		$rName = $table -> info('name');
		$ids =  $this->getDescendantIds();
		$ids[] = $this->customcategory_id;
		$select = $table -> select() -> from($rName) -> where($rName . '.category_id in (?)', $ids)->where('deleted = 0');
		$row = $table -> fetchAll($select);
		return $row;
	}
	
	public function getNextValueMultiSelect($i) {
		$ascendants = $this->getAscendant();
		$trees = $this->getTree();
