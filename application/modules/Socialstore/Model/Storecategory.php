...
class Socialstore_Model_Storecategory extends Core_Model_Item_Abstract {
	...
	public function getAscendant($category_id = null) {
		...
		$select = $table->select();
		if (!$category_id) {
			$select->where('storecategory_id = ?', $this->parent_category_id);
		}
		else {
			$select->where('storecategory_id = ?', $category_id);
		}
	...
	public function getDescendantIds($parent_id = null, $descendantIds = array()) {
		$temp = array();
		if (!$parent_id) {
			$temp = $this->getDirectDescendant($this->storecategory_id);
		}
		else {
	    ...
	    foreach ($temp as $te) {
	        $this->getDescendantIds($te->storecategory_id, $descendantIds);
	    }   
	...
	public function getNextValueMultiSelect($i) {
		...
		if (count($ascendants) > 0) {
			foreach ($ascendants as $ascendant) {
				if ($ascendant->level == $i) {
					return $ascendant->storecategory_id;
				}
			}
		}*/
			$trees = $this->getTree();
