...
include_once  APPLICATION_PATH . '/application/modules/Socialstore/externals/scripts/adaptivelib/Config/paypal_sdk_clientproperties.php' ;
include_once  APPLICATION_PATH . '/application/modules/Socialstore/externals/scripts/adaptivelib/SOAPEncoder/SOAPEncoder.php';
include_once  APPLICATION_PATH . '/application/modules/Socialstore/externals/scripts/adaptivelib/Exceptions/FatalException.php'  ;
include_once  APPLICATION_PATH . '/application/modules/Socialstore/externals/scripts/adaptivelib/Log.php';
...
class CallerServices {
    ...
    private $LastError;
   	 ...
   	 */
   	function call($req, $request,$serviceName) {
		...
		try {
	    	...
	    	if ($sandboxMode == 1) {
	    		$str = 'https://svcs.sandbox.paypal.com/';
	    	}
	    	else {
	    		$str = 'https://svcs.paypal.com/';
	    	}
		    $endpoint=$str.$serviceName;
		 
		    $response = call($req, $request, $endpoint, $this->sandBoxEmailAddress);
		    $isFault = false;
...
function call($req, $MsgStr, $endpoint, $sandboxEmailAddress = '')
{
    ...
    $headers_array = setupHeaders(API_AUTHENTICATION_MODE,$req);
...
function setupHeaders($auth_mode,$req) {
    $headers_arr = array();

	if ($auth_mode == 'ssl' ) {
	   $headers_arr[]="CLIENT_AUTH:  ".'Valid cert';
	} else {
	    $headers_arr[]="X-PAYPAL-SECURITY-SIGNATURE: ".$req->apiSignature;

	}
	 Zend_Registry::get('Zend_Log')->log(print_r($req,true), 7);
	$headers_arr[]="X-PAYPAL-SECURITY-USERID:  ".$req->apiUsername;
	$headers_arr[]="X-PAYPAL-SECURITY-PASSWORD: ".$req->apiPassword;
	//$headers_arr[]="X-PAYPAL-APPLICATION-ID: ".X_PAYPAL_APPLICATION_ID;
	if (!$req->clientDetails) {
		$headers_arr[]="X-PAYPAL-APPLICATION-ID: ".$req->apiAppID;
	}
	else {
		$headers_arr[]="X-PAYPAL-APPLICATION-ID: ".$req->clientDetails->applicationId;
	}
