<?php 
if(!function_exists('renderPlaylistPagination')) 
{ 
    function renderPlaylistPagination($html)
    {
...
?>    
	...
	?>	
<h3> <?php if($this->browse->params['search']):  echo $this->translate('Browse Playlists'); endif;
	if(!$this->browse->params['search']): echo $this->translate('New playlists'); endif; ?></div>    
         </h3>
...
<div class="mp3_browse_album">      
        ...
        <li class="mp3music_browsealbums" style="float: none; overflow: hidden;">
           <div class="mp3music_bgalbums" style="float: left" title="<?php echo $playlist->title;  ?>">
               <a href="javascript:;"  onClick="return openPage('<?php echo $this->url(array('playlist_id'=>$playlist->playlist_id), 'mp3music_playlist');?>',123,565)">
                <?php echo $this->itemPhoto($playlist, 'thumb.normal'); ?>  
                </a>
            </div>
   ...
   <span style="float:right ;"> <?php echo renderPlaylistPagination($this->paginationControl($this->browse->playlistPaginator,null, null, array(
                      'pageAsQuery' => false,
