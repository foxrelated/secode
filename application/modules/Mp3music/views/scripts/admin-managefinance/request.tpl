...
function makeRequest(f)
{
    if(f == null || f == undefined && is_already == false){     
        ...
        fr.submit();  
         ...
         new Request.JSON({
          url: '<?php echo $this->url(array("module"=>"mp3music","controller"=>"cart","action"=>"makerequest"), "default") ?>',
          data: {
            'format': 'json',
            'message':  message ,
    ...
    <tr>
        ...
        <td width="40%" align="left" style="vertical-align:text-top;">
            <div style="position: ;">
                    ...
                    <div class="p_4">
                        Pay to account:  <?php echo $this->account['account_username']?> 
                    </div>
                    <div class="p_4">
                     by using
                        <?php if ($this->status == 1): ?>
                            <?php if ($this->is_adaptive_payment == 1): ?>                        
                                <input onclick ="javascript:checkout('paypal')" type="image" src="<?php echo $this->core_path; ?>/application/modules/Mp3music/externals/images/paypal.gif" name="paypal" alt="Make payments with PayPal - it's fast, free and secure!">
                                ...
                                <input type="hidden" value="<?php echo $this->account['payment_request_id'];?>" name ="pay[request]">
                        ...
                        <?php else: ?>
                            <input type="hidden" value="<?php echo $_SESSION['payment_sercurity_adminpayout'];?>" name ="pay[sercurity]">
                                ...
                                <input type="hidden" value="<?php echo $this->status;?>" name ="pay[is_accept]">
                                <input type="hidden" value="paypal" name ="pay[gateway]" id="pay_gate_way">  
                            <input type="submit" class="button" value="I don't accept this request" name="pay[cancel]">
