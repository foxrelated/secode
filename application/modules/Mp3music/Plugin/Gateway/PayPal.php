<?php
class Mp3music_Plugin_Gateway_PayPal extends Payment_Plugin_Gateway_PayPal
{
  ...
  public function createPackageTransaction(Mp3music_Model_Order $order, array $params = array())
 	{
    ...
    $payment_item = array_merge($payment_item, array(
      'L_PAYMENTREQUEST_0_NAME0' => $view->translate('Buy Songs'),
      'L_PAYMENTREQUEST_0_DESC0' => $view->translate('Buy Songs from %s', $view->layout()->siteinfo['title']),
    ...
    $payment_requests = array_merge($payment_requests, array_merge(array(
      ...
      'PAYMENTREQUEST_0_DESC' => $view->translate('Buy Songs'),
  ...
  public function onPackageTransactionReturn(Mp3music_Model_Order $order, array $params = array())
  {
     ...
     if ( null == ($order = Engine_Api::_()->getDbTable('orders', 'mp3music') -> findRow($request['PAYMENTREQUESTID'])) ) {
       $order->onPaymentFailure();
     ...
     $order = Engine_Api::_()->getDbTable('orders', 'mp3music') -> findRow($info['PAYMENTREQUESTID']);
     ...
     $db->beginTransaction();
     try {
     	Engine_Api::_() -> mp3music() -> processPayment($order);
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
     ...
     if( $paymentStatus == 'okay' ) {
       ...
       if ($order instanceof Mp3music_Model_Order) {
         $order->onPaymentFailure();
         throw new Payment_Model_Exception('There was an error processing your ' .
                             'transaction. Please try  again later.');
       }
