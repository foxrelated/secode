...
document.addEvent('domready', function(){
	...
	if($$('.mp3music_newsalbums').length > 0) {
		var itemWidth =  parseInt($$('.mp3music_newsalbums')[0].getStyle('width'));
		...
		$('mp3music_navigation-slideshow').style.width = slideWidth + "px";
		console.log(itemWidth);
		if($$('.mp3music_newsalbums').length >= 4) {
			$$('.mp3music_newsalbums').each(function(item, index){
