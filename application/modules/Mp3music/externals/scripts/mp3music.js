...
(function($) {
  ...
  // Get an random number
	...
	function next_song(current_item)
	{
		var mp3Container = $(current_item).closest('.mp3music_container');
			var strSongTitle = $(current_item).children('.song-title').text();
			var audio_src = $(current_item).parent().children(".link").text();
			
			$(current_item).parent().addClass('current').siblings().removeClass('current');
