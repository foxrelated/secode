<?php
class Mp3music_TransactionController extends Core_Controller_Action_Standard
{
	protected $_user;
	protected $_session;
	protected $_order;
	...
	public function init()
	{
		...
		if($params['action'] != 'finish')
		{
			if ($order_id)
			{
				$this -> _order = Engine_Api::_() -> getDbTable('orders', 'mp3music') -> findRow($order_id);
			}
			else
			{
				return $this -> _redirector();
	...
	{
		return $this -> _helper -> redirector -> gotoRoute(array('action' => 'process'), 'mp3music_transaction', true);
	}
	public function processAction()
	{
		$api = Engine_Api::_() -> mp3music();
	...
	public function returnAction()
	{
		...
		if (!$this -> _user 
			...
			|| !($order = Engine_Api::_() -> getDbTable('orders', 'mp3music') -> findRow($orderId)) 
		...
		), 'mp3music_transaction', true);
	}

	public function finishAction()
	{
		$this -> view -> status = $status = $this -> _getParam('state');
		$order = Engine_Api::_() -> getItem('mp3music_order', $this->_getParam('order_id'));
		if ($status == 'completed')
		{
			$url = $this -> view -> escape($this -> view -> url(array(), 'mp3music_cart_downloadlist', true));
		}
		else
		{
			//redirect to homepage base on type
			$url = $this -> view -> escape($this -> view -> url(( array()), 'mp3music_browse', true));
