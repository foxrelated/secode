...
class Mp3music_AlbumController extends Core_Controller_Action_Standard
{
	...
	public function createAction()
	{
		...
		$this -> view -> form = $form = new Mp3music_Form_CreateAlbum();
		$this -> view -> album_id = $this -> _getParam('album_id', '0');
		if ((isset($_POST['upload_message']) && $_POST['upload_message'] == 'on') || isset($_POST['price']))
		{
			$this -> view -> upload_message = 1;

		}
		if ($this -> getRequest() -> isPost() && isset($_POST['title']) && $form -> isValid($this -> getRequest() -> getPost()))
		{
			$db = Engine_Api::_() -> getDbTable('albums', 'mp3music') -> getAdapter();
			$db -> beginTransaction();
			try
			{
				if (!empty($_FILES['track']) && $_FILES['track']['tmp_name'])
				{
					if (!preg_match('/\.(mp3)$/', $_FILES['track']['name']))
					{
						$form -> getElement("track") -> addError("Invalid file type");
						return FALSE;
					}
					try
					{
						$song = Engine_Api::_() -> getApi('core', 'mp3music') -> createSong($_FILES['track']);								
						...
						{
							if ($allow_artist)
								$album -> addSong($song -> getIdentity(), $values['music_categorie_id'], $values['music_singer_id'], $values['other_singer']);
							else
								$album -> addSong($song -> getIdentity(), $values['music_categorie_id'], $values['music_singer_id'], $values['other_singer'], $values['music_artist_id']);
						}
					}
	...
	public function editAction()
	{
		...
		foreach ($this->_navigation->getPages() as $page)
			if ($page -> route == 'mp3music_manage_album')
				$page -> setActive(true);
		$this -> view -> form = $form = new Mp3music_Form_EditAlbum();
		$form -> populate($album);
			...
			try
			{
				if (!empty($_FILES['track']) && $_FILES['track']['tmp_name'])
				{
					...
					try
					{
						...
						{
							$song = Engine_Api::_() -> getApi('core', 'mp3music') -> createSong($_FILES['track']);
							$album -> addSong($song -> getIdentity(), 0, 0, 0);
						}
					}
	...
	public function deleteAction()
	{
		...
		try
		{
			...
			$where = $table -> getAdapter() -> quoteInto('album_id = ?', $album -> album_id);
	...
	public function albumAction()
	{
		...
		if (!empty($song))
		{
			if( !$this->_helper->requireAuth->setAuthParams($song, null, 'play')->isValid()) 
			{
		      return;
		    }
	...
	public function editSongAction()
	{
		...
		if ($this -> getRequest() -> isPost() && $this -> view -> form -> isValid($this -> getRequest() -> getPost()))
		{
			...
			try
			{
				$price = $this -> view -> form -> saveValues();
				$db -> commit();
				...
				if ($price)
				{
					if ($price > 0)
						...
						$this -> _forward('success', 'utility', 'core', array(
							...
							'parentRefresh' => true,
							'format' => 'smoothbox',
							'messages' => array($this -> view -> translate('Your song allow free download!'))
						));
				}
			}
			catch (Exception $e)
	...
	public function deleteSongAction()
	{
		...
		try
		{
			...
			$this -> _forward('success', 'utility', 'core', array(
				...
				'parentRefresh' => true,
				'format' => 'smoothbox',
				'messages' => array($this -> view -> translate('Delete song successfully.'))
			));
		}
		catch (Exception $e)
		{
	...
	public function albumArtAction()
	{
		$this -> _helper -> layout -> disableLayout();
		$album_id = $this -> getRequest() -> getParam('album_id');
		...
		{
			$album = Engine_Api::_() -> getItem('mp3music_album', $album_id);
			$this -> view -> pathPhoto = $album -> getPhotoUrl();
		}
		if ($playlist_id)
		{
			$playlist = Engine_Api::_() -> getItem('mp3music_playlist', $playlist_id);
			$this -> view -> pathPhoto = $playlist -> getPhotoUrl();
		}
	}
