...
class Mp3music_Model_Playlist extends Core_Model_Item_Abstract
{
	...
	public function getHref($params = array())
	{
		...
		return Zend_Controller_Front::getInstance() -> getRouter() -> assemble($params, 'mp3music_playlist', true);
	}

	public function getEditHref($params = array())
	{
		$params = array_merge(array('playlist_id' => $this -> playlist_id), $params);
		return Zend_Controller_Front::getInstance() -> getRouter() -> assemble($params, 'mp3music_edit_playlist', true);
	}
	...
	public function getSongs($file_id = null)
	{
		...
		;
		$select -> join($ps_name, "$as_name.song_id = $ps_name.album_song_id", array('playlist_id' => "$ps_name.playlist_id")) -> where('playlist_id = ?', $this -> getIdentity()) -> where("$as_name.is_delete = 0") -> order("$ps_name.order ASC");
		if (!empty($file_id))
	...
	public function getPSongs()
	{
		...
		$select = $table -> select() -> where('playlist_id = ?', $this -> getIdentity()) -> order("order ASC");
	...
	public function getNewPlaylists($limit = NULL)
	{
		...
		$select = $table -> select() -> order('creation_date  DESC') -> where('search = 1') -> limit($limit);
		return $table -> fetchAll($select);
	}
	...
	{
		$table = Engine_Api::_() -> getDbtable('playlists', 'mp3music');
		$select = $table -> select() -> order('creation_date  DESC') -> where('search = 1') -> where('user_id = ?', $user_id);
		if ($limit)
			$select -> limit($limit);
		return $table -> fetchAll($select);
	}
	...
	public function getPlaylists($params = array())
	{
		...
		if ($params['title'])
		{
			$select -> where('title LIKE ?', "%{$params['title']}%") -> order('creation_date  DESC');
		}
		return $table -> fetchAll($select);
	}
	...
	public function setProfile()
	{
		...
		$table -> update(array('profile' => 0, ), $where);
	...
	public function setPhoto($photo)
	{
		...
		$image -> open($file) -> resize(1024, 1024) -> write($path . '/m_' . $name) -> destroy();
