<?php
class Mp3music_Model_Order extends Core_Model_Item_Abstract
{
	protected $_searchTriggers = false;
	protected $_type = 'mp3music_order';
	...
	public function getPackageParams($arr = array())
    {
        ...
        // General
        $params['name'] = $view -> translate('Buy Songs');
        $params['price'] = $arr['price'];
        $params['description'] = $view -> translate('Buy Songs from %s', $view -> layout() -> siteinfo['title']);
    ...
    {
        return 'mp3music_' . $user_id . '_fee_' . $fee;
    }
	...
	public function isChecked()
	{
		if ($this -> status != 'completed')
			return false;
		$table = Engine_Api::_() -> getDbTable('transactions', 'mp3music');
	...
	public function onPackageTransactionReturn(array $params = array())
	{
		...
		if (isset($params['state']) && $params['state'] == 'cancel')
		{
	     ...
	     $db->beginTransaction();
	     try {
			Engine_Api::_() -> mp3music() -> processPayment($this);
		    $db->commit();
	    } catch (Exception $e) {
	      $db->rollBack();
	      throw $e;
