...
class Mp3music_Model_AlbumSong extends Core_Model_Item_Abstract
{
	...
	{
		return vsprintf(Zend_Registry::get('Zend_Translate') -> _(array(
			'%s play',
			'%s play',
			$this -> play_count
		)), Zend_Locale_Format::toNumber($this -> play_count)) . ' - ' . vsprintf(Zend_Registry::get('Zend_Translate') -> _(array(
			'%s USD',
			'%s USD',
			$this -> price
		)), Zend_Locale_Format::toNumber($this -> price));
	}
