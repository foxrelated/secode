...
class Mp3music_Model_Album extends Core_Model_Item_Abstract
{
	...
	public function getCountAlbums($user = null)
	{
		...
		$select = $ab_table -> select() -> from($ab_table, array('COUNT(*) AS count'));
		if ($user)
	...
	public function getCountSongs($user = null)
	{
		...
		$select = $as_table -> select() -> from($as_name, array('COUNT(*) AS count'));
		if ($user)
		{
			$ab_table = Engine_Api::_() -> getDbTable('albums', 'mp3music');
			$ab_name = $ab_table -> info('name');
	...
	public function getReSongs($user_id = null, $limit = null)
	{
		...
		$select -> joinLeft($ab_name, "$ab_name.album_id = $as_name.album_id", '');
		if ($user_id)
			$select -> where("$ab_name.user_id = ?", $user_id);
		$select -> where("$as_name.is_delete = 0");
	...
	public function getFirstSongArtists($artist_id = null)
	{
		...
		$songs = $as_table -> fetchAll($select) -> limit(1);
		return $songs[0];
	}
	...
	public function getSongs($song_id = null, $limit = NULL)
	{
	...
	public function getDLSongs($file_id = null, $limit = null)
	{
		$table = Engine_Api::_() -> getDbtable('albumSongs', 'mp3music');
		$select = $table -> select() -> where('album_id = ?', $this -> getIdentity()) -> order('order ASC');
		if (!empty($file_id))
	...
	public function getListSong($params = array())
	{
		...
		if (!empty($params['search']) && $params['search'] == 'songs' && !empty($params['title']))
		{
			$keyword = $params['title'];
			$select -> where("$as_name.title LIKE ?", "%{$keyword}%");
		}
		if (isset($params['search']) && $params['search'] == 'categories')
		{
			if (!empty($params['id']))
			{
		...
		if (!empty($params['search']) && $params['search'] == 'singer')
		{
			if ($params['title'])
			{
				$keyword = stripslashes($params['title']);
				$title = $params['title'];
				$select -> joinLeft($s_name, "$as_name.singer_id = $s_name.singer_id", '') -> where("$s_name.title LIKE ?", "%{$title}%") -> orWhere("$as_name.other_singer LIKE ?", "%{$keyword}%");
			}
			if ($params['id'])
			{
		...
		else
		{
			if (!empty($params['title']))
				$select -> where("$as_name.title LIKE ?", "%{$params['title']}%");
			if (!empty($params['album']))
			{
				$select -> where("$ab_name.title LIKE ?", "%{$params['album']}%");
			}
		}
		if (!empty($params['search']) && $params['search'] == 'all')
		{
			if ($params['title'])
			{
				$keyword = stripslashes($params['title']);
				$select -> join("engine4_users", "engine4_users.user_id = $ab_name.user_id", '') -> joinLeft($s_name, "$as_name.singer_id = $s_name.singer_id", '') -> Where("$s_name.title LIKE ?", "%{$keyword}%") -> orWhere("$as_name.other_singer LIKE ?", "%{$keyword}%") -> orWhere("engine4_users.displayname LIKE ?", "%{$keyword}%") -> orWhere("$as_name.title LIKE ?", "%{$keyword}%") -> orWhere("$ab_name.title LIKE ?", "%{$keyword}%");
			}
		}
		if (!empty($params['search']) && $params['search'] == 'browse_topsongs')
		{
			$select -> order("$as_name.play_count DESC");
		}
		elseif (!empty($params['search']) && $params['search'] == 'browse_topdownloads')
		{
	...
	public function getTopAlbums($limit = NULL)
	{
		...
		$select = $table -> select() -> order('play_count  DESC') -> where('search = 1');
		$select -> where("is_delete = 0");
		if ($limit != NULL)
			$select -> limit($limit);
		return $table -> fetchAll($select);
	...
	public function getTopAlbumsDownload($limit = NULL)
	{
		$table = Engine_Api::_() -> getDbtable('albums', 'mp3music');
		$select = $table -> select() -> order('download_count  DESC') -> where('search = 1');
		$select -> where("is_delete = 0");
		if ($limit != NULL)
			$select -> limit($limit);
		return $table -> fetchAll($select);
	...
	public function addSong($file_id, $cat_id, $singer_id, $other_singer, $artist = 0)
	{
		...
		if ($file)
		{
			...
			if($this -> type == 1)
				$album_song -> price = $this -> price;
			$album_song -> save();
			
			$auth = Engine_Api::_() -> authorization() -> context; 
			$auth -> setAllowed($album_song, 'everyone', 'play', true);
		}
		return $album_song;
	}
	...
	public function setPhoto($photo)
	{
		...
		{
			throw new Mp3music_Model_Exception('Invalid argument passed to setPhoto: ' . print_r($photo, 1));
		}
