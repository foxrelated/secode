...
class Ynresume_Model_DbTable_Experiences extends Engine_Db_Table 
{
    ...
    public function getExperiencesByResumeId($resume_id, $current = false, $limit = false) {
        $select = $this->select()->where('resume_id = ?', $resume_id)
            ->order(new Zend_Db_Expr('IF(ISNULL(end_year),0,1), end_year DESC'))
            ->order(new Zend_Db_Expr('IF(ISNULL(end_month),0,1), end_month DESC'))
            ->order(new Zend_Db_Expr('start_year DESC'))
            ->order(new Zend_Db_Expr('IF(ISNULL(start_month),1,0), start_month DESC'));
