...
<?php
$this->headLink()
        ->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sitestorereview/externals/styles/style_sitestorereview.css')
...
<script type="text/javascript">
  function showReviewTab(){
    if($('main_tabs')) {
      ...
      ShowContent('<?php echo $this->content_id; ?>', execute_Request_Review, '<?php echo $this->content_id ?>', 'review', 'sitestorereview', 'profile-sitestorereviews', store_showtitle,'<?php echo Engine_Api::_()->sitestore()->getStoreUrl($this->sitestore->store_id)?>', review_ads_display, store_communityad_integration, adwithoutpackage);
    }
    if($('profile_status')) {
...
<ul class="sitestore_sidebar_list sitestorereview_sidebar">
  ...
  <li>
    ...
    <?php if ($this->can_create == 1 && empty($this->is_manageadmin) && !empty($this->content_id)): ?>
      ...
      <?php if ($storelayout) : ?>
        <a href="<?php echo $url; ?>" onclick='showReviewTab();return false;' class="sitestorereview_more_link"><?php echo $this->translate($more_link); ?></a>
      <?php else: ?>
