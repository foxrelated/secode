...
class Socialbridge_Api_Facebook extends Socialbridge_Api_Abstract
{
	...
	public function getOwnerInfo($params = array())
	{
		if ($this -> _objPlugin)
		{
			try
			{
				...
				$extrafields = '?fields=id,about,birthday,email,name,first_name,last_name,gender,picture,link,website';
				$me = $this -> _objPlugin -> api('/me'.$extrafields);
				$me['identity'] = $me['id'];
	...
	public function getAlbums($params = array())
	{
		...
		{
			case 'my':
				$albums = $this -> _objPlugin -> api('/'.$uid.'/albums?fields=id,name,count,cover_photo,from&limit='.$limit.'&after='.$after);
				break;
		}
		$after = '';
		$data = $albums['data'];
		...
		{
			$src_small = $src_thumb = $src_medium = $src_big = 'application/modules/Socialbridge/externals/images/nocover_album.jpg';
			if(!empty($album['cover_photo']))
			{
				$cover = $this -> _objPlugin -> api('/'.$album['cover_photo']['id'].'?fields=images');
				if($cover)
				{
					$covers = $cover['images'];
					$cover_end = end($covers);
					$src_small = $src_thumb =  $src_medium = $cover_end['source'];
					$src_big = $covers[0]['source'];
				}
			}
			$nid = $this -> createNodeId($this -> _plugin, 'album', $album['id']);
			$albums[$nid] = array(
				'nid' => $nid,
	            'id' => $album['id'],
	            'aid' => $album['id'],
	...
	public function getPhotos($params = array())
	{
		...
		switch($extra)
		{
			case 'uid' :
			case 'my':
				$photos = $this -> _objPlugin -> api('/'.$uid.'/photos?fields=id,name,album,images,from&limit='.$limit.'&after='.$after);
				break;
			case 'upload':
				$photos = $this -> _objPlugin -> api('/'.$uid.'/photos/uploaded?fields=id,name,album,images,from&limit='.$limit.'&after='.$after);
				break;
			case 'aid' :
				$photos = $this -> _objPlugin -> api('/'.$aid.'/photos?fields=id,name,album,images,from&limit='.$limit.'&after='.$after);
				break;
			case 'tag' :
				$photos = $this -> _objPlugin -> api('/'.$uid.'/photos/tagged?fields=id,name,album,images,from&limit='.$limit.'&after='.$after);
				break;
		}
		$after = '';
		$data = $photos['data'];
        ...
        {
            $nid = $this -> createNodeId($this -> _plugin, 'photo', $photo['id']);
			$photo_end = end($photo['images']);
            $photos[$nid] = array(
                'nid' => $nid,
                'id' => $photo['id'],
                'aid' => $photo['album']['id'],
                ...
                'description' => '',
                'photo_count' => 1,
                'src_thumb' => $photo_end['source'],
                'src_small' => $photo_end['source'],
                'src_medium' => $photo_end['source'],
                'src_big' => $photo['images'][0]['source'],
                'status' => '',
            );
        }
		unset($data);
		return array($photos, $after);
	}
	...
	public function hasPermission($params = array())
	{
		...
		if ($permissions)
		{
			if(count($permissions['data']) > 1)
			{
				...
				{
					$arr_permission[0][$permission['permission']] = TRUE;
				}
			}
			else {
				$arr_permission = $permissions['data'];
			}
