...
class Ynmusic_IndexController extends Core_Controller_Action_Standard {
	...
	public function validateAction() {
		...
		try {
			...
			$segments = explode('/', $track_url);
			if (strpos($segments[4],'?') !== false)  {
				$subSegments = explode('?', $segments[4]);
				$permalink = $subSegments[0];
			} else {
				$permalink = $segments[4];
			}
		...
		if ($track -> permalink) {
			echo Zend_Json::encode(array('status' => 'true', 'title' => $track -> title, 'permalink' => $track -> permalink, 'idValue' => $this -> _getParam('idValue'), 'idTitle' => $this -> _getParam('idTitle'), 'idEdit' => $this -> _getParam('idEdit'), 'idAction' => $this -> _getParam('idAction'), 'idSave' => $this -> _getParam('idSave')));
		} else {
			echo Zend_Json::encode(array('status' => 'false'));
		} exit ;
	}
