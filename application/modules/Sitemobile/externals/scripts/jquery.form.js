//--------------Custom Fileds Js jsSwitch js------------------
var customProfileFiledsSettings = [];
var customFormPageId = '';
$(document).bind('pageshow', function(event, data) {
  var $page = $(event.target);
  if ($page.data('role') == 'page') {
    customFormPageId = $page.attr('id');
  }
});
function getFieldsElements(selector) {
  if ($.type(customProfileFiledsSettings[customFormPageId]) != 'undefined' && $.type(customProfileFiledsSettings[customFormPageId].elementCache[selector]) != 'undefined') {
    return customProfileFiledsSettings[customFormPageId].elementCache[selector];
  } else {
    if ($.type(customProfileFiledsSettings[customFormPageId]) == 'undefined'){
      customProfileFiledsSettings[customFormPageId]={elementCache:{}};
    }

    return customProfileFiledsSettings[customFormPageId].elementCache[selector] = $(selector);
  }
}
...
var changeFields = window.changeFields = function(element, force, isLoad) {
  ...
  if (!$.type(element) || $.type(element) == 'null') {
    getFieldsElements('.parent_' + customProfileFiledsSettings[customFormPageId].topLevelId).each(function(key, element) {
  ...
  getFieldsElements('.parent_' + field_id).each(function(key, childElement) {
    ...
    if($.type($(childElement).attr('id'))== 'undefined' && !$(childElement).jqmData('field-id')) {
        return;
    }
  ...
  $('#'.customFormPageId).trigger('onFormChangeFields');
};
  ...
  $.fn.ajaxSubmit = function(options) {
    ...
    function fileUploadXhr(a) {
      ...
      for (var i = 0; i < a.length; i++) {
         if(!(a[i].type ==='file' && !!$.mobile.activePage.fileUploader && $.mobile.activePage.fileUploader.length))
        formdata.append(a[i].name, a[i].value);
      }

      for (var i = 0; i < $.mobile.activePage.fileUploader.length; i++) {
        formdata.append($.mobile.activePage.fileUploader[i].name,$.mobile.activePage.fileUploader[i].content,$.mobile.activePage.fileUploader[i].filename);     
      }
