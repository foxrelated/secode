 ...
 * @version    $Id: Edit.php 9800 2012-10-17 01:16:09Z richard $
 * @author     John
 */
 ...
 * @license    http://www.socialengine.com/license/
 */
class Sitemobile_modules_Activity_Form_Edit extends Engine_Form {
...
//    }
  ...
  public function setActionIdentity($comment_id) {
    if (Engine_Api::_()->getApi('settings', 'core')->core_spam_comment) {
      ...
      $this
              ->setAttrib('id', 'activity-edit-form-' . $comment_id)
              ->setAttrib('class', 'activity-comment-form')
              ->setAttrib('style', 'display: none;');
    }
    $this->action_id
            ->setValue($comment_id)
            ->setAttrib('id', 'activity-edit-id-' . $comment_id);
    $this->submit //->getDecorator('HtmlTag')
            ->setAttrib('id', 'activity-edit-submit-' . $comment_id)
    ;

    $this->body
            ->setAttrib('id', 'activity-edit-body-' . $comment_id)
    ;
