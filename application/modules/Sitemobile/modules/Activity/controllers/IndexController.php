...
class Activity_IndexController extends Core_Controller_Action_Standard {
  ...
  public function init() {
    $this->view->addHelperPath(APPLICATION_PATH . '/application/modules/Sitemobile/modules/Activity/View/Helper', 'Activity_View_Helper');
  }
  ...
  public function postAction() {
    ...
    $last_id = 0;
    if ($action) {
      $feed_stream = $this->view->activitySM($action, array('onlyactivity' => true));
...
//    }
  ...
  public function viewlikeAction() {
    ...
    if (null === $this->_getParam('format', null)) {
      ...
      $this->view->body = $this->view->activitySM($action, array('viewAllLikes' => true, 'noList' => $this->_getParam('nolist', false)));
    }
  }
...
//     {
    $method = 'update';
    $this->view->body = $this->view->activitySM($action, array('noList' => true), $method);
    //}
  }
...
//     {
  ...
  public function viewcommentAction() {
    ...
    if (null === $this->_getParam('format', null)) {
      ...
      $this->view->body = $this->view->activitySM($action, array('viewAllComments' => true, 'noList' => $this->_getParam('nolist', false)));
    }
  }
...
//     {
    ...
    $this->view->body = $this->view->activitySM($action, array('noList' => true), $method, $show_all_comments);
    //}
  }
  ...
  public function shareAction() {
    ...
    $this->view->form = $form = new Sitemobile_modules_Activity_Form_Share();
    if (Engine_API::_()->sitemobile()->isApp()) {
      Zend_Registry::set('setFixedCreationForm', true);
      if ($attachment->getMediaType() == "image") 
      Zend_Registry::set('setFixedCreationHeaderTitle', str_replace(' New ', ' ', 'Share Photo'));
      else
      Zend_Registry::set('setFixedCreationHeaderTitle', str_replace(' New ', ' ', 'Share'));    
      Zend_Registry::set('setFixedCreationHeaderSubmit', 'Share');
      $this->view->form->setAttrib('id', 'form_share_creation');
      Zend_Registry::set('setFixedCreationFormId', '#form_share_creation');
      $this->view->form->setTitle('');
      $this->view->form->removeElement('cancel');
      $this->view->form->removeElement('post_to_twitter');
    ...
    if ($attachment->getMediaType() == "image")
      $sharableItem = "photo";
    elseif ($attachment->getMediaType() == "item")
      $sharableItem = $attachment->getShortType();
    else
      $sharableItem = "content";
    ...
    if (Engine_API::_()->sitemobile()->isApp()) {
      $this->_forward('success', 'utility', 'core', array(
          ...
          'messages' => array('The '.$sharableItem.' has been shared successfully.')
      ));
    } else {
      if (null === $this->_helper->contextSwitch->getCurrentContext()) {
        $return_url = $form->getValue('return_url', false);
        ...
        $this->_forward('success', 'utility', 'core', array(
            'smoothboxClose' => 10,
          //'parentRefresh'=> 10,
            'messages' => array('The content has been shared successfully.')
        ));
      }
    }
  }
