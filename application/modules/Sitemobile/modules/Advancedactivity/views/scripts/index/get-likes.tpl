  ...
  <?php if ($this->likes->getTotalItemCount() > 0): // COMMENTS -------  ?>
    ...
    <?php if ($this->page == 1): ?>
      <div class="sm-ui-popup-top ui-header ui-bar-a">
        <?php if (!empty($action)): ?> 
          <a data-iconpos="notext" data-role="button" data-icon="chevron-left" data-corners="true" data-shadow="true" class="ui-btn-left " onclick="$('#comment-activity-item-' + <?php echo $action->action_id ?>).css('display', 'block');$('#like-comment-item-' + <?php echo $action->action_id ?>).css('display', 'none');"><?php //echo $this->translate('back');?></a>
        <?php else : ?>
