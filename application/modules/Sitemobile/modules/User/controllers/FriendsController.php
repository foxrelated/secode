 ...
 * @version    $Id: FriendsController.php 9800 2012-10-17 01:16:09Z richard $
 * @author     John
 */
...
class User_FriendsController extends Core_Controller_Action_User {
    ...
    public function init() {
        $this->view->addHelperPath(APPLICATION_PATH . '/application/modules/Sitemobile/modules/User/View/Helper', 'User_View_Helper');
        // Try to set subject
    ...
    public function addAction() {
        ...
        if ($viewer->isSelf($user)) {
            return $this->_forward('success', 'utility', 'core', array(
                        'smoothboxClose' => true,
                        'parentRefresh' => true,
                        'messages' => array(Zend_Registry::get('Zend_Translate')->_('You cannot be friend yourself.'))
            ));
        }
        ...
        // Make form
        $this->view->form = $form = new Sitemobile_modules_User_Form_Friends_Add(array('user' => $user));
        $this->view->clear_cache = true;
        ...
        try {
            ...
            //CHECK IF THE REQUEST IS JSON REQUEST
            if ($this->_getParam('type') == 'json') {
                $this->view->body = $this->view->userFriendshipSM($user, $viewer);
            } else {
                return $this->_forward('success', 'utility', 'core', array(
                            'smoothboxClose' => true,
                            'parentRefresh' => true,
    ...
    public function cancelAction() {
        ...
        // Make form
        $this->view->form = $form = new Sitemobile_modules_User_Form_Friends_Cancel(array('user' => $user));
        $this->view->clear_cache = true;
        if (!$this->getRequest()->isPost()) {
            $this->view->status = false;
            $this->view->error = Zend_Registry::get('Zend_Translate')->_('No action taken');
    ...
    public function confirmAction() {
        ...
        // Make form
        $this->view->form = $form = new Sitemobile_modules_User_Form_Friends_Confirm(array('user' => $user));
        $this->view->clear_cache = true;
        if (!$this->getRequest()->isPost()) {
            $this->view->status = false;
            $this->view->error = Zend_Registry::get('Zend_Translate')->_('No action taken');
        ...
        try {
            ...
            $message = Zend_Registry::get('Zend_Translate')->_('<div class="tip">You are now friends with %s</div>');
    ...
    public function rejectAction() {
        ...
        // Make form
        $this->view->form = $form = new Sitemobile_modules_User_Form_Friends_Reject(array('user' => $user));
        $this->view->clear_cache = true;
        if (!$this->getRequest()->isPost()) {
            $this->view->status = false;
            $this->view->error = Zend_Registry::get('Zend_Translate')->_('No action taken');
    ...
    public function removeAction() {
        ...
        // Make form
        $this->view->form = $form = new Sitemobile_modules_User_Form_Friends_Remove(array('user' => $user));
        $this->view->clear_cache = true;
        if (!$this->getRequest()->isPost()) {
            $this->view->status = false;
            $this->view->error = Zend_Registry::get('Zend_Translate')->_('No action taken');
    ...
    public function suggestAction() {
        ...
        if (!$viewer->getIdentity()) {
            ...
            if ($this->_getParam('message') && $usersAllowed == "everyone") {
