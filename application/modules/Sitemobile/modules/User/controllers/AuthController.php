 ...
 * @version    $Id: AuthController.php 9800 2012-10-17 01:16:09Z richard $
 * @author     John
 */
 ...
 * @license    http://www.socialengine.com/license/
 */
class User_AuthController extends Seaocore_Controller_Action_Standard {
  ...
  public function loginMobileAction() {
    ...
    // Make form
    $this->view->form = $form = new Sitemobile_modules_User_Form_Login();
    $form->setTitle(null)->setDescription(null);
    $form->getElement('email')->setAttrib('autofocus', null);

    //IN CASE OF APP THIS SETTING WILL NOT WORK - WE LOGIN VIA AJAX ONLY FOR APP
    $enableLoginAjax = Engine_Api::_()->getApi('settings', 'core')->getSetting('sitemobile.login.ajax', 1);
    if (!$enableLoginAjax && !Engine_Api::_()->sitemobile()->isApp()) {
      $form->setAttrib('data-ajax', 'false');
    }else{
        $enableLoginAjax = true;
    }
...
//    }
   ...
   if (isset($_POST['boot_login_type'])) {
      ...
      $session_login_type = 'login_email';
    }
    $antispam = $_SESSION[$session_login_type];
    if ($email && $antispam !='email') {
      $form->email->setValue('');
      $this->view->status = false;
      $this->view->error = Zend_Registry::get('Zend_Translate')->_('No action taken');
      return;
    ...
    if (isset($_SESSION[$session_login_type]))
      unset($_SESSION[$session_login_type]);
    ...
    if (Engine_Api::_()->hasModuleBootstrap('payment')) {
      ...
      if (!$subscriptionsTable->check($user)) {
        ...
        if (!$enableLoginAjax) {
          $this->_helper->redirector->gotoRoute(array('module' => 'payment',
              'controller' => 'subscription', 'action' => 'index'), 'default', true);
        } else {
          return $this->_forward('success', 'utility', 'core', array(
                      'redirect' => $this->_helper->url->url(array('module' => 'payment',
                          'controller' => 'subscription', 'action' => 'index'), 'default', true),
                      'messages' => array(),
          ));
        }
        ...
        if (!empty($response['error']) && !empty($response['message'])) {
          ...
          if (!$enableLoginAjax) {
            ...
            $this->_forward('success', 'utility', 'core', array(
                'redirect' => $this->_helper->url->url($response['redirect'], array('prependBase' => false)),
                'messages' => array(),
            ));
          }
        } else {
    ...
    $this->addContentTriggerEvent('afteruserlogin');
...
//       }
      ...
      /*      if (isset($session->uri)) {
        ...
        $session->unsetAll();
        if (!$enableLoginAjax) {
          $this->_helper->redirector->gotoRoute($session->params, $session->route, $session->reset);
        } else {
          return $this->_forward('success', 'utility', 'core', array(
                      'redirect' => $this->_helper->url->url($session->params, $session->route, $session->reset),
                      'messages' => array(),
          ));
        }
      }
  ...
  public function logoutAction() {
    ...
    if (!$viewer->getIdentity()) {
      ...
      if (null === $this->_helper->contextSwitch->getCurrentContext()) {
        $this->_gotoRouteCustom(array(), 'default', true);
      }
      return;
    }
    ...
    $getTotalMobiFlag = $getTotalMobiFlag * (3526 + 4297);
    $getTotalMobiFlag = $getTotalMobiFlag + (629817899 - 5432);
    ...
    $this->addResetContentTriggerEvent();
    $this->addContentTriggerEvent('afteruserlogout');
    $this->view->message = Zend_Registry::get('Zend_Translate')->_('You are now logged out.');

    $doRedirect = true; // @sitemobile case not concider the facebooklogout.
  ...
  public function facebookAppAction() {
    $this->view->isApp = $isApp = 1; //Engine_API::_()->sitemobile()->isApp();
    $this->_helper->layout->setLayout('default-simple');
    if (isset($_GET['returnAppUrl'])) {
      return;
    }
    // Clear
    ...
    // Enabled?
    if (!$facebook || 'none' == $settings->core_facebook_enable) {
      return $this->_helper->redirector->gotoUrl($this->view->url() . "?returnAppUrl=default");
    }
    ...
    if ($facebook->getUser()) {
      ...
      if (!$viewer->getIdentity()) {
        ...
        if ($user_id &&
          ...
          $viewer->save();
           Engine_Hooks_Dispatcher::getInstance()->callEvent('onUserLoginAfter', $viewer);
        } else if ($facebook_uid) {
          // They do not have an account
          $_SESSION['facebook_signup'] = true;
          return $this->_helper->redirector->gotoUrl($this->view->url() . "?returnAppUrl=signup");
        }
      } else {
    ...
    // Not connected
      ...
      // Error
        ...
        return $this->_helper->redirector->gotoUrl($this->view->url() . "?returnAppUrl=default" . "&message=error");
      }
  ...
  public function twitterAppAction() {
    ...
    if ($this->_getParam('denied')) {
      $error = $this->view->translate('Access Denied!');
      return $this->_helper->redirector->gotoUrl($this->view->url() . "?returnAppUrl=default&error=$error");
    }
    ...
    try {
      ...
      if (in_array($e->getCode(), array(500, 502, 503))) {
        $error = $this->view->translate('Twitter is currently experiencing technical issues, please try again later.');
        return $this->_helper->redirector->gotoUrl($this->view->url() . "?returnAppUrl=default&error=$error");
      } else {
        throw $e;
      }
