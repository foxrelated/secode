 ...
 * @version    $Id: SignupController.php 9800 2012-10-17 01:16:09Z richard $
 * @author     John
 */
 ...
 * @license    http://www.socialengine.com/license/
 */
class User_SignupController extends Seaocore_Controller_Action_Standard {
    ...
    public function indexMobileAction() {
        ...
        if (Engine_Api::_()->user()->getViewer()->getIdentity()) {
            return $this->_gotoRouteCustom(array(), 'default', true);
        }
        ...
        foreach (Engine_Api::_()->getDbtable('signup', 'user')->fetchAll() as $row) {
            if ($row->enable == 1) {
                $class = $row->class;
                if (strpos($class, '_Plugin_Signup_Account')) {
                    $class = 'User_Plugin_Signup_Account';
                }
                $classArray = explode('_', $class);
                $modulename = strtolower($classArray[0]);
                if (!Engine_Api::_()->sitemobile()->isSupportedModule($modulename))
                    continue;
                if (strpos($class, 'Signup_Invite'))
                    continue;
                if (strpos($class, '_Plugin_Signup_Account'))
                    $class = 'Sitemobile_modules_' . $class;
                if (strpos($class, '_Plugin_Signup_Photo'))
                    $class = 'Sitemobile_modules_' . $class;
            ...
            foreach ($event->getResponses() as $response) {
                if (is_array($response)) {
                    ...
                    if (!empty($response['redirect'])) {
                        return $this->_gotoRouteCustom($response['redirect'], array('prependBase' => false));
                    }
                }
            }
        }
            ...
            if (!$subscriptionsTable->check($viewer)) {
                ...
                if (!$defaultSubscription) {
                    ...
                    if (!empty($subscriptionSession->subscription_id)) {
                        return $this->_gotoRouteCustom(array('module' => 'payment',
        ...
        if (!$viewer->enabled) {
            ...
            return $this->_gotoRouteCustom(array('action' => 'confirm'), 'user_signup', true);
        }

        // Handle normal signup
        else {
            Engine_Api::_()->user()->getAuth()->getStorage()->write($viewer->getIdentity());
            if (!Engine_Api::_()->sitemobile()->checkVersion(Engine_Api::_()->getDbtable('modules', 'core')->getModule('core')->version, '4.8.11')) {
                Engine_Hooks_Dispatcher::getInstance()
                        ->callEvent('onUserEnable', $viewer);
            } else {
                Engine_Hooks_Dispatcher::getInstance()->callEvent('onUserEnable', array('user' => $viewer, 'shouldSendEmail' => false));
            }
        }
        ...
        return $this->_gotoRouteCustom(array('action' => 'home'), 'user_general', true);
    }
    ...
    public function resendAction() {
        ...
        Engine_Api::_()->getApi('mail', 'core')->sendSystem(
                $user, 'core_verification', $mailParams
        );
    }
