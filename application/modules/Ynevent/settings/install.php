...
class Ynevent_Installer extends Engine_Package_Installer_Module
{
	...
	private function _addEventProfilepage()
	{
		...
		if (!$page_id)
		{
			...
			// insert ynevent.profile-files
			$db -> insert('engine4_core_content', array(
				'type' => 'widget',
				'name' => 'ynevent.profile-files',
				'page_id' => $page_id,
				'parent_content_id' => $main_container_id,
				'order' => 21,
				'params' => '{"title":"Discussions"}',
	...
	protected function _addEventProfileFileWidget()
	{
		
		// get page profile event
		$db = $this -> getDb();
		$select = new Zend_Db_Select($db);
		$select -> from('engine4_core_pages') -> where('name = ?', 'ynevent_profile_index') -> limit(1);
		...
		$select = new Zend_Db_Select($db);
		$select -> from('engine4_core_content') -> where('page_id = ?', $page_id) -> where('name = ?', 'ynevent.profile-files') -> limit(1);
		$info = $select -> query() -> fetch();
		if(!$info)
		{
			//Profile fields 
	...
	public function onInstall()
	{
		...
		$this ->_addEventProfileFileWidget();
