...
class Ynevent_EventController extends Core_Controller_Action_Standard 
{
	...
	private function setAuth(Ynevent_Model_Event $event, $values) {
		...
		$rolesFile = array('owner', 'member', 'parent_member', 'registered', 'everyone');
		$fileMax = array_search($values['auth_file'], $rolesFile);
		foreach ($rolesFile as $i => $r) {
			$auth -> setAllowed($event, $r, 'file', ($i <= $fileMax));
		}
	...
	public function editAction() 
	{
		...
		foreach ($rolesFile as $i => $r) 
		{
			if (isset($form -> auth_file -> options[$r]) && $auth -> isAllowed($event, $r, 'file')) {
				$form -> auth_file -> setValue($r);
			}
		}
