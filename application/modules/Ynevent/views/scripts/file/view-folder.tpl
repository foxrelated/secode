...
<div class="ynevent-profile-module-header">
    ...
    <div class="ynevent-profile-header-right">
	    <?php echo $this->htmlLink(array('route' => 'event_profile', 'id' => $this->event->getIdentity()), '<i class="fa fa-arrow-left"></i>'.$this->translate('Back to Event'), array(
	    'class' => 'buttonlink'
	  )) ?>
	 	<?php echo $this->htmlLink(array('route' => 'event_extended', 'controller'=>'file', 'action'=>'list', 'event_id' => $this->event->getIdentity()), '<i class="fa fa-arrow-left"></i>'.$this->translate('Back to File Folders'), array(
	    'class' => 'buttonlink'
	  )) ?>
    </div>      
...
<div class="ynfs_block ynfs_controls">
	...
	<?php if ($this->canEdit) : ?>
		<a href="<?php 
			echo $this->url(array('action' => 'edit', 'folder_id' => $this->folder->getIdentity(), 'parent_type' => $this->parentType, 'parent_id' => $this->parentId, 'subject_id' => $this->parentId), 
	...
	<?php if ($this->canEditPerm) : ?>
		<a href="<?php 
			echo $this->url(array('action' => 'edit-perm', 'folder_id' => $this->folder->getIdentity(), 'parent_type' => $this->parentType, 'parent_id' => $this->parentId, 'subject_id' => $this->parentId), 
				'ynfilesharing_folder_specific', true)?>" 
	...
	<ul>
		...
		<?php
			 ...
			 <?php if ( ($topFolder->parent_id == $this->parentId) && ($topFolder->parent_type == $this->parentType) ): ?>
			 	<a href="<?php echo $this->url(
					array('controller'=>'file', 'action'=>'list', 'event_id' => $this->event->getIdentity()), 
					'event_extended', 
					true)?>">
					<?php echo $this->translate('Event Folders')?>
				</a>
			 <?php else: ?>
			 	<?php 
		...
		<?php foreach ($parentFolders as $folder) : ?>
			<li>
				...
				<a href="<?php echo Engine_Api::_() -> ynevent() ->getFolderHref(array('parent_id' => $this->parentId, 'parent_type' => $this->parentType), $folder)?>">
...
<?php 
	echo $this->partial('_browse_folders.tpl', 'ynevent', 
		array(
