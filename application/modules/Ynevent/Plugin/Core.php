				...
				if ($event_id)
				{
					...
					switch ($type) {
						...
						case 'folder':
							...
							// Add activity
							$viewer = Engine_Api::_() -> user() -> getViewer();
							//$action = $activityApi->addActivity($viewer, $event, 'ynevent_folder_create');
							//if($action) {
							//	$activityApi->attachActivity($action, $payload);
							//}
							$key = 'ynevent_predispatch_url:' . 'ynfilesharing' . '.index.manage';
						...
						case 'ynultimatevideo_video':
							...
							$event = Engine_Api::_() -> getItem('event', $event_id);
							$activityApi = Engine_Api::_()->getDbtable('actions', 'activity');
							$action = $activityApi->addActivity($viewer, $event, 'ynevent_video_create');
							if($action) {
								$activityApi->attachActivity($action, $payload);
							}
//							@TODO hightlight ultimate video or not?
							$key = 'ynevent_predispatch_url:' . $request -> getParam('module') . '.index.view';
							$value = $view -> url(array(
			...
			if ($type == 'event')
			{
				...
				if ($event_id)
				{
					...
					switch ($type) {
						...
						case 'folder':
							$key = 'ynevent_predispatch_url:' . $request -> getParam('module') . '.folder.view';
							$value = $view -> url(array(
								'controller' => 'file',
								'action' => 'list',
								'event_id' => $event_id,	
