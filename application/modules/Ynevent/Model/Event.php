...
class Ynevent_Model_Event extends Core_Model_Item_Abstract
{
	...
	public function getYnultimatevideoVideosPaginator($params = array()){
		$paginator = Zend_Paginator::factory($this->getYnultimatevideoVideosSelect($params));
		if( !empty($params['page']) ) {
			$paginator->setCurrentPageNumber($params['page']);
		}
		if( !empty($params['limit']) ) {
	...
	public function getYnultimatevideoVideosSelect($params = array()) {
		$table_video = Engine_Api::_()->getItemTable('ynultimatevideo_video');
		$select = $table_video->select();
	...
	public function getRichContent($view = false, $params = array()) {
	    ...
	    if($view == false){
			return $zend_View -> partial('_event_feed.tpl', 'ynevent', array('item' => $this));
	    }
  	}
}
