...
class Ynevent_Form_Create extends Engine_Form
{
  ...
  public function init()
  {
	...
	$fileOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('event', $user, 'auth_file');
    ...
    if( $this->_parent_type == 'user' ) {
	  ...
	  $fileOptions = array_intersect_key($availableLabels, array_flip($fileOptions));
	...
	// File
    if( !empty($fileOptions) && count($fileOptions) >= 1 ) {
      // Make a hidden field
      if(count($fileOptions) == 1) 
      {
        $this->addElement('hidden', 'auth_file', array('value' => key($fileOptions), 'order' => '31'));
      // Make select box
      } else {
        $this->addElement('Select', 'auth_file', array(
            'label' => 'File Upload',
            'description' => 'Who may upload files for this event?',
            'multiOptions' => $fileOptions,
            'value' => key($fileOptions),
        ));
        $this->auth_file->getDecorator('Description')->setOption('placement', 'append');
      }
    }
