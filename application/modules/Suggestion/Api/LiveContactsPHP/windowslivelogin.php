 ...
 * @version    $Id: windowslivelogin.php 2010-08-17 9:40:21Z SocialEngineAddOns $
...
class WindowsLiveLogin
{
    ...
    public static function initFromXml($settingsFile)
    {
        ...
        $settings['returnurl'] = ( _ENGINE_SSL ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getBaseUrl() . '/suggestion/usercontacts/getwindowlivecontacts';
    ...
    public function setPolicyUrl($policyurl)
    {
        ...
        if (!$policyurl) {
            if ($_force_delauth_nonprovisioned) {
								echo Zend_Registry::get('Zend_Translate')->_('Please make sure that you have entered the policy url for Window Live settings.');die;
