<?php
    ...
    class YounetCore_Api_Core extends Core_Api_Abstract
    {
		...
		const AUTH_HOST =  'http://auth.younetco.com';
        ...
        public function getModules()
        {
			...
			if($modules && $recheck){
				...
				$sql =  'select name, version from engine4_core_modules';
				$rows = $db->fetchPairs($sql);
				foreach ($rows as $key=>$value)
				{
					if(isset($modules[$key])){
						$modules[$key]['current_v'] = $value;	
					}
				}
			}
			
			if($modules){
				$data = $this->getModuleInfoFromServer();
				if(is_array($data))
				{
					foreach($data as $key=>$value)
					{
						if(isset($modules[$key])){
							$modules[$key]['latest_v'] = $value['latest_v'];
                            $modules[$key]['download'] = $value['download'];
						}
					}	
				}
			}
		...
		public function getModuleInfoFromServer(){
			
			$cacheKey = 'ynmodule_latest_plugins';
			...
			$url = self::AUTH_HOST. '/plugins.php?platform=socialengine&version=4';
        ...
        public function getNews()
        {
            ...
            try{
                $rss = Zend_Feed::import('http://socialengine.younetco.com/feed');
                $news = array(
                'title'       => $rss->title(),
                'link'        => $rss->link(),
