<?php
class Ynlistings_Form_Create extends Engine_Form
{
	protected $_category;
	protected $_formArgs;
  ...
  public function init()
  {
   ...
   $this->addElement('TinyMce', 'about_us', array(
    ...
    $this->addElement('Select', 'currency', array(
      'label' => 'Currency',
      'value' => Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD'),
    ));
    ...
    if( !$this->_item ) {
      ...
      $customFields = new Ynlistings_Form_Custom_Fields(array(
        'item' => $this->getItem()
      ));
    }
    if( get_class($this) == 'Ynlistings_Form_Create' ) {
	...
	if(!empty($this -> _formArgsParent))
	{
	    if( !$this->_item ) {
	      ...
	      $customFieldsParent = new Ynlistings_Form_Custom_FieldsParent(array(
	        'item' => $this->getItem()
	      ));
	    }
	    if( get_class($this) == 'Ynlistings_Form_Create' ) {
